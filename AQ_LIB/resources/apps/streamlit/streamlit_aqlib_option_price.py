...
with st.sidebar:
    ...
    st.write("Black-Scholes pricing")
    st.subheader("Engine")
    st.write(f"AlgoQuantLib version: {aq_version}")
    st.write("Python → native C++")
    st.write("Pricing calculated by native engine")
    st.subheader("Demo information")
    st.caption("Professional demonstration of native quantitative analytics for pricing and eTrading.")

st.title("AlgoQuantLib")
st.caption("C++ Quantitative Analytics for Pricing & eTrading")
...
st.caption("European option pricing calculated directly by the native AlgoQuantLib C++ engine.")
...
with input_panel:
    ...
    calculate_price = st.button("Calculate Price", use_container_width=True)

if calculate_price:
    try:
        vol, rate, carry = vol_pct / 100.0, rate_pct / 100.0, carry_pct / 100.0
        args = (call_or_put, float(spot), float(strike), vol, float(time), rate, carry, float(shift))
...
with result_panel:
    if "price_result" in st.session_state:
        x = st.session_state.price_result
        ...
        with result_right:
            ...
            st.metric("Spot / Strike", f"{x['spot'] / x['strike']:.4f}")
    else:
        st.info("Enter the option parameters and calculate the native C++ price.")
...
if "price_result" in st.session_state:
    ...
    for current_spot in spots:
        try:
            ...
            prices.append(float(aq.aqMathBlackScholesPrice(*args)))
        except Exception:
            prices.append(np.nan)
    sensitivity = pd.DataFrame({"Spot": spots, "Option Price": prices})
    st.subheader("Option Price vs Spot")
    st.line_chart(sensitivity, x="Spot", y="Option Price", height=420, use_container_width=True)
    st.caption(f"Strike {x['strike']:.2f} · Volatility {x['vol'] * 100:.2f}% · Rate {x['rate'] * 100:.2f}% · Carry {x['carry'] * 100:.2f}% · Expiry {x['time']:.2f} years")

st.divider()
st.caption("AlgoQuantLib · Native C++ Quantitative Analytics · Pricing · eTrading")
