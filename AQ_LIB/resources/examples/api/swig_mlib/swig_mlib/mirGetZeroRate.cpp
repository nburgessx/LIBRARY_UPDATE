						...
						bool isFwdInterp) throw(std::exception)
{
	...
	try 
	{
		...
		DoubleVector results = validation_api::tryMirGetZeroRate1(g_root, terms, cId, freq, dayC, interp, cName, isFwdInterp);
		ret = results[0];
									...
									bool isFwdInterp) throw(std::exception)
{
	...
	try 
	{
		...
		ret = validation_api::tryMirGetZeroRate1(g_root, Terms, cId, freq, dayC, interp, cName, isFwdInterp);
						...
						bool isFwdInterp) throw(std::exception)
{
	...
	try 
	{
		...
		std::vector<double> results = validation_api::tryMirGetZeroRate2(g_root, terms, cId, freq, dayC, slideRule, cal, interp, cName, isFwdInterp);
		ret = results[0];
									...
									bool isFwdInterp) throw(std::exception)
{
	...
	try 
	{
		...
		ret = validation_api::tryMirGetZeroRate2(g_root, terms, cId, freq, dayC, slideRule, cal, interp, cName, isFwdInterp);
