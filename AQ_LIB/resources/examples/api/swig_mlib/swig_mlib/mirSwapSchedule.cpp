						  ...
						  bool showColumnHeaders) throw(std::exception) 
{
	...
	try 
	{
		...
		LAString fixingCalendar	(fixingCalendar.c_str());
		LAString fixingLag		(fixingLag.c_str());
		...
		StringMatrix temp = validation_api::tryMirSwapSchedule(
										aStartDate, aEndDateOrTenor, aDaycount, aFrequency, aSRule, aCalendar,
										pFrequency, pSRule, pCalendar, pLag,
										fixingSRule, fixingCalendar, fixingLag, fixingAdAr,
