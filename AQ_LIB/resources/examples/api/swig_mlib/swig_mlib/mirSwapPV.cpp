/* 
 * @brief			Swig interface to Java for function mirSwapPV
...
*/
double mirSwapPV(double notional,
				...
				bool InterpFwds,
				bool EomRoll) throw(std::exception)
{
	double ret;
	try 
	{
		// marshall all inputs		
		LAString payRec			(PayRec.c_str());
		...
		ret = validation_api::tryMirSwapPV(g_root,
												notional,
												payRec,
												effectiveDate, 
												maturity, 
