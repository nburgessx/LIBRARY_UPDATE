# CLAUDE.md â€” AlgoQuantLib (AQ_LIB)
...
| Path | Access |
|---|---|
| `LIBRARY_UPDATE\AQ_LIB\` **and all subfolders** | **READ / WRITE** â€” this is the tree being rebranded |
| `REPO\.APPLES\` | **READ ONLY** â€” VS2017 branded original; the port source of truth |
| `REPO\.ALGO_QUANT_LIB\` | **READ ONLY** â€” manual backup of AQ_LIB + the external libs. Never write here. It is a safety copy, not a build tree. |
| `$(AQ_EXTERNAL_LIB_PATH)` (external dependency tree) | **READ ONLY** â€” except the two documented xlOil patches (Â§3) |
...
- Yield-curve framework calibrated on the **forward rate as the state variable**,
  ...
  to discount factors â€” removes forward-curve kinks and the arbitrage they imply.
- **Analytical risk via the Jacobian approach** â€” IR sensitivities without
...
```
AQ_LIB\
â”œâ”€â”€ AlgoQuantLib-VS22.sln                    the solution
â”œâ”€â”€ SetEnvVars.bat    one-time env-var setup (see Â§3.1)
â”œâ”€â”€ Visualizer.natvis             debug visualisers for Julian-day dates (keep current)
â”œâ”€â”€ readme.md                     library readme
â”œâ”€â”€ MIGRATION_PLAN.md             the rebrand / xlOil port plan â€” READ THIS
â”œâ”€â”€ projects\                     all .vcxproj / .filters / .user live here
â”œâ”€â”€ src\
â”‚   â”œâ”€â”€ math\                     LEGACY â€” deprecate / extract (Â§7)
â”‚   â”œâ”€â”€ models\                   LEGACY â€” near-empty (~1.3k lines)
â”‚   â”œâ”€â”€ calibration\              LEGACY â€” deprecate / extract (Â§7)
â”‚   â”œâ”€â”€ etrading\                 core pricing / analytics + LWO handle framework
â”‚   â”œâ”€â”€ validation\               single entry / contract layer (Â§4)  [namespace still validation_api]
â”‚   â”œâ”€â”€ AQ_API\                   SWIG bindings (was AQ_BINDINGS, renamed â€” Â§5.4). Python verified/supported; C#/Java/R shelved (nice-to-have)
â”‚   â”œâ”€â”€ AQ_XLL\                   xlOil Excel add-in â€” POC only, port pending (Â§6)
â”‚   â””â”€â”€ GTEST\              test suite, sees the whole library
â”œâ”€â”€ resources\                    end-user spreadsheets, toolkits, research guides
â””â”€â”€ targets\                      build output (git-ignored, regenerated)
```

### 2.1 Current state (2026-09-11)

**This section was stale for a long time â€” it used to say "the code rebrand
...
- **Toolchain migration: done.** VS2022/v143/C++17, static runtime, `/MP`;
  projects `validation_apiâ†’validation`, `MLIB_CLIENT_API/AQ_BINDINGSâ†’AQ_API`,
  `MLIBQ_ADDINâ†’AQ_XLL`; `ZEROMQ` dropped; external deps upgraded and routed
  through `$(AQ_EXTERNAL_LIB_PATH)`.
- **Phase 3 (identifier rebrand): done.** `validation_api`â†’`validation`,
  `me*`/`mir*`/`LA*`/`MA*`/`MB*` renamed per Â§6, `mir*` deleted wholesale,
  `LWO`â†’`AQObj`.
- **Phase 4 (xlOil XLL port): essentially complete.** `AQ_XLL` is no longer a
  proof of concept â€” **466 worksheet functions, covering 458 of 467
  ...
  have **zero `validation` wrappers today** â€” new design-and-build work, not
  a port. Full detail and the re-runnable gap-audit script: `STATUS.md` Â§2
  and `rebrand\STATUS.md`.
- **Phase 4a (editions): done (2026-09-15).** `AQ_XLL` ships five build
  configurations â€” `Release` (Full) plus `Release_XL_Bond`/
  `Release_XL_Swap`/`Release_XL_Credit`/`Release_XL_Curve` â€” each built from
  the `src\Core`/`src\Optional` file split (Â§4.4). The `AQ_API` runtime
  edition manifest once planned for this phase is **dropped, not
  deferred** â€” `AQ_API` ships one full binary per language, no edition
  concept there.
- **GoogleTest baseline confirmed green (2026-09-15).** Full suite re-run
  ...
  not a rebrand regression â€” pre-existing data staleness, unrelated to any
  rename or code change. Closes the deferral flagged in that entry.
- **Phase 5 (bindings) â€” Python is the supported binding; C#/Java/R shelved
  (decided, Nicholas 2026-09-15).** `AQ_BINDINGSâ†’AQ_API` rename is **done**
  (Â§5.4) and Python is verified end-to-end. **Testing C#, Java and R is
  shelved permanently, not just blocked** â€” no current requirement to use
...
- **âš  Reminder â€” Phase 6.11 (matrix/table-type consolidation), steps 1â€“3
  done (2026-09-20).** `AQLMatrix`â†’`AQLNumericMatrix` rename done, along
  with a review-driven correctness fix (`throw "Invalid Matrix"`), an
  efficiency fix (the four O(nÂ³) decomposition kernels no longer write
  ...
  scoped in detail (79 files / 268 occurrences â€” genuinely bigger than the
  ...
  rename) and then **deliberately paused at Nicholas's request** â€” zero
  files changed. `AQLBoolMatrix` (step 4) remains unstarted too, smaller
  and lower-risk. Full plan and staging in `MIGRATION_PLAN.md` Â§6.11. Don't
  let this quietly slip â€” see `rebrand\STATUS.md`'s "OPEN REMINDER" entry
  and the "Step 5 ... paused" entry for the up-to-date state.

### 2.2 Visualizer.natvis

Dates are Julian day numbers â€” unreadable in the debugger without this file.
...
- **Language standard:** `/std:c++17`. (There is no C++15 â€” VS2015 shipped C++14.)
- **Runtime:** static (`/MT`, `/MTd`). Every dependency â€” Boost, QuantLib, xlOil â€”
  must match. A `/MD` dependency links but misbehaves.
- **Parallelism:** `/MP` (`<MultiProcessorCompilation>true`) on every project. Do
  not remove it â€” it is the difference between a 1â€“2 minute and a 15 minute build.
- **Project settings live in the `.vcxproj` directly**, not in imported `.props`.
  Deliberate â€” one place. Do not "tidy" this into property sheets.
- **Linux MAKE/CMAKE build** exists but lags Windows. **In scope**, lower
  priority (late phase) â€” it must be re-validated *and* fully rebranded: the end
  state is that no file anywhere, Windows or Linux, carries a legacy name.

### 3.1 Environment variables

Set once by `SetEnvVars.bat` in this folder, then **restart Visual
Studio** (MSBuild reads the environment at launch). Used by project properties
*and* the binding build scripts â€” never hard-code these paths in either.
...
**Config folder resolution -- no environment variable.** `config` (calendars +
generator JSON, Â§4.3) always ships next to the built artefact
(`AlgoQuantLib.pyd` / `.xll` / `.dll`), so its location is resolved at
runtime from **the loaded module's own path**, not a working directory or an
env var -- a Python caller (e.g. Streamlit) gets the right config regardless
of where it was launched from. An explicit path passed to
`trySetupAQL`/`tryAqToolLoadCalendarFile`/`tryAqToolLoadStaticData` still
overrides this, same as today.

The script derives its defaults as folders *next to* `AQ_LIB`. If your external
libs live elsewhere (e.g. the backup tree), override at the prompt â€” the script
...
- **xlOil static build.** Needs `XLOIL_STATIC_LIB` + `SPDLOG_COMPILED_LIB`
  ...
  `xlOilHelpers` has no upstream static config â€” it was made by hand.
- **xlOil patch (not upstream â€” must survive any xlOil upgrade).**
...
- **AQ_XLL: every xlOil function must have exactly as many `.arg()` calls as the
  ...
  add-in fails to load â€” the XLL itself is a valid binary, so this looks like a
...
- **C++17 fixes already applied elsewhere and expected again in ports:**
  `_HAS_STD_BYTE` / `byte` ambiguity, `register` keyword, `auto_ptr`â†’`unique_ptr`,
  `Disposable` removal, `mem_fun_ref` / `bind2nd`â†’lambdas, and an ICE in
  `LAPriceCMSSpreadCalibration.cpp` needing `/Od` on that one file.

### 3.3 Dependency licences (the library is sold â€” this matters)

| Dependency | Version | Licence | Note |
|---|---|---|---|
| Boost | 1.91 | BSL-1.0 | permissive |
| QuantLib | 1.43 | modified BSD | permissive |
| xlOil | 0.19.0 | Apache-2.0 | **we ship a patched copy â€” Apache-2.0 requires marking modified files** |
| Eigen | (Eigen3) | MPL-2.0 | file-level copyleft. Linking is fine. **Never patch Eigen â€” wrap it.** |
...
licence text live only in the root `LICENSE` file â€” never in source-file or
console banners; an interim `LICENSE` placeholder goes in early, the final EULA
replaces `LICENSE` later with **zero source churn**. See `..\..\CLAUDE.md` Â§7
for the licence decision and Â§7.1 for the banner-content rule (no copyright, no
company/person names, no tool/migration references â€” technical help only,
...
- **`clang-format`** replaces the legacy AStyle utility. `.clang-format` at this
  ...
  the mass renames (plan Phase 6), never during â€” mid-rename reformatting buries
  the real diff.
- **`clang-tidy`** â€” `readability-identifier-naming` as a rebrand-completeness
...
## 4. Architecture â€” how code flows
...
```
   AQ_API (SWIG: Py/C#/Java/R)        AQ_XLL (xlOil add-in)        GTEST
              ...
              â”‚          validation          â”‚ â—„â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
              â”‚  single entry / contract     â”‚
              â”‚  `try` prefix                â”‚
        ...
        â”‚  LEGACY â€” deprecate / extract (Â§7)      â”‚
        â”‚  calibration â”‚ models â”‚ math           â”‚
        â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```

### 4.1 `validation` is the contract â€” and the GOLDEN SOURCE for names

Every public call â€” the Excel XLL and the Python / C# / Java / R bindings â€”
...
- To name or rename a function, name the `validation` wrapper first; the other
  surfaces follow.
- `tryAqDateYearFraction` â‡” `aqDateYearFraction` in the XLL, `aqDateYearFraction`
  in Python/C#/Java/R, `â€¦AqDateYearFractionâ€¦` in the test name.
- `docs\api_map.csv` and `rebrand\tools\api_pair_check.py` enforce the pairing.

(Today the wrappers are `tryMeâ€¦` in namespace `validation_api` â€” both change in
the rebrand.)
...
### 4.2 Naming carries meaning â€” preserve it
...
- **Project name â‰  output name.** All consumer artefacts are named
  **AlgoQuantLib** so users write `import AlgoQuantLib`:
  `AlgoQuantLib.pyd` / `AlgoQuantLib.xll` / `AlgoQuantLib.dll`.

### 4.3 The `config` folder â€” calendars & generators

`config` ships with **both** the `AQ_API` and `AQ_XLL` packages â€” the no-recompile
customisation surface:

- **Holiday / calendar files** (MarketWire / SwapsWire) â€” loaded at runtime;
...
- **Generators** â€” JSON **instrument & model static-data templates**. Static data
  ...
  conventions. Consumed by the object layer: generator + overrides â†’ handle.
...
### 4.4 Editions â€” DONE (2026-09-15), AQ_XLL-only
...
gated at compile time â€” there is no runtime edition gate, and the
...
- **`src\Core`** â€” files that must build into **every** edition
  ...
  renamed 2026-09-12 â€” Nicholas: `InterestRate`â†’`IR`), promoted 2026-09-12**:
  every priced product depends on discount/forward curves and fixing tables,
  so these two are infrastructure too, not a product edition of their own â€”
  see Â§5.1a for the category rename detail and `MIGRATION_PLAN.md` Â§4a.5 for
  why they moved out of `Optional`.
- **`src\Optional`** â€” one file per product category (`aqBond.cpp`,
  ...
  **Every new `AQ_XLL` category file is added to `src\Optional`, never
  `src\Core`** â€” unless it turns out to be cross-cutting infrastructure like
  `Curve`/`IR`, in which case flag it for `Core` rather than assuming.

**Five build configurations now exist** alongside
`Debug`/`Debug_EditAndContinue`/`Release_Profiler`: **`Release`** (the `Full`
edition â€” excludes nothing, every `src\Optional` file builds into it) and
**`Release_XL_Bond`**, **`Release_XL_Swap`**, **`Release_XL_Credit`**,
**`Release_XL_Curve`** â€” each compiles every `src\Core` file plus only the
...
alongside it; see
`rebrand\STATUS.md` for the manifest-generator tooling) â€” that is a finer
...
every category unconditionally â€” no per-language edition SKUs, since
building N native artefacts per language was the combinatorial blow-up
CLAUDE.md Â§5.2 always ruled out, and the runtime-gate alternative to avoid
...
`library prefix + Category + FunctionName` â€” `aq` + `Date` + `YearFraction`
â†’ **`aqDateYearFraction`**. Gives an IntelliSense-style grouped feel in Excel and
...
**Canonical category list â€” LOCKED (21):**
...
`Curve` is the rates yield-curve framework only â€” *not* bond or credit curves;
...
- **No `Option` category.** An option product is its own category â€”
...
- **`Ois` is NOT its own category (decided, Nicholas 2026-09-11) â€” it lives
  inside `Swap`.** Unlike `AssetSwap`/`CapFloor`/`Swaption` etc., an OIS is not
  a structurally distinct product â€” it is a vanilla swap whose floating leg
  ...
  needed) â€” see the form table below. Code-wise, `aqSwapOis*` lives in
  ...
  `tryAqOisParRate` wrappers (+ `*LVBKeys` companions) â€” which predated the
  current category scheme â€” were renamed to `tryAqSwapOisPV`/
  ...
  the detail.
- **`InterestRate` renamed to `IR`** (decided, Nicholas 2026-09-12) â€” shorter,
  ...
  â†’ `tryAqIR*`; `AQ_XLL`'s `aqInterestRate.cpp` â†’ `aqIR.cpp`, all 13 functions;
  `AQ_API`'s `aqInterestRateFixingTable*` bindings in `aqCurveObject.{h,cpp}`
  â†’ `aqIRFixingTable*`; the matching `GTEST` call sites and 3 fixture files).
  Detail: `rebrand\STATUS.md`.

Use these 21, identically in `validation` / `AQ_XLL` / `AQ_API` / `GTEST`.
Detail: `MIGRATION_PLAN.md` Â§2.2.
...
| `aq<Category><Variant><Function>` | a same-category **product variant** that doesn't earn its own top-level category â€” stateless (`<Variant>` before `Object`, no `Object` word) or, if ever needed, stateful (`<Variant>Object<Function>`) | `aqSwapOisPV`, `aqSwapOisParRate` (stateless OIS-swap forms; not `aqSwapObjectOis*`) |
...
### 5.1a Category-migration sequence â€” always in this order

Whenever a category (or a variant like `Ois`) is migrated or newly exposed, do
the four surfaces **in this order** â€” never start with `AQ_XLL`:

1. **`validation`** â€” confirm or rename the wrapper to the golden-source name
...
2. **`GTEST`** â€” update call sites, and file/suite names where they encode the
   old wrapper name, to match.
3. **`AQ_API`** â€” update the SWIG-bound method name (and source file name) in
   every language binding to match.
4. **`AQ_XLL`** â€” write or rename the worksheet function to the identical name
...
`validation â†’ AQ_API â†’ AQ_XLL â†’ GTEST`. Any drift found between the four
surfaces during a migration gets cleaned up as part of that migration, not
deferred â€” `validation` is always the tie-breaker.

**File-per-category is the default, not a hard rule.** `AQ_XLL` category files
are normally `aq<Category>.cpp` â€” but `BondOption` and `BondFutureOption`
...
instead of a bond spot price â€” there is no `aqBondFutureOptionObjectCreate`).
The category names themselves are unchanged â€” `BondOption` and
...
### 5.2 Two orthogonal groupings â€” do not conflate

- **Discoverability** = category prefix (`aqCurveâ€¦`, `aqSwapâ€¦`).
- **Gating** = shipped edition (Bonds / Swaps / Credit / Curves / Full),
  **`AQ_XLL`-only** (Â§4.4). Editions cut *across* categories, enforced at
  compile time via the `src\Core`/`src\Optional` filter split and the
  `Release_XL_*` build configurations â€” a native `.xll` per edition is cheap
  ...
  light by design â€” the library is not useful without the shipped examples
  and templates.

### 5.3 C++ style (do not "improve" this)

- Self-describing names: `spot_`, `strike_`, `vol_`, `timeToExpiry_`,
  `interestRate_` â€” never `s`, `k`, `v`, `t`, `r`, `x`, `z1`.
...
| `mir` (function prefix) | **deprecate â€” do not migrate** (produce call-graph first) |
| `LA`, `LB` ("Legacy Analytics" type/object prefixes) | **`AQL`** ("AQ Legacy") â€” marks legacy-to-deprecate, greppable vs new `AQ*` |
| `MA`, `MB` (type/object prefixes) | `AQ` â€” confirm per project (`math` had zero real ones) |
| `MLIB_*` macros | `AQ_*` |
| LWO (light-weight objects) â€” **C++ classes** | **`AQObj`** prefix: `AQObjCurve`, `AQObjUtilities`, `AQObjCurveDayAdjustment`, `AQObjHandleEnums`; screaming-snake macros take `AQOBJ_` (`AQOBJ_KEY`); free predicate `isLWOObject â†’ isAQObject`. |
| LWO â€” **public function names** (`meLWOâ€¦`) | `aq<Category>Object<Function>` â€” the `Object` word after the singular category is what separates the handle API from the stateless twin: `aqSwapObjectPV`, `aqBondObjectDirtyPrice`. Named sub-objects don't repeat it: `aqCurveMarketDataDisplay`. Generic lifecycle ops: `aqObjectLoad`, `aqObjectSave`, `aqObjectClearCache`. |
| `mir*` (whole stack: `AQ_API\mir*` 58 files, `validation\tryMir*` ~35, `LAXL.cpp` 156 fns) | **delete wholesale** â€” self-contained, no inbound `aq`/`me` deps (0.5 call-graph) |
| `msc*`, `LoanCalculations`, `SupervisoryRules`, `CashflowClient` + securitisation cluster | **delete â€” client-specific**; keep only what `Credit` genuinely needs (0.6 removal map) |
| project `AQ_BINDINGS` | **`AQ_API`** â€” **done**. `.vcxproj`/`.filters`/`.user`, `.sln`, folder, SWIG `.i`, the 8 `generate*`/`deploy*` batch files and pre/post-build `<Command>` lines all renamed. |
| `validation_api` (namespace) | `validation` |
| calendar holiday-centre delimiter `:` | **`+`** â€” one named constant + one helper; **accept `:` too during transition** unless a `:` parsing collision is found (plan Phase 0.8), then `+` only |
| Excel function names | **clean break** â€” every `me*`, hidden aliases included, becomes `aq*`; **no forwarding aliases**; removed names go in the release notes |
...
over **everything** â€” source, `projects\`, scripts, `resources\`, `examples\`,
docs, the Linux build â€” returns nothing but third-party headers and genuine
...
come, theme, parameter, MEMORY, gammaâ€¦*. A naive replace **will** corrupt the
codebase in ways that compile.

- **Never** run an unanchored, case-insensitive replace of `me`, `MA`, `MB`,
  `LA`, `LB`.
- Word-boundary, case-sensitive, prefix-anchored patterns only â€” e.g.
  `\bme(?=[A-Z])`.
- `MA`/`MB`/`LA`/`LB` anchored to a following uppercase letter **and** checked
  ...
  positives to skip.
- **`LA` â†’ `AQL`** (not `AQ`) â€” "Legacy Analytics" â†’ "AQ Legacy", keeps the
  legacy tree marked and greppable. `MA`/`MB` â†’ `AQ`.
- **Build the identifier list first (`rebrand\tools\prefix_census.py`), get
  sign-off, rename from the approved list.**
- Reviewable batches â€” one project or category at a time. Build between each.
- Never rename inside string literals, third-party headers, or `.APPLES`.
- `LWO` â†’ `AQObj`; predicate stays `isAQObject()`. Never write `AQObjbject` â€”
  when renaming `AQO`â†’`AQObj`, exclude tokens that already contain `AQObject`
  (`AQObjects`, `IsAQObject`, `isAQObject`).

### 5.6 Renaming Excel functions â€” clean break

`meDatesYearFraction` â†’ `aqDateYearFraction` turns every saved sheet into
`#NAME?`. **Decided:** clean break â€” no `me*` spelling survives, no forwarding
...
the existing object/handle framework underneath â€” do **not** use xlOil's cache.
...
- The int is a compact hash of the cell location â€” short numeric sheet handles,
...
`HandleEnums`, `LWOUtilities`, `StructuredExceptionHandler` â€” shells already
present in `src\etrading`), rebrand `LWOâ†’AQObj`, keep the behaviour exactly.

### 6.1 Port source

`.APPLES\APPLE\src\MLIBQ_ADDIN` â€” 40 files, ~82k lines, **653** exported Excel
...
and owns the SEH + exceptionâ†’string that `MLIB_START/END` did. **Only the `_Impl`
body carries across**, and it shrinks (`ExcelObj`/`ExcelArray` vs
`CXlOper`/`CXlStringArg`).

This is **not** a verbatim 653-function copy â€” it is the reference for *which*
...
keep/drop before porting. Most `getVectorOf<T>` / `populateExcelArrayWithâ€¦` /
...
`msc*` add-in files (structured credit, other client) â€” **do not port; delete.**

### 6.2 Porting order

Date â†’ Tool â†’ Curve â†’ Swap â†’ products â†’ Model. Each ported function needs a
`validation` wrapper (Â§4.1) **and** a GoogleTest case before it counts as done.

### 6.3 File organisation for editions â€” `src\Core` / `src\Optional`

Every `AQ_XLL` category `.cpp` under `projects\AQ_XLL.vcxproj.filters` sits in
one of two filters â€” see Â§4.4 for the full edition-gating design:

- **`src\Core`** â€” always-built infrastructure (`aqXllTools.cpp`,
  `aqMain.cpp`, `aqDate.cpp`, `aqObject.cpp`, `aqMath.cpp`, `aqTool.cpp`,
  `aqCurve.cpp`, `aqIR.cpp`).
- **`src\Optional`** â€” one file per product category. **New category files go
  here, not `Core`** â€” unless it's cross-cutting infrastructure every product
...
exist alongside `Debug`/`Debug_EditAndContinue`/`Release_Profiler`/`Release` â€”
...
## 7. Legacy â€” `calibration`, `math`, `models`

Direction: **deprecate as much as possible.** There is no `mapping` project â€” the
...
1. Build a usage map â€” what in `calibration`/`math`/`models` is reachable from
...
Do not delete outright â€” check what is needed first.
...
current build.** Renames are behaviour-preserving by definition â€” after each
...
failing on **stale holiday calendar data** â€” a pre-existing data-currency
issue (Â§4.3's calendar files need a refresh from MarketWire/SwapsWire), not a
...
- Yield-curve framework + Jacobian risk â€” test against analytic cases and against
  bump-and-revalue as the reference for the Jacobian.
- Fixed-income price/yield â€” the standing claim is Bloomberg-exact both ways;
  protect it with a regression test.
- Date logic â€” it is being extracted out of legacy.
...
- Anchored, case-sensitive patterns only (Â§5.5).
...
- **Always update `rebrand\STATUS.md` during this migration project.** Every time
  ...
  `STATUS.md` in the same session â€” what changed, what is verified, what is
...
- `MIGRATION_PLAN.md` â€” the phased plan; the authority on sequence and decisions.
- `..\..\CLAUDE.md` â€” repo-wide context and the rebrand rationale.
- `.APPLES\APPLE\APPLE_2017.sln` â€” the VS2017 original, port source.
- `readme.md` â€” end-user / new-developer entry point.