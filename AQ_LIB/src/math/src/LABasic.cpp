...
template <class T> class LAMath_sub {
public:
/*!
...
static double    wk;
static const double MINIMUM = LAMath::minValue(wk);
static const double EPSILON = LAMath::epsValue(wk);
...
gcf(double& gammcf, double a, double x, double& gln) 
{
    ...
    for (n = 1; n <= ITMAX;++n)
    {
        ...
        if (a1)
        {
            ...
            {
                gammcf =LAMath::exp(-x+a*LAMath::log(x)-gln)*g;
                return;
            }
            gold =g;
        }
...
gser(double& gamser, double a, double x, double& gln) 
{
    ...
    if (x <= 0.0) 
    {
        ...
        for (n=1;n<=ITMAX;n++) {
            ...
            del *= x/ap;
            sum += del;
            if (LAMath::abs(del) < LAMath::abs(sum)*EPSILON) {
                gamser=sum*LAMath::exp(-x+a*LAMath::log(x)-gln);
                return;
            }
        }
        { //error 
...
betacf(double a, double b, double x) 
{
    ...
    if (LAMath::abs(d) < MINIMUM) d=MINIMUM;
    d=(double)1.0/d;
    h=d;
    for (m=1;m<=ITMAX;m++) 
    {
        ...
        if (LAMath::abs(d) < MINIMUM) d=MINIMUM;
        c=(double)1.0+aa/c;
        if (LAMath::abs(c) < MINIMUM) c=MINIMUM;
        d=(double)1.0/d;
        h *= d*c;
        ...
        del=d*c;
        h *= del;
        if (LAMath::abs(del-(double)1.0) < EPSILON) break;
    }
    if (m > ITMAX) 
    {
        char_t  msg[128];
...
*/
double 
LAMath::minValue(double& val) 
{
    static double min;
    return (min == 0.0) ? (val = LAMath_sub<double>().minVal(min)) 
                            : (val = min);
}
/*!
...
*/
double 
LAMath::epsValue(double& val) 
{
    static double eps;
    return (eps == 0.0) ? (val = LAMath_sub<double>().epsilon(eps)) 
                            : (val = eps);
}
...
*/
double
LAMath::dividedDiff(const double* x, const double* y, unsigned int m)
{
    if (m == 0)
    {
        return y[0];
...
*/
double 
LAMath::tpf(double x1, double x2, int m)
{
...
*/
double 
LAMath::pythag(double a, double b)
{
    double absa, absb;
    absa=(double)::fabs(a);
    absb=(double)::fabs(b);
...
*/
double          
LAMath::bico(unsigned int n, unsigned int k)
{
...
*/
double 
LAMath::gammp(double a, double x)
{
    if (x < 0.0 || a <= 0.0) 
    {
        char_t  msg[128];
...
*/
double 
LAMath::betai(double a, double b, double x)
{
    double bt;
