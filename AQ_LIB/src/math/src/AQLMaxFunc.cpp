...
*/
AQLCoreFunctionBase*	
AQLMaxMethod::clone() const
{
    try 
	{
		return new AQLMaxMethod(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
AQLMaxMethod::isTypeOf(function_t id) const
{
	return (id == FN_MAX ? true : AQLFunctionBase::isTypeOf(id));
}
...
*/
double
AQLMaxMethod::operator()(const DoubleArray& x) const
{
...
*/
double
AQLMaxMethod::partialDerivative(const DoubleArray& x, unsigned int pos,
...
*/
double
AQLMaxMethod::partialDerivative2(const DoubleArray& x, unsigned int posi, unsigned int posj,
...
*/
bool
AQLMaxMethod::isDifferentiable(const DoubleArray& x, unsigned int pos) 
const
{
	DoubleArray::const_iterator it = max_element(x.begin(), x.end());
	unsigned int i = static_cast<unsigned int>(it - x.begin());
...
*/	
bool
AQLMaxMethod::isDifferentiable(const DoubleArray& x, unsigned int posi, unsigned int posj) 
const
{
