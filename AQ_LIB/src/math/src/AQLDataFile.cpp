...
MDFile::setItem(
    ...
    unsigned int from, 
    unsigned int to, 
    const AQLString &item)
{
    if (mRead) // if the read-only, throw an exception.
    {
        AQLString    msg("File Open Mode Error"); 
        throw AQLCoreInvalidData(msg.getCString(), __FILE__, __LINE__);
    }
    mModified = true;
    if(item.size() != to-from+1)
    {
        throw AQLCoreInvalidData(item.getCString(), __FILE__, __LINE__);
    }
    ...
    const AQLString& item)
{
        ...
        ++j)
    {
        ...
        if (str[j] == '\0') 
        {
            throw AQLCoreInvalidData(str, __FILE__, __LINE__);
        }
    }
    // return NULL when it is not found
    if (delCount < num) 
    {
        AQLString msg = "Invalid index : ";
        msg += AQLString(int(num));
        throw AQLCoreInvalidData(msg.getCString(), __FILE__, __LINE__);
    }
    if (i != j)
    {
...
MDFile::init()
{
    ...
    if (mpFile == NULL) 
    {
        AQLString msg = "File Open Error for:";
        msg+=mFileName;
        AQLCoreSystemError e(msg.getCString(),__FILE__, __LINE__);
        e+=AQLCoreSystemError(__FILE__, __LINE__);
        throw e;
    }
    for(;!feof(mpFile);)
    {
