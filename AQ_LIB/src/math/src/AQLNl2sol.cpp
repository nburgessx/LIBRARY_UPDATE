...
NL2SOL::NL2SOL(AQLFunctionVector & function) :
m_function(function),
m_f(m_function.lengthOfFunctionVector(), 0.0),
m_x(m_function.lengthOfArgumentVector(), 0.0),
//m_j((int)(m_function.lengthOfFunctionVector2()), (int)(m_function.lengthOfArgumentVector())),
// m_j(m_function.lengthOfFunctionVector2()),
m_lv(105+m_function.lengthOfArgumentVector()*( m_function.lengthOfFunctionVector()+2*m_function.lengthOfArgumentVector()+17)+4*m_function.lengthOfFunctionVector()),
m_liv(80+m_function.lengthOfArgumentVector()),
m_workspace((int)m_lv, 0.0),
m_iv((int)m_liv,(int)0)
{
	int nFunc = m_function.lengthOfFunctionVector();
	int nArg = m_function.lengthOfFunctionVector();

    m_j.resize( nFunc, std::vector<double>() );
	for ( int i=0; i<nFunc; ++i)
	{
		m_j[i].resize( nArg, 0.0 );
	}
};

template <class T> void fill( std::vector< T > & v, const T & x ){ 
...
void NL2SOL::calcJ(int *n, int *p, double *x, int *withinbounds, double *jacobian)
{
   ASSERT(static_cast<unsigned long>(*p) == m_function.lengthOfArgumentVector());
   ASSERT(static_cast<unsigned long>(*n) == m_function.lengthOfFunctionVector());
   ASSERT(&(m_x[0]) == x);
   if (m_function.constraintsAreViolated(m_x)) {
       *withinbounds=0;   //   This indicates out of bounds.
       return;
   }

   unsigned long jindex = 0, i, j;

   for (i=0; i<m_j.size(); i++)
	   fill(m_j[i], 0.0);

   m_function(m_j,m_x);

   for (j=0; j<m_j[0].size(); j++)
	   for (i=0; i<m_j.size(); i++)
			jacobian[jindex++] = m_j[i][j];
}
...
long
NL2SOL::tryToSolve(std::vector<double> &x, double *errormeasure){

   AQLString msg;
   const double tolx = m_function.argumentTolerance(), tolf = m_function.functionTolerance();
   const unsigned long ntrial = m_function.maximumNumberOfIterations();
   ...
   int p = static_cast<int>(m_function.lengthOfArgumentVector());
   int n = static_cast<int>(m_function.lengthOfFunctionVector());

   ASSERT(x.size() == p);
   long k = 0;
   m_workspace[35]  = sqrt(DBL_EPSILON);					//   Finite differencing distance (in the vols) for the Jacobian.
   ...
   m_workspace[30]  = 0.5*tolf*tolf;                        //   This actually controls absolute tolerance but this will do.
   ...
   m_x = x;                                                 //   Now call the ACM-TOMS-573 algorithm.
   m_workspace[9] = ieee::NaN;                              //   Preset the function value to something indicating that no evaluation has occurred yet.
   //nl2sno(&n, &p, &m_x[0], &m_iv[0], &m_workspace[0]);
   nl2sno(&n, &p, &m_x[0], &m_iv[0], &m_workspace[0]);
   x = m_x;
   k = m_iv[30];
   //
   ...
   case 8:
       ...
       optionalDebugOutput("Warning: The ACM-TOMS-573 algorithm (nl2sno) came to a false convergence halt.\n");
       break;
   case 9:
#ifndef NDEBUG
       optionalDebugOutput("Warning: The ACM-TOMS-573 algorithm (nl2sno) reached the function evaluation limit without other convergence.\n");
#endif
       break;
   case 10:
#ifndef NDEBUG
       optionalDebugOutput("Warning: The ACM-TOMS-573 algorithm (nl2sno) reached the iteration limit without other convergence.\n");
#endif
       k++;
       break;
   case 11:   // Interrupted by user or stopNow() was flagged by the functor.
       k = -k;
       break;
   case 13:
       msg = "Invalid starting coordinates passed on to NL2SOL::tryToSolve()";
	   throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);
       break;
   case 15:
       msg = "Unable to calculate Jacobian in NL2SOL::tryToSolve()";
	   throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);
       break;
   case 16:
       if (p>n)
           msg = "The number of arguments must not be larger than the number of errors to minimise in NL2SOL::tryToSolve()";
	       throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);
   default:;
   }
   if (m_iv[0] > 11){   //   A serious error from the NL2SOL algorithm ?
		msg = "The ACM-TOMS-573 algorithm (nl2sno) returned error value ";
		msg += AQLString(m_iv[0]);
		msg += " in NL2SOL::tryToSolve() which is almost certainly a programming error";
		throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);
   }
   if (errormeasure)
       *errormeasure = AQLMath::sqrt(2*m_workspace[9]);
   return k;
}

long
NL2SOL::tryToSolve2(std::vector<double> &x, double *errormeasure){
   ...
   nl2sol(&n, &p, &m_x[0], &m_iv[0], &m_workspace[0]);
   //nl2sno(&n, &p, &m_x[0], &m_iv[0], &m_workspace[0]);
   x = m_x;
   k = m_iv[30];
   //
   ...
   if (m_iv[0] > 11){   //   A serious error from the NL2SOL algorithm ?
		AQLString msg = "The ACM-TOMS-573 algorithm (nl2sno) returned error value ";
		msg += AQLString(m_iv[0]);
		msg += " in NL2SOL::tryToSolve() which is almost certainly a programming error";
		throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);
   }
   if (errormeasure)
       *errormeasure = sqrt(2*m_workspace[9]);
   return k;
}

void
NL2SOL::calcR(int *n, int *p, double *x, int *withinbounds, double *residuals)
{
   ASSERT(static_cast<unsigned long>(*p) == m_function.lengthOfArgumentVector());
   ASSERT(static_cast<unsigned long>(*n) == m_function.lengthOfFunctionVector());
   ASSERT(&(m_x[0]) == x);
   ...
   m_function(m_f,m_x);
   // std::copy(m_f.begin(),m_f.end(),residuals);
   int nSize = m_f.size();
...
int
NL2SOL::nl2sol_defaults(int *iv, double *v)
{
   /* Initialized data */
...
NL2SOL::rmdcon(int k)
{
   ...
   //  ***        k = 2... square dataInstance of 1.001*eta. 
   //  ***        k = 3... unit roundoff = smallest pos. no. machep such 
   //  ***                 that 1 + machep .gt. 1 .and. 1 - machep .lt. 1. 
   //  ***        k = 4... square dataInstance of 0.999*machep. 
   //  ***        k = 5... square dataInstance of 0.999*big (see k = 6). 
...
int
NL2SOL::nl2sol(int *n, int *p, double *x, int *iv, double *v)
//NL2SOL::nl2sol(int *n, int *p, double *x, S_fp calcj, int *iv, double *v, int *uiparm, double *urparm, i_fp ufparm)
{
   /* Initialized data */
   ...
   //                  printing and limit the number of iterations and method- 
   ...
   //             iteration and function evaluation counts, current method- 
   ...
   //             and if this step yields at most twice the predicted method- 
   ...
   //             possible according to the current model, i.e., the method- 
...
L10:
   nf = iv[nfcall];
   calcR(n, p, &x[1], &nf, &v[r1]);
   if (m_function.stopNow()){
       iv[1] = 11;
       goto L900;
   }
   if (strted) {
...
L30:
//   (*calcj)(n, p, &x[1], &iv[nfgcal], &v[j_1], &uiparm[1], &urparm[1], reinterpret_cast<i_fp>(ufparm));
   calcJ(n, p, &x[1], &iv[nfgcal], &v[j_1]);
   if (iv[nfgcal] == 0) {
       goto L50;
   }
   strted = true;
...
L40:
   nl2itr(&v[d1], &iv[1], &v[j_1], n, n, p, &v[r1], &v[1], &x[1]);
   if (m_function.stopNow()){
       iv[1] = 11;
       goto L900;
   }
...
int
NL2SOL::nl2sno(int *n, int *p, double *x, int *iv, double *v)
{
   /* Initialized data */
...
int
NL2SOL::nl2itr(double *d_, int *iv, double *j, int *n, int *nn, int *p, double *r_, double *v, double *x)
{
   /* Initialized data */
...
L190:
   if (! stopx(m_function.stopNow())) {
       goto L200;
   }
   iv[1] = 11;
   goto L205;
   
   //     ***  come here when restarting after method. eval. limit or stopx. 
...
L200:
   ...
   iv[1] = 9;
L205:
   if (m_function.stopNow() || v[f] >= v[f0]) {
       goto L900;
   }
...
L230:
   g1 = iv[g];
   lmstep(&d_[1], &v[g1], &iv[ierr], &iv[ipivot], &iv[kalm], p, &v[qtr1], &
       v[h1], &v[step1], &v[1], &v[w1]);
   goto L310;
...
L300:
   ...
   dig1 = iv[dig];
   lmat1 = iv[lmat];
   gqtstp(&d_[1], &v[dig1], &v[h1], &iv[kagqt], &v[lmat1], p, &v[step1], &
       v[1], &v[w1]);
...
L700:
   ...
   iv[mode] = 0;
L710:
   covclc(&i_, &d_[1], &iv[1], &j[j_offset], n, nn, p, &r_[1], &v[1], &x[
       1]);
   switch (static_cast<int>(i_)) {
   case 1:  goto L720;
   case 2:  goto L720;
...
int
NL2SOL::assess(double *d_, int *iv, int *p, double *step, double *stlstg, double *v, double *x, double *x0)
{
   /* Initialized data */
   ...
   //      v(f) (i/o) on both input and output, v(f) is the objective method- 
...
L130:
   ...
   if (emax < v[gtstep]) {
       ...
       v[radfac] = rfac1 * AQLMath::max(d_1,d_2);
   }
...
int
NL2SOL::covclc(int *covirc, double *d_, int *iv, double *j, int *n, int *nn, int *p, double *r_, double *v, double *x)
{
   /* Initialized data */
   ...
   //  ***  hessian h is computed (using method. and grad. values if 
   //  ***  iv(covreq) is nonnegative, and using only method. values if 
...
L80:
   ...
   //    del = v[delta0] * max(d_2,d_3);
   //
   del = v[delta0] * AQLMath::max(one/d_[m],fabs(x[m]));
...
L105:
   ...
   //
   // PJ:  BUG ALERT !  BUG ALERT !  BUG ALERT ! 
   //
   //
...
L180:
   ...
   //
   //    d_2 = one / d_[m], d_3 = (d_1 = x[m], abs(d_1));
   //    del = v[dltfdc] * max(d_2,d_3);
   //
   del = v[dltfdc] * AQLMath::max(one/d_[m],fabs(x[m]));
   if (x[m] < zero) {
       del = -del;
   }
   v[xmsave] = x[m];
...
double
NL2SOL::dotprd(int *p, double *x, double *y)
{
   /* Initialized data */
   ...
   for (i_ = 1; i_ <= i_1; ++i_) {
       ...
       t = AQLMath::max(fabs(x[i_]),fabs(y[i_]));
       if (t > one) {
           goto L10;
       }
       if (t < sqteta) {
...
int
NL2SOL::dupdat(double *d_, int *iv, double *j, int *n, int *nn, int *p, double *v)
{
   /* Initialized data */
...
L20:
   ...
   for (i_ = 1; i_ <= i_1; ++i_) {
       ...
       if (t < v[jtoli]) {
           ...
           t = AQLMath::max(d_1,d_2);
       }
       // Computing MAX 
       d_1 = vdfac * d_[i_];
       d_[i_] = AQLMath::max(d_1,t);
   }
...
int
NL2SOL::gqtstp(double *d_, double *dig, double *dihdi, int *ka, double *l, int *p, double *step, double *v, double *w)
{
   /* Initialized data */
...
L190:
   ...
   //    lk = max(d_1,d_2);
   lk = AQLMath::max(lk,v[dgnorm] / rad - w[emax]);
   ...
   //    lk = max(d_1,d_2);
   lk = AQLMath::max(lk,phi * w[phipin]);
...
L210:
   ...
   if (-v[dst0] >= alphak || alphak < lk || alphak >= uk) {
       ...
       //        alphak = uk * max(d_1,d_2);
       alphak = uk * AQLMath::max(p001, sqrt(lk / uk));
   }
   k = 0;
   i_1 = *p;
   for (i_ = 1; i_ <= i_1; ++i_) {
...
L260:
   ...
   if (phi < zero) {
       uk = AQLMath::min(uk,alphak);
   }
   alphak += phi / t1 * (dst / t1) * (dst / rad);
   lk = AQLMath::max(lk,alphak);
   goto L210;
...
L330:
   ...
   //    d_1 = lk, d_2 = t - w[emax];
   //    lk = max(d_1,d_2);
   lk = AQLMath::max(lk,t - w[emax]);
   if (v[dst0] > zero) {
       // Computing MAX 
       //
       //        d_1 = lk, d_2 = (v[dst0] - rad) * w[phipin];
       //        lk = max(d_1,d_2);
       lk = AQLMath::max(lk,(v[dst0] - rad) * w[phipin]);
   }
   goto L260;
...
L340:
   ...
   if (alphak > zero) {
       // Computing MIN 
       d_1 = uk, d_2 = w[uk0];
       uk = AQLMath::min(d_1,d_2);
   }
   // Computing MAX 
   d_1 = zero, d_2 = -v[dst0], d_1 = AQLMath::max(d_1,d_2), d_2 = t - w[emax];
   lk = AQLMath::max(d_1,d_2);
   if (v[dst0] > zero) {
       // Computing MAX 
       d_1 = lk, d_2 = (v[dst0] - rad) * w[phipin];
       lk = AQLMath::max(d_1,d_2);
...
L390:
   ...
   sw = dotprd(p, &w[q], &w[1]);
   t1 = (rad + dst) * (rad - dst);
   dataInstance = sqrt(sw * sw + t1);
   if (sw < zero) {
       dataInstance = -dataInstance;
   }
   si = t1 / (sw + dataInstance);
...
int
NL2SOL::linvrt(int *n, double *lin, double *l)
{
   /* Initialized data */
...
int
NL2SOL::litvmu(int *n, double *x, double *l, double *y)
{
   /* Initialized data */
...
int
NL2SOL::livmul(int *n, double *x, double *l, double *y)
{
   /* Initialized data */
...
int
NL2SOL::lmstep(double *d_, double *g, int *ierr, int *ipivot, int *ka, int *p, double *qtr, double *r_, double *step, double *v, double *w)
{
   /* Initialized data */
...
int
NL2SOL::lsqrt(int n1, int *n, double *l, double *a, int *irc)
{
   /* Initialized data */
...
double
NL2SOL::lsvmin(int *p, double *l, double *x, double *y)
{
   /* Initialized data */
   ...
   for (jjj = 1; jjj <= i_1; ++jjj) {
       ...
       ix = ix * 3432 % 9973;
       b = half * (one + static_cast<float>(ix) / r9973);
       xplus = b - x[j];
       xminus = -b - x[j];
       splus = fabs(xplus);
       sminus = fabs(xminus);
...
int
NL2SOL::ltsqar(int *n, double *a, double *l)
{
   /* System generated locals */
   int i_1, i_2, i_3;
...
int
NL2SOL::parchk(int *iv, int *n, int *nn, int *p, double *v)
{
   /* Initialized data */
...
int
NL2SOL::qapply(int *nn, int *n, int *p, double *j, double *r_, int *ierr)
{
   /* System generated locals */
   int j_dim1, j_offset, i_1, i_2;
...
int
NL2SOL::qrfact(int *nm, int *m, int *n, double *qr, double *alpha, int *ipivot, int *ierr, int nopivk, double *sum)
{
   /* Initialized data */
   ...
   //     ..........  rktol is the square dataInstance of the relative precision 
...
double
NL2SOL::reldst(int *p, double *d_, double *x, double *x0)
{
   /* Initialized data */
...
int
NL2SOL::rptmul(int method, int *ipivot, double *j, int *nn, int *p, double *rd, double *x, double *y, double *z_)
{
   /* System generated locals */
   int j_dim1, j_offset, i_1, i_2;
   ...
   //  ***  method = 1... set  y = rmat * (perm**t) * x. 
   //  ***  method = 2... set  y = perm * (rmat**t) * rmat * (perm**t) * x. 
   //  ***  method = 3... set  y = perm * (rmat**t) x. 
...
int
NL2SOL::slupdt(double *a, double *cosmin, int *p, double *size, double *step, double *u, double *w, double *wchmtd, double *wscale, double *y)
{
   /* Initialized data */
   ...
   *wscale = one;
   if (denmin != zero) {
       ...
       //        *wscale = min(d_2,d_3);
       *wscale = AQLMath::min(one,fabs(sdotwm / denmin));
   }
   t = zero;
   if (sdotwm != zero) {
       t = *wscale / sdotwm;
...
int
NL2SOL::slvmul(int *p, double *y, double *s, double *x)
{
   /* System generated locals */
   int i_1, i_2;
...
NL2SOL::stopx(bool stopNow) {
   ...
   return stopNow || Abort::abortHasBeenRequested();
} /* stopx */

int
NL2SOL::vaxpy(int *p, double *w, double a, double *x, double *y)
{
   /* System generated locals */
   int i_1;
...
int
NL2SOL::vcopy(int p, double *y, double *x)
{
   /* System generated locals */
   int i_1;
...
int
NL2SOL::vscopy(int *p, double *y, double s)
{
   /* System generated locals */
   int i_1;
...
L40:
   ...
   //     ***  sqteta is (slightly larger than) the square dataInstance of the 
