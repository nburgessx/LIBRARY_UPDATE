...
AQLProductDistribution::AQLProductDistribution(double rho_)
...
AQLProductDistribution::AQLProductDistribution(double meanX_, double meanY_, double stDevX_, double stDevY_, double rho_)
: AQLBivariateDistribution(meanX_, meanY_, stDevX_, stDevY_)
{
    mRho = rho_;
    mUnivariateX = 0;
    mUnivariateY = 0;
...
AQLProductDistribution::~AQLProductDistribution()
{
    delete mUnivariateX; mUnivariateX = 0;
    delete mUnivariateY; mUnivariateY = 0;
}

double AQLProductDistribution::Cumulative(double x, double y)
{
    ...
    {
        if (mUnivariateY == 0)
            throw AQLCoreInvalidData("Univariate sub-distribution not defined in product distribution", __FILE__, __LINE__);
        else
...
double AQLProductDistribution::TrueBivariateCumulative(double x, double y)
{
    return CumulativeByIntegration(x, y); // By default we do the 1D numerical integral
}

double AQLProductDistribution::CumulativeByIntegration(double x, double y)
{
    double lwBound = -7.0;
    if (y < lwBound)
        return 0.0;
    ...
    {
        AQLFunction* f = BivariateIntegrand(x);
        ...
        else
        {
            ...
            AQLGaussLegendre integrator(32);
