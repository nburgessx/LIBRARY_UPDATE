...
////X///////////////////X///////////////////////////////X///////////////////
//  NAME        :       AQLLinearMethod.cpp
//
//  SYNOPSIS    :       AQLLinearMethod
...
*/
AQLLinearMethod::AQLLinearMethod() 
: AQLFunctionBase()
{
...
*/
AQLCoreFunctionBase*	
AQLLinearMethod::clone() const
{
    try 
	{
		return new AQLLinearMethod(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
AQLLinearMethod::isTypeOf(function_t id) const
{
	return (id == FN_LINEAR ? true : AQLFunctionBase::isTypeOf(id));
}
...
*/
double
AQLLinearMethod::operator()(const DoubleArray& x) const
{
	if (x.size() + 1 != mParam.size())
	{
		throw AQLCoreInvalidData("input data size must be parameter size minus one", __FILE__, __LINE__);
	}
...
*/
double
AQLLinearMethod::integral(const std::vector<std::pair<double,double> >& x)const
{
	if (x.size() + 1 != mParam.size())
	{
		throw AQLCoreInvalidData("input data size must be parameter size minus one", __FILE__, __LINE__);
...
*/
double
AQLLinearMethod::partialDerivative(const DoubleArray& x, unsigned int pos,
...
*/
double
AQLLinearMethod::partialDerivative2(const DoubleArray& x, unsigned int posi, unsigned int posj,
