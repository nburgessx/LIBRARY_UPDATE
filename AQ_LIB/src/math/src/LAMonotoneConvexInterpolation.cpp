// LAMonotoneConvexInterpolation.cpp
...
/*!
    @brief constructor
*/
LAMonotoneConvexInterpolation::LAMonotoneConvexInterpolation()
: LAInterpolationBase()
{
	mpDataProvider = NULL;
...
*/
LAMonotoneConvexInterpolation::LAMonotoneConvexInterpolation(const double dLambda, const bool dIsAllowedNegative)
{
	mpDataProvider=NULL;
	lambda = dLambda;
...
*/
LAMonotoneConvexInterpolation::LAMonotoneConvexInterpolation(const LAMonotoneConvexInterpolation& v)
: LAInterpolationBase(v), mpDataProvider(NULL)
{
    try 
	{	
		mpDataProvider = new LAMonotoneConvexInterpolationDataProvider(*v.mpDataProvider);
...
*/
LACoreFunctionBase*	
LAMonotoneConvexInterpolation::clone() const
{
    try 
	{
		LAMonotoneConvexInterpolation* pTmp;
		pTmp =new LAMonotoneConvexInterpolation(lambda, isAllowedNegative, inputMode, outputMode);

		if (mpDataProvider != NULL)
		{
			pTmp->mpDataProvider = new LAMonotoneConvexInterpolationDataProvider(this->mpDataProvider->index, 
...
*/
bool
LAMonotoneConvexInterpolation::isTypeOf(function_t id) const
{
	return (id == FN_MONOTONECONVEXINTERPOLATION ? true : LAInterpolationBase::isTypeOf(id));
}
...
*/
function_t
LAMonotoneConvexInterpolation::getType() const
{
	return FN_MONOTONECONVEXINTERPOLATION;
}

int
LAMonotoneConvexInterpolation::getInputMode(void) const
{
	return inputMode;
}

int
LAMonotoneConvexInterpolation::getOutputMode(void) const
{
	return outputMode;
}

void
LAMonotoneConvexInterpolation::setInputMode(const int dInputMode)
{
	inputMode = dInputMode;	return;
}

void
LAMonotoneConvexInterpolation::setOutputMode(const int dOutputMode)
{
	outputMode = dOutputMode;
	return;
}
...
*/
void
LAMonotoneConvexInterpolation::set(const DoubleArray& index, const DoubleArray& value)
{
	...
	case INPUTGENERAL://general
		...
		break;
	default:
		throw LACoreInvalidData("the input mode is not supported", __FILE__, __LINE__);
	}
	...
	else
	{
		...
		//(79) ameliorate f0
		if (LAMath::abs(f[0] - fdiscrete[0]) > 0.5 * LAMath::abs(f[1] - fdiscrete[0])) 
			f[0] = fdiscrete[1] - 0.5 * (f[1] - fdiscrete[0]);
		//(80) ameliorate fn
		if (LAMath::abs(f[n] - fdiscrete[n]) > 0.5 * LAMath::abs(f[n - 1] - fdiscrete[n])) 
			f[n] = fdiscrete[n] - 0.5 * (f[n - 1] - fdiscrete[n]);
	}
...
*/
double
LAMonotoneConvexInterpolation::value(const double x1) const
{
	if (mpDataProvider == NULL)
	{
		throw LACoreInvalidData("#Error: Monotone Convex interpolation data has not been set", __FILE__, __LINE__);
	}
	...
	default:
		throw LACoreInvalidData("the output mode is not supported", __FILE__, __LINE__);
	}
} 
...
*/
double
LAMonotoneConvexInterpolation::forward(const double x1) const
{
	//numbering refers to Wilmott paper
...
*/
double
LAMonotoneConvexInterpolation::interpolant(const double x1) const
{
	//numbering refers to Wilmott paper
	int size = mpDataProvider->size;
	double ret = 0.0;
...
*/
double
LAMonotoneConvexInterpolation::interpolantGeneral(const double x1) const
{	
	//numbering refers to Wilmott paper	
	int size = mpDataProvider->size;
	double ret = 0.0;
...
*/
double
LAMonotoneConvexInterpolation::analyzeInterpolant(const double x1) const
{
	int size = mpDataProvider->size;
	double ret = 0.0;
	if (x1 <= 0) 
...
std::tuple<std::vector<double>,std::vector<double>> LAMonotoneConvexInterpolation::getXY() const
{
