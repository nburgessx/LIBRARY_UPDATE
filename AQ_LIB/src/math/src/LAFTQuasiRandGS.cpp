...
////X///////////////////X///////////////////////////////X///////////////////
//  NAME        :       LAFTQuasiRandGS.cpp
//
//  SYNOPSIS    :       LAFTQuasiRandGS
//  DESCRIPTION :
//
//
//
...
#include "LAFTQuasiRandGS.h"
unsigned long LAFTQuasiRandGS::mIrreduciblePoly[1500]  = {	
...
: LARandBase(),mSeedSet(false),mDimSet(false),mMax(0)
{
...
*/
LACoreFunctionBase*
LAFTQuasiRandGS::clone() const
{
    try
	{
		return new LAFTQuasiRandGS(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
LAFTQuasiRandGS::isTypeOf(function_t id) const
{
	return (id == FN_RAND_QUASIGS ? true : LARandBase::isTypeOf(id));
}
...
*/
void
LAFTQuasiRandGS::setSeed(const UlongArray& _seed)
{
	UlongArray seed(4);
	if (_seed.size() == 2)
	{
...
LAFTQuasiRandGS::getEG(unsigned long IPoly,unsigned int randShift)
{
	...
	elementGeneratorMtrx=(unsigned long)(elementGeneratorMtrx)&((LONG_Integer)LAMath::pow(2,mMaxDigit)-1);
...
*/
void
LAFTQuasiRandGS::ftsrand (unsigned long seed)
{
