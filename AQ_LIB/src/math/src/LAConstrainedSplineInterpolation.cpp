// LAConstrainedSplineInterpolation.cpp
...
/*!
    @brief constructor
*/
LAConstrainedSplineInterpolation::LAConstrainedSplineInterpolation()
: LAInterpolationBase()
{
	mpDataProvider=NULL;
...
*/
LAConstrainedSplineInterpolation::LAConstrainedSplineInterpolation(const LAConstrainedSplineInterpolation& v)
: LAInterpolationBase(v), mpDataProvider(NULL)
{
    try 
	{	
		mpDataProvider = new LAConstrainedSplineInterpolationDataProvider(*v.mpDataProvider);
...
*/
LACoreFunctionBase*	
LAConstrainedSplineInterpolation::clone() const
{
    try 
	{
		LAConstrainedSplineInterpolation* pTmp;
		pTmp =new LAConstrainedSplineInterpolation;

		if (mpDataProvider != NULL)
		{
			pTmp->mpDataProvider = new LAConstrainedSplineInterpolationDataProvider(this->mpDataProvider->index ,this->mpDataProvider->value, this->mpDataProvider->a,
...
//===================	LAConstrainedSplineInterpolation.cpp20051014(1)  end  =======================//
}
...
*/
bool
LAConstrainedSplineInterpolation::isTypeOf(function_t id) const
{
	return (id == FN_CONSTRAINEDSPLINEINTERPOLATION ? true : LAInterpolationBase::isTypeOf(id));
}
...
*/
function_t
LAConstrainedSplineInterpolation::getType() const
{
...
LAConstrainedSplineInterpolation::value(const double x1) const
{
	if (mpDataProvider == NULL)
	{
		throw LACoreInvalidData("#Error: Constrained Spline interpolation data has not been set", __FILE__, __LINE__);
	}
	...
	*/
void
LAConstrainedSplineInterpolation::set(const DoubleArray& index, const DoubleArray& value)
{
	...
	{
		throw LACoreInvalidData("index size and value size are not same", __FILE__, __LINE__);
	}
	if (index.size() == 0)
	{
		throw LACoreInvalidData("data size is zero", __FILE__, __LINE__);
	}
    ...
    for (i = 0; i <=  Nmax; i++)
    {
        ...
        else if ( ( LAMath::abs(value[i + 1] - value[i] ) < D_ZERO) || ( LAMath::abs(value[i] - value[i - 1]) < D_ZERO) )
        {
            //Only check for 0 dy. dx assumed NEVER equals 0 !
            gxx[i] = 0.0;
        }
        else if ( LAMath::abs( (index[i + 1] - index[i]) / (value[i + 1] - value[i]) + (index[i] - index[i - 1]) / (value[i] - value[i - 1]) ) < D_ZERO )
        {
        ...
        else
        {
            ...
            gxx[i] = 2.0 / (  LAConstrainedSplineInterpolation::calc_dxx(index[i + 1], index[i]) / (value[i + 1] - value[i]) + LAConstrainedSplineInterpolation::calc_dxx(index[i], index[i - 1]) / (value[i] - value[i - 1])  );
        }
    }
    ...
    gxx[0] = 3.0 / 2 * (value[1] - value[0]) / LAConstrainedSplineInterpolation::calc_dxx(index[1], index[0]) - gxx[1] / 2.0 ;
    slope0 = gxx[0];
    //Last point has 0 2nd derivative
    gxx[Nmax] = 3.0 / 2 * (value[Nmax] - value[Nmax-1]) / LAConstrainedSplineInterpolation::calc_dxx(index[Nmax], index[Nmax-1]) - gxx[Nmax-1] / 2.0 ;
    slope1 = gxx[Nmax];

    //Calc second derivative at points
    for (i = 1; i <=  Nmax  ; i++)
    {
        ggxx[i][0] = -2.0   * (gxx[i] + 2 * gxx[i-1]) / LAConstrainedSplineInterpolation::calc_dxx(index[i], index[i - 1]) + 6.0 * (value[i] - value[i - 1]) / ((LAConstrainedSplineInterpolation::calc_dxx(index[i], index[i - 1]) * LAConstrainedSplineInterpolation::calc_dxx(index[i], index[i - 1]) ));
        ggxx[i][1] = 2.0 * (2 * gxx[i] + gxx[i-1]) / LAConstrainedSplineInterpolation::calc_dxx(index[i], index[i - 1]) - 6.0 * (value[i] - value[i - 1]) / ((LAConstrainedSplineInterpolation::calc_dxx(index[i], index[i - 1]) * LAConstrainedSplineInterpolation::calc_dxx(index[i], index[i - 1])));

        //Calc constants for cubic
        d[i] = 1.0 / 6 * (ggxx[i][1] - ggxx[i][0]) / LAConstrainedSplineInterpolation::calc_dxx(index[i], index[i - 1]);
        c[i] = 1.0 / 2 * (index[i] * ggxx[i][0] - index[i - 1] * ggxx[i][1]) / LAConstrainedSplineInterpolation::calc_dxx(index[i], index[i - 1]);
        b[i] = (value[i] - value[i - 1] - c[i] * ((index[i] * index[i]) - (index[i - 1] * index[i -1]) ) - d[i] * ((index[i] * index[i] * index[i]) - (index[i - 1]  * index[i - 1]  * index[i - 1]) )) / LAConstrainedSplineInterpolation::calc_dxx(index[i], index[i - 1]);
	...
	mpDataProvider = new LAConstrainedSplineInterpolationDataProvider(index, value, a, b, c, d, size, slope0, slope1);
	interpolationData_ = std::make_shared<InterpolationData>( index, value );
	return;
}

double LAConstrainedSplineInterpolation::calc_dxx(const double x1, const double x0)
{
    //Calc Xi - Xi-1 to prevent div by zero
    double dxx = x1 - x0;
    if (LAMath::abs(dxx) < D_ZERO) 
        dxx = D_UPPER;

    return dxx;
};

std::tuple<std::vector<double>,std::vector<double>> LAConstrainedSplineInterpolation::getXY() const
{
