...
NL2SOL::NL2SOL(LAFunctionVector & function) :
m_function(function),
...
NL2SOL::tryToSolve(std::vector<double> &x, double *errormeasure){
   ...
   if (errormeasure)
       *errormeasure = LAMath::sqrt(2*m_workspace[9]);
   return k;
}
...
NL2SOL::tryToSolve2(std::vector<double> &x, double *errormeasure){
   ...
   if (m_iv[0] > 11){   //   A serious error from the NL2SOL algorithm ?
		LAString msg = "The ACM-TOMS-573 algorithm (nl2sno) returned error value ";
		msg += LAString(m_iv[0]);
		msg += " in NL2SOL::tryToSolve() which is almost certainly a programming error";
		throw LACoreInvalidData(msg.getCString(),__FILE__,__LINE__);
   }
...
L130:
   ...
   if (emax < v[gtstep]) {
       ...
       v[radfac] = rfac1 * LAMath::max(d_1,d_2);
   }
...
L80:
   ...
   //
   del = v[delta0] * LAMath::max(one/d_[m],fabs(x[m]));
...
L180:
   ...
   //
   del = v[dltfdc] * LAMath::max(one/d_[m],fabs(x[m]));
   if (x[m] < zero) {
       del = -del;
   }
   v[xmsave] = x[m];
...
NL2SOL::dotprd(int *p, double *x, double *y)
{
   ...
   for (i_ = 1; i_ <= i_1; ++i_) {
       ...
       t = LAMath::max(fabs(x[i_]),fabs(y[i_]));
       if (t > one) {
           goto L10;
       }
       if (t < sqteta) {
...
L20:
   ...
   for (i_ = 1; i_ <= i_1; ++i_) {
       ...
       if (t < v[jtoli]) {
           ...
           t = LAMath::max(d_1,d_2);
       }
       // Computing MAX 
       d_1 = vdfac * d_[i_];
       d_[i_] = LAMath::max(d_1,t);
   }
...
L190:
   ...
   //    lk = max(d_1,d_2);
   lk = LAMath::max(lk,v[dgnorm] / rad - w[emax]);
...
L210:
   ...
   if (-v[dst0] >= alphak || alphak < lk || alphak >= uk) {
       ...
       alphak = uk * LAMath::max(p001, sqrt(lk / uk));
   }
   k = 0;
   i_1 = *p;
   for (i_ = 1; i_ <= i_1; ++i_) {
...
L260:
   ...
   if (phi < zero) {
       uk = LAMath::min(uk,alphak);
   }
   alphak += phi / t1 * (dst / t1) * (dst / rad);
   lk = LAMath::max(lk,alphak);
   goto L210;
...
L330:
   ...
   //    d_1 = lk, d_2 = t - w[emax];
   //    lk = max(d_1,d_2);
   lk = LAMath::max(lk,t - w[emax]);
   if (v[dst0] > zero) {
       // Computing MAX 
       //
       //        d_1 = lk, d_2 = (v[dst0] - rad) * w[phipin];
       //        lk = max(d_1,d_2);
       lk = LAMath::max(lk,(v[dst0] - rad) * w[phipin]);
   }
   goto L260;
...
L340:
   ...
   if (alphak > zero) {
       // Computing MIN 
       d_1 = uk, d_2 = w[uk0];
       uk = LAMath::min(d_1,d_2);
   }
   // Computing MAX 
   d_1 = zero, d_2 = -v[dst0], d_1 = LAMath::max(d_1,d_2), d_2 = t - w[emax];
   lk = LAMath::max(d_1,d_2);
   if (v[dst0] > zero) {
       // Computing MAX 
       d_1 = lk, d_2 = (v[dst0] - rad) * w[phipin];
       lk = LAMath::max(d_1,d_2);
...
NL2SOL::slupdt(double *a, double *cosmin, int *p, double *size, double *step, double *u, double *w, double *wchmtd, double *wscale, double *y)
{
   ...
   *wscale = one;
   if (denmin != zero) {
       ...
       //        *wscale = min(d_2,d_3);
       *wscale = LAMath::min(one,fabs(sdotwm / denmin));
   }
   t = zero;
   if (sdotwm != zero) {
       t = *wscale / sdotwm;
