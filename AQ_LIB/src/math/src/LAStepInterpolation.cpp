...
*/
LAStepInterpolation::LAStepInterpolation(StepType steptype) 
: LAInterpolationBase(), mStepType(steptype), mpDataProvider(NULL)
{
...
/*
    @brief destructor
*/
LAStepInterpolation::~LAStepInterpolation() 
{
...
*/
LAStepInterpolation::LAStepInterpolation(const LAStepInterpolation& v)
: LAInterpolationBase(v), mpDataProvider(NULL)
{
	mStepType = v.mStepType;	
    try 
	{	
		mpDataProvider = new LAStepInterpolationDataProvider(*v.mpDataProvider);
...
*/
LACoreFunctionBase*	LAStepInterpolation::clone() const
{
    try 
	{
		LAStepInterpolation* pTmp;
		pTmp = new LAStepInterpolation(mStepType);

		if (mpDataProvider != nullptr )
		{
			pTmp->mpDataProvider = new LAStepInterpolationDataProvider(this->mpDataProvider->index1, this->mpDataProvider->value1, this->mpDataProvider->size1);
...
*/
bool LAStepInterpolation::isTypeOf(function_t id) const
{
	return (id == FN_STEPINTERPOLATION ? true : LAInterpolationBase::isTypeOf(id));
}
...
*/
function_t LAStepInterpolation::getType() const
{
...
*/
double LAStepInterpolation::value(const double x1) const
{
	// Data Class Uninitialized or Empty
	if (mpDataProvider == nullptr || mpDataProvider->size1 == 0 )
	{
        throw LACoreInvalidData("#Error: Step interpolation data has not been set", __FILE__, __LINE__);
	}
...
double LAStepInterpolation::differentiate(const double & x1) const
{
...
*/
void LAStepInterpolation::set(const DoubleArray& index,const DoubleArray& value)
{
	if (index.size() != value.size())
	{
		throw LACoreInvalidData("#Error: Number of X Data Points does not match the number of Y Value Points", __FILE__, __LINE__);
	}
	if (mpDataProvider != NULL)
	{
		delete mpDataProvider;
	...
	mpDataProvider = new LAStepInterpolationDataProvider(index, value, index.size());
	interpolationData_ = std::make_shared<InterpolationData>( index, value );
	return;
};

std::tuple<std::vector<double>,std::vector<double>> LAStepInterpolation::getXY() const
{
...
double LAStepInterpolation::intervalSlope( const Interval& interval ) const
{
	// Slope
	return 0.0;
}
...
double LAStepInterpolation::intervalArea( const size_t & endIndex, const double & lowerBound, const double & upperBound ) const
{
