...
AQLLinearInterpolation::AQLLinearInterpolation(ExtrapolationType type) 
: AQLInterpolationBase(), mpDataProvider(NULL), mExtrapolationType(type)
{}

AQLLinearInterpolation::~AQLLinearInterpolation() 
{
...
AQLLinearInterpolation::AQLLinearInterpolation(const AQLLinearInterpolation& v)
: AQLInterpolationBase(v), mpDataProvider(NULL)
{
	mExtrapolationType = v.mExtrapolationType;	
    try 
	{	
		mpDataProvider = new AQLLinearInterpolationDataProvider(*v.mpDataProvider);
...
AQLCoreFunctionBase*	AQLLinearInterpolation::clone() const
{
    AQLLinearInterpolation* pTmp = nullptr;
    try 
	{
		pTmp =new AQLLinearInterpolation(mExtrapolationType);

		if (mpDataProvider != NULL)
		{
			pTmp->mpDataProvider = new AQLLinearInterpolationDataProvider(this->mpDataProvider->index1,this->mpDataProvider->value1,this->mpDataProvider->size1);
...
*/
bool AQLLinearInterpolation::isTypeOf(function_t id) const
{
	return (id == FN_LINEARINTERPOLATION ? true : AQLInterpolationBase::isTypeOf(id));
}

/*  @brief Return this function type
    @return function type
*/
function_t AQLLinearInterpolation::getType() const
{
	return FN_LINEARINTERPOLATION;
}

// Interpolation Value
double AQLLinearInterpolation::value(const double x1) const
{
	...
	AQLAlgorithm::locate<DoubleArray,double>( mpDataProvider->index1, x1, mpDataProvider->size1, position );
...
double AQLLinearInterpolation::differentiate(const double & x1) const
{
	AQ_REQUIRE( mpDataProvider != nullptr, "Linear interpolation data has not been set" )
	AQ_REQUIRE( mpDataProvider->size1 > 0, "Linear interpolation data has not been set" )
...
double AQLLinearInterpolation::integrate(const double & lowerBound, const double & upperBound ) const
{
	AQ_REQUIRE( mpDataProvider != nullptr, "Linear interpolation data has not been set" )
	AQ_REQUIRE( mpDataProvider->size1 > 0, "Linear interpolation data has not been set" )
...
*/
void AQLLinearInterpolation::set(const DoubleArray& index, const DoubleArray& value)
{
	...
	mpDataProvider = new AQLLinearInterpolationDataProvider(index, value, index.size());
...
std::tuple<std::vector<double>,std::vector<double>> AQLLinearInterpolation::getXY() const
{
...
double AQLLinearInterpolation::intervalSlope( const Interval& interval ) const
{
	// Slope
...
double AQLLinearInterpolation::intervalArea( const size_t & endIndex, const double & lowerBound, const double & upperBound ) const
{
