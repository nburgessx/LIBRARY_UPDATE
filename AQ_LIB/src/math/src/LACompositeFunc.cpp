...
////X///////////////////X///////////////////////////////X///////////////////
//  NAME        :       LACompositeMethod.cpp
//
//  SYNOPSIS    :       LACompositeMethod
//  DESCRIPTION :       Source code of class to represent composite function of two functions
//                      
//  SEE ALSO    :       
//  TYME        :       CLASS
//  STATUS      :       
...
/*!
	...
	@note Example of f(x, y, z) = f1(x,y) - f2(z,x)
		...
		type = SUBTRACTION
*/
LACompositeMethod::LACompositeMethod(const LAStringVector& var,
					const LAFunctionBase& func1, const LAStringVector& var1,
					const LAFunctionBase& func2, const LAStringVector& var2,
					OPERATION_TYPE type,	
					bool checkflag) 
: LAFunctionBase(checkflag), mType(type), mpF1(NULL), mpF2(NULL)
{
	mpF1 = dynamic_cast<LAFunctionBase*>(func1.clone());
	mpF2 = dynamic_cast<LAFunctionBase*>(func2.clone());
	...
	{
		//error
		throw LACoreInvalidData("Input is something wrong", __FILE__, __LINE__);
	}	
...
*/
LACompositeMethod::LACompositeMethod(const LACompositeMethod& v) 
: LAFunctionBase(v), 
mType(v.mType), mpF1(NULL), mpF2(NULL), mVarPos1(v.mVarPos1), mVarPos2(v.mVarPos2)
, mVar1(v.mVar1), mVar2(v.mVar2)
{
	mpF1 = dynamic_cast<LAFunctionBase*>(v.mpF1->clone());
	mpF2 = dynamic_cast<LAFunctionBase*>(v.mpF2->clone());
}

/*!
	@brief destructor
*/
LACompositeMethod::~LACompositeMethod() 
{
	delete mpF1;
	delete mpF2;
}
...
*/
LACoreFunctionBase*	
LACompositeMethod::clone() const
{
    try 
	{
		return new LACompositeMethod(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
LACompositeMethod::isTypeOf(function_t id) const
{
	return (id == FN_COMPOSITEFUNC ? true : LAFunctionBase::isTypeOf(id));
}
...
*/
function_t
LACompositeMethod::getType() const
{
...
*/
double
LACompositeMethod::operator()(const DoubleArray& x) const
{
...
*/
double
LACompositeMethod::partialDerivative(const DoubleArray& x, unsigned int pos,
...
*/
double
LACompositeMethod::partialDerivative2(const DoubleArray& x, unsigned int posi,unsigned int posj,
...
*/
bool
LACompositeMethod::isInDomain(const DoubleArray& x)const
{
	try
	{
		if (mpF1->isCheck())
...
*/
bool
LACompositeMethod::isDifferentiable(const DoubleArray& x, unsigned int pos)const
{
	try
	{
		if (mpF1->isCheck())
...
*/	
bool
LACompositeMethod::isDifferentiable(const DoubleArray& x, unsigned int posi, unsigned int posj) const
{
	try
	{
		if (mpF1->isCheck())
...
*/	
void
LACompositeMethod::setCheckFlag(bool checkflag)
{
