...
*/
AQLLinearSplineInterpolation::AQLLinearSplineInterpolation() 
: AQLInterpolationBase(), 
  ...
  extrapolationType_(LINEAR_EXTRAPOLATION_TYPE)
{
	...
	splineInterpolation_ = std::shared_ptr<AQLSplineInterpolation>(new AQLSplineInterpolation(true));
}
...
/*!
	...
	// LINEAR_EXTRAPOLATION = Linear Extrapolation
*/
AQLLinearSplineInterpolation::AQLLinearSplineInterpolation( ExtrapolationType extrapolationType, const bool useNaturalSpline ) 
: AQLInterpolationBase(), 
  isJoinDateSet_(false), 
  linearInitialised_(false),
  splineInitialised_(false),
  extrapolationType_(extrapolationType)
{
	...
	splineInterpolation_ = std::shared_ptr<AQLSplineInterpolation>(new AQLSplineInterpolation(useNaturalSpline));
}
...
*/
AQLLinearSplineInterpolation::AQLLinearSplineInterpolation( const AQLLinearSplineInterpolation& v )
...
{
	splineInterpolation_    = std::shared_ptr<AQLSplineInterpolation>(dynamic_cast<AQLSplineInterpolation*>(v.splineInterpolation_->clone()));
...
*/
AQLCoreFunctionBase*	
AQLLinearSplineInterpolation::clone() const
{
    AQLLinearSplineInterpolation* pTmp = nullptr;
    try 
	{
	    pTmp = new AQLLinearSplineInterpolation( *this );
...
*/
bool
AQLLinearSplineInterpolation::isTypeOf(function_t id) const
{
	return ( id == FN_LINEARSPLINEINTERPOLATION ? true : AQLInterpolationBase::isTypeOf( id ) );
}
...
*/
function_t
AQLLinearSplineInterpolation::getType() const
{
...
*/
double
AQLLinearSplineInterpolation::value( const double x1 ) const
{
...
double AQLLinearSplineInterpolation::differentiate(const double & x1) const
{
...
double AQLLinearSplineInterpolation::integrate(const double & lowerBound, const double & upperBound ) const
{
	AQ_REQUIRE( interpolationData_ != nullptr, "Linear-Spline interpolation data has not been set" )
	AQ_REQUIRE( interpolationData_->size_ > 0, "Linear-Spline interpolation data has not been set" )
...
*/
const double
AQLLinearSplineInterpolation::getJoinDateAsDouble() const
{
...
*/
void AQLLinearSplineInterpolation::set( const DoubleArray& index, const DoubleArray& value, const double& joinDateAsDouble )
{
...
*/
void
AQLLinearSplineInterpolation::setJoinDateAsDouble( const double& joinDateAsDouble )
{
...
*/
void
AQLLinearSplineInterpolation::set( const DoubleArray& index, const DoubleArray& value )
{
    // This parameter is set in the 'setLinearInterpolationEndDateAsDouble' method above.
    if( !isJoinDateSet_ )
    {
        throw AQLCoreInvalidData("#Error: Join date has not been set in Linear Spline interpolation scheme", __FILE__, __LINE__);
    }

    if( index.size() != value.size() )
    {
        throw AQLCoreInvalidData("#Error: Interpolation index size and value sizes are not same.", __FILE__, __LINE__);
    }
    ...
    if( linearIndex.size() >= 2 && splineIndex.size() >= 2 ) // Extrapolation Conditions to prevent access violation errors
    {
		...
		if (lastLinearIndex < firstSplineIndex)
		{
			...
			std::shared_ptr<AQLSplineInterpolation> transitionInter(new AQLSplineInterpolation(true)); // use natural spline
...
std::tuple<std::vector<double>,std::vector<double>> AQLLinearSplineInterpolation::getXY() const
{
