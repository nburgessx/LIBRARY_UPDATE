...
*/
AQLPriceDataDayCount::AQLPriceDataDayCount(const staticData_t attr) : AQLPriceDataType(attr)
{
    mDayCount = ACT_ACT;
}
...
*/
AQLPriceDataDayCount::AQLPriceDataDayCount(const AQLPriceDataDayCount& attr) 
...
*/
AQLPriceDataDayCount::AQLPriceDataDayCount(const DayCount dc)

    : AQLPriceDataType(DATA_DAYCOUNT)
{
    mDayCount = dc;
    setNull(false);
}
...
/*!
    @brief destructor
*/
AQLPriceDataDayCount::~AQLPriceDataDayCount(void)
{
}
...
*/
AQLPriceDataType*    
AQLPriceDataDayCount::clone() const
{
    try {
        AQLPriceDataDayCount*   pAttr = new AQLPriceDataDayCount(*this);
        return pAttr;
    }
    catch (bad_alloc & e){
...
/*!
    @brief Return DayCount-rule by AQLString
           ex.) "ACT_365_ISDA"
...
*/
AQLString   
AQLPriceDataDayCount::convertToString(void) const
{
    if (isNull()) return NULL_STR;
    AQLString ret(gDayCount[mDayCount]);
    return ret;
}
...
*/
double
AQLPriceDataDayCount::getTerm(const AQLDate& fromDate, const AQLDate& toDate, bool includelast) const
{
	...
	case ACT_ACT_ICMA:
		{
			...
			std::vector<AQLDate> CouponStartEndDates;
			...
			if(i1 < 0 || i2 < 0)
			{
				AQLString msg = "Daycount ACT/ACT.ICMA: FromDate or ToDate are out of range.";
...
double
AQLPriceDataDayCount::getTerm(const AQLDate& fromDate, const double& termDays, bool includelast) const
{
    double ret = 0;
    switch (mDayCount)
    {
   ...
   //     break;
	...
	case ACT_ACT_ICMA:
		{
			throw AQLCoreInvalidData("GetTerm(FromDate, TermDays) is not supported for daycount ACT/ACT.ICMA.", __FILE__, __LINE__);
		}
		break;
...
int
AQLPriceDataDayCount::getNumerator(const AQLDate& fromDate, const AQLDate& toDate, bool includelast) const
{
    if (toDate < fromDate) return -getTerm(toDate, fromDate);
	if (fromDate == toDate) return 0;
	...
	case ACT_365_ISDA:
		{
			...
			throw AQLCoreInvalidData(" ACT_ACT and ACT_365_ISDA are not supported for this method", __FILE__, __LINE__);	
	...
	case ACT_ACT_ICMA:
		{
			throw AQLCoreInvalidData(" This daycount is not supported for this method", __FILE__, __LINE__);	
		}
		break;
...
int
AQLPriceDataDayCount::getDenominator() const
{
...
AQLPriceDataDayCount::dayCountEnum( const DayCount & daycount )
{
	...
	switch ( daycount )
    {
		...
		default:
		{
			AQ_THROW("Invalid Daycount: Daycount not supported in AQLPriceDataDayCount class")
			break;
		}
	}
    return result;
...
etrading::DayCountEnum AQLPriceDataDayCount::dayCountEnum() const
{
	etrading::DayCountEnum result = AQLPriceDataDayCount::dayCountEnum( mDayCount );
	return result;
}
...
double
AQLPriceDataDayCount::getDayTerm(const AQLDate& fromDate, const double& termY, bool includelast) const
{
   ...
   //     break;
	...
	case E30_360:	case ACT_365_FJ:
		{
			throw AQLCoreInvalidData("Daycounts: N30_360, E30_360 and ACT_365_FJ are not supported in this method", __FILE__, __LINE__);
		}
		break;
	case ACT_ACT:
	case ACT_365_ISDA:
	...
	case ACT_ACT_ICMA:
		{
			throw AQLCoreInvalidData("GetDayTerm(FromDate, TermYear) is not supported for daycount ACT/ACT.ICMA.", __FILE__, __LINE__);
		}
		break;
    }
    return ret;
...
*/
void          
AQLPriceDataDayCount::convertFromString(const AQLString& str)
{
    AQLString data;
...
*/
void                  
AQLPriceDataDayCount::setDayCount(const DayCount dc)
{
	if (mDayCount != dc)
	{
		mDayCount = dc;
...
/*!
    ...
    @param[in] dc DayCount by AQLString
*/
void                  
AQLPriceDataDayCount::setDayCount(const AQLString& dc)
{
    int i;
    for (i = 0; i < DAY_C_SIZE; i++)
    {
    ...
    if (i == DAY_C_SIZE)
    {
        AQLString msg("Invalid String for setDayCount : ");
        msg += dc;
        throw AQLCoreInvalidData(msg.getCString(), __FILE__, __LINE__);
    }
    setNull(false);
...
AQLPriceDataType& 
AQLPriceDataDayCount::assignment(const AQLPriceDataType& a)
{
    if (this == &a) return *this;

    if (a.getType() != DATA_DAYCOUNT) 
    {
        AQLString err = "Assignement error for AQLPriceDataDayCount : from ";
        err += AQLString(a.getType());
        throw AQLCoreInvalidData(err.getCString(), __FILE__, __LINE__);
    }
    mDayCount = dynamic_cast<const AQLPriceDataDayCount&>(a).mDayCount;
...
/*!
	...
	if argument is not AQLPriceDataDayCount, then error
...
int          
AQLPriceDataDayCount::compare(const AQLPriceDataType& a) const
{
    if (a.getType() != DATA_DAYCOUNT) 
    {
        AQLString err = "Compare error for AQLPriceDataDayCount : from ";
        err += AQLString(a.getType());
        throw AQLCoreInvalidData(err.getCString(), __FILE__, __LINE__);
    }
    const AQLPriceDataDayCount& at = dynamic_cast<const AQLPriceDataDayCount&>(a);
