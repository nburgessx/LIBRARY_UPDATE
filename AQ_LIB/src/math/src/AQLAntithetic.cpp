...
*/
AQLAntithetic::AQLAntithetic(const AQLAntithetic& v):
AQLRandBase(v), mpInner(NULL)
{
	if (v.mpInner) mpInner = dynamic_cast<AQLRandBase*>( v.mpInner->clone() );
	isOdd = v.isOdd;
...
/*!
	...
	@return pointer to AQLCoreFunctionBase
*/
/*!
...
*/
AQLCoreFunctionBase*
AQLAntithetic::clone() const
{
    try 
	{
		return new AQLAntithetic(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
void
AQLAntithetic::setInnerGenerator(const AQLRandBase& v)
{
	if (mpInner) delete mpInner;
	mpInner = NULL;
	
	mpInner = dynamic_cast<AQLRandBase*>( v.clone() );
...
*/
bool
AQLAntithetic::isTypeOf(function_t id) const
{
	return (id == FN_RAND_ANTI ? true : AQLRandBase::isTypeOf(id));
}
...
*/
void
AQLAntithetic::getUniforms(DoubleArray& variates)
{
	// if the following block affects performance, remove it.
	if (mpInner == NULL)
	{
		// exception
		throw AQLCoreSystemError("Inner generator is not set.", __FILE__, __LINE__);
	}
...
*/
void
AQLAntithetic::setDim(const UintArray& dimValue)
{
...
*/
void
AQLAntithetic::setSeed(const UlongArray& seed)
{
