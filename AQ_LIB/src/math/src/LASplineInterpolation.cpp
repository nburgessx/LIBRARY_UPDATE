...
/*!
    @brief constructor
*/
LASplineInterpolation::LASplineInterpolation()
: LAInterpolationBase()
{
	mpDataProvider=NULL;
...
*/
LASplineInterpolation::LASplineInterpolation( const bool useNaturalSpline )
: LAInterpolationBase()
{
	mpDataProvider=NULL;
...
*/
LASplineInterpolation::LASplineInterpolation(const LASplineInterpolation& v)
: LAInterpolationBase(v), mpDataProvider(NULL)
{
    try 
	{	
        useNaturalSpline_ = v.useNaturalSpline_;
		mpDataProvider = new LASplineInterpolationDataProvider(*v.mpDataProvider);
...
*/
LACoreFunctionBase*	
LASplineInterpolation::clone() const
{
    try 
	{
		LASplineInterpolation* pTmp;
		pTmp = new LASplineInterpolation;
        pTmp->useNaturalSpline_ = this->useNaturalSpline_;

		if (mpDataProvider != NULL)
		{
			pTmp->mpDataProvider = new LASplineInterpolationDataProvider(this->mpDataProvider->index ,this->mpDataProvider->value ,this->mpDataProvider->coeff ,this->mpDataProvider->size);
...
*/
bool
LASplineInterpolation::isTypeOf(function_t id) const
{
...
*/
function_t
LASplineInterpolation::getType() const
{
...
*/
double
LASplineInterpolation::value(const double x1) const
{
	if (mpDataProvider == NULL)
	{
		throw LACoreInvalidData("#Error: Spline interpolation data has not been set", __FILE__, __LINE__);
	}
...
*/
void
LASplineInterpolation::set( const DoubleArray& index, const DoubleArray& value )
{
    // Important Note:
    // ---------------
	...
	{
        throw LACoreInvalidData("#Error: Spline interpolation error, the index size and value size are not same.", __FILE__, __LINE__);
	}
	if (index.size() == 0)
	{
        throw LACoreInvalidData("# Error: Spline interpolation error. No data provided.", __FILE__, __LINE__);
	}
    ...
    else
    {
        ...
        if ( width1 == 0 )
        {
            throw LACoreInvalidData("#Error: Invalid spline first derivative at start-point. The underlying data is invalid having an infinite slope.", __FILE__, __LINE__);
        }
    ...
    else
    {
        ...
        if ( widthN == 0 )
        {
            throw LACoreInvalidData("#Error: Invalid spline first derivative at end-point. The underlying data is invalid having an infinite slope.", __FILE__, __LINE__);
        }
	...
	mpDataProvider = new LASplineInterpolationDataProvider(index, value, secDev, size);
	interpolationData_ = std::make_shared<InterpolationData>( index, value );
	return;
};

std::tuple<std::vector<double>,std::vector<double>> LASplineInterpolation::getXY() const
{
...
double LASplineInterpolation::intervalSlope( const Interval& interval, const double& x1) const
{
	int klo, khi;
	double h, b, a, dydx;
...
double LASplineInterpolation::intervalArea( const size_t & endIndex, const double & lowerBound, const double & upperBound ) const
{
