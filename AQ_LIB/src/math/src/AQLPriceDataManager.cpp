...
*/
void
AQLPriceDataManager::setUpDefault(void)
{
#if 0 // unused
	// set Data Object
#endif
...
*/
AQLPriceDataManager::AQLPriceDataManager()
...
/*!
    @brief destructor
*/
AQLPriceDataManager::~AQLPriceDataManager(void)
{
}
...
*/
const AQLDataHolder& 
AQLPriceDataManager::getData(const AQLString& name) const
{
...
const AQLDataHolder&
AQLPriceDataManager::getData( const AQLString& name, const AQLString& alias ) const
{
	// get the name data if missing try the alias data
	AQLDataHolder dh = getData( name );
...
*/
const AQLDataHolder& 
AQLPriceDataManager::getData(object_t id) const
{
	map<object_t, AQLDataHolder>::const_iterator it = mObjectCollection.find(id);
...
const AQLDataHolder&
AQLPriceDataManager::getData( object_t id, object_t alias_id ) const
{
...
*/
vector<object_t>
AQLPriceDataManager::getDataArray(void) const
{
	vector<object_t> ret;
	map<object_t, AQLDataHolder>::const_iterator it = 
...
*/
vector<AQLString>
AQLPriceDataManager::getNameArray(void) const
{
...
*/
const AQLDataHolder&	
AQLPriceDataManager::setData(const AQLString& name, object_t id)
{
	const AQLDataHolder* dh = &mAttrCollection.getData(name, NOCHECK);
	if (dh->isDefined())
	{
		if(dh->getType() != id)
		{
	...
	const AQLDataHolder& h = getData(id);
	if (!h.isDefined())
	{
		AQLString msg("No object_t[");
		msg += AQLString(id) + "] is registered";
		throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);
	}
...
/*!
	...
	AQLPriceDataType Master holds ownership of Data object
...
*/
const AQLDataHolder&	
AQLPriceDataManager::setData(AQLPriceDataType* att)
{
	if (att == NULL)
	{
		throw AQLCoreInvalidData("Argument is NULL", __FILE__,__LINE__);
	}
	AQLDataHolder& h = mObjectCollection[att->getType()];
	if (h.isDefined())
	{
...
void
AQLPriceDataManager::removeData(object_t type)
{
...
*/
void
AQLPriceDataManager::removeData(const AQLString& name)
{
...
*/
AQLPriceDataManager::AQLPriceDataManager(const AQLPriceDataManager& a)
