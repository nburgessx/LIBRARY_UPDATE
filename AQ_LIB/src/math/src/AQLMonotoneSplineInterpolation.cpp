...
AQLMonotoneSplineInterpolation::AQLMonotoneSplineInterpolation()
	: AQLInterpolationBase()
{
	// Default to MonotoneCubic
	monotoneInterpolationType_ = AQLMonotoneSplineInterpolation::FRITSCH_BUTLAND;

}

// @brief Constructor allowing the setting of the monotone interpolation sub-type
AQLMonotoneSplineInterpolation::AQLMonotoneSplineInterpolation( const MonotoneInterpolationEnum monotoneInterpolationType )
	: AQLInterpolationBase()
{
	monotoneInterpolationType_ = monotoneInterpolationType;

    if ( ! ( monotoneInterpolationType == AQLMonotoneSplineInterpolation::FRITSCH_BUTLAND || monotoneInterpolationType == AQLMonotoneSplineInterpolation::MONOTONE_PARABOLIC ) )
	{
        throw AQLCoreInvalidData("#Error: Unsupported Montone Spline method.", __FILE__, __LINE__);
	}
}
...
AQLMonotoneSplineInterpolation::AQLMonotoneSplineInterpolation(const AQLMonotoneSplineInterpolation& v)
: AQLInterpolationBase(v), monotoneInterpolationType_(v.monotoneInterpolationType_), qlInterpolator_( v.qlInterpolator_), xValues_( v.xValues_ ), yValues_( v.yValues_ )
{
	// Base Class Data
...
AQLCoreFunctionBase*	AQLMonotoneSplineInterpolation::clone() const
{
    try 
	{
		AQLMonotoneSplineInterpolation* pTmp;
		pTmp = new AQLMonotoneSplineInterpolation();
...
bool AQLMonotoneSplineInterpolation::isTypeOf(function_t id) const
{
...
function_t AQLMonotoneSplineInterpolation::getType() const
{
	...
	case MONOTONE_PARABOLIC:
		...
		break;
	default:
		throw AQLCoreInvalidData("#Error: Unsupported Monotone-Spline method.", __FILE__, __LINE__);
	}
...
*/
double AQLMonotoneSplineInterpolation::value(const double x1) const
{
	if ( qlInterpolator_ == nullptr )
	{
		throw AQLCoreInvalidData("#Error: Monotone-Spline interpolation data has not been set", __FILE__, __LINE__);
	}
	...
	catch( std::exception & e)
	{
		...
		errorMsg += e.what();
		throw AQLCoreInvalidData( errorMsg.c_str(), __FILE__, __LINE__ );
	}
	return interpValue;
} 
...
*/
void AQLMonotoneSplineInterpolation::set( const DoubleArray& index, const DoubleArray& value )
{
    if (index.size() != value.size())
	{
        throw AQLCoreInvalidData("#Error: Monotone-Spline interpolation error, the index size and value size are not same.", __FILE__, __LINE__);
	}
	if (index.size() == 0)
	{
        throw AQLCoreInvalidData("# Error: Monotone-Spline interpolation error. No data provided.", __FILE__, __LINE__);
	}
...
*/
std::tuple<std::vector<double>,std::vector<double>> AQLMonotoneSplineInterpolation::getXY() const
{
