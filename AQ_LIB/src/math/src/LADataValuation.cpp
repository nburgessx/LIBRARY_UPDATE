...
*/
LADataValuation::LADataValuation(void) : 
    LAPriceDataType(DATA_VALUATION),  
...
*/
LADataValuation::LADataValuation(const LADataValuation& v) : 
    LAPriceDataType(DATA_VALUATION), 
    ...
    mDate(), mFuncHolder(v.mFuncHolder), mName("")
{
    if (! v.isNull())
    {
        if (mVersion != -1)
        {
            mpMethods = &(dynamic_cast<const class LACoreValuation&>(mFuncHolder.get()));
...
*/
LADataValuation::LADataValuation(const LACoreFunctionHolder& h, const LAString& name) : 
    LAPriceDataType(DATA_VALUATION), 
    mVersion(-1), mpMethods(NULL), mpDataProvider(NULL), mValue(0),
...
/*!
    ...
    @param[in] name function name
*/
LADataValuation::LADataValuation(const LACoreValuation* v, const LAString& name) : 
    LAPriceDataType(DATA_VALUATION), 
    mVersion(-1), mpMethods(NULL), mpDataProvider(NULL), mValue(0),
    mDate(), mFuncHolder(), mName("")
{
...
/*!
    @brief destructor
*/
LADataValuation::~LADataValuation()
{
...
*/
LAPriceDataType*    
LADataValuation::clone() const
{
    try {
        LADataValuation*    pAttr = new LADataValuation(*this);
        return pAttr;
    }
    catch (bad_alloc & e){
...
*/
bool                
LADataValuation::isTypeOf(function_t id) const
{
...
/*!
    ...
    @return function to be set(LACoreValuation)
*/
const LACoreValuation&
LADataValuation::getMethod(void) const 
{
...
*/
LAString          
LADataValuation::convertToString(void) const
{
    if (isNull()) return NULL_STR;

    LAString ret(mValue, DOUBLE_PRECISION);
    if(mName != "")
    {
        ret+="(";
        ret+=mName;
...
*/
void          
LADataValuation::convertFromString(const LAString& str)
{
    LAString data;
...
*/
void          
LADataValuation::setMethod(const LAString& str)
{
    LAString eqstr;
    double value=0.0;
    ...
    if(pos > (int)str.size()-2)
    {
        ...
        // exception
        throw LACoreInvalidData(str.getCString(), __FILE__, __LINE__);      
    }

    // setting of content
    if(pos==-1) // no exist'(' 
    {
        LAString firststr = str.subString(0,0);
        if(firststr == LAString('-') || (firststr >= LAString('0') &&  firststr <= LAString('9'))) // str has numerical
        {
...
*/
void 
LADataValuation::setMethod(const LACoreValuation* v, const LAString& name) 
{
    // update
    update();
    // clear Buggage
...
*/
void                
LADataValuation::setValue(const double v)
{
    // update
    update();
    // clear Buggage
...
*/
void                
LADataValuation::setNoValuation(void) const
{
    mVersion  = -1;
}
...
*/
void                
LADataValuation::setValuation(void) const
{
    mVersion = 0;
}
...
*/
double              
LADataValuation::value(const LADate& basedate)
{
    if (mVersion != -1) 
    { // calculation mode
        // get Instruemnt
...
// 28JUN05:ysuzuki          if (mVersion != inst->getModel() || basedate != mDate)
        ...
        {
            // error
            throw LACoreInvalidData("instrument is not set for LADataValuation",
                            __FILE__, __LINE__);
        }
    }
    return mValue;
...
*/
void                
LADataValuation::setDataProvider(LADataProvider* dataProvider) const
{
...
*/
LAPriceDataType& 
LADataValuation::assignment(const LAPriceDataType& a)
{
    if (this == &a) return *this;

    if (a.getType() != DATA_VALUATION) 
    {
        LAString err = "Assignment error for LADataValuation : from ";
        err += LAString(a.getType());
        throw LACoreInvalidData(err.getCString(), __FILE__, __LINE__);
    }

    const LADataValuation& in = dynamic_cast<const LADataValuation&>(a);
...
/*!
    ...
    @param[in] a LADataValuation object to be compared

    @return  (Function ID of the Function class that this Data has) - (Function ID of the Function class that Data "a" has)
*/
int          
LADataValuation::compare(const LAPriceDataType& a) const
{
    if (a.getType() != DATA_VALUATION) 
    {
        LAString err = "Compare error for LADataValuation : from ";
        err += LAString(a.getType());
        throw LACoreInvalidData(err.getCString(), __FILE__, __LINE__);
    }
    const LADataValuation& in = dynamic_cast<const LADataValuation&>(a);
...
*/
void                
LADataValuation::setHolder(LADataHolder* holder)
{
    ...
    if (e != NULL && mName != "")
    {
        ...
        if (dataInstance != NULL)
        {
            ...
            if (! h.isDefined() || !h.get().isTypeOf(FN_VALUATION)) 
            {
                ...
                mVersion = -1;
                mName = "";
                msg += " is not found in Function Master as LACoreValuation";
