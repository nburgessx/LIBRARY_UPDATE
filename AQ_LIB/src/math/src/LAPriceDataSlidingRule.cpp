...
*/
LAPriceDataSlidingRule::LAPriceDataSlidingRule(SlidingRuleType sr)
		: LAPriceDataType(DATA_SLIDINGRULE), mSlidingRuleType(sr) 
{
	setNull(false);
}
...
*/
LAPriceDataSlidingRule::LAPriceDataSlidingRule(const LAPriceDataSlidingRule& attr)
		: LAPriceDataType(DATA_SLIDINGRULE), 
...
/*!
    @brief destructor
*/
LAPriceDataSlidingRule::~LAPriceDataSlidingRule(void) 
{ 
}
...
*/
LAPriceDataType*
LAPriceDataSlidingRule::clone() const
{
    try {
    	LAPriceDataSlidingRule* pAttr = new LAPriceDataSlidingRule(*this);
    	return pAttr;
    }
    catch (bad_alloc & e){
...
*/
LAString
LAPriceDataSlidingRule::convertToString(void) const
{
...
*/
LADate LAPriceDataSlidingRule::getDate(const LADate& d, const LAMathCalendar& c)const
{       
	LADate	ret = d;
	if (c.isHoliday(ret))
	{
...
/*!
    ...
    @return the date adjusted the holiday
*/
LADate LAPriceDataSlidingRule::getDate(const LADate& d, const LAPriceDataCalendar& c)const
{
...
*/
void
LAPriceDataSlidingRule::convertFromString(const LAString& str)
{
	...
	else
	{
		...
		else
		{
			setNull();
			// exception
			LAString	msg("InvalidData for LAPriceDataSlidingRule : ");
			msg += str;
...
*/
LAPriceDataType&
LAPriceDataSlidingRule::assignment(const LAPriceDataType& a)
{
	if (this == &a ) return *this;
	if (a.getType() != DATA_SLIDINGRULE)
	{
		// exception
		LAString	msg = "Assignment error for LAPriceDataSlidingRule : from ";
		msg += LAString(a.getType());
		throw LACoreInvalidData(msg.getCString(), __FILE__, __LINE__);
	}
	update();
	LAPriceDataSlidingRule b = dynamic_cast<const LAPriceDataSlidingRule&>(a);
...
*/
int
LAPriceDataSlidingRule::compare(const LAPriceDataType& a) const
{
	if (a.getType() != DATA_SLIDINGRULE)
	{
		// exception
		LAString	msg = "Compare error for LAPriceDataSlidingRule : from ";
		msg += LAString(a.getType());
		throw LACoreInvalidData(msg.getCString(), __FILE__, __LINE__);
	}
	if (isNull() && a.isNull())
...
*/
bool
LAPriceDataSlidingRule::isBusinessDay(const LADate &d, const LAPriceDataCalendar& c) const
{
