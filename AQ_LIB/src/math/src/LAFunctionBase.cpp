...
////X///////////////////X///////////////////////////////X///////////////////
//  NAME        :       LAFunctionBase.cpp
//
//  SYNOPSIS    :       LAFunctionBase
...
*/
LAFunctionBase::LAFunctionBase(bool checkflag) 
: LACoreFunctionBase(), mPos(0), mUpdateFlag(true), mCheckFlag(checkflag)
{
	mX.resize(1);
}
/*!
...
*/
/*LAFunctionBase::LAFunctionBase(const LAFunctionBase& v) 
...
*/
bool
LAFunctionBase::isTypeOf(function_t id) const
{
	return (id==FN_FUNCTION ? true : LACoreFunctionBase::isTypeOf(id));
}
...
*/
double
LAFunctionBase::operator()(double x) const
{
	DoubleArray xx = mX;
	xx[mPos] = x;
	return operator()(xx);
...
*/
LAFunctionBase&
LAFunctionBase::operator()(unsigned int pos, const DoubleArray& x)
{
	mPos = pos;
	mX = x;
/*!	mX.resize(x.size()+1);
...
*/
double
LAFunctionBase::integral(const std::vector<std::pair<double,double> >& x,const LAIntegralBase* pIntegral) const
{
	if (pIntegral == NULL)
	{
		return integral(x);	
...
*/
double
LAFunctionBase::integral(const std::vector<std::pair<double,double> >& x) const
{	
	(void)x;
	throw LACoreNumericalError("This function does not support analyticaly integrated method", __FILE__, __LINE__);
}
/*!
    @brief Return integral result
...
*/
double
LAFunctionBase::integral(double xl, double xu, const LA1DIntegral* pIntegral) const
{
	if (pIntegral == NULL)
	{
...
*/
/*double
LAFunctionBase::integral(double xl, double xu, const LA1DIntegral* pIntegral, const DoubleArray& grids) const
{
	if (pIntegral == NULL)
	{
		return integral(xl, xu);
...
*/
double
LAFunctionBase::integral(double xl, double xu) const
{
	(void)xl; (void)xu;
	throw LACoreNumericalError("This function does not support analyticaly integrated method", __FILE__, __LINE__);
}
...
*/
LACombinationMethod
LAFunctionBase::operator+(const LAFunctionBase& method) const
{
	if (method.getType() == FN_COMBINATIONFUNC)return method+(*this);
	
	LACombinationMethod combifunc(mCheckFlag && method.mCheckFlag);
	vector<LAFunctionBase*> vfunc,vfunc2;
	vfunc.push_back(dynamic_cast<LAFunctionBase*>(clone()));
	vfunc2.push_back(dynamic_cast<LAFunctionBase*>(method.clone()));	
	combifunc.mFuncs.push_back(pair<double,vector<LAFunctionBase*> >(1,vfunc));
	combifunc.mFuncs.push_back(pair<double,vector<LAFunctionBase*> >(1,vfunc2));
...
*/
LACombinationMethod
LAFunctionBase::operator+(double a) const
{
	LACombinationMethod combifunc(mCheckFlag);
	combifunc.mConst = a;
...
LAFunctionBase::operator*(double a) const
{
	...
	vector<LAFunctionBase*> vfunc;
	vfunc.push_back(dynamic_cast<LAFunctionBase*>(clone()));
	combifunc.mFuncs.push_back(pair<double,vector<LAFunctionBase*> >(a,vfunc));
	return combifunc;
}
...
*/
double
LAFunctionBase::partialDerivative(const DoubleArray& x,unsigned int pos,
								CALC_TYPE calctype, DIFF_TYPE difftype,double delta) const 
{
	if (calctype == NUMERICAL || calctype == DEFAULT)
	{
		...
		{
			LAString msg = "Not derivable";
			throw LACoreNumericalError(msg.getCString() , __FILE__, __LINE__);
		}
		if (difftype==ONESIDE)
		{
			DoubleArray x1 = x;
		...
		throw LACoreNumericalError("This function does not support analytical derivative", __FILE__, __LINE__);
...
*/
double
LAFunctionBase::partialDerivative2(const DoubleArray& x, unsigned int posi,unsigned int posj,
...
*/
void
LAFunctionBase::gradient(const DoubleArray& x, DoubleArray& out,
						 CALC_TYPE calctype, DIFF_TYPE difftype,double delta) const
{
...
*/
void
LAFunctionBase::hessian(const DoubleArray& x, DoubleMatrix& out,
						CALC_TYPE calctype, double delta) const
{
	for (unsigned int i=0;i<x.size();i++)
	{
...
*/
bool
LAFunctionBase::isInDomain(const DoubleArray& x)const
{
	(void)x;
...
*/
bool
LAFunctionBase::isDifferentiable(const DoubleArray& x, unsigned int pos)const
{
	(void)pos; (void)x;
	throw LACoreNumericalError("This function does not support this method", __FILE__, __LINE__);
}
...
*/
bool
LAFunctionBase::isDifferentiable(const DoubleArray& x, unsigned int posi,unsigned int posj)const
{
...
*/
void
LAFunctionBase::setCheckFlag(bool checkflag)
{
...
*/
void
LAFunctionBase::setParam(const DoubleArray& param)
{
	mUpdateFlag = true;
...
*/
void
LAFunctionBase::setDiscontPoints(const std::set<double>& points)
{
...
*/
LAString
LAFunctionBase::convertToString(void) const
{
	LADataDoubles ret(mParam);
...
*/
void
LAFunctionBase::convertFromString(const LAString& str)
{
	LADataDoubles ret;
...
*/
LAFunctionBase&
LAFunctionBase::operator = (const LAFunctionBase& v)
{
