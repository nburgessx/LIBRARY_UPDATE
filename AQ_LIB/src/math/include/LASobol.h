...
// Function ID
#define FN_RAND_SOBOL     27841102 
// Function Name
#define FN_RAND_SOBOL_STR	"fn_rand_sobol"
...
/*! 
    @brief Class to generate Sobol.
*/
class LASobol : public LARandBase
{
public:
//  LIFECYCLE
    explicit LASobol(const LAString &DI, unsigned int factor = 1, bool isHalleyMod = true); // Default Construcotr
    virtual ~LASobol();// Destructor
    // copy constructor
    LASobol(const LASobol& rhs);
...
//  QUERY
    ...
    // 
///	virtual void                setInicial_function(const UlongArray& seed);
								//======================================
								// set dimension
	...
	LASobol & operator=( const LASobol & ) { return *this; }
...
protected:
	...
	void						initialize(void);
								//======================================
								// next int sequence
	const std::vector<unsigned int>& nextInt32Sequence() const;
    static const int mBits;     // bits
    static const double mNormalizationFactor; // normalization factor 
	LAString mDI;                    // direction integers
	unsigned int mFactor;                     // factor
	mutable bool mIsInitialized;              // initialize flag
    mutable unsigned int mSequenceCounter;   // sequence counter
    mutable bool mIsFirstDraw;                  // first draw flag
    mutable std::vector<unsigned int> mIntegerSequence; // integer sequence
    std::vector<std::vector<unsigned int> > mDirectionIntegers; // direction integers
