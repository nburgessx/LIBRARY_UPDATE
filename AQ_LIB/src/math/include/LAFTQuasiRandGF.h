...
// LAFTQuasiRandGF Function ID
#define FN_RAND_QUASIGF     1106
// LAFTQuasiRandGF Function Name
...
class LAFTQuasiRandGF : public LARandBase
{
public:
...
//  LIFECYCLE
	...
	//copy constructor
//	LAFTQuasiRandGF(const LAFTQuasiRandGF&);
	
	//destructor
    virtual ~LAFTQuasiRandGF();
...
//  QUERY
	...
	LAFTQuasiRandGF & operator=( const LAFTQuasiRandGF & ) { return *this; }
...
private:
	...
	*/
	...
	public:
		...
		~LAMatrixData();
		// copy constructor
		LAMatrixData(const LAMatrixData& m);
		...
		const int*				operator[](unsigned int n) const {return mpData[n];}
								...
								// assignment
		void					operator =(const LAMatrixData& other);

								// product of the matrix
		LAMatrixData			matMult(const LAMatrixData& other,int base) const;
		void					print(void);
