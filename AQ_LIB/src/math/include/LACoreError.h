...
// TODO: Allow LACoreError to inherit from std::exception on all platforms.
// With Linux / Intel Compiler we currently would have to decorate the destructor with the throw() qualifier.
#if defined(WIN32) || defined(WIN64)
class LACoreError : public virtual std::exception
#else
class LACoreError
#endif
{
...
//  LIFECYCLE
    ...
    LACoreError(void);
    // constructor
    LACoreError(const char_t* msg, const char_t* file, unsigned int line);
    // copy constructor
    LACoreError(const LACoreError& e);
    // destructor
    virtual ~LACoreError(void);
...
//  QUERY
    ...
    const LACoreError&      print(void) const;
    LACoreError&            print(const char_t* fileName);
    const LACoreError&      print(const char_t* fileName) const;
...
//  OPERATOR
    // assignment operator
    const LACoreError&      operator=(const LACoreError& e);
    // addition assignment operator 
    const LACoreError&      operator+=(const LACoreError& e);

private:
    LACoreErrorInfo*        mpErrInfo;  // error information
};
