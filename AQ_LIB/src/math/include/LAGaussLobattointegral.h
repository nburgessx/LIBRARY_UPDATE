...
protected:
								  ...
								  ) const
   {
		...
		{
			throw LACoreInvalidData("max number of iterations reached @ GaussLobattoIntegral::adaptivGaussLobattoStep", __FILE__, __LINE__);
		}
		...
		if(dist==acc || mll<=a || b<=mrr)
		{
		   	if( !(m>a && b>m) )
			{
				throw LACoreInvalidData("Interval contains no more machine number @ GaussLobattoIntegral::adaptivGaussLobattoStep ", __FILE__, __LINE__);
			}
			return integral1;
	    }
		else
								...
								) const
	{
		...
		if(acc == 0.)
		{
			throw LACoreInvalidData("can not calculate absolute accuracy from relative accuracy @ GaussLobattoIntegral::adaptivGaussLobattoStep", __FILE__, __LINE__);
		}
