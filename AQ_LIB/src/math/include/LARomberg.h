...
/*! 
	...
	This class derives from abstract base class "LAIntegralBase", and 
...
*/
class LARomberg : public LA1DIntegral
{
public:
//  LIFECYCLE
	// constructor
	LARomberg(unsigned short mpowertopoints, unsigned short morderofromberg=2, double romberg_eps = 1.0e-5);
	// destructor
	virtual ~LARomberg();
...
//  QUERY
	...
	virtual double				integrate(const LAFunctionBase& f, double xl, double xu) 
								const;
	...
	// assignment operator
	LARomberg & operator=( const LARomberg & ) { return *this; }
...
private:
	...
	double						trapzd(const LAFunctionBase &f,
								const double a,
								const double b,
								const int n) const;
