...
namespace etrading
{
	...
	SwaptionTrade::SwaptionTrade( const std::string& objectName, const LabelValueBlock& dealLVB ) : Option(objectName)
	{
        ...
        feeDate_                = dealLVB.getOptionalValueAsDate( SWAPTION_KEYS::FEE_DATE, AQLDate() );
        ...
        {
            AQ_REQUIRE( feeDate_ != AQLDate(), "Fee Date required - Must provide a feeDate when specifying fee payments")
        ...
        if ( notificationDays_ != "0D" )
        {
            ...
            AQLString notificationTenorString = AQLString("-") + AQLString( notificationDays_.c_str() );
	...
	*/
	LabelValueBlock SwaptionTrade::setupSwapExpressionLVBforSwaption( const AQLDate& swapStartDate, const LabelValueBlock& dealLVB ) const
	{
