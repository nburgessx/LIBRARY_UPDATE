...
namespace etrading
{
								     ...
								     AQLString daycount, AQLString interpolation, AQLString foreCurveName, AQLString dfCurveName, bool isFWDInter)
    {
	    ...
	    if(asofdate > datevec[0])
		    AQ_THROW( " fromdate < asofdate is not supported" );  
	
	    int N = datevec.size()-1;
	    for(int i=1;i<N;i++)
	    {
		    if(datevec[i-1] >= datevec[i])
			    AQ_THROW( "Wrong input of DateVector " );
	    }
	    ...
	    if ( isFWDInter && !setUpForwardDayCount(dataInstance, curveid, foreCurveName, yc) )
		    AQ_THROW( "Forward interpolation failed. Check whether the forward rate was generated by AQLMathYieldCurvePro !!" );
    ...
    */
									    ...
									    const AQLString& useCurveName)
    {
	    ...
	    {
		    AQ_THROW_IF( fixingDates[i-1] >= fixingDates[i], "Invalid Fixing Date(s)." );
	    }
	
	    size_t nCurveNames = curveNames.size();
	    size_t nCurveTenors = curveNames.size();
	    size_t nFixings = tenorCurveFixings.size();
	    AQ_THROW_IF( nCurveNames != nCurveTenors, "The number of curve names must equal the number of curve tenors" );

	    AQ_THROW_IF( nFixings != 0 && nFixings != nCurveNames, "If past fixings are provided, they must be provided for each curve name" );
								      ...
								      AQLString slidingRule)
    {
	    ...
	    AQ_THROW_IF( isFWDInter && !setUpForwardDayCount(dataInstance, curveid, foreCurveName, yc) && !isOIS, "Forward interpolation failed. Check whether the forward rate was generated by AQLMathYieldCurvePro !!" );
                                              ...
                                              const AQLDate&    settlementDate )
    {
	    ...
	    AQ_THROW_IF( isFWDInter && !setUpForwardDayCount(dataInstance, curveid, foreCurveName, yc), "Forward interpolation failed. Check whether the forward rate was generated by AQLMathYieldCurvePro !!" );
								     ...
								     AQLString dfCurveName, bool isFWDInter)
    {
	    ...
	    if(fromDate > toDate || asofdate > fromDate)
		    AQ_THROW( "toDate < fromdate and asofdate > fromDate are not supported" );  
                                  ...
                                  const AQLString* roll_convention)
    {
	    ...
	    {	
		    ...
		    ret = AQLCurveForwardRateHelpers::getDF(fromdate,term,dataInstance,curveid,daycount,slidingrule,calendar,interpolation,false,dfCurveName);
			    ...
			    AQ_THROW( "This rate type is not supported" );
	    return ret;
    }
							    ...
							    bool isbasisflag, AQLString curveName)
    {
	    ...
	    if(isbasisflag==true && ( !dh->isDefined() || dh->isNull() ) ) 
            AQ_THROW( "BasisCurve is not set" );
     ...
     */
     ...
     AQLCurveForwardRateHelpers::setCalendarForCurveID(AQLCurvePricingObject& yc, const AQLString& calendar)
     {
	     ...
	     {
		    AQ_THROW( "Error has occured in AQLCurveForwardRateHelpers::setCalednarForCurvID." );
	     }
...
{
    AQ_THROW( "Invalid Input, the 'fixingAdvanceOrArrears' parameter must be set to 'advance', 'arrears', 'adv', 'arr' or left blank." );
}
	...
	// *** Duplicate OF AQLMathCurveFuncUtility.cpp UpdateAccrualFrequency ***
    static void updateAccrualFrequency(AQLString& accrualFreq, const AQLString& paymentFreq, bool fixingInAdvance=true)
    {
	    ...
	    if (paymentFreq != AQLString())
	    {
		    ...
		    {
                AQ_THROW_IF( !fixingInAdvance, "Unable to fix in Arrears, since floating payments cannot be made before their fixing dates." );
    ...
    */
    static DateVector updatePaymentDates(const DateVector& accrualDates, const DateVector& paymentDates)
    {
        ...
        AQ_THROW_IF( accrualSize == 0, "The size of Accrual Dates cannot be empty." );
        AQ_THROW_IF( paymentSize == 0, "The size of Payment Dates cannot be empty." );
        ...
        AQ_THROW_IF( accrualDates.size() != result.size(), "The size of Accrual Dates and Payment Dates must be the same." );
    ...
    */
																	    ...
																	    const DateVector& combinedAccrualDates)
    {
        ...
        AQ_THROW_IF( nCombinedAccrualDates < 1, "Swap Accrual Dates Error, cannot have an accrual start date and no accrual end date" );
