...
namespace etrading
{
    ...
    typename to_cached_object_type<CURVE_DEPRECATED>::type createCacheableObject<CURVE_DEPRECATED>( const rapidjson::Value& jsonDoc )
    {
        ...
        // the data for building an LWOCurve instead of using rapidjson (and having to check the formation of the file all the time).
        ...
        if( hasCurveBuildProperties )
        {
            ...
            return std::move( LWOCurve(	objectName, dates, discountFactors, cbp, forwardRates ) );
        }
        else
        {
