    ...
    */
    AQLDate validateMaturityDate( const AQLDate& effectiveDate, const AQLString& maturityDateStr )
    {
        ...
        if( !isMaturityInDateFormat )
        {
            ...
            AQ_REQUIRE( AQLDateSchedule::isValidDate( effectiveDate ), "Invalid Date: Unable to convert MaturityTenor to a date" )
            maturityDate = AQLDateSchedule::getDate( effectiveDate,
                                                         maturityDateStr,		// maturity is Tenor
                                                         AQLString(),         // BusinessDayAdjustment is NO_CHANGE
                                                         AQLString());       // Calendar is NONE
        }
        
        AQ_REQUIRE( AQLDateSchedule::isValidDate( maturityDate ), "Invalid Maturity Date " + maturityDateStr )
        return maturityDate;
    }
