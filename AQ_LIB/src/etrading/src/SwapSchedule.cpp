...
namespace etrading
{
    ...
    {
        // Date Validation
        AQ_THROW_IF( !AQLDateSchedule::isValidDate( accrualStartDate ), "Invalid 'AccrualStartDate'." );

        AQ_THROW_IF( !AQLDateSchedule::isValidDate( accrualEndDate ), "Invalid 'AccrualEndDate'." );

        AQ_THROW_IF( accrualStartDate >= accrualEndDate, "The 'AccrualStartDate' must be strictly greater than the 'AccrualEndDate'." );
        ...
        if ( pFirstStub != NULL )
        {
            AQ_THROW_IF( !AQLDateSchedule::isValidDate( firstStub ), "Invalid 'FirstStub'." );
        }

        if ( pLastStub != NULL )
        {
            AQ_THROW_IF( !AQLDateSchedule::isValidDate( lastStub ), "Invalid 'LastStub'." );
        }

        AQ_THROW_IF( pFirstStub != NULL && firstStub < accrualStartDate, "The 'FirstStub' cannot be before the 'AccrualStartDate'." );

        AQ_THROW_IF( pLastStub != NULL && lastStub > accrualEndDate, "The 'LastStub' cannot be after the 'AccrualEndDate'." );

        AQ_THROW_IF( pFirstStub != NULL && pLastStub != NULL && firstStub > lastStub, "The 'LastStub' cannot be before the 'FirstStub'." );
        ...
        else AQ_THROW_IF( dayCount != AQLString( "ACT/360" )      && dayCount != AQLString( "30/360" )
                  && dayCount != AQLString( "ACT/365" )      && dayCount != AQLString( "30E/360" )
                  && dayCount != AQLString( "ACT/365_ISDA" ) && dayCount != AQLString( "ACT/365FJ" ), "Invalid 'AccrualDaycount', must be 'Act/360', 'Act/365', 'Act/365_ISDA', 'Act/365FJ', '30/360', '30E/360' or left blank." );
        ...
        if ( rollDayOrConvention != AQLString( "" ) )
        {
            ...
            {
                AQ_THROW( "'RollDayOrConvention' must be a day of the month or a date convention i.e. IMM or EOM (End of Month)." );
            }
        }

        // Generate the Accrual Base Dates: This is a one dimensional vector representing accrual start and end dates
        accrualBaseDates = AQLDateSchedule::generateSchedule( accrualStartDate,
                           accrualEndDate,
                           accFrequency,
        ...
        AQ_THROW_IF( accrualBaseDates.empty() || accrualBaseDates.size() < MINIMUM_NUMBER_OF_DATES, "Unable to generate accrual dates with the parameters specified." );
        ...
        paymentBaseDates = AQLDateSchedule::generateSchedule( accrualStartDate,
																	accrualEndDate,
																	payFrequency,
																	payRollConvention,
																	payCalendar,
        ...
        AQ_THROW_IF( paymentBaseDates.empty() || paymentBaseDates.size() < MINIMUM_NUMBER_OF_DATES, "Unable to generate the payment dates using the parameters specified." );
        ...
        if (  paymentBaseDates.size() >= accrualBaseDates.size() )
        {
            ...
            for( it = paymentBaseDates.begin(); it != paymentBaseDates.end(); ++it )
            {
                ...
                {
                    AQ_THROW_IF( accrualRowIndex == 0 || accrualRowIndex > accrualBaseDates.size(), "Unable to build schedule. Incorrect accrual schedule dimensions." );

                    AQ_THROW_IF( paymentRowIndex == 0 || paymentRowIndex > paymentBaseDates.size(), "Unable to build schedule. Incorrect accrual schedule dimensions." );
                    ...
                    AQ_THROW_IF( !isFixingInAdvance && paymentBaseDates.size() > accrualBaseDates.size(), "Unable to fix in Arrears, since floating paymennts cannot be made before their fixing dates." );

                    AQ_THROW_IF( ( accrualRowIndex - 1 ) < 0, "Unable to generate fixing dates. Access Violation Error." );
                    ...
                    // Apply Payment Lag
                    thisFixingDateWithLag       = AQLDateSchedule::getDate( isFixingInAdvance ? thisFixingDateNoLag : thisEndDate,
                                                  fixingLag,
                                                  fixingRollConvention,
                                                  fixingCalendar );

                    thisPaymentDateWithLag      = AQLDateSchedule::getDate( thisPaymentDateNoLag,
                                                  paymentLag,
                    ...
                    thisAccrualPeriod           = AQLDateSchedule::getTerm( thisStartDate,
                                                  thisEndDate,
                                                  dayCount,
                                                  DONT_INCLUDE_LAST_PAYMENT_DATE );

                    AQ_THROW_IF( thisFixingDateWithLag > thisPaymentDateWithLag, "Payments cannot be made before fixing dates. Please check if the schedule payment and fixing lags and other inputs are correct." );

                    AQ_THROW_IF( thisStartDate > thisEndDate, "Accrual start dates cannot be after the accrual end dates. Please check that the schedule inputs are correct." );

                    // Update the schedule's current row of results
                    scheduleRow.push_back( ( double )( AQLDateSchedule::getExcelDate( thisFixingDateWithLag ) ) );
                    scheduleRow.push_back( ( double )( AQLDateSchedule::getExcelDate( thisStartDate ) ) );
                    scheduleRow.push_back( ( double )( AQLDateSchedule::getExcelDate( thisEndDate ) ) );
                    scheduleRow.push_back( thisAccrualPeriod );
                    scheduleRow.push_back( ( double )( AQLDateSchedule::getExcelDate( thisPaymentDateWithLag ) ) );
        ...
        else
        {
            ...
            for( it = accrualBaseDates.begin(); it != accrualBaseDates.end(); ++it )
            {
                ...
                else
                {
                    ...
                    thisFixingDateWithLag       = AQLDateSchedule::getDate( isFixingInAdvance ? thisStartDate : thisEndDate,
                                                  fixingLag,
                                                  fixingRollConvention,
                                                  fixingCalendar );
