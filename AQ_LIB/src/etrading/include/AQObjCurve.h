...
namespace etrading
{
    ...
    public:
        ...
        AQObjCurve( const std::string& objName );

        AQObjCurve( AQObjCurve&& );

        AQObjCurve( const AQObjCurve& ); // Only doing this so we can copy from the cache (but do we really need to do that?)

        virtual ~AQObjCurve(); // virtual in case we want to make this a base
