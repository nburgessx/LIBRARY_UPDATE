...
*/
AQLMarketDataLMM::AQLMarketDataLMM(void)
{
}
...
*/
void
AQLMarketDataLMM::getCanonicalGrid(DoubleArray &tenor_30_360, DoubleArray &tenor, 
						...
						const AQLString &freq, int max, AQLStringVector &exTenor, const bool isDataOut) 
{
	...
	AQLString str_tenor = AQLCoreDataService::getContext(CONTEXT_KEY_LMM_TENOR);
	AQLString str_tenor_30_360 = AQLCoreDataService::getContext(CONTEXT_KEY_LMM_30_360_TENOR);
	AQLString str_deltatenor = AQLCoreDataService::getContext(CONTEXT_KEY_LMM_DELTATENOR);
	...
	if (isDataOut)
    {
 		const AQLString fileSuffix = AQLCoreDataService::getContext(ARG_KEY_FILENUM) + "_" + freq;
...
*/
double
AQLMarketDataLMM::getSkew(const AQLString &currency)
{
	...
	AQLFileAccessor file(AQLMarketData::getNumFileName(skew_fPath));
	...
	{
		AQLString msg = "LMM skew file, format is wrong. file = " + AQLMarketData::getNumFileName(skew_fPath);
		throw AQLCoreInvalidData(msg.getCString(), __FILE__, __LINE__);
	}
...
*/
double
AQLMarketDataLMM::getConstShift(const AQLString &currency)
{
	AQLString key_ccy = currency;
	key_ccy.toLower();
	...
	AQLFileAccessor file(AQLMarketData::getNumFileName(constShift_fPath));
	...
	{
		AQLString msg = "LMM con file, format is wrong. file = " + AQLMarketData::getNumFileName(constShift_fPath);
		throw AQLCoreInvalidData(msg.getCString(), __FILE__, __LINE__);
	}
...
*/
double
AQLMarketDataLMM::getMaxTerm(const AQLString &currency)
{
	AQLString key_ccy = currency;
	key_ccy.toLower();
	...
	AQLFileAccessor file(AQLMarketData::getNumFileName(maxterm_fPath));
	...
	{
		AQLString msg = "LMM max term file, format is wrong. file = " + AQLMarketData::getNumFileName(maxterm_fPath);
		throw AQLCoreInvalidData(msg.getCString(), __FILE__, __LINE__);
	}
...
*/
AQLStringVector
AQLMarketDataLMM::getGridExTenor()
{
	AQLStaticData &staticData = AQLCoreDataService::getStaticDataManager().getStaticData();
	...
	AQLStringVector exTenorForCanonicalGrid = AQLMarketDataLMM::getCanonicalGridExTenor();
	for (unsigned int i = 0; i < exTenorForCanonicalGrid.size(); ++i)
	{
		if(std::find(ret.begin(), ret.end(), exTenorForCanonicalGrid[i]) == ret.end())
		{
			if((exTenorForCanonicalGrid[i] == "3M" && AQLCoreDataService::getContext(ARG_KEY_SIMFREQ) == "QA") ||
			   (exTenorForCanonicalGrid[i] == "6M" && AQLCoreDataService::getContext(ARG_KEY_SIMFREQ) == "SA"))
			{
				continue;
			}
...
*/
AQLStringVector
AQLMarketDataLMM::getCanonicalGridExTenor()
{
	AQLStaticData &staticData = AQLCoreDataService::getStaticDataManager().getStaticData();
