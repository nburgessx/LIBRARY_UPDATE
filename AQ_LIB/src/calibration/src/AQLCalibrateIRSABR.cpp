...
*/
AQLCalibrateIRSABR::AQLCalibrateIRSABR()
:AQLCalibrate()
{
}
...
*/
void
AQLCalibrateIRSABR::setUp(AQLObjectPool &objPool, const AQLScenarioParam &param, AQLCalibrationFunc *method, int gridPos)
{
	
	cout << static_cast<int>(AQLCoreThread::getThreadID()) << " AQLCalibrateIRSABR set up called" << endl;
	if (!method)
	{
	...
	for (unsigned int i_under = 0; i_under < calibInfoNames.size(); ++i_under)
	{
		...
		AQLString calibTarget_str = AQLCoreDataService::getContext(ccy + "." CONTEXT_KEY_DEAL_IRVOL + "." + underlying);
	...
	cout << static_cast<int>(AQLCoreThread::getThreadID()) << " AQLCalibrateIRSABR set up end" << endl;
}
...
*/
void
AQLCalibrateIRSABR::doCalibrate()
{

	//calibration start
	cout << static_cast<int>(AQLCoreThread::getThreadID()) << " AQLCalibrateIRSABR calibrate called.." << endl;
	...
	delete method;
	cout << static_cast<int>(AQLCoreThread::getThreadID()) << " AQLCalibrateIRSABR calibrate end.." << endl;
}
