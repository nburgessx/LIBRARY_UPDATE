...
//  2007, AlgoQuantHub.
////X///////////////////X///////////////////////////////X///////////////////
//  NAME        :       AQLCalibrationParametersIRSABR.cpp
//
...
*/
AQLCalibrationParametersIRSABR::AQLCalibrationParametersIRSABR(void)
...
*/
AQLString 
AQLCalibrationParametersIRSABR::createCalibrationInfo(AQLObjectPool &objPool, const AQLString &ccy)
{
	...
	for (unsigned int i = 0; i < underlyings.size(); ++i)
	{
		...
		for (unsigned int k = 0; k < forSize; k++)
		{
			...
			if (0 == k)
			{
				file =  AQLMarketData::getNumFileName(mpCalibStaticData->getStaticData(tmpCurrency + STATIC_DATA_KEY_CALIB_SWAPTION_FILE + "." + underlyings[i]));
				addstr =  AQLString("_") + PRICING_DATA_VOLID;
			}
			else if (1 == k)
			{
				file =  AQLMarketData::getNumFileName(mpCalibStaticData->getStaticData(tmpCurrency + STATIC_DATA_KEY_CALIB_SABR_ALPHA_FILE + "." + underlyings[i]));
				addstr =  AQLString("_") + PRICING_DATA_ALPHAID;
			}
			else if (2 == k)
			{
				file =  AQLMarketData::getNumFileName(mpCalibStaticData->getStaticData(tmpCurrency + STATIC_DATA_KEY_CALIB_SABR_BETA_FILE + "." + underlyings[i]));
				addstr =  AQLString("_") + PRICING_DATA_BETAID;
			}
			else if (3 == k)
			{
				file =  AQLMarketData::getNumFileName(mpCalibStaticData->getStaticData(tmpCurrency + STATIC_DATA_KEY_CALIB_SABR_RHO_FILE + "." + underlyings[i]));
				addstr =  AQLString("_") + PRICING_DATA_RHOID;
			}
			else if (4 == k)
			{
				file =  AQLMarketData::getNumFileName(mpCalibStaticData->getStaticData(tmpCurrency + STATIC_DATA_KEY_CALIB_SABR_NU_FILE + "." + underlyings[i]));
			...
			if (0==k || !isonlyatmvol) //atmvol or usualcalibration case
			{
				for (unsigned int row = 0; row < valmat.size(); ++row)
					for (unsigned int col = 0; col < valmat[row].size(); ++col)
						valmat[row][col] = AQLMarketData::getVolatilityVal(file, optionMatVec[row], sTenorVec[col]);
			}
			...
			if (k == 0)
			{
				...
				{
					const AQLString _premFileName =  AQLMarketData::getNumFileName(premFileName);
					...
					for (unsigned int row = 0; row < premiummat.size(); ++row)
						for (unsigned int col = 0; col < premiummat[row].size(); ++col)
							premiummat[row][col] = AQLMarketData::getVolatilityVal(_premFileName, optionMatVec[row], sTenorVec[col]);
