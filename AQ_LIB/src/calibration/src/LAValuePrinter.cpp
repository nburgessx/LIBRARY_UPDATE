...
*/
int 
MAValuePrinter::printCF(const vector<AQLString> &mainTradeVec, const vector<AQLString> &fileNumVec,
			...
			(objHolder.getData(CALIBRATION_DATA_VALUE, ISNOTNULL).get()).getType() == FN_IR_PORTFOLIOVALUE)
		{
			...
			if (staticData.getStaticData(KEY_DEAL_SUMMARY_INFO + AQLString(".")  + portName.toLower()) != AQ_NO_DATA)
			{
				...
				if (summaryInfo.size() != tradeNames.size())
					throw AQLCoreInvalidData("MAValuePrinter::printValue failed! Sizes of summary information are inconsistent!",__FILE__,__LINE__);
...
*/
void
MAValuePrinter::printFeeCF(AQLObjectHolder& tradeEntity, ofstream& outFile)
{
	// ! print fee value
...
*/
int 
MAValuePrinter::printValue(const vector<AQLString> &mainTradeVec, const vector<AQLString> &fileNumVec,
						   const vector<AQLDataInstance *> &rootVec, const vector<AQLString> &ccyVec, AQLString calc)
{
	vector<bool> sFlags;
	LAStaticData &staticData = LACoreDataService::getStaticDataManager().getStaticData();
	...
	for (unsigned int i = 0; i < portNum; ++i)
	{
			...
			(objHolder.getData(CALIBRATION_DATA_VALUE, ISNOTNULL).get()).getType() == FN_IR_PORTFOLIOVALUE)
		{
			...
			{				
				...
				if (staticData.getStaticData(KEY_DEAL_SUMMARY_INFO + AQLString(".")  + portName.toLower()) == AQ_NO_DATA)
					throw AQLCoreInvalidData("MAValuePrinter::printValue failed! No summary information!",__FILE__,__LINE__);
				AQLStringVector summaryInfo = staticData.getStaticData(KEY_DEAL_SUMMARY_INFO + AQLString(".")  + portName.toLower()).toToken(';');
				if (summaryInfo.size() != tradeNames.size())
					throw AQLCoreInvalidData("MAValuePrinter::printValue failed! Sizes of summary information are inconsistent!",__FILE__,__LINE__);
				for (unsigned int j = 0; j < summaryInfo.size(); ++j)
...
*/
int
MAValuePrinter::printFeeValue(const vector<AQLString> &mainTradeVec, const vector<AQLString> &fileNumVec,
...
*/
bool 
MAValuePrinter::printPV(const AQLString &name, const vector<const AQLObject*> &tradeEntities, ofstream &file, const AQLString &ccys, bool isDetail, bool isvanilla)
{
	const AQLDataHolder* ah_c;	
	// size check
	if (tradeEntities.size() == 0)
	{
		throw AQLCoreInvalidData("MAValuePrinter::printPV failed! The number of trade is zero!",__FILE__,__LINE__);
	}
...
*/
bool 
MAValuePrinter::printRisk(const AQLString &name, const AQLString &riskName, const vector<const AQLObject*> &tradeEntities, const AQLObject &riske, ofstream &file, bool isBase)
{
	// size check
	if (tradeEntities.size() == 0)
		throw AQLCoreInvalidData("MAValuePrinter::printRisk failed! The number of trade is zero!",__FILE__,__LINE__);
	bool ret = true;
	...
	if (isRiskAttrExist)
	{
		if (attrRisk->getType() == DATA_DOUBLES) //isGridSensitivity=TRUE
		{
			...
			AQLString isOmitStart_str = LACoreDataService::getContext(CONTEXT_KEY_RISKGRID_ISOMITSTART).toUpper();
			AQLString isOmitEnd_str = LACoreDataService::getContext(CONTEXT_KEY_RISKGRID_ISOMITEND).toUpper();
...
bool
MAValuePrinter::printIMMFwdRisk(const AQLString &name, const AQLString &riskName, const vector<const AQLObject*> &tradeEntities, const AQLObject &riske, ofstream &file)
{
	// size check
	if (tradeEntities.size() == 0)
		throw AQLCoreInvalidData("MAValuePrinter::printIMMFwdRisk failed! The number of trade is zero!",__FILE__,__LINE__);
	bool ret = true;
	const AQLDataHolder *dh;
...
*/
bool 
MAValuePrinter::printAttr(AQLDataInstance *dataInstance, ofstream &file, const AQLString &ccys, bool isDetail, bool isvanilla, bool isSummary)
{
...
void
MAValuePrinter::changeSemiAnalyticResult(AQLDataInstance& dataInstance,const AQLString& infileName, std::ofstream &file)
{
...
int MAValuePrinter::printEntityDump(const std::vector< AQLString >& fileNumVec,
                                    ...
                                    )
{
    ...
    try
    {
        AQLString fnameBase = LACoreDataService::getStaticDataManager().getStaticData().getStaticData(KEY_ENTITY_DUMP_FILE);
...
int MAValuePrinter::printCompoundedRates(const AQLObject& trade,
...
void MAValuePrinter::printFxOptionMatrix(AQLDataInstance* dataInstance, const AQLString& filenum)
{
    const AQLString fname_base = LACoreDataService::getStaticDataManager().getStaticData().getStaticData(KEY_DEAL_FXOPT_BULK_OUTFILE);
    ...
    AQLDataStringMatrix temp; temp.convertFromString(LACoreDataService::getStaticDataManager().getStaticData().getStaticData(KEY_DEAL_FXOPT_BULK));
    ...
    AQLDataStrings temp2; temp2.convertFromString(LACoreDataService::getStaticDataManager().getStaticData().getStaticData(KEY_DEAL_FXOPT_BULK_LABEL));
