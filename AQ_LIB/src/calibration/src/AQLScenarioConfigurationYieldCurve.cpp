...
*/
AQLScenarioConfigurationYieldCurve::AQLScenarioConfigurationYieldCurve(void)
{
}
...
AQLScenarioConfigurationYieldCurve::createScenario(AQLDataInstance &dataInstance, const AQLScenarioParam &param) const
{
	...
	if (param.isParallel)
	{
		...
		if (param.isAudExtra && tmpCurrency == CURRENCY_AUD && aud_firstRefString.size() > 0)
		{
			...
			AQLMarketData::resetMarketDataUseL(bYieldPro, tmpCurrency, &param.targetCurveType);
			// restore market ref
		...
		{
			AQLMarketData::restoreSwapRateFromL(bYieldPro, aud_origSwapRate, tmpCurrency, &param.targetCurveType);
		}
	...
	if (param.isGrid)
	{
		...
		for (unsigned int i = 0; i < gridSize; ++i)
		{
			...
			for (unsigned int j = targetSize; j > 0; --j)
			{
				...
				{
					AQLMarketData::restoreSwapRateFromL(bYieldPro, aud_waveSwapRate, tmpCurrency, &param.targetCurveType);
				}
				++t_it_first;
			}
		}
