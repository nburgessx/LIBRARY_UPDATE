...
AQLCalibrationParametersLMM::createCalibrationInfo(AQLObjectPool &objPool, const AQLString &ccy)
{
	...
	double gridsizeG = AQLDateCalculations::getPeriodFrequencyInMonths(frequencyG)/12.0;
	...
	AQLDate _00_pay = etrading::AQLDateSchedule::getDate(asOfDate, capSpotLag, liborSlidingRule, liborCalendar);
	...
	for (unsigned int i = 0; i < capTermSize; ++i)
	{
		...
		_0_pay   = etrading::AQLDateSchedule::getDate(_00_pay, capTenor, liborSlidingRule, liborCalendar); // start date?
		last_pay = etrading::AQLDateSchedule::getDate(_0_pay, capTerm, liborSlidingRule, liborCalendar); // expiry date?

		date_stream = etrading::AQLDateSchedule::generateSchedule(_0_pay, last_pay, liborFrequency, liborSlidingRule, liborCalendar);
		...
		{
			T_pay_cap[j] = etrading::AQLDateSchedule::getTerm( asOfDate, date_stream[j + 1], dayCountStr, true );

			tmp_D = etrading::AQLDateSchedule::getDate(date_stream[j], capSpotLag_mns, liborSlidingRule, liborCalendar);
			T_fix_cap[j] = etrading::AQLDateSchedule::getTerm( asOfDate, tmp_D, dayCountStr, true );

			tau_L_cap[j] = etrading::AQLDateSchedule::getTerm( date_stream[j], date_stream[j + 1], liborDayCount, liborIncludeLast );
			tau_cap[j] = etrading::AQLDateSchedule::getTerm( date_stream[j], date_stream[j + 1], capDayCount, capIncludeLast );
		}
	...
	for (unsigned int i = 0; i < optionMatSize; ++i)
	{
		...
		_00_pay = etrading::AQLDateSchedule::getDate(asOfDate, optionMat, swaptionSlidingRule, swaptionCalendar);
		_0_pay = etrading::AQLDateSchedule::getDate(_00_pay, swaptionSpotLag, swaptionSlidingRule, swaptionCalendar);
		if (swaptionForecastCurve == swaptionDiscountCurve && liborFrequency == swaptionFrequency)
		{
			...
			_00_pay = etrading::AQLDateSchedule::getDate(asOfDate, swaptionSpotLag, swaptionSlidingRule, swaptionCalendar);
			_0_pay = etrading::AQLDateSchedule::getDate(_00_pay, optionMat, swaptionSlidingRule, swaptionCalendar);
		}
		for(unsigned int j = 0; j < swapTenorSize; ++j)
		{
			const AQLString swapTenor = swapTenorVec[j];

			last_pay_L    = etrading::AQLDateSchedule::getDate(_0_pay, swapTenor, liborSlidingRule, liborCalendar);
			date_stream_L = etrading::AQLDateSchedule::generateSchedule(_0_pay, last_pay_L, liborFrequency, liborSlidingRule, liborCalendar);
			...
			{
				T_pay_L_swaption[k] = etrading::AQLDateSchedule::getTerm( asOfDate, date_stream_L[k + 1], dayCountStr, true );
				T_fix_L_swaption[k] = etrading::AQLDateSchedule::getTerm( asOfDate, date_stream_L[k], dayCountStr, true );
				tau_L_swaption[k] = etrading::AQLDateSchedule::getTerm( date_stream_L[k], date_stream_L[k + 1], liborDayCount, liborIncludeLast );
			}

			last_pay    = etrading::AQLDateSchedule::getDate(_0_pay, swapTenor, swaptionSlidingRule, swaptionCalendar);
			date_stream = etrading::AQLDateSchedule::generateSchedule(_0_pay, last_pay, swaptionFrequency, swaptionSlidingRule, swaptionCalendar);
			...
			{
				T_pay_swaption[k] = etrading::AQLDateSchedule::getTerm( asOfDate, date_stream[k + 1], dayCountStr, true );
				tau_swaption[k] = etrading::AQLDateSchedule::getTerm( date_stream[k], date_stream[k + 1], swaptionDayCount, swaptionIncludeLast );
			}
