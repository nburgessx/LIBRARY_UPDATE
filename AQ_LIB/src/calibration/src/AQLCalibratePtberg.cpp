...
*/
AQLCalibratePtberg::AQLCalibratePtberg()
...
*/
void
AQLCalibratePtberg::setUp(AQLObjectPool &objPool,  const AQLScenarioParam &param, AQLCalibrationFunc *method, int gridPos)
{
	cout << static_cast<int>(AQLCoreThread::getThreadID()) << " AQLCalibratePtberg set up called" << endl;
	if (!method)
	{
...
//			AQLDate date = AQLDateCalculations::getDate(asofDate, optionMatVec[i], termSliding, &termCal, true);
	...
	cout << static_cast<int>(AQLCoreThread::getThreadID()) << " AQLCalibratePtberg set up end" << endl;
}
...
{
	//calibration start
	cout << static_cast<int>(AQLCoreThread::getThreadID()) << " AQLCalibratePtberg calibrate called.." << endl;
...
double time = (double)(cend - cstart) / CLOCKS_PER_SEC;
	cout << static_cast<int>(AQLCoreThread::getThreadID()) << " AQLCalibratePtberg calibrate end.." << endl;
cout << "-> time = " << time << endl;
}
