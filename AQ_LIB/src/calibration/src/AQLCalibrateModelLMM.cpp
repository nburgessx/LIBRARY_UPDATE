...
*/
AQLCalibrateModelLMM::AQLCalibrateModelLMM(const AQLString &baseCurrency)
...
*/
AQLCalibrateModelLMM::~AQLCalibrateModelLMM(void)
{
}
...
*/
void
AQLCalibrateModelLMM::setUp(void)
{
	...
	AQLMarketDataLMM::getCanonicalGrid(mTenor_30_360, mTenor, mDeltatenor, mExtraTenorFlag, asOfDate, dayCount, freq, term, exTenor, isDataOut);
}
...
*/
SDE_TYPE
AQLCalibrateModelLMM::getSDEType(const AQLString &currency) const
{
...
*/
bool 
AQLCalibrateModelLMM::isLJ(const AQLString &currency) const
{
	AQLString key_ccy = currency;
...
*/
AQLRatesSDEBase *
AQLCalibrateModelLMM::createSDEInstance(const AQLString &currency, AQLDataInstance &dataInstance) const
{
	(void)dataInstance;
...
*/
void
AQLCalibrateModelLMM::setVolatility(const AQLString &currency, AQLRatesSDEBase &sde) const
{
	AQLString key_ccy = currency;
...
*/
void
AQLCalibrateModelLMM::setDrift(const AQLString &currency, AQLRatesSDEBase &sde) const
{
	AQLString key_ccy = currency;
	AQLString sdeName = mpStaticData->getStaticData(key_ccy.toLower() + STATIC_DATA_FX_KEY_SDE_NAME);
	const int size = mTenor.size() - 2;
	...
	if (!isCancelForFunding(currency))
		skew = AQLMarketDataLMM::getSkew(currency);
...
*/
void
AQLCalibrateModelLMM::setNumeraire(const AQLString &currency, AQLRatesSDEBase &sde) const
{
	if (AQLDealUtils::getSDECurrencys().size() != 1)
	{
...
*/
void
AQLCalibrateModelLMM::setOutputTemplate(const AQLString &currency, AQLRatesSDEBase &sde) const
{
	...
	if (!isCancelForFunding(currency))
	{
		...
		skew = AQLMarketDataLMM::getSkew(currency);
		constShift = AQLMarketDataLMM::getConstShift(currency);
	}
	if (skew <= 0. || skew >= 2.)
	{
...
*/
void
AQLCalibrateModelLMM::setIntegralFunction(const AQLString &currency, AQLRatesSDEBase &sde) const
{
	AQLString key_ccy = currency;
...
*/
AQLString 
AQLCalibrateModelLMM::getFunctionMasterResistName(const AQLString &currency) const
{
...
*/
AQLString
AQLCalibrateModelLMM::getCorTye(const AQLString &currency) const
{
	AQLString key_ccy = currency;
...
*/
void
AQLCalibrateModelLMM::setUpCorFactor(const AQLString &currency, AQLMathCorrelation &cor, AQLDataInstance &dataInstance) const
{
	(void)dataInstance;
	...
	AQLFileAccessor file(AQLMarketData::getNumFileName(filePath));
...
*/
void
AQLCalibrateModelLMM::setUpCorData(const AQLString &currency, AQLMathCorrelation &cor, AQLDataInstance &dataInstance) const
{
	(void)dataInstance;
	setUpCorEntity(currency, cor);
	AQLString key_ccy = currency;
...
*/
void
AQLCalibrateModelLMM::setUpCorFunc(const AQLString &currency, AQLMathCorrelation &cor, AQLDataInstance &dataInstance) const
{
	(void)dataInstance;
	setUpCorEntity(currency, cor);
	AQLString key_ccy = currency;
	...
	if (param.isCalib)
	{
		AQLCalibrationParametersLMM cInfo;
		AQLCalibCorrelationParam calibCorParam;
	...
	else
	{
		...
		// read method param
		AQLFileAccessor paramFile(AQLMarketData::getNumFileName(paramFilePath));
		AQLStringMatrix funcParamMtx;
		paramFile.readAllData(MARKET_DATA_DELIMITER, funcParamMtx);
		paramFile.close();

		// read max param
		AQLFileAccessor maxFile(AQLMarketData::getNumFileName(maxFilePath));
...
*/
AQLString
AQLCalibrateModelLMM::getVolType(const AQLString &currency) const
{
	AQLString key_ccy = currency;
...
*/
void
AQLCalibrateModelLMM::setUpVolFunc(const AQLString &currency, AQLMathVolatility &vol, AQLDataInstance &dataInstance) const
{
	...
	fileVec[0] = AQLMarketData::getNumFileName(mpStaticData->getStaticData(key_ccy + STATIC_DATA_KEY_LMM_VOLATILITY_FUNC_CALIB_FILE));
	fileVec[1] = AQLMarketData::getNumFileName(mpStaticData->getStaticData(key_ccy + STATIC_DATA_KEY_LMM_VOLATILITY_FUNC_ADJ_FILE));
	fileVec[2] = AQLMarketData::getNumFileName(mpStaticData->getStaticData(key_ccy + STATIC_DATA_KEY_LMM_MAXTERM_FILE));

	if (AQLDealUtils::getSDECurrencys().size() == 1)
	{
		fileVec[3] = AQLMarketData::getNumFileName(mpStaticData->getStaticData(key_ccy + STATIC_DATA_KEY_LMM_SKEW_SINGLE_FILE));
	}
	else
	{
		fileVec[3] = AQLMarketData::getNumFileName(mpStaticData->getStaticData(key_ccy + STATIC_DATA_KEY_LMM_SKEW_CROSS_FILE));
	}

	
	AQLCalibrateVolatilityLMM volCreator;
	// create method vec
...
*/
void
AQLCalibrateModelLMM::setInterpolationMethod(const AQLString &currency, AQLRatesSDEBase &sde) const
{
	AQLString key_ccy = currency;
	key_ccy.toLower();
	AQLString interType = mpStaticData->getStaticData(key_ccy + STATIC_DATA_KEY_LMM_PATH_INTERPOLATION);
...
//		key_ccy.toLower();
//
//		bool isCalibTarget = AQLCalibrateModel::isCalibTarget(currency); 
//
//		if (! isCalibTarget)
//		{
...
AQLCalibrateModelLMM::createCurveLogLinearInterpolation() const
{
...
*/
void
AQLCalibrateModelLMM::setUpVolData(const AQLString &currency, AQLMathVolatility &vol, AQLDataInstance &dataInstance) const
{
	(void)dataInstance;
	setUpVolEntity(currency, vol);
	AQLString key_ccy = currency;
...
*/
void
AQLCalibrateModelLMM::setUpCorEntity(const AQLString &currency, AQLMathCorrelation &cor) const
{
	// set grid_T
	DoubleArray grid_T;
	...
	if (isCalibTarget(currency))
	{
		...
		AQLCalibrateModelLMM::setUpTenorForCalib(currency, tenor_30_360_forCalib, grid_T, deltatenor_forCalib, extraTenorFlag);
	}
	else
	{
		grid_T = mTenor;
...
*/
void
AQLCalibrateModelLMM::setUpVolEntity(const AQLString &currency, AQLMathVolatility &vol) const
{
	// set interpolation
	AQLString tmpCurrency = currency;
...
*/
void
AQLCalibrateModelLMM::setUpTenorForCalib(const AQLString &currency, DoubleArray &tenor_30_360, DoubleArray &tenor, DoubleArray &deltatenor, BoolVector &extraTenorFlag) const
{
	int maxTerm = (int)AQLMarketDataLMM::getMaxTerm(currency);
	...
	AQLStringVector exTenor = AQLMarketDataLMM::getCanonicalGridExTenor();
 	AQLMarketDataLMM::getCanonicalGrid(tenor_30_360, tenor, deltatenor, extraTenorFlag, asOfDate, dayCount, freq, maxTerm, exTenor);
}
