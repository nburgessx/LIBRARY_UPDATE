...
AQLRiskConfigurationYieldIRDeltaSimple::createMarketBumpYieldEntity(const AQLString &ccy, AQLDataInstance &dataInstance, SCENARIONUM scenarioNum, int index) const
{
...
//  getMarketTerms(ccy, param.paraTerm);
    ...
    std::shared_ptr<AQLScenarioConfiguration> creator(AQLScenarioConfigurationManager::getInstance()->createScenarioCreator(RISK_SCENARIO_YIELD));
