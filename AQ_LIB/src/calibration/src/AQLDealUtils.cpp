...
*/
AQLDealUtils::AQLDealUtils(void)
{
}
...
*/
AQLStringVector 
AQLDealUtils::getSDECurrencys(const bool isIncludeVol)
{
	// get currencys
	AQLStringVector ret;
	AQLString ccys = AQLCoreDataService::getContext(ARG_KEY_CURRENCY);
	ret = ccys.toToken(':');
	if (isIncludeVol)
	{
...
*/
AQLStringVector 
AQLDealUtils::getSimulationSDECurrencys(const bool isIncludeVol)
{
	// get currencys
	AQLStringVector ret;
	AQLString ccys = AQLCoreDataService::getContext(ARG_KEY_SIMCURRENCY);
	if (ccys != AQ_NO_DATA)
	{
		ret = ccys.toToken(':');
		if (isIncludeVol)
...
*/
AQLStringVector 
AQLDealUtils::getSDECurrencysAll()
{
	// get currencys
	AQLStringVector ret;
	AQLString ccys = AQLCoreDataService::getContext(ARG_KEY_CURRENCY);
...
*/
AQLStringVector 
AQLDealUtils::getAllSingleCurrencys()
{
	// get currencys
	AQLStringVector ret;
	AQLString ccys = AQLCoreDataService::getContext(ARG_KEY_CURRENCY);
...
*/
int
AQLDealUtils::getCMSYears(const AQLDataMultiReference &ref, const AQLString &indexAttr)
{
	int ret = 0;
...
*/
int
AQLDealUtils::getMaxTerm(AQLObjectPool &objPool, const AQLDate &asOfDate, AQLString tradetype)
{
...
*/
int
AQLDealUtils::getMaxTerm(const AQLObjectHolder &objHolder, const AQLDate &asOfDate)
{
	// max date
	AQLDate maxDate;
...
*/
int
AQLDealUtils::getMaxTermFromPlainVanilla(const AQLObjectHolder &objHolder, const AQLDate &asOfDate)
{
	// max date
	AQLDate maxDate;
	maxDate.setSystemDate();
	...
	{
		return AQLDealUtils::getMaxTerm(objHolder, asOfDate) + addYears;
	}
...
*/
AQLString 
AQLDealUtils::getAppMat()
{
	AQLString appMat = AQLCoreDataService::getContext(ARG_KEY_APPMAT);
...
*/
BoolMatrix 
AQLDealUtils::getCalibTargetIRVolGrids(AQLObjectPool &objPool, 
	...
	if (isPropSource)
	{
		...
		for (unsigned int i = 0; i < dataMtx.size(); ++i)
		{
			...
			else if (tenorInfos.size() == 3)
			{
				...
				const AQLString frequency = AQLMarketData::getFrequency(accessory);
				//get tenor
...
void 
AQLDealUtils::addCalibTargetFlag( BoolMatrix& calibTaretMat,
								 double expiry, 
								 double tenor, 
...
void 
AQLDealUtils::addStochasticVolIndex(AQLStringVector& ccys)
{
