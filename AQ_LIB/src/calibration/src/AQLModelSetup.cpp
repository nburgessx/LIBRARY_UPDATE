...
{
	AQLCoreDataService::setContext(CONTEXT_KEY_TIMEGRID_DAYCOUNT, DAYCOUNT_ACT_365_ISDA);
	//max term
	...
	AQLCoreDataService::setIStringStream(AQLMarketData::getNumFileName(volFileName), pVolStream);
	fileCont = "";
	// spot fx
	...
	AQLCoreDataService::setIStringStream(AQLMarketData::getNumFileName(fxFileName), pFXStream);
	ccys[0].toLower(), ccys[1].toLower();
	fileCont = "";
							...
							const AQLString& type)
{
	...
	AQLCoreDataService::setIStringStream(AQLMarketData::getNumFileName(corFileName), pCorStream);
	ccys[0].toLower(), ccys[1].toLower();
	fileCont = "";
	...
	AQLString hwParamName_d = AQLMarketData::getCalibDataName(KEY_PV, AQLMarketData::getYieldDataName(objPool, AQLMarketData::getBaseYieldName(ccys[0])));
	...
	AQLString hwParamName_f = AQLMarketData::getCalibDataName(KEY_PV, AQLMarketData::getYieldDataName(objPool, AQLMarketData::getBaseYieldName(ccys[1])));
	...
	AQLCoreDataService::setIStringStream(AQLMarketData::getNumFileName(initalFileName), pInitialStream);
...
setUpCalibPreparation(AQLDataInstance &dataInstance, const AQLString& fx)
{
	...
	{
		AQLCoreDataService::setContext(CONTEXT_KEY_ASOFDATE, "19000101");
	}
	AQLStringVector ccys;
	AQLMarketData::convertToCurrency(fx, ccys);
	// set currency
	AQLCoreDataService::setContext(ARG_KEY_CURRENCY, ccys[0].toUpper() + ":" + ccys[1].toUpper() + ":" + tmpFX.toUpper());
	...
	//AQLString initalFileName = calibProp.getStaticData(tmpFX + FX_KEY_CALIB_PTBERG_INITIALPARAM_FILE);
	AQLString fileCont = "";
	//istringstream *pInitailStream = AQLCoreDataService::getIStringStream(AQLMarketData::getNumFileName(initalFileName));
	//if (!pInitailStream)
	//{
	...
	//	AQLCoreDataService::setIStringStream(AQLMarketData::getNumFileName(initalFileName), pInitailStream);
	//	fileCont = "";
	//}
	// correlation file
	AQLStaticData &irProp = AQLCoreDataService::getStaticDataManager().getStaticData();
	...
	istringstream *pVolStream = AQLCoreDataService::getIStringStream(AQLMarketData::getNumFileName(volFileName));
	if (!pVolStream)
	{
