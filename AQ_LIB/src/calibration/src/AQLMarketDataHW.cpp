...
*/
AQLMarketDataHW::AQLMarketDataHW(void)
{
}
...
*/
void
AQLMarketDataHW::getCanonicalGrid(DoubleArray &tenor, const AQLDate &asOfDate, const AQLPriceDataDayCount &dayCount, int max, bool isReflesh, const AQLString *p_freq_str)
{
	isReflesh;
	tenor.clear();

	AQLString str_tenor = AQLCoreDataService::getContext(CONTEXT_KEY_HW_TENOR);
...
*/
void
AQLMarketDataHW::getHullWhiteParam(const AQLString &fileName, DoubleArray &t_grid, DoubleArray &alpha, DoubleArray &sigma)
{
	AQLFileAccessor paramFile(fileName);
