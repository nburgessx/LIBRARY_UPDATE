#ifndef AQLMathSwaptionVolUtility_h
#define AQLMathSwaptionVolUtility_h
...
class AQLMathSwaptionVolUtility
{
friend class AQLMathLeastSquareSABRCapFloorCostFuncQlib;
...
private:
    static AQLMathSABR* createSABR(const AQLString& approxMethod, double alpha, double beta, double nu, double rho, bool isLognormal = true);

	AQLMathSwaptionVolUtility(void);
	~AQLMathSwaptionVolUtility(void);
	AQLMathSwaptionVolUtility(const AQLMathSwaptionVolUtility &rhs);
	AQLMathSwaptionVolUtility &operator=(const AQLMathSwaptionVolUtility &rhs);
};
#endif 