...
*/
class LAMathYieldCurve : public LAObject
{
public:
// LIFECYCLE
	// constructor
	LAMathYieldCurve(LADataInstance* dataInstance, const LAString *const pCurveType = 0);
	// copy constructor
...
//  QUERY
	...
	//	get Interpolation
	const LAPriceDataInterpolation&
						getInterpolation(const LAString &curveType) const;
	//	get Interpolation
	LAPriceDataInterpolation&
						getInterpolation(const LAString &curveType);	
	...
	double				getParRate(const LAString& term_str, LAString foreCurveName = STD, 
								   LAString dfCurveName = STD, bool isFWDInter = true, const LAString* roll_convention=NULL);
	
    //	get ParRate from DateVector 
	double				getParRate(const DateVector& dates, const DateVector& dates_float, LAString foreCurveName = STD, 
...
//  OPERATION 
	...
	double getCurBasisSwapValue(const LAMathYieldCurve& yc, const LADate& valueDate, const LADate& startDate, 
	...
	// get DayCountConvension
	const LAPriceDataDayCount&
						getDayCount(const LAString &curveType) const;
    // get DayCountConvension
	...
	void				getCurveConvention(LAString& freq, LAPriceDataCalendar& cal, LAPriceDataSlidingRule& sld,
										   LAPriceDataDayCount& dc, LAString& accessary, const LAString& curveName) const;

	// get convension from yield curve data object
	void				getCurveConvention(LAString& freq, LAPriceDataCalendar& cal, LAPriceDataSlidingRule& sld,
										   LAPriceDataDayCount& dc, LAString& accessary, const LAString& curveName);
...
protected:
	...
	LAString    mCurveType;		// curve Type
