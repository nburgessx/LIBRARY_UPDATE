...
typedef std::vector<AQLRatesPathElementBase*>	ONEPATH;   // vector of pointer to path element
typedef std::vector<ONEPATH>				MCPATH;   // matrix of pointer to path element

/*! 
    @brief Class to represent MC path.
*/
class AQLMathPathEntity : public AQLObject
{
public:
// LIFECYCLE
    // default constructor
	AQLMathPathEntity(AQLDataInstance* dataInstance);
    // copy constructor
	AQLMathPathEntity(const AQLMathPathEntity& irse);
    // destructor
	virtual ~AQLMathPathEntity();
...
private:
	...
	std::vector<AQLRatesSDEBase*>	mSDEs;			// vector of pointer to sde
...
//	int							mCacheSize;		// cash size 
	...
	std::set<AQLRatesBM*>			mpBMs;			// Brownian Motion
