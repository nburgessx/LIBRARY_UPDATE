...
// ID for AQLRatesPathElementSMMCurve
...
*/
class AQLRatesPathElementSMMCurve : public AQLRatesPathElementCurve
{
public:
//  LIFECYCLE
	// Default constructor
	explicit AQLRatesPathElementSMMCurve(const DoubleArray& tenor, const DoubleArray& delta_tenor, double t = 0);
	// Destructor
	virtual ~AQLRatesPathElementSMMCurve();
	// copy constructor
	AQLRatesPathElementSMMCurve(const AQLRatesPathElementSMMCurve& v);
...
//  QUERY
    ...
    */
    virtual	AQLRatesPathElementSMMCurve&
								operator = (const AQLRatesPathElementSMMCurve& a);
...
/*	virtual void				set(const void* pdata) 
	...
	*/	
	...
	// get libor rate
	double						getL (double ts, double te, double delta, const AQLRatesPathElementCurve& curve) const;
	// get swap rate
	//double					getS (double ts, double te, double annuity, const AQLRatesPathElementCurve& curve) const;
	double						getS (DoubleArray tenor, DoubleArray deltatenor, const AQLRatesPathElementCurve& curve) const;

private:
	// set start time of this curve
	void						set_t (void); 
	// get annuity
	double						getA (DoubleArray tenor, DoubleArray deltatenor, const AQLRatesPathElementCurve& curve) const;
