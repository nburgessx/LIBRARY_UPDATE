...
//  LIFECYCLE
	...
	AQLPriceCouponTool();
    // destructor	
	virtual ~AQLPriceCouponTool();
    // copy constructor	
	AQLPriceCouponTool(const AQLPriceCouponTool& v);
	/*!
	...
	*/		
	...
	std::vector<AQLPriceIndexToolBase*> getIndexs(void) const {return mpIndexs;}// index
...
protected:
	...
	*/
	void						round(double& coupon) const
								{
									if (!mIsRound) return;
									coupon = AQLPriceCFGenUtility::round(coupon, mRoundFunction, mRoundDigit);									
								}
	/*!
	...
	*/
	void						capfloor(double& coupon) const
								{
										...
										|| (mIsFloor && mpFloor->isTypeOf(FN_ACCRUEDCOUPONFUNCBASE)))
									{
										...
										const AQLPricePayOffTool *mp = &(mpPayOff->getPayOffMaster())[mLegNo][mCurrentPos].getPayOff();	
	...
	virtual AQLPriceIndexToolBase* createIndexTool(const AQLObject& indexInfo, const AQLDate& baseDate);
						
	
	std::vector<AQLPriceIndexToolBase*> mpIndexs;// index
	AQLFunctionBase* mpOperator;// operator 
	
    const AQLPricePayOff* mpPayOff;// payoff
...
class AQLPriceCouponToolCompound : public AQLPriceCouponTool
{
public:
	virtual	void setUp(const AQLDate& basedate,	
                       const AQLObject& trade,
