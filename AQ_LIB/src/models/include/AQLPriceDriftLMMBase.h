...
*/
class AQLPriceDriftLMMBase : public AQLMathDriftFuncBase
{
public:
//  LIFECYCLE
	// Default constructor
	explicit AQLPriceDriftLMMBase(double Q = 1.0);
	// constructor
	AQLPriceDriftLMMBase(const AQLString& sdeAttrName, unsigned int i, const DoubleArray& tenor, const DoubleArray& delta_tenor, double Q = 1.0);	
	//	Copy constructor
	AQLPriceDriftLMMBase(const AQLPriceDriftLMMBase& v);
	// Destructor
	virtual ~AQLPriceDriftLMMBase();
...
protected:
	...
	AQLRatesCovariance*								mpCovariance;	// covariance class
