...
// Funciton ID of LARatesCpnCapFloorFuncForTARN
#define FN_CPNCAPFLOORFORTARN	10003
// Function Name of LARatesCpnCapFloorFuncForTARN
#define FN_CPNCAPFLOORFORTARN_STR	"fn_couponcapfloorfuncfortarn"
...
*/
class LARatesCpnCapFloorFuncForTARN : public LAFunctionBase
{
public:
//  LIFECYCLE
	// constructor
	LARatesCpnCapFloorFuncForTARN();
	LARatesCpnCapFloorFuncForTARN(const DoubleArray& x); 
	// destructor
	virtual ~LARatesCpnCapFloorFuncForTARN();

	//20061017--David--Remove warning:C4512
	LARatesCpnCapFloorFuncForTARN & operator=( const LARatesCpnCapFloorFuncForTARN & ) { return *this; }
...
//  QUERY
	...
	virtual	bool				isInDomain(const DoubleArray& x) const { (void)x; //20061017--David--Remove warning:C4100 20070411--Nagase--åxçêçÌèúÇgccÇ…Ç‡ëŒâû
                            ...
                            // check derivable or not.
	virtual	bool				isDifferentiable(const DoubleArray& x, unsigned int pos) const { (void)x; (void)pos; //20061017--David--Remove warning:C4100 20070411--Nagase--åxçêçÌèúÇgccÇ…Ç‡ëŒâû
												return true;};
								//==========================================
	                            // check double derivable or not.	
	virtual	bool				isDifferentiable(const DoubleArray& x, unsigned int posi, unsigned int posj) const { (void)x; (void)posi; (void)posj; //20061017--David--Remove warning:C4100 20070411--Nagase--åxçêçÌèúÇgccÇ…Ç‡ëŒâû
												return true;};
