...
// AQLPriceTradeValue's function id
#define FN_IR_TRADEVALUE				2012
// AQLPriceTradeValue's function name
...
*/
class AQLPriceTradeValue : public AQLCoreValuation
{
public:
    // constructor	
	explicit AQLPriceTradeValue(AQLPriceAccruedInterest* pacc = 0);
    // destructor	
	virtual ~AQLPriceTradeValue();
	...
	AQLPriceTradeValue & operator=( const AQLPriceTradeValue & ) { return *this; }

	/*!
		@brief cache class for performance up
	*/
	class AQLPriceTradeValueDataProvider : public AQLDataProvider
	{
	public:
		virtual ~AQLPriceTradeValueDataProvider(); 
		
		AQLMathPathEntity*					pPath;		// path object
		std::set<AQLMathIndexEntity*>		indexs;		// reference index vector
		AQLRatesNumeraireBase*				pNumeraire; // numeraire
		int								mcnum;		// mc num
		...
		AQLPricePayOff						payoff;		// payoff class
		...
		std::vector<AQLPolynomialBase*>	polies;			// polynomial functions
		std::vector<AQLPolynomialBase*>	polies_rebate;		// polynomial functions
	...
	virtual double calcFeeValueExo(AQLObject &tradeEntity, AQLPriceTradeValueDataProvider* dataProvider) const;

protected:
    // copy constructor
    AQLPriceTradeValue(const AQLPriceTradeValue& v);	
	...
	double calcValueTermRatio(const AQLPriceTradeValueDataProvider &dataProvider) const;
	// get FXEntity
	const AQLMathFXEntity &getFXEntity(AQLObject &object) const;

	AQLPriceAccruedInterest*		mpAcc;// < accrued interest calulatetor
	void clearLSMCPointers(AQLPriceTradeValueDataProvider &dataProvider) const; // clear LSMC pointers
private:
