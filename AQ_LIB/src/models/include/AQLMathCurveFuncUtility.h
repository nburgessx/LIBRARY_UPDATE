#ifndef AQLMathCurveFuncUtility_h
#define AQLMathCurveFuncUtility_h
...
public:
	...
	static DoubleArray getMultiForwardRate(const DateVector& fromdate, const DateVector& todate, AQLDataInstance* dataInstance, const AQLString& curveid,
							AQLString frequency, AQLString daycount, AQLString slidingrule, AQLString calendar, AQLString interpolation, 
							AQLString curveName = STD, bool isFWDInter = true, bool useFwdData = false); // useFwdData: false = imply forwards from DiscountFactors, true = use forward data directly
	
    // Function to get the curve frequency
    static AQLString getCurveFrequency( const AQLString& curveCollection, const AQLString& curveName ); // CurveCollection also known as CurveID
   ...
   static double getBasisSwapValue(AQLDataInstance* dataInstance, const AQLDate& valueDate, const AQLDate& startDate, AQLString& term, const  AQLString& arbFreeCurveID,
	...
	static  AQLMathYieldCurve& getYieldCurveForCurveID(AQLDataInstance* dataInstance, const AQLString& curveid);
	static void setCalendarForCurveID(AQLMathYieldCurve& yc, const AQLString& calendar);
	static bool setUpForwardDayCount(AQLDataInstance *dataInstance, const AQLString &curveid, const AQLString &curveName, AQLMathYieldCurve &yc);
	...
	*/
	...
	static std::tuple<std::vector<double>,std::vector<double>> AQLMathCurveFuncUtility::getXY(AQLDataInstance* dataInstance, const AQLString& curveCollection);
...
private:
	...
	static void setUpYieldCurveConvention(const AQLString &foreCurveName, const AQLString &inter, const AQLString &dc, const AQLString &sld, 
													const AQLString &cal, const AQLString &freq, const AQLString &curveid, AQLMathYieldCurve &yc);
	...
	AQLMathCurveFuncUtility(void);
	~AQLMathCurveFuncUtility(void);
	AQLMathCurveFuncUtility(const AQLMathCurveFuncUtility &rhs);
	AQLMathCurveFuncUtility &operator=(const AQLMathCurveFuncUtility &rhs);
