...
AQLMathLeastSquareCostFuncSZ::AQLMathLeastSquareCostFuncSZ(double S0_,
	double V0_,
	double rho_,
	...
	)
{
	...
	mCostFunction = std::make_shared<AQLMathCostFuncSZ>(S0_, V0_, rho_, curve_d_, curve_f_, T_param_, beta_, theta_, kappa_, epsilon_, calibFlag_);
}


//
//AQLMathLeastSquareCostFuncSZ::AQLMathLeastSquareCostFuncSZ(const AQLMathLeastSquareCostFuncSZ& rhs)
//{
//}


AQLMathLeastSquareCostFuncSZ::~AQLMathLeastSquareCostFuncSZ()
{
}
...
AQLMathLeastSquareConstraintSZ::AQLMathLeastSquareConstraintSZ(double max_beta,
														double min_beta,
														double max_theta,
														double min_theta,
														double max_kappa,
...
{
	mConstraint = std::shared_ptr<QuantLib::Constraint>(new AQLMathConstraintSZ(max_beta, min_beta, max_theta, min_theta, max_kappa, min_kappa, max_epsilon, min_epsilon, calibFlag));

}

//AQLMathLeastSquareConstraintSZ::AQLMathLeastSquareConstraintSZ(const AQLMathLeastSquareConstraintSZ& rhs)
//{
//}


AQLMathLeastSquareConstraintSZ::~AQLMathLeastSquareConstraintSZ()
{
}

// method to overload to compute the cost function value in x
Real AQLMathLeastSquareCostFuncSZ::value(const Array& x) const
{
...
Array AQLMathLeastSquareCostFuncSZ::values(const Array& x) const
{
	double m = x.size();
...
void AQLMathLeastSquareCostFuncSZ::Set_t2f_LS(double T_,
						  const vector<double>& strike_,
						  const vector<double>& weight_,
						  const vector<double>& t2f_
						 )
{
	dynamic_pointer_cast<AQLMathCostFuncSZ>(mCostFunction)->Set_t2f(T_, strike_, weight_, t2f_);
}

double AQLMathLeastSquareCostFuncSZ::Get_Call_Opt(double t, double K)
{
	return ddsv_approx->Get_Call_Opt(t, K, true);
}

double AQLMathLeastSquareCostFuncSZ::Get_ImpVol(double t, double K)
{
	return ddsv_approx->Get_imp_vol(t, K, true);
}


void AQLMathLeastSquareCostFuncSZ::set_params(const Array& x) const
{
	size_t counter = 0;
