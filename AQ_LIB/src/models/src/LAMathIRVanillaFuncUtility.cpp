...
const size_t LAMathIRVanillaFuncUtility::busDayPerYear = 250;
...
//	LAString daycount(AC_365I);
	...
	//Optimize
	double ret = LAMathIRVanillaFuncUtility::optimize(low, high, prem, param, p1, p2);//Bisection Method}
					   ...
					   double forwardrate, LAString foreCurveName, LAString dfCurveName, const bool isFWDInter)
{
	...
	DateVector fixVec(N);
	for(int i =0; i<N;i++)
		fixVec[i] = LAMathDateCalculations::getDate(payVec[i],spotlag,sr2,&cal2,false); 
    ...
    //we do not slide dates in LAMathYieldCurve class
	...
	LAString foreCurveName, LAString dfCurveName, const bool isFWDInter, double shiftvalue)
{
	...
	for (int i = pos; i<N - 1; i++)
	{
		...
		param.Te = conv.getTerm(valuedate, fixVec[i]);
		param.Vol = LAMathSwaptionVolUtility::getSABRVol4(dataInstance, param.Te, tenorstr, param.F, param.K, alphaid, betaid, nuid, rhoid,
				...
				
{
	...
	//paymentvec;
	DateVector payVec; 
	LAMathDateCalculations::generateSchedule(tmpstart,endpay,frequency,true,NULL,NULL,&roll,payVec,&sr,&cal);
	if(fixingdate > valuedate)
									...
									const bool isIRRModel)
{
	...
	if (!isIRRModel)
	{
		numeraire = LAMathIRVanillaFuncUtility::getAnnuity(tmpDate,swapterm,yc);
	}
	else
	{
		numeraire = LAMathIRVanillaFuncUtility::getCashSettledAnnuity(tmpDate,swapterm,yc,forwardrate);
	...
	AnalyticBKParam param;
	if(-10.0==forwardrate)
		param.F = LAMathCurveFuncUtility::getParRate(tmpDate,swapterm,dataInstance,curveid,frequency,daycount,slidingrule,calendar,
									  ...
									  const bool isIRRModel)
{
	...
	double forward;
	if(forwardRate == NULL)
		forward = LAMathCurveFuncUtility::getParRate(tmpDate,swapterm,dataInstance,curveid,frequency,daycount,slidingrule,calendar,interpolation,foreCurveName,dfCurveName);
	else
		forward = *forwardRate;
	...
	{
		numeraire = LAMathIRVanillaFuncUtility::getCashSettledAnnuity(tmpDate,swapterm,yc,forward);
		numeraire *= yc.getDF(conv.getTerm(asOfDate, tmpDate), &dc_act);
									          ...
									          const bool isIRRModel)
{
	...
	if(-10.0==forwardrate)
	{
		forwardrate = LAMathCurveFuncUtility::getParRate(tmpDate,swapterm,dataInstance,curveid,frequency,daycount,slidingrule,calendar,
	...
	if		(PAYERS == callput)
	{
		ret = LAMathAnalyticalFormula::BlackFormulaDD(forwardrate, t_vol, strike, 1, 0);
	}
	else if(RECEIVERS==callput)
	{
		ret = LAMathAnalyticalFormula::BlackFormulaDD(forwardrate, t_vol, strike, -1, 0);
						...
						)
{
	...
	LAMathConvexityAdust_Antonov swaption_prem(T, S0, sabr_params[0], sabr_params[1], sabr_params[2], sabr_params[3], num_int_step);
...
double
LAMathIRVanillaFuncUtility::optimize( double x1, double x2, double x3, AnalyticParam& param, LABlackScholesBase*& p1, LABlackScholesBase*& p2)
{
...
double
LAMathIRVanillaFuncUtility::optimize( double x1, double x2, double x3, AnalyticParam& param, LABlackScholesBase*& p1, LABlackScholesBase*& p2, LABlackScholesBase*& p3, LABlackScholesBase*& p4)
{
...
double
LAMathIRVanillaFuncUtility::optimize2( double x1, double x2, double x3, AnalyticParam& param, LABlackScholesBase*& p1, LABlackScholesBase*& p2)
{
...
LASABRCAIntegral::~LASABRCAIntegral() 
{ 
	delete sabr; 
	sabr = 0; 
}
...
void 
LASABRCAIntegral::setIntegralParams(const double start, const double step, size_t num)
{
...
double
LASABRCAIntegral::integralVol(unsigned int num) const
{
...
double 
LASABRCAIntegral::operator()(double x) const
{
