...
void  
AQLMathCurveGenerateFuncUtility::setUpCurveFromMarket(AQLDataInstance* dataInstance, const AQLString& CurveID, 
						...
						AQLStringMatrix ylddata, bool isCheckDF, bool isAUDadjust)
{
	...
	AQLDate		asofdate			= AQLMathDateUtilities::getLADate(chgrow(ylddata,CURVEINPUT_ASOFDATE,1));
	...
	AQLMathYieldCurvePro * yp = NULL; 
	if(!ehCur.isDefined())
	{	
		yp= new AQLMathYieldCurvePro(dataInstance);
		objPool.set(CurveIDTool,yp);
	}
	else
	{
		dynamic_cast<AQLMathYieldCurvePro &>(objPool.getObject(CurveIDTool).get()).reset();
		yp	= &dynamic_cast<AQLMathYieldCurvePro &>(objPool.getObject(CurveIDTool).get());
	}
	...
	double rate=0.0;
	//Money Market
    //spotdate	= AQLMathDateUtilities::getLADate(chgrow(mdata,CURVEINPUT_SPOTDATE,1));
	...
	////LiborData
	//spotdate	= AQLMathDateUtilities::getLADate(chgrow(ldata,CURVEINPUT_SPOTDATE,1));
	...
	////SwapData
	//spotdate	= AQLMathDateUtilities::getLADate(chgrow(sdata,CURVEINPUT_SPOTDATE,1));
	...
	if(isfuteruse)
	{
		//spotdate	= AQLMathDateUtilities::getLADate(chgrow(fdata,CURVEINPUT_SPOTDATE,1));
		...
		for(int i=1; i<N; i++)
		{
			AQLDate	StartDate	=	AQLMathDateUtilities::getLADate(chgcol(fgrid,CURVEINPUT_STARTDATE,	i));
			AQLDate EndDate		=	AQLMathDateUtilities::getLADate(chgcol(fgrid,CURVEINPUT_ENDDATE,		i));
...
void 
AQLMathCurveGenerateFuncUtility::setUpXccyBasisCurve(AQLDataInstance* dataInstance,const AQLString& BCurveID, 
						AQLStringMatrix ylddata, AQLStringMatrix bgrid, AQLStringMatrix bdata)
{
	...
	AQLString term;
    AQLDate spotdate;
	//AQLDate spotdate		= AQLMathDateUtilities::getLADate(chgrow(bdata,CURVEINPUT_SPOTDATE,1));		
    ...
    {
        spotdate = AQLMathDateUtilities::getLADate(chgrow(bdata,CURVEINPUT_SPOTDATE,1));
    }
	...
	AQLMathYieldCurve* ycp = NULL;
	if(!ehyld.isDefined())
	{
		ycp = new AQLMathYieldCurve(dataInstance);
		objPool.set(yldname,ycp);
	}
	else
	{
		dynamic_cast<AQLMathYieldCurve &>(objPool.getObject(yldname).get()).reset();
		ycp = &dynamic_cast<AQLMathYieldCurve &>(objPool.getObject(yldname).get());
	}
	...
	{
		//dynamic_cast<AQLMathYieldCurve &>(objPool.getObject(CurveIDTool).get()).reset();
		yp	= &dynamic_cast<AQLMathYieldCurvePro &>(objPool.getObject(CurveIDTool).get());
	}
...
void 
AQLMathCurveGenerateFuncUtility::setUpBasisCurve(AQLDataInstance* dataInstance,
...
void 
AQLMathCurveGenerateFuncUtility::setUpBasisCurveFromMarket(AQLDataInstance* dataInstance,const AQLString& CurveID, 
...
{
	AQLMathCurveGenerateFuncUtility::setUpCurveFromMarket(dataInstance,CurveID,mgrid,lgrid,fgrid,sgrid,mdata,ldata,fdata,sdata,ylddata,isCheckDF,isAUDadjust);
	AQLMathCurveGenerateFuncUtility::setUpXccyBasisCurve(dataInstance,CurveID,ylddata,bgrid,bdata);
}

void  
AQLMathCurveGenerateFuncUtility::
                             ...
                             AQLStringMatrix yldData, AQLString& warning )
{
    ...
    {
        lspotdate = AQLMathDateUtilities::getLADate(chgrow(swapConv,CURVEINPUT_SPOTDATE,1));
    }
	/**/
    ...
    {
        spotdate = AQLMathDateUtilities::getLADate(chgrow(swapConv,CURVEINPUT_SPOTDATE,1));
    }
    ...
    if( Currency != CURRENCY_USD )
    {
		...
		for (size_t i=0; i<term.size(); i++)
		{
			...
			{
				spotdate = AQLMathDateUtilities::getLADate(chgrow(currConv,CURVEINPUT_SPOTDATE,1));
			}
    ...
    if(isFRAUse)
    {
        ...
        {
            spotdate = AQLMathDateUtilities::getLADate(chgrow(fraConv,CURVEINPUT_SPOTDATE,1));
        }
...
void 
AQLMathCurveGenerateFuncUtility::setUpForecastCurveFromMarket(AQLDataInstance* dataInstance,
...
void 
AQLMathCurveGenerateFuncUtility::setUpForecastCurve(AQLDataInstance* dataInstance,
...
void 
AQLMathCurveGenerateFuncUtility::setUpCdtDFCurve(AQLDataInstance* dataInstance, AQLString curveID, AQLStringMatrix data)
{
    ...
    {
        spotdate = AQLMathDateUtilities::getLADate(chgrow(data,CURVEINPUT_SPOTDATE,1));
    }
...
AQLString 
AQLMathCurveGenerateFuncUtility::getCurveGenerateTime(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLString& curvename)
{
...
void	
AQLMathCurveGenerateFuncUtility::setUpCurve2(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLDate& asofdate, 
										  const DoubleMatrix& mat, const AQLString& curveName)
{
	AQLMathCurveGenerateFuncUtility::setUpCurve(dataInstance,curveid,asofdate,mat,curveName);
	AQLString suffix;
	if (curveName == STD)
	{
		suffix = AQLString("");
...
void 
AQLMathCurveGenerateFuncUtility::setUpCurve(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLDate& asofdate, 
										 const DoubleMatrix& mat, const AQLString& curveName)
{
	unsigned int N=mat.size();
	if (N < 2) 
...
/////////////furuytos20120813///////////
void 
AQLMathCurveGenerateFuncUtility::setUpCurve3(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLDate& asofdate, 
...
DoubleArray 
AQLMathCurveGenerateFuncUtility::outPutCurveFromYieldData(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLString& curveName)
{
	AQLObjectPool& objPool	= dataInstance->getObjectPool();
...
DoubleArray 
AQLMathCurveGenerateFuncUtility::outPutCurveFromYieldData2(AQLDataInstance* dataInstance, const AQLString& curveID, const AQLString& curveID2,
...
*/
DiscountFactorTable 
AQLMathCurveGenerateFuncUtility::getCurveDiscountFactorTable(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLString& curveName)
{
	AQLObjectPool& objPool	= dataInstance->getObjectPool();
...
*/
void AQLMathCurveGenerateFuncUtility::setCurveDiscountFactorTable(AQLDataInstance* dataInstance, const AQLString& curveid, const std::vector<AQLString>& curveNames, const DiscountFactorTable& discountFactorTable )
{
...
*/
ForwardRateTable AQLMathCurveGenerateFuncUtility::getCurveForwardRateTable( AQLDataInstance* dataInstance, const AQLString& curveid, const AQLString& curveName )
{
    AQLObjectPool& objPool = dataInstance->getObjectPool();
...
*/
void AQLMathCurveGenerateFuncUtility::setCurveForwardRateTable( AQLDataInstance* dataInstance, const AQLString& curveid, const std::vector<AQLString>& curveNames, const ForwardRateTable& forwardRateTable )
{
        // 1. Get the terms and discount factors from the discountFactorTable
    // ---------------------------------------
...
{
	//AQLDate spotDate	= AQLMathDateUtilities::getLADate(chgrow(sdata,CURVEINPUT_SPOTDATE,1));
	std::map<AQLString, AQLString>& dvar = AQLCoreComponentManager::getDayCountMap();
...
{
	//AQLDate spotDate	= AQLMathDateUtilities::getLADate(chgrow(ldata,CURVEINPUT_SPOTDATE,1));
	std::map<AQLString, AQLString>& dvar = AQLCoreComponentManager::getDayCountMap();
