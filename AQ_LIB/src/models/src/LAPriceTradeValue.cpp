...
*/
LAPriceTradeValue::~LAPriceTradeValue()
{
	delete mpAcc;
}
/*!
...
LAPriceTradeValue::clone() const
{
    try 
	{
  		return new LAPriceTradeValue(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
function_t			
LAPriceTradeValue::getType() const
{
						...
						unsigned int startpathnum) const
{
	...
	for (unsigned int i = 0; i < dataProvider->cashvec.size(); i++)
	{
		...
		{
			// get coupon and calc
			const LAPricePayOffTool &pt = dataProvider->cashvec[i][j].getPayOff();
			pt.calcPayOff();
								...
								const AQLDataValuation& att) const
{
	...
	for (unsigned int i = 0; i < names.size(); i++)
	{
		...
		LAMathAttrSDE& attr = dynamic_cast<LAMathAttrSDE&>(dh->get());
...
//		dataProvider->payoff.setLSMCCoefficient(dataProvider->coefficient->get());
	...
	{
		dataProvider->polies[i] = dynamic_cast<LAPolynomialBase*>(dataProvider->poly->clone());
		dataProvider->polies_rebate[i] = dynamic_cast<LAPolynomialBase*>(dataProvider->poly->clone());
...
{
	callval.clear();
	const LAPriceTradeValueDataProvider* dataProvider = dynamic_cast<const LAPriceTradeValueDataProvider*>(dp);
...
*/
double
LAPriceTradeValue::calcValueTermRatio(const LAPriceTradeValueDataProvider &dataProvider) const
{
	// valuedate calc
...
LAPriceTradeValue::getFXEntity(AQLObject &object) const
{
	...
	for (unsigned int i = 0; i < initSize; i++)
	{
		...
		if (-1 != ccy.findString('/'))
		{
			return dynamic_cast<LAMathFXEntity &>(initialrefs.get(i).get());
...
/*!
	...
	@param[in, out] LAPriceTradeValueDataProvider
*/
void LAPriceTradeValue::clearLSMCPointers(LAPriceTradeValueDataProvider &dataProvider) const
{
	dataProvider.poly = 0;
	...
	if (dh->isDefined() && !dh->isNull())
	{
		...
		for (size_t i = 0; i < feeAmounts.size(); i++)
		{
			...
			for (unsigned int j = 0; j < sdeNames.getSize(); j++)
			{
				...
				{
					yieldCurve = dynamic_cast<const LAMathYieldCurve*>(&initialRefs.get(j).get());
				}
			}
			...
			{
				const LAMathFXEntity& fxEntity = getFXEntity(tradeEntity);
...
#endif
LAPriceTradeValue::LAPriceTradeValueDataProvider::~LAPriceTradeValueDataProvider()
{
