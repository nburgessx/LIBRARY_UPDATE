...
/*!
    @brief Constructor
*/
LAPriceEventNotionalChange::LAPriceEventNotionalChange()
: LAPriceEventBase()
{
...
LAPriceEventNotionalChange::clone() const    
{
    try 
	{
        return new LAPriceEventNotionalChange(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
LAPriceEventNotionalChange::isTypeOf(function_t id) const
{
	return (id == FN_IR_TRIGGERACTIONNOTIONALCHANGE ? true : LAPriceEventBase::isTypeOf(id));
}
...
*/
function_t			
LAPriceEventNotionalChange::getType() const
{
										...
										vector<PayOffToolHolderIter>& iter)
{
	...
	{	
		...
		if (mIsNotionalExchangeAtEnd[i])
		{
			...
			{
				const vector<LAPriceEventBase*>& action = pastaction[i]->getAction();
				...
				*it = ph;
				...
				if (mIsNotionalExchangeAtEnd[i] && flag)
				{
					...
					if (_action != NULL)
					{
						...
						vector<LAPriceEventHolder*> dummy2;
												 ...
												 double amount) const
{
	...
	if (isamountsetting)
	{
		...
		amount_ = LAPriceCFGenUtility::round(amount_, ROUND_DOWN, 0);
	}
	else
	{
		amount_ = notional1st / count;
		amount_ = LAPriceCFGenUtility::round(amount_, roundfunction, rounddigit);
	}
