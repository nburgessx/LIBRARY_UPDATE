...
#include "AQLPriceCMSSpreadFormula.h"
#include "AQLMathOptionTools.h"

double AQLPriceCMSSpreadFormula::Price(double t, double K, bool isCall, double alpha, double beta,
                                 double S1, double S2, double vol1, double vol2, double rho)
{
    AQLPriceCMSSpreadCallPricer pricer;
    if (isCall)
        pricer = AQLPriceCMSSpreadCallPricer(t, K, alpha, beta, S1, S2, vol1, vol2);
    else
        pricer = AQLPriceCMSSpreadCallPricer(t, -K, -alpha, -beta, S1, S2, vol1, vol2);
...
double AQLPriceCMSSpreadFormula::ImpliedCorrelation(double fwdPrice, double t, double K, bool isCall, double alpha, double beta,
...
AQLPriceCMSSpreadCallPricer::AQLPriceCMSSpreadCallPricer(double t_, double K_, double alpha_, double beta_, double S1_, double S2_,
...
double AQLPriceCMSSpreadCallPricer::operator()(double x) const
{
    return Price(x) - mFwdPrice;
}

double AQLPriceCMSSpreadCallPricer::Price(double rho) const
{
