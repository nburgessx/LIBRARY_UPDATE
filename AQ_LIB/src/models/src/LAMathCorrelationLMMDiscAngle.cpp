/*!
    @file LAMathCorrelationLMMDiscAngle.cpp
*/
...
//////////////////////////////////////////////////////////////////
//
LAMathCorrelationLMMDiscAngle::LAMathCorrelationLMMDiscAngle()
: LAMathCorrelationLMMDisc()
{
}

//
LAMathCorrelationLMMDiscAngle::LAMathCorrelationLMMDiscAngle(LAMathCorrelationFuncLMM*	corr,
														 const DoubleVector&		T_fix,
														 size_t						no_factors
		)
:
LAMathCorrelationLMMDisc( corr, T_fix, no_factors )
{
    if( no_factors > 1 ) mTheta = angleFromFactorLoading(mFactorLoading);
}

//
LAMathCorrelationLMMDiscAngle::LAMathCorrelationLMMDiscAngle(const DoubleMatrix&	corr_mat_,
														 const DoubleVector&	T_fix,
														 size_t					no_factors,
														 bool					full_mat
		)
:
LAMathCorrelationLMMDisc( corr_mat_, T_fix, no_factors, full_mat )
{
    if( no_factors > 1 ) mTheta = angleFromFactorLoading(mFactorLoading);
    if (full_mat)
    {
        ...
        //{
        //    throw invalid_argument("full-length corr_mat is incompatible with T_fix : LAMathCorrelationLMMDisc::LAMathCorrelationLMMDisc");
        //}
    ...
    else
    {
        ...
        //{
        //    throw invalid_argument("reduced-length corr_mat is incompatible with T_fix : LAMathCorrelationLMMDisc::LAMathCorrelationLMMDisc");
        //}
...
//
LAMathCorrelationLMMDiscAngle::LAMathCorrelationLMMDiscAngle(const DoubleMatrix&	factor_loading_,
														 const DoubleVector&	T_fix,
														 bool					full_mat
		)
:
LAMathCorrelationLMMDisc( factor_loading_, T_fix, full_mat)
{
    ...
    if (full_mat)
    {
        ...
        //{
        //    throw invalid_argument("full-length factor loading is incompatible with T_fix : LAMathCorrelationLMMDisc::LAMathCorrelationLMMDisc");
        //} 
    ...
    else
    {
        ...
        //{
        //    throw invalid_argument("reduced-length factors loading is incompatible with T_fix : LAMathCorrelationLMMDisc::LAMathCorrelationLMMDisc");
        //}
        //corr_mat = Corr_from_Factor_loading( factor_loading );
        //for (size_t i = 0; i < corr_mat.size(); i++ )
        //{
...
LAMathCorrelationLMMDiscAngle::LAMathCorrelationLMMDiscAngle(const DoubleMatrix& theta_,
														 const DoubleVector& T_fix
		)
:
LAMathCorrelationLMMDisc( factorLoadingFromAngle(theta_), T_fix ),
mTheta(theta_)
{
}

void LAMathCorrelationLMMDiscAngle::setTheta(const DoubleMatrix& theta_)
{ 
	mTheta = theta_;
...
DoubleMatrix LAMathCorrelationLMMDiscAngle::angleFromFactorLoading( const DoubleMatrix& factor_loading_, double eps )
{
	//error check
	...
	for(size_t i = 0; i < n; ++i)
	{
		...
		if( LAModelUtilities::sign(sin_) != LAModelUtilities::sign(mFactorLoading[i][m-1]) )
		{
			theta_[i][m-2] *= -1.0;
			sin_ *= -1.0;
		}
...
DoubleMatrix LAMathCorrelationLMMDiscAngle::factorLoadingFromAngle( const DoubleMatrix& theta_ )
{
	//
