...
*/
void
LAMathVolFuncSZDD::setFwdFX(const DoubleArray &fx)
{
...
/*!
	@brief destructor
*/
LAMathVolFuncSZDD::~LAMathVolFuncSZDD(void)
{
}
...
*/
LAMathVolFuncSZDD::LAMathVolFuncSZDD(const LAMathVolFuncSZDD &rhs) 
...
LAMathVolFuncSZDD::clone() const
{
    try 
	{
		return new LAMathVolFuncSZDD(*this);
    }
    catch (bad_alloc &e)
	{
...
*/
function_t
LAMathVolFuncSZDD::getType() const
{
...
*/
double
LAMathVolFuncSZDD::operator()(const DoubleArray& x) const
{
...
*/
double
LAMathVolFuncSZDD::integral(const vector<pair<double,double> >& x) const
{
	if (mTimeGrid.size() == 0 || x.size() < 2)
	{
		throw LACoreInvalidData("No data is set or argument size is less than two ", __FILE__, __LINE__);
...
double
LAMathVolFuncSZDD::getIntegralofV(double ts, double te) const
{
...
double
LAMathVolFuncSZDD::getIntegralofSVV(double ts, double te) const
{
	double ret1 = 0.0, ret2 = 0.0;
	if ( !is_cache[ts] ) 
	{
...
double
LAMathVolFuncSZDD::getIntegralofSV(double ts, double te) const
{
	double ret1 = 0.0, ret2 = 0.0;
	if ( !is_cache[ts] ) 
	{
