...
*/
AQLMathCapFloorConvention::AQLMathCapFloorConvention
...
/*!
	@brief Default constructor
*/
AQLMathCapFloorSABR::AQLMathCapFloorSABR
    ( const IntVector& expiDate_, const vector<AQLMathSABR_Hagan>& sabr_, 
      AQLInterpolationBase* pInter_, const AQLMathCapFloorConvention& conv_, const AQLString& curveID_,
...
AQLMathSABR_Hagan 
AQLMathCapFloorSABR::getSABRParam(unsigned int pos)
{
...
void                                 
AQLMathCapFloorSABR::calibrateToCapVol
    ( AQLDataInstance* dataInstance, AQLMathSABRLimiter sabrLimiter, 
      ...
      const AQLString& target, DoubleMatrix& capletVol, DoubleMatrix& capVol )
{
    ...
    unsigned int span = AQLMathYieldCurve::setSpanFromFrequency( conv.freq );
    ...
    while( rowPos_mk<capTerm_mon.size() && colPos<=strikeVec.size() )
    {
        ...
        if( capMkVol<eps )
        {
			for(; span*(rowPos_caplet+1)<=capTerm-span; rowPos_caplet++)
			{
				...
				AQLMathSABRCalibrator calibrator(sabr[rowPos_caplet], strike_calib, vol_calib, forward_calib,
...
void
AQLMathCapFloorSABR::getCapletVol( unsigned int expiPos, unsigned int columPos, 
                                DoubleMatrix& capletVols, double strike, double aveVol )
{
    ...
    else
    {
        ...
        map<AQLString, AQLBlackScholesBase*> &var = AQLCoreComponentManager::getBlackComponentMap();
        AQLString bscomponent = AQLString(BK)+AQLString(PREM)+AQLString(CALL);
        map<AQLString, AQLBlackScholesBase*>::iterator it = var.find(bscomponent);
        ...
        = AQLMathIRVanillaFuncUtility::optimize(0.000001, 10., capletPrem, param, p1, p2);//Bisection Method
    }
}

double
AQLMathCapFloorSABR::getCapVol( unsigned int expiPos, unsigned int columPos, 
                             const DoubleMatrix& capletVols, double strike )
{
    ...
    else
    {
        ...
        it = var.find(bscomponent); 
	    if(it==var.end()) throw AQLCoreInvalidData("Product Type is not supported",__FILE__,__LINE__);
        AQLBlackScholesBase* p3 = it->second;
        ...
        ret = AQLMathIRVanillaFuncUtility::optimize(0.000001, 10., capPrem, param2, p2, p3);
    }
...
void
AQLMathCapFloorSABR::setCalibrationStack(AQLDataInstance* dataInstance)
{
	...
	AQLMathYieldCurve* yc2 = dynamic_cast<AQLMathYieldCurve* >(yc.clone());
...
double
AQLMathCapFloorSABR::getCapFloorPrem( unsigned int expiPos, double strike, double aveVol )
{
    //error check
...
void                                 
AQLMathCapFloorSABR::getCapletVolMat
...
double
AQLMathCapFloorSABR::getVol( AQLDataInstance* dataInstance, double fixingTerm, double strike )
{
    unsigned int span = AQLMathYieldCurve::setSpanFromFrequency( conv.freq );
...
void                                 
AQLMathCapFloorSABR::getCapletVolMat2
    ( AQLDataInstance* dataInstance, const DoubleMatrix& capVol_mk, const DoubleVector& strikeVec, 
      const IntVector& capTerm_mon, DoubleMatrix& capletVol, DoubleMatrix& capVol )
{
    const double eps = 0.000000001;
    ...
    for(size_t i=1; i<capVol_mk.size(); i++)
    {
        ...
        for(size_t j=0; j<strikeVec.size(); j++)
        {
            AQLMathCapVolCalibrator calibrator(capletVol_[j],Te_,F_,Nu_,strikeVec[j],capVol_mk[i][j],pInter);
            x[0] = capVol_mk[i][j];
            NL2SOL solver( calibrator );
            solver.tryToSolve( x );
...
AQLMathCapVolCalibrator::AQLMathCapVolCalibrator
...
void
AQLMathCapVolCalibrator::operator()(DoubleArray& f, const DoubleArray& x)
{
...
DoubleArray 
AQLMathCapVolCalibrator::getCapletVol(const DoubleArray& x)
{
    size_t N = Te.size();
...
bool 
AQLMathCapVolCalibrator::constraintsAreViolated(const DoubleArray& x)
{
