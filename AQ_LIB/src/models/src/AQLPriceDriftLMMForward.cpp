...
*/
AQLPriceDriftLMMForward::AQLPriceDriftLMMForward(unsigned int point, double Q)
: AQLPriceDriftLMMBase(Q), mPoint(point)
{
}
/*!
	@brief default constructor
	...
	@param[in] Q val for displaced diffusion case
*/
AQLPriceDriftLMMForward::AQLPriceDriftLMMForward(const AQLString& sdeAttrName, unsigned int i, const DoubleArray& tenor, const DoubleArray& delta_tenor, unsigned int point, double Q)
: AQLPriceDriftLMMBase(sdeAttrName, i, tenor, delta_tenor, Q), mPoint(point)
{
	if (tenor.at(0) == 0.0)
	{
		if (mPoint == 0)
...
*/
AQLPriceDriftLMMForward::AQLPriceDriftLMMForward(const AQLPriceDriftLMMForward& v) 
: AQLPriceDriftLMMBase(v), mPoint(v.mPoint)
{
...
/*!
	@brief destructor
*/
AQLPriceDriftLMMForward::~AQLPriceDriftLMMForward() 
{
...
*/
AQLCoreFunctionBase*	
AQLPriceDriftLMMForward::clone() const	
{
    try 
	{
		return new AQLPriceDriftLMMForward(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
AQLPriceDriftLMMForward::isTypeOf(function_t id) const
{
	return (id==FN_DRIFTLMMFORWARD ? true : AQLMathDriftFuncBase::isTypeOf(id));
}
...
*/
function_t
AQLPriceDriftLMMForward::getType() const
{
...
*/
double
AQLPriceDriftLMMForward::operator()(const DoubleArray& x) const
{
...
*/
void
AQLPriceDriftLMMForward::convertFromString(const AQLString& str)
{
	AQLDataStrings tmp;
...
*/
void
AQLPriceDriftLMMForward::setTenor(const DoubleArray& tenor, const DoubleArray& delta_tenor)
{
	...
	AQLPriceDriftLMMBase::setTenor(tenor, delta_tenor);	
...
*/
double
AQLPriceDriftLMMForward::integral(const std::vector<std::pair<double,double> >& x) const
{
...
*/
void
AQLPriceDriftLMMForward::setUp(AQLMathPathEntity& path)
{
	if (mPoint + 1 >= mTenor.size())
	{
		//error
	...
	AQLPriceDriftLMMBase::setUp(path);
