...
/*!
    @brief Constructor
*/
LAPriceEventTerminate::LAPriceEventTerminate()
: LAPriceEventBase()
{
...
LAPriceEventTerminate::clone() const    
{
    try 
	{
        return new LAPriceEventTerminate(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
LAPriceEventTerminate::isTypeOf(function_t id) const
{
	return (id == FN_IR_TRIGGERACTIONTERMINATE ? true : LAPriceEventBase::isTypeOf(id));
}
...
*/
function_t			
LAPriceEventTerminate::getType() const
{
									 ...
									 vector<PayOffToolHolderIter>& iter)
{
	...
	/*	for (; iter[i] != payoff[i].end() && iter[i]->getPayOff().getPaymentDate() <= actiondate; iter[i]++)
		...
		{
			const LAPricePayOffTool* ptool = NULL; 
			double accrued = 0.0;
