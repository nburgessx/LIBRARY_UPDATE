...
DoubleComplex AQLMathDisplacedHeston::zeta_DD(DoubleComplex phi, 
                                           double beta_,
				                           double kappa_, 
				                           double epsilon_, 
                                           double rho_
...
DoubleComplex AQLMathDisplacedHeston::psi_DD(DoubleComplex phi, 
				                          DoubleComplex zeta, 
                                          double beta_,
				                          double kappa_, 
                                          double epsilon_, 
...
DoubleComplex AQLMathDisplacedHeston::B_DD(DoubleComplex psi_plus, 
...
DoubleComplex AQLMathDisplacedHeston::G_DD(DoubleComplex phi, 
				                        double T, 
				                        double FX0,
                                        double beta_,
				                        ...
				                        )
{
    ...
    AQLMathJumpDiffusion::AdjustHestonParam(heston_params_,T);
    ...
    //Get A,B
    DoubleComplex zeta_ = AQLMathDisplacedHeston::zeta_DD(phi, beta_, kappa_, epsilon_, rho_);
    DoubleComplex psi_plus = AQLMathDisplacedHeston::psi_DD(phi, zeta_, beta_, kappa_, epsilon_, rho_, 1);
    DoubleComplex psi_minus = AQLMathDisplacedHeston::psi_DD(phi, zeta_, beta_, kappa_, epsilon_, rho_, -1);
    DoubleComplex  A_ = AQLMathJumpDiffusion::A(psi_plus, psi_minus, zeta_, T_, kappa_, theta_, epsilon_);
    DoubleComplex  B_ = AQLMathDisplacedHeston::B_DD(psi_plus, psi_minus, zeta_, phi, T, beta_);
...
double AQLMathDisplacedHeston::F_Integral_DD(double z, 
					                      double T, 
					                      double FX0, 
					                      double K, 
					                      double beta_,
					                      ...
					                      )
{
	...
	DoubleComplex G_ = AQLMathDisplacedHeston::G_DD(phi, T, FX0, beta_, heston_params);
...
double AQLMathDisplacedHeston::Get_F_DD(double T, 
				                     double FX0, 
				                     double K, 
				                     double beta_, 
				                     const HestonParams& heston_params,
				                     ...
				                     )
{
	...
	for(size_t i=0;i<GL_Number;i++)
	{
		...
		integral = AQLMathDisplacedHeston::F_Integral_DD(x[i],T, FX0, K, beta_, heston_params);
		tmp += integral * weight[i];
	}
...
double AQLMathDisplacedHeston::BS_DDHeston(double T,
				                        double FX0,
				                        double K, 
				                        double P0,
				                        int sgn,
