...
/*!
    @brief  constructor
*/
LAPriceArbFreeGenerator::LAPriceArbFreeGenerator()
{
}
/*!
    @brief  destructor
...
LAPriceArbFreeGenerator::clone() const
{
    try 
    {
        return new LAPriceArbFreeGenerator();
    }
    catch (bad_alloc & e)
    {
	...
	bool isUSD)
{
	...
	//calibration
    LAMathAFCurveCalibrator calibrator(basedate, threeMLTerms_DF, threeMLDF, sixMLTerms_DF, sixMLDF, dfTerms, dfCurve, isRenAdj, 
...
DoubleVector 
LAPriceArbFreeGenerator::getSwapValue(	
	const LiborMarket& libMkt, 
	const SwapMarket& swapMkt, 
	const DateVector& dateGrid_3MRoll,
	const DoubleArray& termGrid_3MRoll,
...
DoubleVector 
LAPriceArbFreeGenerator::getLibBasisValue(	
	const LiborMarket& libMkt, 
	const SwapMarket& swapMkt, 
	const LiborBasisMarket& libBasisMkt,
	const DateVector& dateGrid_3MRoll,
...
SwapMarket( vector<const AQLObject* > mktData, vector<const AQLObject* > tenorSwapMktData, const LiborMarket& libMkt, const AQLDate& basedate )
{
	...
	startDate = LAMathDateCalculations::getDate(spotDate, terms_str.front(), sld, pCal, true, &roll_conv);
	...
	endDate = LAMathDateCalculations::getDate(spotDate, terms_str.back(), sld, pCal, true, &roll_conv);
...
LAPriceArbFreeGenerator::SwapMarket::SwapMarket(const SwapMarket& v)
				 ...
				 const AQLDate& basedate )
{
	...
	if (!isUSD)
	{
		...
		endDate = LAMathDateCalculations::getDate(spotDate, swapMkt.endTerm, sld, pCal, true, &roll_conv);
		...
		if (fCurveName.findString(DUMMY) >= 0)
		{
			...
			// floaterPrices is 0
			int y,m,d,w;
			LAMathDateCalculations::termStrtoYMDW(swapMkt.endTerm,y,m,d,w);
...
LAPriceArbFreeGenerator::XCCYBasisMarket::XCCYBasisMarket(const XCCYBasisMarket& v)
...
LiborBasisMarket( vector<const AQLObject* > mktData, const SwapMarket& swapMkt, const AQLDate& basedate )
{
	...
	endDate = LAMathDateCalculations::getDate(spotDate, swapMkt.endTerm, swapMkt.sld, swapMkt.pCal, true, &roll_conv);

	generateSchedule(basedate, spotDate, endDate, 3, pCal, sld, dateGrid_3MRoll, termGrid_3MRoll, isEOMRoll);  
	generateSchedule(basedate, spotDate, endDate, 6, pCal, sld, dateGrid_6MRoll, termGrid_6MRoll, isEOMRoll);  
}

LAPriceArbFreeGenerator::LiborBasisMarket::LiborBasisMarket(const LiborBasisMarket& v)
...
void
LAPriceArbFreeGenerator::LAMathAFCurveCalibrator::operator()(DoubleArray& f, const DoubleArray& x)
{
	unsigned int pos = 0;
...
bool
LAPriceArbFreeGenerator::LAMathAFCurveCalibrator::constraintsAreViolated( const DoubleArray& x )
{
	for (size_t i=0; i<x.size(); i++)
	{
