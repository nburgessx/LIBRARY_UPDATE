...
ddsv_approx(new LAMathSwaptionFormulaSZ(curve_d_,
                 		            curve_f_,
									T_param_,
			                        beta_,      
						            rho_,                          
...
LAMathCostFuncSZ::~LAMathCostFuncSZ()
{
	delete ddsv_approx;
	ddsv_approx = 0;
}
...
Array LAMathCostFuncSZ::values(const Array& x) const
{
	double m = x.size();
...
//
void LAMathCostFuncSZ::Set_t2f(double T_,
						 ...
						 )
{
	...
	idx = LAModelUtilities::getIndex(T_, T_param);
...
double LAMathCostFuncSZ::Get_ImpVol(double t, double K)
{
	return ddsv_approx->Get_imp_vol(t, K, true);
}


void LAMathCostFuncSZ::set_params(const Array& x) const
{
	size_t counter = 0;
