...
void LAMathDisplacedHestonTDP::AdjustHestonParam_TDP(HestonParams_TDP& params, double T)
	{
...
DoubleComplex LAMathDisplacedHestonTDP::B_DD_TDP(DoubleComplex B,
...
DoubleComplex LAMathDisplacedHestonTDP::A_TDP(DoubleComplex B,
					                        DoubleComplex psi_plus, 
					                        DoubleComplex psi_minus,
					                        DoubleComplex zeta, 
					                        double tau_, 
...
DoubleComplex LAMathDisplacedHestonTDP::G_DD_TDP(DoubleComplex phi, 
                                               double T,
					                           double FX0, 
					                           double beta_, 
					                           ...
					                           )
{
    ...
    LAMathDisplacedHestonTDP::AdjustHestonParam_TDP(heston_params_,T);
	...
	for(int i = n-1; i>=0 ; i--)
	{
        DoubleComplex zeta_ = LAMathDisplacedHeston::zeta_DD(phi, beta_, kappa_[i], epsilon_[i], rho_[i]);
		DoubleComplex psi_plus = LAMathDisplacedHeston::psi_DD(phi, zeta_, beta_, kappa_[i], epsilon_[i], rho_[i], 1);
		DoubleComplex psi_minus = LAMathDisplacedHeston::psi_DD(phi, zeta_, beta_, kappa_[i], epsilon_[i], rho_[i], -1);
...
double LAMathDisplacedHestonTDP::F_Integral_DD_TDP(double z,
                                                 double T, 
						                         double FX0, 
						                         double K, 
						                         double beta_,
...
double LAMathDisplacedHestonTDP::Get_F_DD_TDP(double T,
                                            double FX0, 
					                        double K,
					                        double beta_, 
					                        const HestonParams_TDP& heston_params,
...
double LAMathDisplacedHestonTDP::BS_DDHestonImpVol_TDP(double T,
                                                     double FX0,
					                                 double K, 
					                                 double P0,
					                                 int sgn,
														...
														)
{
    ...
    LAMathDisplacedHestonTDP::AdjustHestonParam_TDP(hestonParam_,datas[termSize-1].T);
    ...
    for(k=0;k<GL_Size;k++)
	{
        ...
        for(i=0;i<termSize;i++)
		{            
			zeta[i][k] = LAMathDisplacedHeston::zeta_DD(phi[k], 1., kappa[i], epsilon[i], rho[i]);
			psi_plus[i][k] = LAMathDisplacedHeston::psi_DD(phi[k], zeta[i][k], 1., kappa[i], epsilon[i], rho[i], 1);
			psi_minus[i][k] = LAMathDisplacedHeston::psi_DD(phi[k], zeta[i][k], 1., kappa[i], epsilon[i], rho[i], -1);
        }
    }
...
DoubleMatrix LAMathDisplacedHestonTDP::CalibrationHelperGL( const std::vector<FXOptionData >& datas,
														  const DoubleMatrix& strikes,
														  const IntMatrix& sgns,
														  HestonParams_TDP& hestonParam
														  )
{
    ...
    for(k=0;k<GL_Size;k++)
	{
        ...
        for(i=0;i<termSize;i++)
		{            
			zeta[k] = LAMathDisplacedHeston::zeta_DD(phi[k], 1., kappa, epsilon, rho);
			psi_plus[k] = LAMathDisplacedHeston::psi_DD(phi[k], zeta[k], 1., kappa, epsilon, rho, 1);
			psi_minus[k] = LAMathDisplacedHeston::psi_DD(phi[k], zeta[k], 1., kappa, epsilon, rho, -1);
        }
    }
...
public:
    ...
    QuantLib::Real value(const QuantLib::Array& x) const 
    {
        ...
        DoubleMatrix modelVol = LAMathDisplacedHestonTDP::CalibrationHelperGL( datas, strikes, sgns, hestonParam );
        for(i=0;i<termSize;i++)
        {
            for(j=0;j<strikeSize;j++)
            {
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonGlobal( const std::vector<FXOptionData >& datas,
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonGlobalV0Fix( const std::vector<FXOptionData >& datas,
                                                             const DoubleMatrix& vols,
                                                             const DoubleMatrix& strikes,
                                                             const IntMatrix& sgns,
                                                             HestonParams_TDP& hestonParam
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonGlobalKappaFix( const std::vector<FXOptionData >& datas,
                                                                const DoubleMatrix& vols,
                                                                const DoubleMatrix& strikes,
                                                                const IntMatrix& sgns,
                                                                HestonParams_TDP& hestonParam
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonGlobalThetaFix( const std::vector<FXOptionData >& datas,
                                                                const DoubleMatrix& vols,
                                                                const DoubleMatrix& strikes,
                                                                const IntMatrix& sgns,
                                                                HestonParams_TDP& hestonParam
...
public:
    ...
    QuantLib::Real value(const QuantLib::Array& x) const 
    {
        ...
        DoubleMatrix modelVol = LAMathDisplacedHestonTDP::CalibrationHelper( datas, strikes, sgns, hestonParam );
        for(i=0;i<termSize;i++)
        {
            for(j=0;j<strikeSize;j++)
            {
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonTDP( const std::vector<FXOptionData >& datas,
													 const DoubleMatrix& vols,
													 const DoubleMatrix& strikes,
													 const IntMatrix& sgns,
													 HestonParams_TDP& hestonParam
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonTDPV0Fix( const std::vector<FXOptionData >& datas,
													      const DoubleMatrix& vols,
													      const DoubleMatrix& strikes,
													      const IntMatrix& sgns,
													      HestonParams_TDP& hestonParam
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonTDPRhoFix( const std::vector<FXOptionData >& datas,
														   const DoubleMatrix& vols,
														   const DoubleMatrix& strikes,
														   const IntMatrix& sgns,
														   HestonParams_TDP& hestonParam
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonTDPKappaFix( const std::vector<FXOptionData >& datas,
														     const DoubleMatrix& vols,
														     const DoubleMatrix& strikes,
														     const IntMatrix& sgns,
														     HestonParams_TDP& hestonParam
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonTDPThetaFix( const std::vector<FXOptionData >& datas,
														     const DoubleMatrix& vols,
														     const DoubleMatrix& strikes,
														     const IntMatrix& sgns,
														     HestonParams_TDP& hestonParam
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonTDP_Boot( const std::vector<FXOptionData >& datas,
														  const DoubleMatrix& vols,
													      const DoubleMatrix& strikes,
												          const IntMatrix& sgns,
													      HestonParams_TDP& hestonParam
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonTDTheta( const std::vector<FXOptionData >& datas,
                                                         const DoubleMatrix& vols,
                                                         const DoubleMatrix& strikes,
                                                         const IntMatrix& sgns,
                                                         HestonParams_TDP& hestonParam
...
void LAMathDisplacedHestonTDP::FXCalibrationHestonTDThetaFix( const std::vector<FXOptionData >& datas,
                                                            const DoubleMatrix& vols,
                                                            const DoubleMatrix& strikes,
                                                            const IntMatrix& sgns,
                                                            HestonParams_TDP& hestonParam )
...
double LAMathDisplacedHestonTDP::GetWingFactorFromHeston( const FXOptionData& x,
