...
int 
AQLMathDateUtilities::getExcelDate( const AQLDate & date )
{
...
StubDateAndType AQLMathDateUtilities::getStubDateAndType( const AQLDate & unadjustedStartDate, const AQLDate & unadjustedEndDate, AQLString & term, const AQLPriceDataSlidingRule & busDayAdj, const AQLPriceDataCalendar & calendar, const AQLString* rollConvention, const etrading::StubTypeEnum & stubType )
{
	// Validation
	upper( term );
	...
	{ 
		...
		{
			// Exit Condition
			nextStubDate = AQLMathDateCalculations::getDate( thisStubDate, term, AQLPriceDataSlidingRule(), &AQLPriceDataCalendar(), false, rollConvention ); // rollForwards = false
		...
		{ 
			...
			// *** Short Stubs ***
			thisStubDate		= AQLMathDateCalculations::getDate( thisStubDate, "0D", busDayAdj, &calendar, false, NULL ); // rollForwards = false
		...
		{
			// *** Long Stubs ***
			previousStubDate	= AQLMathDateCalculations::getDate( previousStubDate, "0D", busDayAdj, &calendar, false, NULL ); // rollForwards = false
		...
		// Note: Don't apply roll convention twice, since boundary start and end dates must not have roll convention applied to them
		AQLDate adjustedStartDate = AQLMathDateCalculations::getDate( unadjustedStartDate, "0D", busDayAdj, &calendar, false, NULL ); // rollForwards = false
	...
	else
	{
		...
		{
			// Exit Condition
			nextStubDate = AQLMathDateCalculations::getDate( thisStubDate, term, AQLPriceDataSlidingRule(), &AQLPriceDataCalendar(), true, rollConvention ); // IsAfter = true i.e. roll forwards;
		...
		{ 
			...
			// *** Short Stubs ***
			thisStubDate		= AQLMathDateCalculations::getDate( thisStubDate, "0D", busDayAdj, &calendar, true, NULL ); // rollForwards = true
			stubInfo.stubDate_ = thisStubDate;
		...
		{
			// *** Long Stubs ***
			previousStubDate	= AQLMathDateCalculations::getDate( previousStubDate, "0D", busDayAdj, &calendar, true, NULL ); // rollForwards = true
			stubInfo.stubDate_ = previousStubDate;
		}
		...
		// Check if Regular Schedule
		// Note: Don't apply roll convention twice, since boundary start and end dates must not have roll convention applied to them
		AQLDate adjustedEndDate = AQLMathDateCalculations::getDate( unadjustedEndDate, "0D", busDayAdj, &calendar, false, NULL ); // rollForwards = false
		stubInfo.isRegularSchedule_ = false;
...
AQLDate AQLMathDateUtilities::firstStubDateFromStubType( const AQLDate & startDate, const AQLDate & endDate, AQLString & term )
{
    upper( term );
    bool isAfter    = false; // This tells getDate to calculate dates backwards from the End Date
    
    AQLDate result = AQLMathDateCalculations::getDate( endDate, term, AQLPriceDataSlidingRule(), &AQLPriceDataCalendar(), isAfter, NULL );
    ...
    while ( shortStubDate > startDate )
    {
        ...
        shortStubDate = AQLMathDateCalculations::getDate( shortStubDate, term, AQLPriceDataSlidingRule(), &AQLPriceDataCalendar(), isAfter, NULL );
...
AQLDate
AQLMathDateUtilities::lastStubDateFromStubType( const AQLDate & startDate, const AQLDate & endDate, AQLString & term)
{
    upper( term );
    bool isAfter                    = true; // This tells getDate to calculate dates forwards from the Start Date

    AQLDate result = AQLMathDateCalculations::getDate( startDate, term, AQLPriceDataSlidingRule(), &AQLPriceDataCalendar(), isAfter, NULL );
...
DateVector
AQLMathDateUtilities::calcDatesWithLag( const DateVector &           dates,
                                       ...
                                       const AQLString *             rollConvention )
{
    ...
    {
        results.push_back( AQLMathDateCalculations::getDate( dates[i], term, slidingRule, pCalendar, isAfter, rollConvention ) );
    }
												 ...
												 const AQLString* stubType)
{
    ...
    AQLMathDateCalculations::generateSchedule(unadjustedStart,
											 unadjustedEnd,
											 data_frequency,
											 true,
											 pFirstStubDate,
...
*/
bool
AQLMathDateUtilities::isValidDate( const AQLDate & dateToValidate )
{
...
AQLDate
AQLMathDateUtilities::getDateFromTerm(const AQLDate& fromdate, const double termy, const AQLPriceDataDayCount& daycount, bool includelast)
{
...
AQLDate 
AQLMathDateUtilities::getDateFromTerm(AQLDate& fromdate, double termy, AQLString& dayCountString, bool includelast)
{
	includelast;
	//change nospace & upper
...
double 
AQLMathDateUtilities::getDayFromTerm(AQLDate& fromdate, double termy, AQLString& daycount, bool includelast)
{
	//change nospace & upper
	upper(daycount);
...
double 
AQLMathDateUtilities::getTermFromDay(AQLDate& fromdate, double termd, AQLString& daycount, bool includelast)
{
	//change nospace & upper
	upper(daycount);
...
double 
AQLMathDateUtilities::getTerm(const AQLDate& fromdate, const AQLDate& todate, AQLString& daycount, bool includelast, 
...
AQLDate
AQLMathDateUtilities::getDate(const AQLDate& basedate, const AQLString& term, const AQLString& slidingrule, const AQLString& calendar)
{
	//calendar
	...
	AQLDate ret = AQLMathDateCalculations::getDate(basedate,term,sr,&cal,true); 
	return ret;
}

AQLDate
AQLMathDateUtilities::getDateWithRollConv(const AQLDate& basedate, const AQLString& term, const AQLString& slidingRule, const AQLString& calendar, const AQLString* roll_conv)
{
    //calendar
	AQLPriceDataCalendar cal;
	cal.convertFromString(calendar);
    ...
    AQLDate ret = AQLMathDateCalculations::getDate(basedate, term, sr, &cal, true, roll_conv);
	
    return ret;
}

DateVector
AQLMathDateUtilities::getMultiDate(const DateVector& basedate, const AQLString& term, const AQLString& slidingrule, const AQLString& calendar, const AQLString* roll_conv)
{
	//calendar
	AQLPriceDataCalendar cal;
	cal.convertFromString(calendar);
	...
	{
		ret.push_back(AQLMathDateCalculations::getDate(basedate[i], term, sr, &cal, true, roll_conv));
	}
	return ret;
}

AQLDate
AQLMathDateUtilities::getDateWithRoll(AQLDate& basedate, AQLString& term, AQLString& slidingrule, AQLString& calendar, int roll)
{
	//change nospace & upper
	upper(term);
	upper(slidingrule);
	...
	AQLDate ret = AQLMathDateCalculations::getDate(basedate,multiterm,sr,&cal,true);
	return ret;
}

AQLDate
AQLMathDateUtilities::getIMMDate1(const int& year, const int& month, AQLString& calendar, AQLString& slidingRule)
{
	upper(slidingRule);
	upper(calendar);
	AQLPriceDataSlidingRule sr;
	...
	AQLDate date = AQLMathDateCalculations::getIMMDate(year, month);
	return sr.getDate(date, cal);
}

AQLDate
AQLMathDateUtilities::getIMMDate2(const int& year, const int& number, AQLString& calendar, AQLString& slidingRule)
{
	...
	AQLDate date = AQLMathDateCalculations::getIMMDate(year, number * 3);
	return sr.getDate(date, cal);
}

AQLDate
AQLMathDateUtilities::getIMMDate3(const AQLDate& basedate, const int& number, AQLString& calendar, AQLString& slidingRule)
{
	upper(slidingRule);
	upper(calendar);
	AQLPriceDataSlidingRule sr;
	...
	if (m_baseDate % 3 == 0)
	{
		AQLDate IMMDate_baseDateMonth = AQLMathDateCalculations::getIMMDate(y_baseDate, m_baseDate);
	...
	nextIMMDate = AQLMathDateCalculations::getIMMDate(nextIMMDate.yearOfEra(), nextIMMDate.monthOfYear());
	return sr.getDate(nextIMMDate, cal);
}

// calc regular (non-stub) payment dates for daycount ACT/ACT.ICMA
DateMatrix
AQLMathDateUtilities::calcRegularDates(const AQLString& frequency,
									  const AQLString& Calendar,
									  const AQLString& SlidingRule,
								      const std::vector<AQLDate>& startdates,
								      const std::vector<AQLDate>& enddates)
{
	...
	AQLString term = (AQLString)(AQLMathDateCalculations::getPeriodFrequencyInMonths(frequency)) + "M";
	...
	for(unsigned int i = 0; i < startdates.size(); ++i)
	{
		...
		if(i == 0)
		{
			regular_startdates.push_back(AQLMathDateCalculations::getDate(enddate, "-" + term, slidingrule, &calendar, true));
			...
			while(regular_startdates.back() > startdate) // Long at start
			{
				...
				regular_startdates.push_back(AQLMathDateCalculations::getDate(regular_enddates.back(), "-" + term, slidingrule, &calendar, true));
			}
		}
		else if(i == startdates.size() - 1)
		{
			regular_startdates.push_back(startdate);
			regular_enddates.push_back(AQLMathDateCalculations::getDate(startdate, term, slidingrule, &calendar, true));
			while(regular_enddates.back() < enddate) // Long at end
			{
				regular_startdates.push_back(regular_enddates.back());
				regular_enddates.push_back(AQLMathDateCalculations::getDate(regular_startdates.back(), term, slidingrule, &calendar, true));
			}
		}
		else
		{
...
{
	const AQLDate next_day = AQLMathDateUtilities::getDate(d, "1d", "FOLLOWING", cal);
...
AQLDate CalendarAdvance(AQLDate baseDate, AQLString term, AQLPriceDataSlidingRule slidingRule, AQLPriceDataCalendar calendar)
{
    ...
    return AQLMathDateCalculations::getDate(baseDate, term, slidingRule, &calendar, true, roll_conv);
}
