...
//
//  SYNOPSIS    :       LAPriceDriftSZDDVolFactor
...
*/
LAPriceDriftSZDDVolFactor::LAPriceDriftSZDDVolFactor()
: 
LAPriceDriftHW(),
mpThetaFunc(0),
mInitialValue(0.)
...
*/
LAPriceDriftSZDDVolFactor::LAPriceDriftSZDDVolFactor(const LAPriceDriftSZDDVolFactor& rhs) 
:
LAPriceDriftHW(rhs),
...
LAPriceDriftSZDDVolFactor::clone() const
{
    try 
	{
		return new LAPriceDriftSZDDVolFactor(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
LAPriceDriftSZDDVolFactor::isTypeOf(function_t id) const
{
	return (id==FN_DRIFTSZDDVOLAFACTOR ? true : LAMathDriftFuncBase::isTypeOf(id));
}
...
*/
function_t
LAPriceDriftSZDDVolFactor::getType() const
{
...
*/
void
LAPriceDriftSZDDVolFactor::setUp(LAMathPathEntity& path)
{
	...
	//delete mpHWtool;
	//mpHWtool =  new LAMathHWFuncToolForVar(*dynamic_cast<LAMathHWFuncMR*>(MR.clone()),
	//									*dynamic_cast<LAMathHWFuncSigma*>(Sigma.clone()));
	//delete mpHWtool2;
	//mpHWtool2 =  new LAMathHWFuncToolForMR(*dynamic_cast<LAMathHWFuncMR*>(MR.clone()),
	//									*dynamic_cast<LAMathHWFuncSigma*>(Sigma.clone()));
	//delete mpHWtool3;
	//mpHWtool3 = new LAMathHWFuncToolForMRIntegral(*dynamic_cast<LAMathHWFuncMR*>(MR.clone()),
	//									*dynamic_cast<LAMathHWFuncSigma*>(Sigma.clone()));

	//mFuncExpInt_a_inv.SetFunc(*this, &LAPriceDriftSZDDVolFactor::getExpInt_a_inv);
	//mFuncExpInt_a_sigma_SQ.SetFunc(*this, &LAPriceDriftSZDDVolFactor::getExpInt_a_sigma_SQ);
	...
	//	pcurve->mpHWtoolMR = new LAMathHWFuncToolForMR(*dynamic_cast<LAMathHWFuncMR*>(MR.clone()),
	//												*dynamic_cast<LAMathHWFuncSigma*>(Sigma.clone()));

	//	delete pcurve->mpHWtoolVar; 
	//	pcurve->mpHWtoolVar = new LAMathHWFuncToolForVar(*dynamic_cast<LAMathHWFuncMR*>(MR.clone()),
	//												*dynamic_cast<LAMathHWFuncSigma*>(Sigma.clone()));
	//}
...
*/
double
LAPriceDriftSZDDVolFactor::operator()(const DoubleArray& x) const
{	
    if ( x.size() != 3 )
	{
...
*/
double
LAPriceDriftSZDDVolFactor::getAlpha(const double t) const
{
...
*/
double
LAPriceDriftSZDDVolFactor::getExpInt_a_theta(const double t) const
{
