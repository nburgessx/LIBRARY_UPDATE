...
*/
AQLRatesLJSpotSDE::AQLRatesLJSpotSDE(SDE_TYPE type, AQLFunctionBase* pTransformer, AQLFunctionBase* pInvTransformer, AQLFunctionBase* pAdjuster)
: AQLRatesSpotSDE(type, pTransformer, pInvTransformer, pAdjuster), mpNumeraire_LJ(0)
{
...
*/
AQLRatesLJSpotSDE::AQLRatesLJSpotSDE(const AQLRatesLJSpotSDE& v) 
: AQLRatesSpotSDE(v), mpNumeraire_LJ(0), mWt(v.mWt), mIntegralRegion(v.mIntegralRegion)
{
	mPath_LJ.resize(v.mPath_LJ.size());
	for (unsigned int i = 0; i < mPath_LJ.size(); i++)
		mPath_LJ[i] = v.mPath_LJ[i]->clone();
	if (v.mpNumeraire_LJ != 0)
		mpNumeraire_LJ = dynamic_cast<AQLRatesNumeraireBase*>(v.mpNumeraire->clone());
...
/*!
	@brief destructor
*/
AQLRatesLJSpotSDE::~AQLRatesLJSpotSDE() 
{
	for (unsigned int i = 0; i < mPath_LJ.size(); i++)
		delete mPath_LJ[i];
	mPath_LJ.clear();
...
*/
AQLCoreFunctionBase*	
AQLRatesLJSpotSDE::clone() const
{
    try 
	{
		return new AQLRatesLJSpotSDE(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
AQLRatesLJSpotSDE::isTypeOf(function_t id) const
{
	return (id==FN_LJSPOTSDE ? true : AQLRatesSpotSDE::isTypeOf(id));
}
...
*/
function_t
AQLRatesLJSpotSDE::getType() const
{
	return FN_LJSPOTSDE;
}
...
*/
bool
AQLRatesLJSpotSDE::check(void) const
{
...
AQLRatesLJSpotSDE::getPath(void)
{
...
*/	
void
AQLRatesLJSpotSDE::calcPath(unsigned int pos)
{
	unsigned int pos_s;
...
*/
void
AQLRatesLJSpotSDE::setNumeraire(AQLRatesNumeraireBase* pnumeraire)
{
