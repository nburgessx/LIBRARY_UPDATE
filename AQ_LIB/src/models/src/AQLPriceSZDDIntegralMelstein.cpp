...
*/
//AQLPriceSZDDIntegralMelstein::AQLPriceSZDDIntegralMelstein()
//: AQLRatesSDEIntegralBase(NORMAL_INTEGRAL), mPos_old(0)
//{
//	mVar.resize(3);
//}
...
*/
AQLPriceSZDDIntegralMelstein::AQLPriceSZDDIntegralMelstein(const AQLString &sdeAttrName, const AQLString &sdeAttrNameVol)
: AQLRatesSDEIntegralBase(NORMAL_INTEGRAL, sdeAttrName), mpVolSDE(0), mSDEAttrNameVol(sdeAttrNameVol), mPos_old(0), mCorr(0)
{
	mVar.resize(3);
}
...
*/
AQLPriceSZDDIntegralMelstein::AQLPriceSZDDIntegralMelstein(const AQLPriceSZDDIntegralMelstein& v) 
: AQLRatesSDEIntegralBase(v), mVar(v.mVar), mpVolSDE(v.mpVolSDE), mSDEAttrNameVol(v.mSDEAttrNameVol), mPos_old(0), mCorr(0)
{
...
*/
AQLCoreFunctionBase*	
AQLPriceSZDDIntegralMelstein::clone() const	
{
    try 
	{
		return new AQLPriceSZDDIntegralMelstein(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
AQLPriceSZDDIntegralMelstein::isTypeOf(function_t id) const
{
	return (id==FN_SZDDINTEGRAL2 ? true : AQLRatesSDEIntegralBase::isTypeOf(id));
}
...
*/
function_t
AQLPriceSZDDIntegralMelstein::getType() const
{
...
*/
void
AQLPriceSZDDIntegralMelstein::integral(double ts, double te, 
							...
							) const
{
	...
	{
		//error
		throw AQLCoreInvalidData("volatility class must be AQLMathVolFuncSZDD!", __FILE__, __LINE__);
	}
	const AQLMathVolFuncSZDD* volfuncsz =dynamic_cast<const AQLMathVolFuncSZDD*>(volfunc);
	...
	//--------
		...
		if (!mpVolSDE)
		{
			throw AQLCoreInvalidData("Vol SDE is not set in AQLPriceSZDDIntegralMelstein", __FILE__, __LINE__);
		}
	   ...
	   //
		...
		AQLRatesBM* bm1 = mpVolSDE->getBM();	//vol
		...
		//const AQLPriceDriftFX *pDrift = 0;
...
void AQLPriceSZDDIntegralMelstein::setUp(AQLMathPathEntity& path)
{
	//fx sde 
	AQLDataHolder *dh = &path.getData(mSDEAttrNameVol, ISNOTNULL);
	mpVolSDE = &dynamic_cast<AQLMathAttrSDE&>(dh->get()).getSDE();
