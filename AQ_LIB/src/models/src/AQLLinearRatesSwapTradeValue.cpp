...
*/
AQLLinearRatesSwapTradeValue::AQLLinearRatesSwapTradeValue(AQLPriceAccruedInterest* pacc) :
AQLPriceTradeValue(pacc)
{
}
/*!
...
*/
AQLLinearRatesSwapTradeValue::AQLLinearRatesSwapTradeValue(const AQLLinearRatesSwapTradeValue& v)
...
*/
AQLLinearRatesSwapTradeValue::~AQLLinearRatesSwapTradeValue()
{
}
/*!
...
*/
bool
AQLLinearRatesSwapTradeValue::isTypeOf(function_t id) const
{
	return (id == FN_IR_PLAINVANILLASWAPTRADEVALUE ? true : AQLPriceTradeValue::isTypeOf(id));
}
/*!
...
*/
AQLCoreFunctionBase*
AQLLinearRatesSwapTradeValue::clone() const
{
    try 
	{
  		return new AQLLinearRatesSwapTradeValue(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
function_t			
AQLLinearRatesSwapTradeValue::getType() const
{
...
*/
void
AQLLinearRatesSwapTradeValue::registerData(AQLPriceDataManager& dm) const
{
...
*/
double
AQLLinearRatesSwapTradeValue::do_value(const AQLDate& basedate, AQLObject& object,
                                         const AQLDataValuation& att) const
{
	...
	AQLLinearRatesSwapTradeValueDataProvider* dp = NULL;
	if (att.isNullDataProvider())
		dp = dynamic_cast<AQLLinearRatesSwapTradeValueDataProvider*>(setUpDataProvider(basedate, object, att));
	else
		dp = &dynamic_cast<AQLLinearRatesSwapTradeValueDataProvider&>(att.getDataProvider());

	dh = &(object.getData(PRICING_DATA_VALUATIONCURRENCY, NOCHECK));
	if (dh->isDefined() && !dh->isNull())
		dp->basecur = dynamic_cast<const AQLDataString&>(dh->get()).get();

	AQLLinearRatesSwapTradeValueDataProvider* dataProvider = dp;
			...
			{	
				...
				for ( size_t j = 0; j < payOff[i].size(); j++ )
				{
                    if(payOff[i][j].getPayOff().isCompounding()){
                        fradfs[i][j] = 0;
                        const AQLPricePayOffToolCompound* temp = dynamic_cast<const AQLPricePayOffToolCompound*>(&payOff[i][j].getPayOff());
                        if(temp==NULL){
                            AQLString msg;
                            msg += "CAST ERROR:";
                            msg += "payoff tool cannot be casted to AQLPriceCouponToolCompound";
					...
					vector<AQLPriceCouponTool *> coupons = payOff[i][j].getPayOff().mpCoupons;

					if (coupons.size() == 1)
					{
						vector<AQLPriceIndexToolBase *> indexs = (*coupons[0]).getIndexs();
						...
						if (indexs.size() == 1)
						{
							...
							{
								const UintArray &indexPos = dynamic_cast<AQLPriceIndexTool *>(indexs[0])->getIndexPos();
								if (!indexPos.empty())
								{
									// get last index value
									forwards[i][j] = dynamic_cast<AQLPriceIndexTool *>(indexs[0])->getIndexEntity()->getIndex()[indexPos.back()];
									fixingDates[i][j] = dynamic_cast<AQLPriceIndexTool *>(indexs[0])->getIndexEntity()->getGrid()[indexPos.back()];
							...
							{
								forwards[i][j] = dynamic_cast<AQLPriceIndexToolFixed *>(indexs[0])->getFixedRate();
								AQLDate* pFixingDate = dynamic_cast<AQLPriceIndexToolFixed *>(indexs[0])->getFixingDate();
								if (pFixingDate) 
								{
					...
					else
					{
						...
						// for digital
						AQLPriceCouponBase* opb = payOff[i][j].getPayOff().mpCpnSelectOperator;
						if (coupons.size() == 6 && opb->isTypeOf(FN_CPNSLTOPERATORFORDIGITAL2))
						{
							AQLPriceCouponForDigital2* op = dynamic_cast<AQLPriceCouponForDigital2*>(opb);
					...
					if (coupons.size() == 1)
					{
						...
						if (indexs.size() == 1)
						{
							if (indexs[0]->isTypeOf(INDEX_TOOL))
							{
								...
								{
									// get last index value
									AQLString indextype = dynamic_cast<AQLPriceIndexTool *>(indexs[0])->getIndexEntity()->getIndexType();
									bool isDelayed = dynamic_cast<AQLPriceIndexTool *>(indexs[0])->getIndexEntity()->isDelayedConvexityAdjusted();
									if (indextype == "CMS" || isDelayed)
									{
										convexityAdjusts[i][j] = dynamic_cast<AQLPriceIndexTool *>(indexs[0])->getIndexEntity()->getConvexityAdjust()[indexPos.back()];
										convexityAdjustVols[i][j] = dynamic_cast<AQLPriceIndexTool *>(indexs[0])->getIndexEntity()->getConvexityAdjustVol()[indexPos.back()];
...
*/
double
AQLLinearRatesSwapTradeValue::value(const AQLDate& basedate, 
						AQLObject& object, 
						AQLDataProvider* dp,
...
*/
AQLDataProvider*
AQLLinearRatesSwapTradeValue::setUpDataProvider(const AQLDate& basedate, AQLObject& object, 
								const AQLDataValuation& att) const
{
	AQLDataHolder* dh;
	AQLLinearRatesSwapTradeValueDataProvider* dataProvider = dynamic_cast<AQLLinearRatesSwapTradeValueDataProvider*>(createNewDataProvider());
	...
	dataProvider->pVanilla = &dynamic_cast<AQLMathPlainVanillaEntity&>(attr.get().get());
	...
	if ( (dataProvider->basecur != dataProvider->paycur || dataProvider->basecur != dataProvider->reccur) && legs.getSize() > 1 )
	{
		...
		//dataProvider->pFX = &dynamic_cast<AQLMathFXEntity&>(fx.get().get());
	}
...
double
AQLLinearRatesSwapTradeValue::value_backward(const DoubleMatrix& time,
...
AQLLinearRatesSwapTradeValue::getFXEntity(AQLObject &object) const
{
...
*/
void						
AQLLinearRatesSwapTradeValue::setUpForAnnuityCalc(AQLObject& object, AQLDataProvider* dp) const
{
	AQLLinearRatesSwapTradeValueDataProvider* dataProvider = dynamic_cast<AQLLinearRatesSwapTradeValueDataProvider*>(dp);
...
//for stub swap
void
AQLLinearRatesSwapTradeValue::setUpStubCoefficient(AQLObject& trade, AQLDataProvider* dp) const
{
	AQLDataHolder* dh;
	...
	AQLMathPlainVanillaEntity *pvanilla = dataProvider->pVanilla;
	if (!pvanilla)
	{
...
*/
AQLDataProvider*
AQLLinearRatesSwapTradeValue::createNewDataProvider() const
{
	AQLLinearRatesSwapTradeValueDataProvider* dataProvider = NULL;
	try 
	{
		dataProvider = new AQLLinearRatesSwapTradeValueDataProvider();
	}
	catch (bad_alloc & e)
	{
		throw AQLCoreSystemError(e.what(), __FILE__, __LINE__);
...
AQLLinearRatesSwapTradeValue::AQLLinearRatesSwapTradeValueDataProvider::~AQLLinearRatesSwapTradeValueDataProvider()
{
}

void AQLLinearRatesSwapTradeValue::setCompoundedRate(AQLObject& trade, const AQLPricePayOff& payoff) const{
...
void AQLLinearRatesSwapTradeValue::doSetUpStubCoefficient(AQLObject& coupon, AQLObject& cashlet, AQLMathPlainVanillaEntity* pvanilla) const
{
    AQLDataHolder* dh;
...
bool AQLLinearRatesSwapTradeValue::hasCashflow(const AQLObject& trade) const
{
...
double
AQLLinearRatesSwapTradeValue::value(const AQLDate& basedate, AQLObject& object,
                                      const AQLDataValuation& att) const
{
...
void
AQLLinearRatesSwapTradeValue::removeNotionalExposure(AQLObject& trade, AQLDataProvider* dp) const
{
	AQLDataHolder* dh = 0;
	...
	if (isRenotional)
	{
		...
		const AQLDate asofDate = dynamic_cast<AQLLinearRatesSwapTradeValueDataProvider*>(dp)->pVanilla->getAsOfDate().get(); 
...
*/
double 
AQLLinearRatesSwapTradeValue::calcFeeValueVanilla(AQLObject &tradeEntity,const AQLString& baseCurrency)
{
	double pvFee = 0.;
	AQLDataHolder* dh;
	...
	if (dh->isDefined() && !dh->isNull())
	{
		...
		for (size_t i = 0; i < feeAmounts.size(); i++)
		{
			...
			const AQLMathPlainVanillaEntity& vanillaEntity = dynamic_cast<const AQLMathPlainVanillaEntity&>(pathEntity->get().get());
			...
			if (feeCurrencies[i] != baseCurrency)
			{
				...
				const AQLMathFXEntity& fxEntity = dynamic_cast<const AQLMathFXEntity&>(fXEntityRef.get().get());
