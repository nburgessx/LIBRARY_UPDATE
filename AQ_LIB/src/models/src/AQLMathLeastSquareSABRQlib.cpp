...
AQLMathLeastSquareSABRCostFuncQlib::AQLMathLeastSquareSABRCostFuncQlib( AQLMathSABR& sabr_,
														  double expiry_,
														  double forward_,
														  double numeraire_,
														  double atmVol_,
...
cloned(false)
{
	if(weight.size() != n)
	{
		throw AQLCoreInvalidData(" strike.size() != weight.size() : AQLMathLeastSquareSABRCostFuncQlib::AQLMathLeastSquareSABRCostFuncQlib", __FILE__, __LINE__);
	}
	
	if(param0.size() != 4)
	{
		throw AQLCoreInvalidData("param0.size() != 4 : AQLMathLeastSquareSABRCostFuncQlib::AQLMathLeastSquareSABRCostFuncQlib", __FILE__, __LINE__);
	}

	if(flg.size() != 4)
	{
		throw AQLCoreInvalidData("flg.size() != 4 : AQLMathLeastSquareSABRCostFuncQlib::AQLMathLeastSquareSABRCostFuncQlib", __FILE__, __LINE__);
	}
	...
	for(size_t i = 0; i < n; ++i)
	{
		if(sgn[i] != 1 && sgn[i] != -1)
		{
			throw AQLCoreInvalidData("sgn must -1 or 1 : AQLMathLeastSquareSABRCostFuncQlib::AQLMathLeastSquareSABRCostFuncQlib", __FILE__, __LINE__);
		}
	}
...
//
AQLMathLeastSquareSABRCostFuncQlib::AQLMathLeastSquareSABRCostFuncQlib(const AQLMathLeastSquareSABRCostFuncQlib& rhs)
:
CostFunction(rhs),
n(rhs.n),
expiry(rhs.expiry),
...
AQLMathLeastSquareSABRCostFuncQlib::~AQLMathLeastSquareSABRCostFuncQlib()
{
	if(cloned)
	{
		delete sabr;
...
QuantLib::Real AQLMathLeastSquareSABRCostFuncQlib::value(const QuantLib::Array& x) const
{   
	size_t m = x.size();
	
	if(m == 0 || m > 4)
		throw AQLCoreInvalidData("m == 0 || m > 4 : AQLMathLeastSquareSABRCostFuncQlib::value", __FILE__, __LINE__);
...
QuantLib::Array AQLMathLeastSquareSABRCostFuncQlib::values(const QuantLib::Array& x) const
{
	size_t m = x.size();
...
void AQLMathLeastSquareSABRCostFuncQlib::set_params(const QuantLib::Array& x) const
{
	// 1st
	if(!flg[0])
	{
		if(!flg[1])
		{
			if(!flg[2])
			{
				if(!flg[3])
				{
					throw AQLCoreInvalidData("All element of flg is false : AQLMathLeastSquareSABRCostFuncQlib::set_params", __FILE__, __LINE__);
				}
				else rho = x[0];
			}
			else
...
//AQLMathLeastSquareSABRConstraintQlib::AQLMathLeastSquareSABRConstraintQlib(vector<double>& param0,
//															 double max_alpha,
//								                             double min_alpha,
//								                             double max_beta,
//								                             double min_beta,
...
//								                             double min_rho
//								                            )
//: QuantLib::Constraint(boost::shared_ptr<QuantLib::Constraint::Impl>( new AQLMathLeastSquareSABRConstraintQlib::Impl(param0,
//																											  max_alpha,
//																											  min_alpha,
//																											  max_beta,
//																											  min_beta,
...
//
AQLMathLeastSquareSABRConstraintQlib::Impl::Impl(vector<double>& param0_,
										  const vector<bool>& flg_,
										  double max_alpha_,
							              double min_alpha_,
							              double max_beta_,
...
//
//
bool AQLMathLeastSquareSABRConstraintQlib::Impl::test(const QuantLib::Array& p) const
{
 	size_t m = p.size();
...
void AQLMathLeastSquareSABRConstraintQlib::Impl::set_params(const QuantLib::Array& x) const
{
	// 1st
	if(!flg[0])
	{
...
/*AQLMathLeastSquareSABRConstraint_AntonovQlib::AQLMathLeastSquareSABRConstraint_AntonovQlib( double fwd,
...
//
AQLMathLeastSquareSABRConstraint_AntonovQlib::Impl::Impl( double fwd_,
...
//
bool AQLMathLeastSquareSABRConstraint_AntonovQlib::Impl::test(const QuantLib::Array& p) const
{
 	size_t m = p.size();
...
void AQLMathLeastSquareSABRConstraint_AntonovQlib::Impl::set_params(const QuantLib::Array& x) const
{
	if(!flg[0])
	{
		if(!flg[1])
...
AQLMathLeastSquareSABRCapFloorCostFuncQlib::AQLMathLeastSquareSABRCapFloorCostFuncQlib(
	...
	tenor_point(AQLMathSwaptionVolUtility::getTenorPoint(tenor_point_str_)),
	...
	cloned(false)
{
	...
	mpSabr.reset(AQLMathSwaptionVolUtility::createSABR(approxmethod, alpha, beta, nu, rho));
}


//
AQLMathLeastSquareSABRCapFloorCostFuncQlib::AQLMathLeastSquareSABRCapFloorCostFuncQlib(const AQLMathLeastSquareSABRCapFloorCostFuncQlib& rhs)
	:
	CostFunction(rhs),
	n(rhs.n),
...
QuantLib::Real AQLMathLeastSquareSABRCapFloorCostFuncQlib::value(const QuantLib::Array& x) const
{
	size_t m = x.size();

	if (m == 0 || m > 4)
		throw AQLCoreInvalidData("m == 0 || m > 4 : AQLMathLeastSquareSABRCapFloorCostFuncQlib::value", __FILE__, __LINE__);
	...
	AQLMathSwaptionMatrix alphaMat_(alphamat_temp, expiry_capfloor, tenor);
	AQLMathSwaptionMatrix betaMat_(betamat_temp, expiry_capfloor, tenor);
	AQLMathSwaptionMatrix nuMat_(numat_temp, expiry_capfloor, tenor);
	AQLMathSwaptionMatrix rhoMat_(rhomat_temp, expiry_capfloor, tenor);
...
QuantLib::Array AQLMathLeastSquareSABRCapFloorCostFuncQlib::values(const QuantLib::Array& x) const
{
	size_t m = x.size();

	if (m == 0 || m > 4)
		throw AQLCoreInvalidData("m == 0 || m > 4 : AQLMathLeastSquareSABRCapFloorCostFuncQlib::values", __FILE__, __LINE__);
...
void AQLMathLeastSquareSABRCapFloorCostFuncQlib::set_params(const QuantLib::Array& x) const
{
	// 1st
	if (!flg[0])
	{
		if (!flg[1])
		{
			if (!flg[2])
			{
				if (!flg[3])
				{
					throw AQLCoreInvalidData("All element of flg is false : AQLMathLeastSquareSABRCapFloorCostFuncQlib::set_params", __FILE__, __LINE__);
				}
				else rho = x[0];
			}
			else
