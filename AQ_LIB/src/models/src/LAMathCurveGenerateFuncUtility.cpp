...
#include "LAMathCurveGenerateFuncUtility.h"
...
void  
LAMathCurveGenerateFuncUtility::setUpCurveFromMarket(AQLDataInstance* dataInstance, const AQLString& CurveID, 
						...
						AQLStringMatrix ylddata, bool isCheckDF, bool isAUDadjust)
{
	...
	AQLDate		asofdate			= LAMathDateUtilities::getLADate(chgrow(ylddata,CURVEINPUT_ASOFDATE,1));
	...
	LAMathYieldCurvePro * yp = NULL; 
	if(!ehCur.isDefined())
	{	
		yp= new LAMathYieldCurvePro(dataInstance);
		objPool.set(CurveIDTool,yp);
	}
	else
	{
		dynamic_cast<LAMathYieldCurvePro &>(objPool.getObject(CurveIDTool).get()).reset();
		yp	= &dynamic_cast<LAMathYieldCurvePro &>(objPool.getObject(CurveIDTool).get());
	}
	yp->getName().convertFromString(CurveIDTool);
	yp->setDFGenerator(FN_IRYIELDGENERATOR_STR);
	yp->getAsOfDate().set(asofdate);
	yp->getIsFutureUse().set(isfuteruse);
	yp->setInterpolation(interpolation);
	yp->getInterpolation_fw().convertFromString(futureinterpolation);
	yp->getInterpolation_yg().convertFromString(yieldgeninterpolation);
	yp->getYieldData().convertFromString(CurveID);
	yp->getDayCount().convertFromString(AC_365I);
	yp->getIsSwapTenorAdjust().set(false);
	yp->getIsFRAUse().set(false);
	if (!isSTD)
	{
		yp->AQLObject::add(IR_CALIBRATION_DATA_GENTARGETDF, new AQLDataString(curveType));
		AQLStringVector tmpCurveType(1,curveType);
		yp->AQLObject::add(IR_CALIBRATION_DATA_GENERATEDFS, new AQLDataStrings(tmpCurveType));

		yp->setAssignedCurveMktMap(curveType,curveType);
	}
	//RatePriority
	...
	if (ratepriority == "SWAP:LIBOR:FUTURE"||ratepriority == "SWAP:FUTURE:LIBOR"||ratepriority == "FUTURE:LIBOR:SWAP"||ratepriority == "FUTURE:SWAP:LIBOR"||ratepriority == "LIBOR:SWAP:FUTURE"||ratepriority == "LIBOR:FUTURE:SWAP")
		yp->getRatePriority().convertFromString(ratepriority);
	else if (ratepriority == "")
	{	
		const AQLStringVector tmpstrvec(0);
		yp->getRatePriority().set(tmpstrvec);
	}
	else
	...
	double rate=0.0;
	//Money Market
    //spotdate	= LAMathDateUtilities::getLADate(chgrow(mdata,CURVEINPUT_SPOTDATE,1));
	...
	{		
        ...
        spotdate = LAMathDateUtilities::getDate(asofdate,spotLag,srule,calendar);
    }
    else if(tmprow2>=0)
    {
        spotdate = LAMathDateUtilities::getLADate(chgrow(mdata,CURVEINPUT_SPOTDATE,1));
	...
	////LiborData
	//spotdate	= LAMathDateUtilities::getLADate(chgrow(ldata,CURVEINPUT_SPOTDATE,1));
	...
	////SwapData
	//spotdate	= LAMathDateUtilities::getLADate(chgrow(sdata,CURVEINPUT_SPOTDATE,1));
	...
	if(isfuteruse)
	{
		//spotdate	= LAMathDateUtilities::getLADate(chgrow(fdata,CURVEINPUT_SPOTDATE,1));
		...
		for(int i=1; i<N; i++)
		{
			AQLDate	StartDate	=	LAMathDateUtilities::getLADate(chgcol(fgrid,CURVEINPUT_STARTDATE,	i));
			AQLDate EndDate		=	LAMathDateUtilities::getLADate(chgcol(fgrid,CURVEINPUT_ENDDATE,		i));
	...
	if (isSTD)
	{
		yp->getMarketData().convertFromString(ref);
	}
	else
	{
		yp->AQLObject::add(CALIBRATION_DATA_MARKETDATA + suffix, new AQLDataMultiReference()).convertFromString(ref);
	}
	//generate Object
	dataInstance->getReferencePool().completeDependency();
	...
	modelDataObj.calibrateModel(asofdate);
	if (!resetFlg)
		restoreSwapRateFromL(*yp,Currency,aud_origSwapRate,sdata);
	if (!isSTD)
	{
		yp->AQLObject::remove(CALIBRATION_DATA_MARKETDATA + suffix);
	}
	if(isCheckDF)
	{
		// check df
...
void 
LAMathCurveGenerateFuncUtility::setUpXccyBasisCurve(AQLDataInstance* dataInstance,const AQLString& BCurveID, 
						AQLStringMatrix ylddata, AQLStringMatrix bgrid, AQLStringMatrix bdata)
{
	...
	AQLString term;
    AQLDate spotdate;
	//AQLDate spotdate		= LAMathDateUtilities::getLADate(chgrow(bdata,CURVEINPUT_SPOTDATE,1));		
    ...
    else if(tmprow1>=0)
    {
        ...
        spotdate = LAMathDateUtilities::getDate(asofdate,spotLag,bsrule,bcalendar);
    }
    else
    {
        spotdate = LAMathDateUtilities::getLADate(chgrow(bdata,CURVEINPUT_SPOTDATE,1));
    }
	...
	LAMathYieldCurve* ycp = NULL;
	if(!ehyld.isDefined())
	{
		ycp = new LAMathYieldCurve(dataInstance);
		objPool.set(yldname,ycp);
	}
	else
	{
		dynamic_cast<LAMathYieldCurve &>(objPool.getObject(yldname).get()).reset();
		ycp = &dynamic_cast<LAMathYieldCurve &>(objPool.getObject(yldname).get());
	}
	...
	{
		//dynamic_cast<LAMathYieldCurve &>(objPool.getObject(CurveIDTool).get()).reset();
		yp	= &dynamic_cast<LAMathYieldCurvePro &>(objPool.getObject(CurveIDTool).get());
	}
	
	AQLString ref = bref.subString(0,bref.size()-2);
	yp->getYieldData().convertFromString(BCurveID);
	yp->getBaseYieldCurve().convertFromString(yldname);
	yp->getInterpolation_bs().convertFromString(FN_SPLINEINTERPOLATION_STR);
	yp->getInterpolation().convertFromString(FN_SPLINEINTERPOLATION_STR);
	yp->getDayCount().convertFromString(AC_365I);
	AQLDataMultiReference &refMarket = yp->getMarketData();
	if (!refMarket.isNull())
	{
		AQLString n_ref = "";
		...
		n_ref += ref;
		yp->getMarketData().convertFromString(n_ref);
	}
	else
	{
		yp->getMarketData().convertFromString(ref);
...
void 
LAMathCurveGenerateFuncUtility::setUpBasisCurve(AQLDataInstance* dataInstance,
                                              ...
                                              AQLStringMatrix bdata)
{
	...
	AQLString ref = bref.subString(0,bref.size()-2);
	yp->getYieldData().convertFromString(stdCurveID);
	yp->getBaseYieldCurve().convertFromString(yldname);
	yp->getInterpolation_bs().convertFromString(FN_SPLINEINTERPOLATION_STR);
	yp->getInterpolation().convertFromString(FN_SPLINEINTERPOLATION_STR);
	yp->getDayCount().convertFromString(AC_365I);
	...
	dataInstance->getReferencePool().completeDependency();
	yp->setBasisRates2(basisCurveID);
}

void 
LAMathCurveGenerateFuncUtility::setUpBasisCurveFromMarket(AQLDataInstance* dataInstance,const AQLString& CurveID, 
...
{
	LAMathCurveGenerateFuncUtility::setUpCurveFromMarket(dataInstance,CurveID,mgrid,lgrid,fgrid,sgrid,mdata,ldata,fdata,sdata,ylddata,isCheckDF,isAUDadjust);
	LAMathCurveGenerateFuncUtility::setUpXccyBasisCurve(dataInstance,CurveID,ylddata,bgrid,bdata);
}

void  
LAMathCurveGenerateFuncUtility::
                             ...
                             AQLStringMatrix yldData, AQLString& warning )
{
	...
	{
		yp->getData(CALIBRATION_DATA_CURVEGENERATOR, NOCHECK).convertFromString(FN_IRARBFREEGENERATOR_JPYBASE_STR);
	}
	else
	{*/
	yp->getData(CALIBRATION_DATA_CURVEGENERATOR, NOCHECK).convertFromString(FN_IRARBFREEGENERATOR_STR);
	//}

	yp->getIsFRAUse().set(isFRAUse);
	yp->getIsFutureUse().set(false);
    yp->getName().convertFromString(CurveIDTool);
	yp->getAsOfDate().set(asofdate);
	yp->getYieldData().convertFromString(CurveID);
    yp->setInterpolation(interpolation);
	yp->getDayCount().convertFromString(AC_365I);
	yp->AQLObject::add(IR_CALIBRATION_DATA_ISRENOTIONALADJUST, new AQLDataBool(isRenAdj));
	yp->AQLObject::add(IR_CALIBRATION_DATA_ISXCCYMARKEDTOMARKET, new AQLDataBool(isRenAdj)); // alias for isRenotionalAdjust
	...
	yp->setArbFreeCurveName(curveNames_6ML,curveNames_DF,curveNames_3ML);
    ...
    else if(ltmprow1>=0)
    {
		...
		lspotdate = LAMathDateUtilities::getDate(asofdate,lspotLag,lrule,lcalendar);
    }
    else if(ltmprow2>=0)
    {
        lspotdate = LAMathDateUtilities::getLADate(chgrow(swapConv,CURVEINPUT_SPOTDATE,1));
    }
	/**/
    ...
    {
        spotdate = LAMathDateUtilities::getLADate(chgrow(swapConv,CURVEINPUT_SPOTDATE,1));
    }
    ...
    if( Currency != CURRENCY_USD )
    {
		...
		for (size_t i=0; i<term.size(); i++)
		{
			...
			{
				spotdate = LAMathDateUtilities::getLADate(chgrow(currConv,CURVEINPUT_SPOTDATE,1));
			}
			...
			else
			{
				if (isRenAdj)
				{
					yp->getForeignYieldData().convertFromString(usdCurve);
				...
				{
					yp->getForeignYieldData().convertFromString(parCurve);
		...
		yp->AQLObject::add(CALIBRATION_DATA_MARKETDATA + AQLString("_") + XCCYBASIS, new AQLDataMultiReference()).convertFromString(ref_XCCYBasis);
    }
	else
	{
		AQLObject* be	=	NULL;
	...
	yp->AQLObject::add(CALIBRATION_DATA_MARKETDATA + AQLString("_") + THREESIXBASIS, new AQLDataMultiReference()).convertFromString(ref_36Basis);
    ...
    if(isFRAUse)
    {
        ...
        {
            spotdate = LAMathDateUtilities::getLADate(chgrow(fraConv,CURVEINPUT_SPOTDATE,1));
        }
	...
	yp->AQLObject::add(IR_CALIBRATION_DATA_MAINBASISDF, new AQLDataString("DF"));
	yp->setBasisRates();
...
void 
LAMathCurveGenerateFuncUtility::setUpForecastCurveFromMarket(AQLDataInstance* dataInstance,
...
void 
LAMathCurveGenerateFuncUtility::setUpForecastCurve(AQLDataInstance* dataInstance,
...
void 
LAMathCurveGenerateFuncUtility::setUpCdtDFCurve(AQLDataInstance* dataInstance, AQLString curveID, AQLStringMatrix data)
{
	...
	{		
        ...
        spotdate = LAMathDateUtilities::getDate(asOfDate,spotLag,srule_str,calendar_str);
    }
    else if(tmprow2>=0)
    {
        spotdate = LAMathDateUtilities::getLADate(chgrow(data,CURVEINPUT_SPOTDATE,1));
    }
...
AQLString 
LAMathCurveGenerateFuncUtility::getCurveGenerateTime(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLString& curvename)
{
...
void	
LAMathCurveGenerateFuncUtility::setUpCurve2(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLDate& asofdate, 
										  const DoubleMatrix& mat, const AQLString& curveName)
{
	LAMathCurveGenerateFuncUtility::setUpCurve(dataInstance,curveid,asofdate,mat,curveName);
	AQLString suffix;
	if (curveName == STD)
	{
		suffix = AQLString("");
...
void 
LAMathCurveGenerateFuncUtility::setUpCurve(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLDate& asofdate, 
										 const DoubleMatrix& mat, const AQLString& curveName)
{
	unsigned int N=mat.size();
	if (N < 2) 
...
/////////////furuytos20120813///////////
void 
LAMathCurveGenerateFuncUtility::setUpCurve3(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLDate& asofdate, 
...
DoubleArray 
LAMathCurveGenerateFuncUtility::outPutCurveFromYieldData(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLString& curveName)
{
	AQLObjectPool& objPool	= dataInstance->getObjectPool();
...
DoubleArray 
LAMathCurveGenerateFuncUtility::outPutCurveFromYieldData2(AQLDataInstance* dataInstance, const AQLString& curveID, const AQLString& curveID2,
...
*/
DiscountFactorTable 
LAMathCurveGenerateFuncUtility::getCurveDiscountFactorTable(AQLDataInstance* dataInstance, const AQLString& curveid, const AQLString& curveName)
{
	AQLObjectPool& objPool	= dataInstance->getObjectPool();
    ...
    for( size_t i = 0; i<termsVector.size() ; ++i )
    {
        ...
        paymentDatesVector[i] = LAMathDateUtilities::getDateFromTerm( asOfDate, termsVector[i], ACT_365_ISDA);
    }
...
*/
void LAMathCurveGenerateFuncUtility::setCurveDiscountFactorTable(AQLDataInstance* dataInstance, const AQLString& curveid, const std::vector<AQLString>& curveNames, const DiscountFactorTable& discountFactorTable )
{
...
*/
ForwardRateTable LAMathCurveGenerateFuncUtility::getCurveForwardRateTable( AQLDataInstance* dataInstance, const AQLString& curveid, const AQLString& curveName )
{
    AQLObjectPool& objPool = dataInstance->getObjectPool();
    ...
    for( size_t i = 0; i<termsMatrix[0].size() ; ++i )
    {
        // Curve terms are always calculated using ACT/365_ISDA as defined in ConstantDeclarations.h [sic] - see #define AC_365I
		AQLString ACT_365_ISDA = AQLString("ACT/365_ISDA");
        fixingDatesVector[i] = LAMathDateUtilities::getDateFromTerm( asOfDate, termsMatrix[0][i], ACT_365_ISDA);
    }
...
*/
void LAMathCurveGenerateFuncUtility::setCurveForwardRateTable( AQLDataInstance* dataInstance, const AQLString& curveid, const std::vector<AQLString>& curveNames, const ForwardRateTable& forwardRateTable )
{
        // 1. Get the terms and discount factors from the discountFactorTable
    // ---------------------------------------
...
void 
LAMathCurveGenerateFuncUtility::restoreSwapRateFromL(LAMathYieldCurvePro &curve, const AQLString &ccy, const std::map<AQLString, double> &sRateMap, AQLStringMatrix sdata)
{
	//AQLDate spotDate	= LAMathDateUtilities::getLADate(chgrow(sdata,CURVEINPUT_SPOTDATE,1));
	std::map<AQLString, AQLString>& dvar = AQLCoreComponentManager::getDayCountMap();
	...
	{		
		AQLString spotLag = AQLFunctionUtilities::findElement(sdata,CURVEINPUT_SPOTLAG,0,1,true);
        spotDate = LAMathDateUtilities::getDate(asOfDate,spotLag,slidingStr,calStr);
    }
    else if(tmprow2>=0)
    {
        spotDate = LAMathDateUtilities::getLADate(chgrow(sdata,CURVEINPUT_SPOTDATE,1));
...
void 
LAMathCurveGenerateFuncUtility::resetMarketDataUseL(LAMathYieldCurvePro &curve, const AQLString &ccy, AQLStringMatrix ldata)
{
	//AQLDate spotDate	= LAMathDateUtilities::getLADate(chgrow(ldata,CURVEINPUT_SPOTDATE,1));
	std::map<AQLString, AQLString>& dvar = AQLCoreComponentManager::getDayCountMap();
