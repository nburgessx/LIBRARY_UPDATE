...
void  
LAMathCurveGenerateFuncUtility::setUpCurveFromMarket(LADataInstance* dataInstance, const LAString& CurveID, 
						...
						LAStringMatrix ylddata, bool isCheckDF, bool isAUDadjust)
{
	...
	LADate		asofdate			= LAMathDateUtilities::getLADate(chgrow(ylddata,CURVEINPUT_ASOFDATE,1));
	...
	if (!isSTD)
	{
		yp->LAObject::add(IR_CALIBRATION_DATA_GENTARGETDF, new LADataString(curveType));
		LAStringVector tmpCurveType(1,curveType);
		yp->LAObject::add(IR_CALIBRATION_DATA_GENERATEDFS, new LADataStrings(tmpCurveType));
	...
	if(isfuteruse)
	{
		...
		for(int i=1; i<N; i++)
		{
			LADate	StartDate	=	LAMathDateUtilities::getLADate(chgcol(fgrid,CURVEINPUT_STARTDATE,	i));
			LADate EndDate		=	LAMathDateUtilities::getLADate(chgcol(fgrid,CURVEINPUT_ENDDATE,		i));
	...
	{
		yp->LAObject::add(CALIBRATION_DATA_MARKETDATA + suffix, new LADataMultiReference()).convertFromString(ref);
	}
	//generate Object
	dataInstance->getReferencePool().completeDependency();
	...
	if (!isSTD)
	{
		yp->LAObject::remove(CALIBRATION_DATA_MARKETDATA + suffix);
	}
	if(isCheckDF)
	{
		// check df
...
void 
LAMathCurveGenerateFuncUtility::setUpXccyBasisCurve(LADataInstance* dataInstance,const LAString& BCurveID, 
						LAStringMatrix ylddata, LAStringMatrix bgrid, LAStringMatrix bdata)
{
	...
	LAString term;
    LADate spotdate;
	//LADate spotdate		= LAMathDateUtilities::getLADate(chgrow(bdata,CURVEINPUT_SPOTDATE,1));		
	...
	yp->getDayCount().convertFromString(AC_365I);
	LADataMultiReference &refMarket = yp->getMarketData();
	if (!refMarket.isNull())
	{
		LAString n_ref = "";
...
void 
LAMathCurveGenerateFuncUtility::setUpBasisCurve(LADataInstance* dataInstance,
...
void 
LAMathCurveGenerateFuncUtility::setUpBasisCurveFromMarket(LADataInstance* dataInstance,const LAString& CurveID, 
                             ...
                             LAStringMatrix yldData, LAString& warning )
{
    ...
    yp->setInterpolation(interpolation);
	yp->getDayCount().convertFromString(AC_365I);
	yp->LAObject::add(IR_CALIBRATION_DATA_ISRENOTIONALADJUST, new LADataBool(isRenAdj));
	yp->LAObject::add(IR_CALIBRATION_DATA_ISXCCYMARKEDTOMARKET, new LADataBool(isRenAdj)); // alias for isRenotionalAdjust
    ...
    if( Currency != CURRENCY_USD )
    {
		...
		yp->LAObject::add(CALIBRATION_DATA_MARKETDATA + LAString("_") + XCCYBASIS, new LADataMultiReference()).convertFromString(ref_XCCYBasis);
    }
	else
	{
		LAObject* be	=	NULL;
	...
	yp->LAObject::add(CALIBRATION_DATA_MARKETDATA + LAString("_") + THREESIXBASIS, new LADataMultiReference()).convertFromString(ref_36Basis);
	...
	yp->LAObject::add(IR_CALIBRATION_DATA_MAINBASISDF, new LADataString("DF"));
	yp->setBasisRates();
...
void 
LAMathCurveGenerateFuncUtility::setUpForecastCurveFromMarket(LADataInstance* dataInstance,
...
void 
LAMathCurveGenerateFuncUtility::setUpForecastCurve(LADataInstance* dataInstance,
...
void 
LAMathCurveGenerateFuncUtility::setUpCdtDFCurve(LADataInstance* dataInstance, LAString curveID, LAStringMatrix data)
{
...
LAString 
LAMathCurveGenerateFuncUtility::getCurveGenerateTime(LADataInstance* dataInstance, const LAString& curveid, const LAString& curvename)
{
...
void	
LAMathCurveGenerateFuncUtility::setUpCurve2(LADataInstance* dataInstance, const LAString& curveid, const LADate& asofdate, 
...
void 
LAMathCurveGenerateFuncUtility::setUpCurve(LADataInstance* dataInstance, const LAString& curveid, const LADate& asofdate, 
										 const DoubleMatrix& mat, const LAString& curveName)
{
	unsigned int N=mat.size();
	if (N < 2) 
...
/////////////furuytos20120813///////////
void 
LAMathCurveGenerateFuncUtility::setUpCurve3(LADataInstance* dataInstance, const LAString& curveid, const LADate& asofdate, 
...
DoubleArray 
LAMathCurveGenerateFuncUtility::outPutCurveFromYieldData(LADataInstance* dataInstance, const LAString& curveid, const LAString& curveName)
{
	LAObjectPool& objPool	= dataInstance->getObjectPool();
...
DoubleArray 
LAMathCurveGenerateFuncUtility::outPutCurveFromYieldData2(LADataInstance* dataInstance, const LAString& curveID, const LAString& curveID2,
...
*/
DiscountFactorTable 
LAMathCurveGenerateFuncUtility::getCurveDiscountFactorTable(LADataInstance* dataInstance, const LAString& curveid, const LAString& curveName)
{
	LAObjectPool& objPool	= dataInstance->getObjectPool();
    ...
    for( size_t i = 0; i<termsVector.size() ; ++i )
    {
		...
		LAString ACT_365_ISDA = LAString("ACT/365_ISDA");
...
*/
void LAMathCurveGenerateFuncUtility::setCurveDiscountFactorTable(LADataInstance* dataInstance, const LAString& curveid, const std::vector<LAString>& curveNames, const DiscountFactorTable& discountFactorTable )
{
...
*/
ForwardRateTable LAMathCurveGenerateFuncUtility::getCurveForwardRateTable( LADataInstance* dataInstance, const LAString& curveid, const LAString& curveName )
{
    LAObjectPool& objPool = dataInstance->getObjectPool();
...
*/
void LAMathCurveGenerateFuncUtility::setCurveForwardRateTable( LADataInstance* dataInstance, const LAString& curveid, const std::vector<LAString>& curveNames, const ForwardRateTable& forwardRateTable )
{
        // 1. Get the terms and discount factors from the discountFactorTable
    // ---------------------------------------
...
void 
LAMathCurveGenerateFuncUtility::restoreSwapRateFromL(LAMathYieldCurvePro &curve, const LAString &ccy, const std::map<LAString, double> &sRateMap, LAStringMatrix sdata)
{
	//LADate spotDate	= LAMathDateUtilities::getLADate(chgrow(sdata,CURVEINPUT_SPOTDATE,1));
	std::map<LAString, LAString>& dvar = LACoreComponentManager::getDayCountMap();
...
void 
LAMathCurveGenerateFuncUtility::resetMarketDataUseL(LAMathYieldCurvePro &curve, const LAString &ccy, LAStringMatrix ldata)
{
	//LADate spotDate	= LAMathDateUtilities::getLADate(chgrow(ldata,CURVEINPUT_SPOTDATE,1));
	std::map<LAString, LAString>& dvar = LACoreComponentManager::getDayCountMap();
