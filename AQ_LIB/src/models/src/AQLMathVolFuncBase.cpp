...
////X///////////////////X///////////////////////////////X///////////////////
//  NAME        :       AQLMathVolFuncBase.cpp
//
//  SYNOPSIS    :       AQLMathVolFuncBase
...
*/
AQLMathVolFuncBase::AQLMathVolFuncBase(bool isMultiVariables)
...
/*!
	...
	@param[in] isMultiVariables volatility function depends sde variables(L,FX,...) or not(only t depend)
*/
AQLMathVolFuncBase::AQLMathVolFuncBase(const AQLString& sdeAttrName, unsigned int i, unsigned int j, bool isMultiVariables)
...
*/
AQLMathVolFuncBase::AQLMathVolFuncBase(const AQLMathVolFuncBase& v) 
...
/*!
	@brief destructor
*/
AQLMathVolFuncBase::~AQLMathVolFuncBase() 
{
	delete mpVolatility;
}
/*!
...
*/
AQLCoreFunctionBase*	
AQLMathVolFuncBase::clone() const
{
    try 
	{
		return new AQLMathVolFuncBase(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
AQLMathVolFuncBase::isTypeOf(function_t id) const
{
...
*/
function_t
AQLMathVolFuncBase::getType() const
{
...
*/
double
AQLMathVolFuncBase::operator()(const DoubleArray& x) const
{
...
*/
AQLString
AQLMathVolFuncBase::convertToString(void) const
{
	AQLString ret;
	ret += mSDEAttrName;
	ret += ":";
...
*/
void
AQLMathVolFuncBase::convertFromString(const AQLString& str)
{
	AQLDataStrings tmp;
...
*/
void
AQLMathVolFuncBase::setUp(AQLMathPathEntity& path)
{
	AQLString name = VOL;
	name += "_";
	name += mSDEAttrName;
	...
	{	
		...
		AQLMathVolFuncFX *pFxVol  = dynamic_cast<AQLMathVolFuncFX *>(mpVolatility);
		...
		const AQLMathYieldCurve &dYield = dynamic_cast<const AQLMathYieldCurve &>(initialValues.get(pos_dIR).get());
		const AQLMathYieldCurve &fYield = dynamic_cast<const AQLMathYieldCurve &>(initialValues.get(pos_fIR).get());
		const AQLMathFXEntity &fxEntity = dynamic_cast<const AQLMathFXEntity &>(initialValues.get(pos_FX).get());
		...
		{
			dynamic_cast<AQLMathVolFuncFXDD *>(pFxVol)->setFwdFX(fwdFX);
		}
		else if (mpVolatility->isTypeOf(FN_VOLFUNCSZDD))
		{
			dynamic_cast<AQLMathVolFuncSZDD *>(pFxVol)->setFwdFX(fwdFX);
		}
	}
}
...
*/
double
AQLMathVolFuncBase::integral(const std::vector<std::pair<double,double> >& x) const
{
	if (mIsMultiVariables) 
...
*/
double
AQLMathVolFuncBase::integral(double t1, double t2, AQL1DIntegral* pIntegral) const
{
