/*! @file AQLMathLeastSquareCorrelationLMMQlib.cpp
...
AQLMathLeastSquareCorrelationLMMQlib::AQLMathLeastSquareCorrelationLMMQlib(AQLMathCorrelationLMMDiscAngle& cor_)
:
...
mCloned(false)
{
    if( mn != mCorTarget[0].size() ) throw AQLCoreInvalidData("Target Cor is not correct! : AQLMathLeastSquareCorrelationLMMQlib::targetAndValue", __FILE__, __LINE__);
}

AQLMathLeastSquareCorrelationLMMQlib::AQLMathLeastSquareCorrelationLMMQlib( const AQLMathLeastSquareCorrelationLMMQlib& rhs )
:
mCorTarget(rhs.mCorTarget),
mn(rhs.mn),
mCor(rhs.mCor != 0 ? dynamic_cast<AQLMathCorrelationLMMDiscAngle*>(rhs.mCor->clone()) : 0),
...
AQLMathLeastSquareCorrelationLMMQlib::~AQLMathLeastSquareCorrelationLMMQlib()
{
    if(mCloned)
    {
        delete mCor;
...
double AQLMathLeastSquareCorrelationLMMQlib::value(const QuantLib::Array& x) const
{
    getfct2fit(x);
...
QuantLib::Array AQLMathLeastSquareCorrelationLMMQlib::values(const QuantLib::Array& x) const
{
...
void AQLMathLeastSquareCorrelationLMMQlib::getfct2fit(const QuantLib::Array& x) const
{
...
void AQLMathLeastSquareCorrelationLMMQlib::gradient(QuantLib::Array& grad_f, const QuantLib::Array& x) const
{
    //
...
AQLMathLeastSquareCorrelationLMMQlib& AQLMathLeastSquareCorrelationLMMQlib::operator =(const AQLMathLeastSquareCorrelationLMMQlib& rhs)
{
    if ( this != &rhs )
    {
        delete mCor;
