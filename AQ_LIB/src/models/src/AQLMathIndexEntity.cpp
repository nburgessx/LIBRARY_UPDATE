...
*/
AQLMathIndexEntity::AQLMathIndexEntity(AQLDataInstance* dataInstance) : 
...
/*!
	...
	@param[in] index AQLMathIndexEntity object
*/
AQLMathIndexEntity::AQLMathIndexEntity(
	const AQLMathIndexEntity& index) : 
	AQLObject(index),
...
*/
object_t	
AQLMathIndexEntity::getType(void) const
{
	return ENTITY_INDEX;
}
/*!
...
*/
bool
AQLMathIndexEntity::isTypeOf(object_t id) const
{
...
*/
const AQLDataString&	
AQLMathIndexEntity::getName() const	
{
...
*/
const AQLDataString&	
AQLMathIndexEntity::getIndexType() const	
{
...
*/
AQLDataString&	
AQLMathIndexEntity::getIndexType()
{
...
*/
const AQLDataString&	
AQLMathIndexEntity::getAccessory() const	
{
...
*/
AQLDataString&	
AQLMathIndexEntity::getAccessory()
{
...
*/
const AQLDataString&	
AQLMathIndexEntity::getCurrency() const	
{
...
*/
AQLDataString&	
AQLMathIndexEntity::getCurrency()
{
...
AQLMathIndexEntity::getDayCount() const
{
...
AQLMathIndexEntity::getDayCount()
{
...
*/
const AQLDataString&
AQLMathIndexEntity::getFrequency() const
{
...
*/
AQLDataString&
AQLMathIndexEntity::getFrequency()
{
...
AQLMathIndexEntity::getCalendar() const
{
...
AQLMathIndexEntity::getCalendar()
{
...
AQLMathIndexEntity::getSlidingRule() const
{
...
AQLMathIndexEntity::getSlidingRule()
{
...
AQLMathIndexEntity::getPathEntity() const	
{
...
*/
AQLDataReference&	
AQLMathIndexEntity::getPathEntity()
{
...
*/
const AQLDataReference&	
AQLMathIndexEntity::getFXEntity() const	
{
...
*/
AQLDataReference&	
AQLMathIndexEntity::getFXEntity()
{
...
*/
const AQLDataString&	
AQLMathIndexEntity::getBasis() const	
{
...
*/
const AQLDataString&	
AQLMathIndexEntity::getDiscountCurve() const	
{
...
*/
AQLDataString&	
AQLMathIndexEntity::getDiscountCurve()
{
...
AQLMathIndexEntity::getBasisInterpolation() const	
{
...
AQLMathIndexEntity::getBasisInterpolation()
{
...
*/
const AQLDataBool&	
AQLMathIndexEntity::getIsFWDInterpolation() const	
{
...
*/
AQLDataBool&	
AQLMathIndexEntity::getIsFWDInterpolation()
{
...
*/
const AQLPriceDataInterpolation&	
AQLMathIndexEntity::getFWDInterpolation() const	
{
...
*/
AQLPriceDataInterpolation&	
AQLMathIndexEntity::getFWDInterpolation()
{
...
*/
const AQLDataString&	
AQLMathIndexEntity::getVolUnderlying() const	
{
...
*/
AQLDataString&	
AQLMathIndexEntity::getVolUnderlying()
{
...
*/
void
AQLMathIndexEntity::setNextIndex()
{
...
*/
int
AQLMathIndexEntity::getCacheSize() const
{
...
*/
void
AQLMathIndexEntity::setCacheSize(unsigned int size)
{
...
*/
void
AQLMathIndexEntity::addGrid(const AQLDate& date)
{
	double term = 0.0;
	if (getPathEntity().get().get().isTypeOf(ENTITY_PATH))
	{
		const AQLMathPathEntity& path = dynamic_cast<const AQLMathPathEntity&>(getPathEntity().get().get());
		term = path.getDayCount().getTerm(path.getAsOfDate().get(), date);
	}
	else if (getPathEntity().get().get().isTypeOf(ENTITY_PLAINVANILLA))
	{
		const AQLMathPlainVanillaEntity& path = dynamic_cast<const AQLMathPlainVanillaEntity&>(getPathEntity().get().get());
		term = path.getDayCount().getTerm(path.getAsOfDate().get(), date);
	}
	else 
...
*/
void AQLMathIndexEntity::addConvexityGrids(const AQLDate& fixingdate, const AQLDate& paymentdate, const AQLDate& enddate)
{
...
*/
void
AQLMathIndexEntity::clearCache(void)
{
...
*/
void
AQLMathIndexEntity::clearGrid(void)
{
	mDateGrid.clear();
	mTimeGrid.clear();
	mFixingGrid .clear();
...
*/
void
AQLMathIndexEntity::setUpforMC(void)
{
	...
	{		
		//path
		mpPath = &dynamic_cast<AQLMathPathEntity&>(getPathEntity().get().get());
	...
	if (mSDEType == IR)
	{
			...
			if (mSpreadMat.empty())
			{
				for (unsigned int i = 0; i < mFwdsGrid.size(); i++)
				{
					const AQLRatesPathElementCurve& curve = 
						dynamic_cast<const AQLRatesPathElementCurve&>(mpPath->getPath(mSDEPos, mFwdsGrid[i]));
		...
		else
		{
			if (mSpreadMat.empty())
			{
				...
				{
					const AQLRatesPathElementCurve& curve = 
						dynamic_cast<const AQLRatesPathElementCurve&>(mpPath->getPath(mSDEPos, mTimeGrid[i]));
...
*/
void
AQLMathIndexEntity::calcIndexforPlainVanilla()
{
	if (mSDEType == IR)
	{
		const AQLMathYieldCurve& curve = dynamic_cast<const AQLMathYieldCurve &>(mpVanilla->getIRCurves().get(mSDEPos).get());
			...
			for (unsigned int i = 0; i < mTimeGrid.size(); i++)
			{
				if (mGridMat[i].size() <= 2)
				{
					...
					if ((i >= offset) && isDelayedConvexityAdjusted())
					{
						...
						const AQLMathVolFuncIRSABR* method = dynamic_cast<const AQLMathVolFuncIRSABR*>(mpVanilla->getIRVolFunc(getCurrency().get()));
...
*/
AQLObject* 
AQLMathIndexEntity::clone() const
{
    try 
	{
    	AQLMathIndexEntity* pIndexEntiy = new AQLMathIndexEntity(*this);
    	return pIndexEntiy;
    }
    catch (bad_alloc & e)
	{
...
*/
void                
AQLMathIndexEntity::update(const unsigned int type) 
{
	...
	const AQLObject& e)
{
	...
	{
		AQLString err = "Assignement error for AQLMathIndexEntity : from ";
...
//	mCacheSize = dynamic_cast<const AQLMathIndexEntity&>(e).mCacheSize; 
	mCache = dynamic_cast<const AQLMathIndexEntity&>(e).mCache; 
	mAntiCache = dynamic_cast<const AQLMathIndexEntity&>(e).mAntiCache; 
	mCachePos =  dynamic_cast<const AQLMathIndexEntity&>(e).mCachePos;
	mAntiCachePos =  dynamic_cast<const AQLMathIndexEntity&>(e).mAntiCachePos;
	mPos =  dynamic_cast<const AQLMathIndexEntity&>(e).mPos;
	mDateGrid = dynamic_cast<const AQLMathIndexEntity&>(e).mDateGrid; 
	mTimeGrid = dynamic_cast<const AQLMathIndexEntity&>(e).mTimeGrid; 
	mIndexArray = dynamic_cast<const AQLMathIndexEntity&>(e).mIndexArray; 
	mpIndexArray = &mIndexArray;
	mIsAntithetic = dynamic_cast<const AQLMathIndexEntity&>(e).mIsAntithetic; 
	mSDEPos = dynamic_cast<const AQLMathIndexEntity&>(e).mSDEPos;
	mpPath = dynamic_cast<const AQLMathIndexEntity&>(e).mpPath;	
	mpFX = dynamic_cast<const AQLMathIndexEntity&>(e).mpFX;		
	mIndexVersion = dynamic_cast<const AQLMathIndexEntity&>(e).mIndexVersion;	
	mSDEType = dynamic_cast<const AQLMathIndexEntity&>(e).mSDEType; 
	mGridMat = dynamic_cast<const AQLMathIndexEntity&>(e).mGridMat; 
	mDFMat = dynamic_cast<const AQLMathIndexEntity&>(e).mDFMat; 
	mTermMat = dynamic_cast<const AQLMathIndexEntity&>(e).mTermMat; 
	mFloatTermMat = dynamic_cast<const AQLMathIndexEntity&>(e).mFloatTermMat; 
	mForwardTimes = dynamic_cast<const AQLMathIndexEntity&>(e).mForwardTimes; 
	mFromCurrency = dynamic_cast<const AQLMathIndexEntity&>(e).mFromCurrency; 
	mToCurrency = dynamic_cast<const AQLMathIndexEntity&>(e).mToCurrency; 
	mDateGrid_old = dynamic_cast<const AQLMathIndexEntity&>(e).mDateGrid_old;

	mIsSameFwds = dynamic_cast<const AQLMathIndexEntity&>(e).mIsSameFwds;
	mpVanilla = dynamic_cast<const AQLMathIndexEntity&>(e).mpVanilla;	
	mSpotLag = dynamic_cast<const AQLMathIndexEntity&>(e).mSpotLag;
	mDatesForGenerate = dynamic_cast<const AQLMathIndexEntity&>(e).mDatesForGenerate;
...
*/
AQLDataHolder&
AQLMathIndexEntity::add(const AQLString& name)
{
	AQLDataInstance* dataInstance = getDataInstance();
	AQLPriceDataManager& dm = dataInstance->getDataMaster();
...
*/
void
AQLMathIndexEntity::setUpforPlainVanilla(void)
{

	//path
	mpVanilla = &dynamic_cast<AQLMathPlainVanillaEntity&>(getPathEntity().get().get());
...
*/
void
AQLMathIndexEntity::setUpforIR(void)
{
	...
	for (i = 0; i < sde_attrnames.size(); i++)
	{
		const AQLMathAttrSDE& attrsde = 
			dynamic_cast<const AQLMathAttrSDE&>(mpPath->getData(sde_attrnames[i], ISNOTNULL).get());
	...
	if(iscoterm)
	{
		...
		if (mpBasis->isDefined() && !mpBasis->isNull())
		{
			...
			for (i = 0; i < sde_attrnames_.size(); i++)
			{
				...
				if (attrsde.getSDEPathType() == mSDEType && attrsde.getCurrency() == getCurrency().get())
				{
					const AQLMathYieldCurve &curve = dynamic_cast<AQLMathYieldCurve &>(mpPath->getInitialValues().get(i).get());
					calcSpreadMat(curve, mGridMat);
					break;
				}		
			}
	...
	if (mpIsFWDInter->isDefined() && !mpIsFWDInter->isNull() && dynamic_cast<const AQLDataBool &>(mpIsFWDInter->get()).get())
	{
		...
		if (indexType == LIBOR)
		{
			...
			AQLMathYieldCurve& curve =  dynamic_cast<AQLMathYieldCurve &>(mpPath->getInitialValues().get(mSDEPos).get());
...
*/
void
AQLMathIndexEntity::calcSpreadMat(const AQLMathYieldCurve &curve, const DoubleMatrix &gridMat)
{	
	// get basis df
...
*/
void
AQLMathIndexEntity::setUpforIRforPlainVanilla(void)
{
	...
	if(iscoterm)
	{
		...
		//if (mpBasis->isDefined() && !mpBasis->isNull())
		//{
		//	AQLMathYieldCurve& curve = dynamic_cast<AQLMathYieldCurve &>(mpVanilla->getIRCurves().get(mSDEPos).get());
		//	calcSpreadMat(curve, mGridMat);
...
*/
void
AQLMathIndexEntity::setUpforFX(void)
{
	mpFX = &dynamic_cast<AQLMathFXEntity&>(getFXEntity().get().get());
...
*/
void
AQLMathIndexEntity::setUpforFXforPlainVanilla(void)
{
	mpFX = &dynamic_cast<AQLMathFXEntity&>(getFXEntity().get().get());
...
// set spot lag
void 
AQLMathIndexEntity::setSpotLag(const AQLString &ccy, unsigned int lag)
{
...
void 
AQLMathIndexEntity::setDatesForIndexGenerates(const std::map<AQLDate, DateVector>& datesmap)
{
...
bool
AQLMathIndexEntity::isDelayedConvexityAdjusted() const
{
	const AQLString& indextype = getIndexType();

	return (indextype == AQLString(LIBOR)) && AQLMathIndexEntity::isDelayedConvexityAdjustModel(mConvexityAdjustModel);
}

bool
AQLMathIndexEntity::isDelayedConvexityAdjustModel(const AQLString& caModel)
{
...
double
AQLMathIndexEntity::getLIBORConvexityAdjust(double forward, double fixingterm, double timingterm, double dfEnd, double dfPayment, unsigned int curpos)
{
//because vol model is not set
	...
	const AQLMathVolFuncIRSABR* method = dynamic_cast<const AQLMathVolFuncIRSABR*>(mpVanilla->getIRVolFunc(currency));
	if (!method->isTypeOf(FN_VOLFUNCIRSABR))
	{
		//throw AQLCoreInvalidData("Convexity Adjust needs only SABR model now", __FILE__, __LINE__);
		return 0.0;
	...
	double vol = AQLMathSABR_Hagan(alpha, beta, nu, rho).getSABRVol(fixingterm, AQLMath::max(F, MIN_RATE), AQLMath::max(F, MIN_RATE));
...
// get 
double 
AQLMathIndexEntity::getConvexityAdjust(double forward, double optionterm, unsigned int curpos)
{
//because vol model is not set
#ifndef VISUAL_STUDIO_2010_ANALYTICS
	double ret = 0.0;
	...
	else if (mConvexityAdjustModel.findString(CMS_CA_SABR) >= 0)
	{
		...
		if (mConvexityAdjustModel == CMS_CA_SABR || mConvexityAdjustModel == CMS_CA_SABR_HAGAN)
		{
			...
			ret = AQLMathIRVanillaFuncUtility::getSABRConvexityAdjust(	AQLMath::max(forward, MIN_RATE),
																	optionterm,
																	tau,
																	mCouponTerm[curpos],
																	...
																	mConvexityAdjustIC);
			// calculate SABR vol
			AQLMathSABR_Hagan sabr(sabr_params[0], sabr_params[1], sabr_params[2], sabr_params[3]);
		...
		else if (mConvexityAdjustModel == CMS_CA_SABR_ANTONOV)
		{
			...
			ret = AQLMathIRVanillaFuncUtility::getSABRConvexityAdjust2(AQLMath::max(forward, MIN_RATE),
																	optionterm,
																	tau,
																	mCouponTerm[curpos],
																	mTermMat[curpos].size(),
			...
			// calculate SABR vol
			AQLMathSABR_Antonov sabr(sabr_params[0], sabr_params[1], sabr_params[2], sabr_params[3]);
			mConvexityAdjustVol[curpos] = sabr.getSABRVol(optionterm, AQLMath::max(forward, MIN_RATE), AQLMath::max(forward, MIN_RATE));
...
//
//	@return AQLMathYieldCurvePro
//*/
//const AQLMathYieldCurvePro& 
//AQLMathIndexEntity::getYieldCurvePro(void) const 
//{
//	if (mpVanilla)
//	{
//		return dynamic_cast<const AQLMathYieldCurvePro &>(mpVanilla->getIRCurvePros().get(mSDEPos).get());
//	}
//	else if (mpPath)
//	{
//		return dynamic_cast<const AQLMathYieldCurvePro &>(mpPath->getIRCurvePros().get(mSDEPos).get());
//	}
//	else
//	{
