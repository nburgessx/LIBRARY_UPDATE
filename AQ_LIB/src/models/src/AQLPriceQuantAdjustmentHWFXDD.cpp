...
//================ AQLPriceQuantAdjustmentHWFXDD ===================================
/*!
	@brief default constructor
*/
AQLPriceQuantAdjustmentHWFXDD::AQLPriceQuantAdjustmentHWFXDD(const AQLString& sdeAttrNameIR_F, const AQLString& sdeAttrNameFX, const bool isAdjustInPath, const double fxCriteria, const double quantoCriteria )
: AQLPriceQuantAdjustmentFuncBase(), mSDEAttrNameIR_F(sdeAttrNameIR_F), mSDEAttrNameFX(sdeAttrNameFX), mIsAdjustInPath(isAdjustInPath), mFXCriteria(fxCriteria), mQuantoCriteria(quantoCriteria),
...
*/
AQLPriceQuantAdjustmentHWFXDD::AQLPriceQuantAdjustmentHWFXDD(const AQLPriceQuantAdjustmentHWFXDD& v) 
: AQLPriceQuantAdjustmentFuncBase(v), 
...
/*!
	@brief destructor
*/
AQLPriceQuantAdjustmentHWFXDD::~AQLPriceQuantAdjustmentHWFXDD() 
{
...
*/
AQLCoreFunctionBase*	
AQLPriceQuantAdjustmentHWFXDD::clone() const
{
    try 
	{
		return new AQLPriceQuantAdjustmentHWFXDD(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
AQLPriceQuantAdjustmentHWFXDD::isTypeOf(function_t id) const
{
	return (id==FN_QUANTADJUSTMENTFXDD ? true : AQLPriceQuantAdjustmentFuncBase::isTypeOf(id));
}
...
*/
function_t
AQLPriceQuantAdjustmentHWFXDD::getType() const
{
...
*/
void
AQLPriceQuantAdjustmentHWFXDD::setUp(AQLMathPathEntity& path)
{
	//foreign numeraire
	...
	{
		throw AQLCoreInvalidData("Foreign ir drift class must be AQLPriceDriftHWQuantAdjustment", __FILE__, __LINE__);
	}
	mpDriftIR = dynamic_cast<AQLPriceDriftHWQuantAdjustment *>(driftVec[0])->getDrift();
	//drift ir setup
...
*/
void
AQLPriceQuantAdjustmentHWFXDD::setUp() const
{
...
*/
double
AQLPriceQuantAdjustmentHWFXDD::getQuantAdjust(const double ts) const
{
	unsigned int pos;
...
*/
void
AQLPriceQuantAdjustmentHWFXDD::calcQuantAdjust(const double ts, const double te, const double r) const
{
	if (!mIsSetUped) setUp();
	...
	//set fxnumerarie
	AQLRatesPathElementHW1FCurveTMDPT pathCurve(te);
	pathCurve.AQLRatesPathElementSRCurve::set(r);
...
*/
double
AQLPriceQuantAdjustmentHWFXDD::getFXQuantAdjustNoFX(const double ts, const double te) const
{
	if (!mIsSetUped) setUp();
...
*/
double
AQLPriceQuantAdjustmentHWFXDD::getFXQuantAdjustFXMulti(const double ts, const double te) const
{
	if (!mIsSetUped) setUp();
...
*/
void
AQLPriceQuantAdjustmentHWFXDD::searchTimeGridPos(const double t, unsigned int &pos)  const
{
	const DoubleArray& timegrid = mpSDEFX->getBM()->getTimeGrid();
	if (t == 0.0) pos = 0;
