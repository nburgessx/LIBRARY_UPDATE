...
*/
AQLRatesBM_BB::AQLRatesBM_BB(const DoubleArray& timegrid, const AQLRandBase& rand, unsigned int factor_num)
:AQLRatesBM(timegrid, rand, factor_num), mNumberOfSteps(timegrid.size() - 1), mNumFactor(factor_num), mLeftIndex(mNumberOfSteps), mRightIndex(mNumberOfSteps), 
...
*/
AQLRatesBM_BB::AQLRatesBM_BB(const DoubleArray& timegrid, const AQLRandBase& rand, const std::vector<DoubleMatrix>& loading)
: AQLRatesBM(timegrid, rand, loading), mNumberOfSteps(timegrid.size() - 1), mNumFactor(1), mLeftIndex(mNumberOfSteps), mRightIndex(mNumberOfSteps), mBridgeIndex(mNumberOfSteps),
...
/*!
	...
	@param[in] loading factor loading(i * j * time)
	@param[in] deleteflag delete flag of input AQLRatesBM_BB	
*/
AQLRatesBM_BB::AQLRatesBM_BB(std::vector<std::pair<AQLRatesBM*, unsigned int> >& bm,
                     const std::vector<DoubleMatrix>& loading,
                     bool deleteflag
                     )
: AQLRatesBM(bm, loading, deleteflag), mNumberOfSteps(bm[0].first->getTimeGrid().size() - 1),  mNumFactor(1), mLeftIndex(mNumberOfSteps), mRightIndex(mNumberOfSteps),
mBridgeIndex(mNumberOfSteps), mLeftWeight(mNumberOfSteps), mRightWeight(mNumberOfSteps), mStdDiv(mNumberOfSteps),mSqrtdt(mNumberOfSteps)
{
...
*/
AQLRatesBM_BB::AQLRatesBM_BB(const AQLRatesBM_BB& rhs)
: AQLRatesBM(rhs), mNumberOfSteps(rhs.mNumberOfSteps), mNumFactor(rhs.mNumFactor), mLeftIndex(rhs.mLeftIndex), mRightIndex(rhs.mRightIndex),
...
/*!
	...
	@return Adress to AQLRatesBM_BB
*/
AQLRatesBM*
AQLRatesBM_BB::clone() const
{
    try
    {
		return new AQLRatesBM_BB(*this);
    }
    catch (bad_alloc &e)
	{
...
*/
void
AQLRatesBM_BB::calcBM(bool isIdling)
{
	if (isIdling)
	{
		if (mType == GENERATOR)
...
*/
void
AQLRatesBM_BB::buildPath(DoubleArray &path, const vector<double> &gaussians)
{
