/*!
    @file LAMathVolatilityLMMDiscModel.cpp
...
//---------------------------------------------------------
//
LAMathVolatilityLMMDiscModel::LAMathVolatilityLMMDiscModel(const DoubleVector& paramV_,
...
:
LAMathVolatilityLMMDisc( T_fix, num_small_step_ ),
...
LAMathVolatilityLMMDiscModel::LAMathVolatilityLMMDiscModel( const LAMathVolatilityLMMDiscModel& rhs )
:
LAMathVolatilityLMMDisc(rhs),
...
//
double LAMathVolatilityLMMDiscModel::get(double t, size_t i)
{
    ...
    if( t_idx < mDividedT_fix.size() ) t_idx = LAModelUtilities::eq(mDividedT_fix[t_idx+1], t, 7. / 367.) ? t_idx + 1 : t_idx;

    return mCacheVolMatrix[t_idx][i];
};

//
void LAMathVolatilityLMMDiscModel::createCacheVolMatrix(size_t i)//, const DoubleVector& T_fix_special, const DoubleVector& paramV, const DoubleVector& paramF, double G)
{
    mVol_special[i] = 0.0 < mDividedT_fix_special[0] ? LAEigenSystems::sigma0( 0.5 * mT_fix_special[0], mT_fix_special[i], mParamV, mParamF, mG[i]) : 0.0;
    ...
    else
    {
        while ( j < i )
        {
            ...
            {
                mCacheVolMatrix[k][i] = LAEigenSystems::sigma0( 0.5 * (mDividedT_fix_special[k + 1] + mDividedT_fix_special[k]), mT_fix_special[i], mParamV, mParamF, mG[i]);
            }
            j++;
        }
    }
...
void LAMathVolatilityLMMDiscModel::setParam(const DoubleVector& paramV_,
										  const DoubleVector& paramF_,
										  const DoubleVector& G_
		)
{
...
void LAMathVolatilityLMMDiscModel::setParamV(const DoubleVector& paramV_)
{
    mParamV = paramV_;
...
void LAMathVolatilityLMMDiscModel::setParamF(const DoubleVector& paramF_)
{
    mParamF = paramF_;
...
void LAMathVolatilityLMMDiscModel::setParamG(const DoubleVector& G_)
{
	if (mpInterG.get() == 0)
	{
	    mG = G_;
