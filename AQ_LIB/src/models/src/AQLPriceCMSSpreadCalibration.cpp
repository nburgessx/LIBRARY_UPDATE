...
//================ Calibration ===================================
AQLString AQLPriceCMSSpreadCalibration::Calibrate(AQLDataInstance* dataInstance, AQLStringMatrix calibrationConfig, AQLStringMatrix legScheduler, AQLStringMatrix cmsScheduler,
                                           AQLStringMatrix inputTypes, const AQLStringVector& optionTypes, const DoubleVector& smileWeights)
{
    ...
    AQLPriceCMSSpreadUtility::CheckInitialParameters(dataInstance, ccy, initExpiryTerms, initIndexes);
    // Check index consistency
    bool calibrate = AQLPriceCMSSpreadUtility::CheckModes(modes);
    AQLPriceCMSSpreadUtility::CheckIndexes(dataInstance, ccy, calibrate, pairIDs, initIndexes);

    //// Create time grid ////
    // Get time grid of SLs
    AQLStringVector slTerms;
    AQLPriceCMSSpreadUtility::ReadSLGrid(dataInstance, ccy, slTerms);
    ...
    for (size_t pairIdx = 0; pairIdx < nPairs; pairIdx++)
    {
        ...
        theta1[pairIdx] = AQLPriceCMSSpreadUtility::InterpolateParameters(dataInstance, gridDates, pairID, AQ_THETA1_IN, ccy);
        theta2[pairIdx] = AQLPriceCMSSpreadUtility::InterpolateParameters(dataInstance, gridDates, pairID, AQ_THETA2_IN, ccy);
        rho[pairIdx] = AQLPriceCMSSpreadUtility::InterpolateParameters(dataInstance, gridDates, pairID, AQ_COPRHO_IN, ccy);
        // Calibrate
        if (mode == "Fixed")
        {
            // Do nothing
        ...
        else if (mode == "ATM" || mode == "Smile")
        {
            ...
            theta1Init = AQLPriceCMSSpreadUtility::InterpolateParameters(dataInstance, slDates, pairID, AQ_THETA1_IN, ccy);
            theta2Init = AQLPriceCMSSpreadUtility::InterpolateParameters(dataInstance, slDates, pairID, AQ_THETA2_IN, ccy);
            rhoInit = AQLPriceCMSSpreadUtility::InterpolateParameters(dataInstance, slDates, pairID, AQ_COPRHO_IN, ccy);
            ...
            if (mode == "ATM")
            {
                ...
                {
                    // Define target
                    CashFlowTiming cf = AQLMathScheduleUtility::CashFlowSchedule(valDate, slTerms[timeIdx], legScheduler, cmsScheduler);
                    double quote = AQLMathParameterObject::LookUpParameterMatrix(dataInstance, AQLPriceCMSObject::MatrixID(AQ_SL_ATM, ccy), slDates[timeIdx],
                                                                             pairID, "Linear");

                    double theta1_ = theta1Init[timeIdx];
                    double theta2_ = theta2Init[timeIdx];
                    AQLPriceCMSSpreadATMTarget target(valDate, cf, discCurveInfo, rateInfo1, rateInfo2, quote, atmIsCall,
                                                theta1_, theta2_);
                ...
                {
                    rho[pairIdx][gridIdx] = AQLMathInterpolationUtilities::interpolate(slTimes, rhoSL, gridTimes[gridIdx], interpolation);
                    targets[pairIdx][gridIdx] = AQLMathInterpolationUtilities::interpolate(slTimes, targetsSL, gridTimes[gridIdx], interpolation);
                }
            }
            else // It's Smile
            {
                ...
                {
                    AQLString quoteMatrixID = AQLPriceCMSObject::MatrixID(AQLString(AQ_ML_SMILE + pairID + "_"), ccy);
                    AQLStringMatrix quoteMatrix = AQLMathParameterObject::ParameterMatrix(dataInstance, quoteMatrixID);
                    AQLPriceCMSSpreadStrip::Strip(dataInstance, convID, ccy,  valDate, pairID, legScheduler, cmsScheduler, quoteMatrix, isCalls,
                                            discCurveInfo, cmsCurveInfo, repConfig, shift, slTerms, slDates, smileStrikes, smilePrices);
                }
                else if (smileType == "SL") // Just read SL
                {
                    AQLString quoteMatrixID = AQLPriceCMSObject::MatrixID(AQLString(AQ_SL_SMILE + pairID + "_"), ccy);
                    AQLStringMatrix quoteMatrix = AQLMathParameterObject::ParameterMatrix(dataInstance, quoteMatrixID);
                ...
                AQLMathParameterObject::SetMatrixAxis(qID, slTerms, indexes, outMatrix);

                // Calibrate on the SL times
                DoubleVector theta1SL(nSLTimes, 1.0), theta2SL(nSLTimes, 1.0);
                for (size_t timeIdx = 0; timeIdx < nSLTimes; timeIdx++)
                {
                    allQuotes[0] = AQLMathParameterObject::LookUpParameterMatrix(dataInstance, AQLPriceCMSObject::MatrixID(AQ_SL_ATM, ccy), slDates[timeIdx],
                                                                             pairID, "Linear");
                    for (size_t k = 0; k < nStrikes; k++)
                        allQuotes[k + 1] = smilePrices[k][timeIdx];

                    CashFlowTiming cf = AQLMathScheduleUtility::CashFlowSchedule(valDate, slTerms[timeIdx], legScheduler, cmsScheduler);
                    AQLPriceCMSSpreadSmileTarget target(valDate, cf, discCurveInfo, rateInfo1, rateInfo2,
                ...
                AQLMathParameterObject::SetMatrixData(dataMatrix, outMatrix);
                AQLMathParameterObject::SetParameterMatrix(dataInstance, AQLPriceCMSObject::MatrixID(qID, ccy), convID, outMatrix);

                // Interpolate results on SL times to total grid
                for (size_t gridIdx = 0; gridIdx < nGrid; gridIdx++)
                {
                    theta1[pairIdx][gridIdx] = AQLMathInterpolationUtilities::interpolate(slTimes, theta1SL, gridTimes[gridIdx], interpolation);
                    theta2[pairIdx][gridIdx] = AQLMathInterpolationUtilities::interpolate(slTimes, theta2SL, gridTimes[gridIdx], interpolation);
                    rho[pairIdx][gridIdx] = AQLMathInterpolationUtilities::interpolate(slTimes, rhoSL, gridTimes[gridIdx], interpolation);
                }
            }
        }
    ...
    AQLMathParameterObject::SetParameterMatrix(dataInstance, AQLPriceCMSObject::MatrixID(theta1Out[0][0], ccy), convID, theta1Out);
    AQLMathParameterObject::SetParameterMatrix(dataInstance, AQLPriceCMSObject::MatrixID(theta2Out[0][0], ccy), convID, theta2Out);
    AQLMathParameterObject::SetParameterMatrix(dataInstance, AQLPriceCMSObject::MatrixID(rhoOut[0][0], ccy), convID, rhoOut);
    AQLMathParameterObject::SetParameterMatrix(dataInstance, AQLPriceCMSObject::MatrixID(targetsOut[0][0], ccy), convID, targetsOut);

    //// Output result ////
    return "Success";
}

void AQLPriceCMSSpreadCalibration::ParseSLSmile(const AQLStringMatrix& quoteMatrix, const AQLStringVector& refTerms,
                                          ...
                                          DoubleVector& smileStrikes, DoubleMatrix& smilePrices)
{
    ...
    AQLPriceCMSSpreadUtility::CheckStringVectors(refTerms, expiryTerms);
...
//================ Calibration Targets ===================================
AQLPriceCMSSpreadATMTarget::AQLPriceCMSSpreadATMTarget(AQLDate valDate, CashFlowTiming cf, CurveInfo discCurveInfo,
...
double AQLPriceCMSSpreadATMTarget::operator()(double x) const
{
    // Set parameter
    ...
    params[2] = x;
    // Calculate PV
    AQLPriceCopulaCMSSpread calculator(mConfidence);
...
AQLPriceCMSSpreadSmileTarget::AQLPriceCMSSpreadSmileTarget(AQLDate valDate, CashFlowTiming cf, CurveInfo discCurveInfo,
                                               const SwapRateInfo& swapRate1, const SwapRateInfo& swapRate2,
...
void AQLPriceCMSSpreadSmileTarget::operator()(DoubleVector& f, const DoubleVector& x)
{
    f.resize(mArgSize);
    if (x.size() != mArgSize)
...
void AQLPriceCMSSpreadSmileTarget::RetrieveCache(DoubleVector& v)
{
    v[0] = mExpiry;
    v[1] = mS1;
    v[2] = mVol1;
...
bool AQLPriceCMSSpreadSmileTarget::constraintsAreViolated(const DoubleVector& x)
{
