...
//
DoubleMatrix AQLEigenSystems::factorloading2corre( const DoubleMatrix& factor_loading )
{
...
//
DoubleMatrix AQLEigenSystems::corre2factorloading( const DoubleMatrix& corr, size_t no_factors )
{
    ...
    return AQLEigenSystems::normalizeMat(factor_loading);
}

DoubleMatrix AQLEigenSystems::normalizeMat( DoubleMatrix& Mat )
{
	size_t n = Mat.size();
	size_t m = Mat[0].size();
...
void AQLEigenSystems::eigensystem(const DoubleMatrix&	orig_mat,
						  DoubleVector&			e_vals,
						  DoubleMatrix&			e_vecs
		)
{
    ...
    for (size_t i = 0; i < mat_rank; i++)
    {
        ...
        {
            if ( !AQLModelUtilities::eq(orig_mat[i][j],orig_mat[j][i]) )
            {
...
//
void AQLEigenSystems::tred2(DoubleMatrix &a, DoubleVector &d, DoubleVector &e)
{
    size_t l, k, j, i;
    ...
    for (i = n - 1; i > 0; i--)
    {
        ...
        if (l > 0)
        {
            for (k = 0; k < l + 1; k++)
                scale += fabs(a[i][k]);
//          if (scale == 0.)
            if (AQLModelUtilities::eq(scale,0.))
            {
                e[i] = a[i][l];
            }
            else
            {
                ...
                f = a[i][l];
                g = ( (f>0. || AQLModelUtilities::eq(f,0.)) ? -sqrt(h) : sqrt(h) );
                e[i] = scale * g;
                h -= f * g;
                a[i][l] = f - g;
                f = 0.;
...
//
void AQLEigenSystems::tqli(DoubleVector &d, DoubleVector &e, DoubleMatrix &z)
{
    int m, l, iter, i, k;
...
//              if (fabs(e[m]) + dd == dd)
                if ( AQLModelUtilities::eq(fabs(e[m]) + dd, dd) )
                {
                    break;
                }
            }
            if (m != l)
            {
                ...
                //form shift
                g = (d[l+1] - d[l]) / (2. * e[l]);
                r = AQLModelUtilities::pythag(g,1.);
                g = d[m] - d[l] + e[l] / (g + AQLModelUtilities::sign(r,g));// this is d_m - k_s
                s = c = 1.;
                p = 0.;
                for (i = m - 1; i >= l; i--)
                {
                    ...
                    f = s * e[i];
                    b = c * e[i];
                    e[i+1] = (r = AQLModelUtilities::pythag(f,g));
//                  if (r == 0.)
                    if ( AQLModelUtilities::eq(r,0.) )
                    {
...
//              if (r == 0. && i >= l)
                if ( AQLModelUtilities::eq(r,0.) && i >= l)
                {
                    continue;
                }
                d[l] -= p;
...
//
void AQLEigenSystems::eigsrt(DoubleVector &d, DoubleMatrix &v)
{
    size_t i,j,k;
    double p;
    size_t n = d.size();
    ...
    for (i = 0; i < n - 1; i++)
    {
        ...
        for (j = i; j < n; j++)
        {
            if (d[j]>p || AQLModelUtilities::eq(d[j],p) )
            {
                p = d[k = j];
            }
        }
...
double AQLEigenSystems::volV( double tT, const DoubleVector& paramV )
{
    if (tT < 0 ) return 0;
...
//
double AQLEigenSystems::volF( double t, const DoubleVector& paramF )
{
...
//
double AQLEigenSystems::sigma0(double t,
					   double T,
...
{
    if ( T < t ) return 0.0;
    double ret = AQLEigenSystems::volV(T - t, paramV) * AQLEigenSystems::volF(t, paramF) * G;
