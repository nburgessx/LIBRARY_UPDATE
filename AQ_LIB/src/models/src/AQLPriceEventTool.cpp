...
/*!
    @brief Constructor
*/
AQLPriceEventTool::AQLPriceEventTool()
...
*/
void
AQLPriceEventTool::setUp(const AQLDate& basedate, const AQLObject& trade,
						AQLObject& trigger,
						...
						bool isLSMC)
{
		...
		for (unsigned int i = 0; i < indexinfos.getSize(); i++)
		{
			...
			if (indextype == FIXED_RATE)
				for (unsigned int j = 0; j < mIndex.size(); j++)	
					mIndex[j][i] = new AQLPriceIndexToolFixed();
			else if (indextype == CPN)
				for (unsigned int j = 0; j < mIndex.size(); j++)		
					mIndex[j][i] = new AQLPriceIndexToolCpn();
			else if (indextype == CPNCF)
				for (unsigned int j = 0; j < mIndex.size(); j++)			
					mIndex[j][i] = new AQLPriceIndexToolCpnCF();
			else
			{
				for (unsigned int j = 0; j < mIndex.size(); j++)
				...
				//		mIndex[j][i] = new AQLPriceIndexToolFixed();
				//	else
						mIndex[j][i] = new AQLPriceIndexTool();		
			}
			for (unsigned int j = 0; j < mIndex.size(); j++)
	...
	if (dh->isDefined() && !dh->isNull())
	{
		...
		for (unsigned int i = 0; i < names.size(); i++)
		{
			...
			if (name.findString(CPN) == 0)// Coupon
			{
				...
				for (unsigned int j = 0; j < mIndex.size(); j++)
				{
					mIndex[j][oldsize + i] = new AQLPriceIndexToolCpn();
			...
			else if (name.findString(CPNCF) == 0)//CouponCF
			{
				...
				for (unsigned int j = 0; j < mIndex.size(); j++)
				{
					mIndex[j][oldsize + i] = new AQLPriceIndexToolCpnCF();	
					mIndex[j][oldsize + i]->setUp(basedate, trade, tmp, expirydates[startpos + j], payoff);		
				}
			}
			else//index
			{
				...
				if (indextype == FIXED_RATE)
					for (unsigned int j = 0; j < mIndex.size(); j++)	
						mIndex[j][oldsize + i] = new AQLPriceIndexToolFixed();
				else if (indextype == CPN)
					for (unsigned int j = 0; j < mIndex.size(); j++)		
						mIndex[j][oldsize + i] = new AQLPriceIndexToolCpn();
				else if (indextype == CPNCF)
				...
				else
				{
					...
					//		mIndex[j][oldsize + i] = new AQLPriceIndexToolFixed();
					//	else
							mIndex[j][oldsize + i] = new AQLPriceIndexTool();		
				}
				for (unsigned int j = 0; j < mIndex.size(); j++)
	...
	if (!isCall) // case trigger info
	{
		...
		if (dh->isDefined() && !dh->isNull()
			&& dynamic_cast<const AQLDataBool&>(dh->get()).get())
				mActionMaster.push_back(new AQLPriceEventTerminate());
		else
		{
			//coupon change
			dh = &(trigger.getData(PRICING_DATA_COUPONCHANGEINFO, NOCHECK));
			if (dh->isDefined() && !dh->isNull())
				mActionMaster.push_back(new AQLPriceEventCpnChange());
			else
			{
			...
			if (dh->isDefined() && !dh->isNull())
				mActionMaster.push_back(new AQLPriceEventNotionalChange());
		}
	...
	const AQLRatesNumeraireBase* pNumeraire = NULL;
	BoolVector	isRcvPay;
	if (isCall)
	{
		const AQLMathPathEntity& path = dynamic_cast<const AQLMathPathEntity&>(ref.get().get());
		...
		for (unsigned int i = 0; i < names.size(); i++)
		{
			...
			const AQLMathAttrSDE& attr = dynamic_cast<const AQLMathAttrSDE&>(dh->get());
	...
	{
		mAction[i] = new AQLPriceEventHolder();
		mAction[i]->mID = id;
