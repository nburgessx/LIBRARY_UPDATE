...
#include "AQLMathCapFloorVolUtility.h"
...
void
AQLMathCapFloorVolUtility::setUpCapFloorVolSurf( AQLDataInstance* dataInstance, const AQLString& capFloorID,
		...
		const AQLString& curveID, AQLString foreCurveName, AQLString dfCurveName)
{
    ...
    AQLMathCapFloorConvention conv(freq, spotLag, daycount, paySlr, payCal, fixCal);
    ...
    if( isSABR )
    {
        ...
        vector<AQLMathSABR_Hagan> sabrParam(expiryMonth.size());
        ...
        AQLMathCapFloorSABR sabr(expiryMonth, sabrParam, pInterpo, conv, curveID_, foreCurveName, dfCurveName);
...
void
AQLMathCapFloorVolUtility::outPutCapFloor( AQLDataInstance* dataInstance, const AQLString& capFloorID, AQLString capFloorFlag, 
...
double 
AQLMathCapFloorVolUtility::lookUpCapFloorVol
( AQLDataInstance* dataInstance, const AQLString& capFloorID, double strike, const AQLDate& fixingDate )
{
    ...
    AQLMathCapFloorSABR sabr(expiryMonth, sabrParam, pInter, conv, curveID);
...
//double 
//AQLMathCapFloorVolUtility::lookUpCapFloorPrem( AQLDataInstance* dataInstance, const AQLString& capFloorID, const AQLDate& firstFixDate, 
...
//    AQLMathCapFloorConvention conv(curveID, freq, spotLag, daycount, paySlr, payCal, fixCal);
//
//    const IntVector& expiryMonth = 
//        dynamic_cast<const AQLDataInts& >(object.getData("EXPIRYMONTH",ISDEFINED).get()).get();
//
...
//
//    AQLMathCapFloorSABR sabr(expiryMonth, sabrPARAM, &inter, conv);
//
//    int y,m,d;
//    int capTerm_m;
...
//                                get().getData(CALIBRATION_DATA_ASOFDATE,ISNOTNULL).get()).get();
//    //AQLMathYieldCurve
//	AQLMathYieldCurve& yc = AQLMathCurveFuncUtility::getYieldCurveForCurveID(dataInstance,conv.curveID);
...
IntVector 
AQLMathCapFloorVolUtility::changeDateFromStringToMonth(const AQLStringVector& expiryDate)
{
    int y,m,d,w;
    IntVector expiryMonth;
...
AQLStringVector
AQLMathCapFloorVolUtility::changeDateFromMonthToString(const IntVector& expiryMonth)
{
