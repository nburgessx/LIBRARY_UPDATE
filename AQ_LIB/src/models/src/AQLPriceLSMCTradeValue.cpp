...
*/
AQLPriceLSMCTradeValue::AQLPriceLSMCTradeValue(AQLPolyFitBase* polyfit, AQLPriceAccruedInterest* pacc) :
AQLPriceTradeValue(pacc), mpPolyFit(polyfit)
{
}
...
*/
AQLPriceLSMCTradeValue::AQLPriceLSMCTradeValue(const AQLPriceLSMCTradeValue& v)
: AQLPriceTradeValue(v), mpPolyFit(0)
{
	if (v.mpPolyFit != 0)
		mpPolyFit = dynamic_cast<AQLPolyFitBase*>(v.mpPolyFit->clone());
}
...
*/
AQLPriceLSMCTradeValue::~AQLPriceLSMCTradeValue()
{
	delete 	mpPolyFit;
}
/*!
...
*/
bool
AQLPriceLSMCTradeValue::isTypeOf(function_t id) const
{
	return (id == FN_IR_LSMCTRADEVALUE ? true : AQLPriceTradeValue::isTypeOf(id));
}
/*!
...
*/
AQLCoreFunctionBase*
AQLPriceLSMCTradeValue::clone() const
{
    try 
	{
    	return new AQLPriceLSMCTradeValue(*this);	
    }
    catch (bad_alloc & e)
	{
...
*/
function_t			
AQLPriceLSMCTradeValue::getType() const
{
...
*/
void
AQLPriceLSMCTradeValue::registerData(AQLPriceDataManager& dm) const
{
...
*/
double
AQLPriceLSMCTradeValue::value(const AQLDate& basedate, AQLObject& object,
					const AQLDataValuation& att) const
{
	...
	if (dh->isDefined() && !dh->isNull()) 
	{
		...
		return AQLPriceTradeValue::value(basedate, object, att);	
	...
	if (dh->isNull())
	{
		dynamic_cast<AQLPriceDataFunction&>(dh->get()).setFunction(new AQLPriceLSMCOperator(), "lsmcoperator");
		...
		dynamic_cast<AQLPriceLSMCOperator&>(method).mPoly = &dynamic_cast<AQLPolynomialBase&>(poly);
	...
	if (dataProvider->lsmcnum == 0 || dataProvider->actiontimes.size() == 0)
		return AQLPriceTradeValue::value(basedate, object, dataProvider, 0);	
    ...
    dynamic_cast<AQLPolyFitLS*>(mpPolyFit)->setRegParam(dataProvider->isexcludeoutlier_y,
		...
		else
		{
			...
			if (dataProvider->lsmcnum < dataProvider->mcnum)
			{
				pv = pv * double(dataProvider->lsmcnum)
					+ AQLPriceTradeValue::value(basedate, object, dataProvider, dataProvider->lsmcnum) * double(dataProvider->mcnum - dataProvider->lsmcnum);
...
*/
double
AQLPriceLSMCTradeValue::value_backward(const vector<DoubleMatrix>& time,
...
{
	const AQLPriceTradeValueDataProvider* dataProvider_ = dynamic_cast<const AQLPriceTradeValueDataProvider*>(dataProvider);
...
*/
/*AQLDataProvider*
AQLPriceLSMCTradeValue::setUpDataProvider(const AQLDate& basedate, AQLObject& object, 
								const AQLDataValuation& att) const
{
	AQLDataHolder* dh;

	AQLPriceTradeValue::setUpDataProvider(basedate, object, att);
	AQLPriceTradeValueDataProvider* dataProvider = &dynamic_cast<AQLPriceTradeValueDataProvider&>(att.getDataProvider());
...
*/
/*AQLDataProvider*
AQLPriceLSMCTradeValue::createNewDataProvider() const
{
	AQLPriceTradeValueDataProvider* dataProvider = NULL;
	try 
	{
...
*/
AQLCoreFunctionBase*
AQLPriceLSMCTradeValue::AQLPriceLSMCOperator::clone() const
{
    try 
	{
        return new AQLPriceLSMCOperator(*this);
    }
    catch (bad_alloc & e)
	{
        throw AQLCoreSystemError(e.what(), __FILE__, __LINE__);
...
*/
double
AQLPriceLSMCTradeValue::AQLPriceLSMCOperator::operator()(const DoubleArray& x) const
{
	DoubleArray xx = x;
	xx.resize(x.size() - 1);
...
*/
void
AQLPriceLSMCTradeValue::setStandardizationParam(AQLPriceTradeValueDataProvider &dataProvider, const BoolVector &isconvert_xy, const vector<DoubleArray>& standardization_y, const vector<DoubleMatrix>& standardization_x,
