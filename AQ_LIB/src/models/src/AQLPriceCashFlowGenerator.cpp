...
//This definition must be coonsistent with the definition in AQLMathIndexEntity.cpp
#ifndef SPOTLAG
#define SPOTLAG 0
#endif

AQLPriceCashFlowGenerator::CashletCreator::CashletCreator(const AQLPriceCashFlowGenerator* cf_gen_,
                                                      AQLObject* leg_)
: is_first_stub(false),
  is_last_stub(false)
{
...
void
AQLPriceCashFlowGenerator::CashletCreator::setup()
{
    AQLDataHolder* dh;
    ...
    is_arrear                  = AQLPriceCFGenUtility::isArrear(timing);
...
AQLObject*
AQLPriceCashFlowGenerator::CashletCreator::createNotionalCashlet(const size_t cf_pos,
...
AQLObject* 
AQLPriceCashFlowGenerator::CashletCreator::createCashlet(const size_t cf_pos,
                                                     const AQLString& name,
													 ...
													 const AQLString* p_basename)
{
    ...
    else
    {
        ...
        advancepaymentdate = AQLPriceCFGenUtility::getDate(startdate, 
                                                       *leg, 
    ...
    if (is_renotional)
    {
        ...
        else
        {
            ...
            advancepaymentdate = AQLPriceCFGenUtility::getDate(temp_startdate, 
                                                           *leg, 
                                                           CALIBRATION_DATA_SLIDINGRULE, 
                                                           CALIBRATION_DATA_CALENDAR);
        }
				...
				if(dh->isDefined() && !dh->isNull())
				{
					...
					for (unsigned int j = 0; j < boundaryIndexInfos.getSize(); j++)
					{
						...
						else
						{
							...
							advancepaymentdate = AQLPriceCFGenUtility::getDate(temp_startdate, *leg, 
	...
	if (dh->isDefined() && !dh->isNull())
	{
		...
		AQLPriceCFGenUtility::getBusDayRuleAndCalendar(*leg,
...
*/
AQLPriceCashFlowGenerator::AQLPriceCashFlowGenerator()
: AQLCoreProcedure()
{
}
/*!
...
*/
bool
AQLPriceCashFlowGenerator::isTypeOf(function_t id) const
{
...
*/
AQLCoreFunctionBase*		
AQLPriceCashFlowGenerator::clone() const
{
    try 
	{
        return new AQLPriceCashFlowGenerator();
    }
    catch (bad_alloc & e)
	{
...
*/
function_t			
AQLPriceCashFlowGenerator::getType() const
{
...
*/
void
AQLPriceCashFlowGenerator::registerData(AQLPriceDataManager& dm) const
{
...
*/
void	            
AQLPriceCashFlowGenerator::calibrateModel(const AQLDate& basedate, 
										AQLObject& object, 
...
*/
void
AQLPriceCashFlowGenerator::createCashlets(AQLObject& leg) const  
{
...
*/
AQLObject*
AQLPriceCashFlowGenerator::createRangeAccrueInfo(const AQLString name, const AQLObject& rangeinfo, const AQLDate& startdate, 
    ...
    if(dh->isDefined() && !dh->isNull())
    {
        ...
        for (unsigned int j = 0; j < boundaryIndexInfos.getSize(); j++)
        {
            ...
            else
            {
                ...
                advancepaymentdate = AQLPriceCFGenUtility::getDate(temp_startdate, *prangeinfo, 
                    CALIBRATION_DATA_SLIDINGRULE, CALIBRATION_DATA_CALENDAR);
            }
...
*/
void
AQLPriceCashFlowGenerator::calcPaymentDates(AQLObject& leg, 
...
*/
void
AQLPriceCashFlowGenerator::calcNotionalArray(const AQLObject& leg,
										...
										DoubleArray& notional_array) const
{
	...
	if (amortize_type == EQUALIZATION)
	{
		...
		amount = AQLPriceCFGenUtility::round(amount, roundfunction, rounddigit);		
	}
...
*/
void
AQLPriceCashFlowGenerator::clearCashlets(AQLObject& leg) const
{
	AQLDataHolder* dh;
...
AQLObject*
AQLPriceCashFlowGenerator::createCouponInfo(const AQLString name,
										...
										) const
{
	...
	if (dh->isDefined() && !dh->isNull())
	{
		...
		AQLPriceCFGenUtility::getBusDayRuleAndCalendar(*pcouponinfo, 
...
*/
AQLObject*
AQLPriceCashFlowGenerator::createIndexInfo(const AQLString name,
										const AQLDate& advancepaymentdate,
										const AQLDate& paymentdate,
										const AQLDate& paymentdate_unadjust, 
										const AQLDate& startdate,
										...
										) const
{
	...
	if (dh->isDefined() && !dh->isNull())
	{
		...
		const AQLPriceDataSlidingRule* psrule;
		const AQLPriceDataCalendar* pcal;
		AQLPriceCFGenUtility::getBusDayRuleAndCalendar(*pindex, 
													PRICING_DATA_OBSERVATIONSLIDINGRULE,
													PRICING_DATA_OBSERVATIONCALENDAR,
													*pindex, 
													CALIBRATION_DATA_SLIDINGRULE,
			...
			else // case of fixing special date
			{
				...
				AQLPriceCFGenUtility::getBusDayRuleAndCalendar(*pindex, 
															...
															*pindex,
															...
															CALIBRATION_DATA_CALENDAR,
															psrule, pcal);		
				fixingdates[i] = AQLPriceCFGenUtility::getDate(fixingbasedates[i], offset, specialday, *psrule, pcal);
	...
	else
	{
		...
		else // case of fixing special date
		{
			...
			fixingdate = AQLPriceCFGenUtility::getDate(fixingbasedate, offset, specialday, *psrule, pcal);
			pindex->remove(PRICING_DATA_FIXINGSLIDINGRULE);
			pindex->remove(PRICING_DATA_FIXINGCALENDAR);
		}
...
*/
DateVector
AQLPriceCashFlowGenerator::getFixedDates(const AQLObject& leg) const
{
...
*/
AQLObject*
AQLPriceCashFlowGenerator::createRangeAccrueIndexInfo(const AQLString name,
												  const AQLDate& startdate,
												  const AQLDate& enddate,
...
*/
void
AQLPriceCashFlowGenerator::setUpLegMaturity(AQLObject& trade) const
{
	AQLDataHolder* dh;
	//leg object
...
*/
void
AQLPriceCashFlowGenerator::setUpCallSchedule(AQLObject& trade) const
{
	const AQLDataHolder* dh;
	AQLDataHolder* ah_;
	//call info 
	...
	if (schtype == PAYMENTDATE)	// call cation dates are same as payment dates
	{
		...
		bool isfixingarrear = AQLPriceCFGenUtility::isArrear(fixingtiming);
		...
		for (unsigned int i = 0; i < cashlets.getSize(); i++)
		{
			...
			if (flag)
			{
				...
				isactionarrear = AQLPriceCFGenUtility::isArrear(actiontiming);
		...
		if (isactionarrear)
		{
			...
			if (!isfixingarrear)
			{
				...
				fixingbasedates.insert(fixingbasedates.begin(), 
									AQLPriceCFGenUtility::getDate(startdate_trade, leg.get(), 
									CALIBRATION_DATA_SLIDINGRULE, CALIBRATION_DATA_CALENDAR)); 
			}
		}
	...
	else if (schtype == DETAIL) // case of detail input for call schedule
	{
		...
		////////////////////////////////////////////////
		AQLPriceCFGenUtility::getBusDayRuleAndCalendar(callinfo.get(), 
													PRICING_DATA_OBSERVATIONSLIDINGRULE,
													PRICING_DATA_OBSERVATIONCALENDAR,
													leg.get(),
													CALIBRATION_DATA_SLIDINGRULE,
		...
		const AQLString& actiontiming = dynamic_cast<const AQLDataString&>(dh->get()).get(); 
		bool isactionarrear = AQLPriceCFGenUtility::isArrear(actiontiming);
		if (isfixingarrear == true && isactionarrear == false)
		{
			//error
			AQLString msg = "call fixing timing is arrear but action timing is advance";
		...
		//call schedule
		AQLPriceCFGenUtility::generateSchedule(callinfo.get(), startdate, enddate,
	...
	else//special offset date
	{
		...
		for (unsigned int i = 0; i < calldates.size(); i++)
			fixingdates[i] = AQLPriceCFGenUtility::getDate(fixingbasedates[i], offset, specialday, *pSrule_fixing, pCal_fixing);	
	}
...
*/
void
AQLPriceCashFlowGenerator::setUpTriggerSchedule(AQLObject& trade) const
{
	const AQLDataHolder* dh;
	AQLDataHolder* ah_;
	...
	for (unsigned int i = 0; i < triggers.getSize(); i++)
	{
		...
		AQLPriceCFGenUtility::getBusDayRuleAndCalendar(triggers.get(i).get(), 
													PRICING_DATA_OBSERVATIONSLIDINGRULE,
													PRICING_DATA_OBSERVATIONCALENDAR,
													pleg->get(),
													CALIBRATION_DATA_SLIDINGRULE,
		...
		AQLPriceCFGenUtility::generateSchedule(triggers.get(i).get(), startdate, enddate,
		...
		else//special offset date
		{
			...
			for (unsigned int j = 0; j < triggerdates.size(); j++)
				fixingdates[j] = AQLPriceCFGenUtility::getDate(triggerdates[j], offset, specialday, *pSrule_fixing, pCal_fixing);	
		}
		...
		if (isdetail)
		{
			...
			else//special offset date
			{
				...
				for (unsigned int j = 0; j < triggerdates.size(); j++)
					tmpdates[j] = AQLPriceCFGenUtility::getDate(actiondates[j], offset, specialday, *pSrule_fixing, pCal_fixing);	
			}
...
*/
void
AQLPriceCashFlowGenerator::setUpTriggerCpnChange(AQLObject& trade, AQLObject& trigger) const
{
	AQLDataHolder* dh;
	...
	for (unsigned int i = 0; i < legs.getSize(); i++)
	{
		...
		for (unsigned int j = 0; j < paymentdates.size(); j++)
		{
			...
			else
			{
				...
				advancepaymentdate = AQLPriceCFGenUtility::getDate(startdate, legs.get(i).get(), 
									CALIBRATION_DATA_SLIDINGRULE, CALIBRATION_DATA_CALENDAR);
			}
...
*/
bool
AQLPriceCashFlowGenerator::isSameStartDates(const AQLDataMultiReference& legs) const
{
...
*/
bool
AQLPriceCashFlowGenerator::isSameEndDates(const AQLDataMultiReference& legs) const
{
...
*/
void
AQLPriceCashFlowGenerator::setUpFundingChange(AQLObject& trade) const
{
...
*/
void
AQLPriceCashFlowGenerator::createFundingSpread(DoubleMatrix &spreadmat, DoubleVector &fixedrates, AQLObject& trade, AQLObject& fundingleg, AQLObject& strleg, AQLObject& fginfo) const
{
	// declare variables
	...
	AQLMathYieldCurve* pbasecurve = 0;
	AQLMathYieldCurve* pmdycurve = 0;
	if (pathref.get().get().isTypeOf(ENTITY_PATH))
	{
		AQLMathPathEntity* ppath = &(dynamic_cast<AQLMathPathEntity&>(pathref.get().get()));
		...
		{
			const AQLMathAttrSDE &sde = dynamic_cast<const AQLMathAttrSDE &>(ppath->getData(sdenames[i]).get());
			...
			if (sde.getSDEPathType() == IR)
			{
				...
				if (ccy == basecur)
					pbasecurve = dynamic_cast<AQLMathYieldCurve *>(&initialrefs.get(i).get());
				else if (ccy == mdycur)
					pmdycurve = dynamic_cast<AQLMathYieldCurve *>(&initialrefs.get(i).get());
			...
			if (sde.getSDEPathType() == FX)
			{
				...
				pfx = dynamic_cast<AQLMathFXEntity *>(&initialrefs.get(i).get());
				break;
			}
		}
	}
	...
	{
		AQLMathPlainVanillaEntity& path = dynamic_cast<AQLMathPlainVanillaEntity&>(pathref.get().get());
		asof = path.getAsOfDate();
		pbasecurve = &path.getIRCurve(basecur);
		pmdycurve = &path.getIRCurve(mdycur);
		pfx = dynamic_cast<AQLMathFXEntity *>(&path.getFXEntity().get().get());
	}
	...
	for (unsigned int i = 0; i < vecSize; i++)
	{
		...
		// Start date: AQLPricePayOff::SetUpEntityOfIndex(AQLDate&, MBentity&, AQLObject&, DateVector*, bool)
		// End date: AQLMathIndexEntity::setUpForIR(void)
		// And variable SPOTLAG must be consistent with definition in AQLMathIndexEntity.cpp
...
*/
void
AQLPriceCashFlowGenerator::createFundingChangeAttribute(AQLObject& trade, AQLObject& fundingleg, AQLObject& strleg, AQLObject& fginfo) const
{
	// get path object
...
*/
void
AQLPriceCashFlowGenerator::setUpRenotional(AQLObject& trade) const
{
...
void
AQLPriceCashFlowGenerator::createRenotional( AQLObject& trade, AQLObject& leg ) const
{
	// ! Get PathEntity
	...
	{
		pathEntity = &( dynamic_cast< AQLMathPathEntity& >( arPath.get().get() ) );
		
	}
	else if ( arPath.get().get().isTypeOf( ENTITY_PLAINVANILLA ) )
	{
		pathEntity = &( dynamic_cast< AQLMathPlainVanillaEntity& >( arPath.get().get() ) );
	}
	else
	{
		AQLString msg = "pathEntity type is wrong! AQLPriceCashFlowGenerator::createRenotional";
		throw AQLCoreInvalidData( msg.getCString(), __FILE__, __LINE__ );
	}
	...
	{
		AQLMathYieldCurve& yc_base = dynamic_cast< AQLMathPlainVanillaEntity *>(pathEntity)->getIRCurve(baseCcy);
		//xccybasis adjust
	...
	AQLMathFXEntity* fxEntity = dynamic_cast< AQLMathFXEntity* >( &( ar->get().get() ) );
...
*/
void
AQLPriceCashFlowGenerator::createOddTradeEntity(AQLObject& trade) const
{
...
*/
void
AQLPriceCashFlowGenerator::setUpEquivalentStrike(AQLObject& trade) const
{
	AQLDataHolder* dh;
...
void
AQLPriceCashFlowGenerator::createRenotionalInfo(const AQLDate& advancepaymentdate,
										const AQLDate& paymentdate,
										const AQLDate& paymentdate_unadjust, 
										const AQLDate& startdate,
										const AQLDate& enddate,
...
bool
AQLPriceCashFlowGenerator::isConsistentFrequencyPair(const AQLString& rollfreq, const AQLString& payfreq) const
{
	bool ret = false;
...
void
AQLPriceCashFlowGenerator::modifyLegInfo(AQLObject& leg) const
{
    AQLDataHolder* dh;
...
void
AQLPriceCashFlowGenerator::detectStubCoupon(bool& is_first_stub,
                                        bool& is_last_stub,
	                                    const AQLString& freq,
										const AQLDate& start_date,
										const AQLDate& end_date,
...
void
AQLPriceCashFlowGenerator::setUpBondFundingLeg(AQLObject& trade) const
{
	AQLDataHolder* dh;
	...
	const AQLMathFXEntity* pFwd = nullptr;
	if (pathEntity->isTypeOf(ENTITY_PATH))
	{
		AQLMathPathEntity& path = *(dynamic_cast<AQLMathPathEntity *>(pathEntity));
		...
		for (unsigned int i = 0; i < initSize; i++)
		{
			...
			{
				pFwd = dynamic_cast<AQLMathFXEntity *>(&initialrefs.get(i).get());
				break;
			}
		}
	}
	...
	{
		AQLMathPlainVanillaEntity& path = *(dynamic_cast<AQLMathPlainVanillaEntity *>(pathEntity));
		pFwd = dynamic_cast<AQLMathFXEntity *>(&path.getFXEntity().get().get());
	}
...
*/
void
AQLPriceCashFlowGenerator::setUpNonDeliverable(AQLObject& trade) const
{
	...
	{
		AQLMathPlainVanillaEntity* pPath = &(dynamic_cast< AQLMathPlainVanillaEntity&>(attrPath.get().get()));
		if (pPath->getFXEntity().isNull())
		{
			return;
		}
		pFX = dynamic_cast<AQLMathFXEntity *>(&pPath->getFXEntity().get().get());
	...
	{
		AQLMathPathEntity* pPath = &dynamic_cast<AQLMathPathEntity&>(attrPath.get().get());
		...
		simccy = dynamic_cast<AQLMathAttrSDE&>(dh->get()).getCurrency();

		const AQLDataMultiReference& initialrefs = pPath->getInitialValues();
		for (unsigned int i = 0; i < sdenames.size(); ++i)
		{
			const AQLMathAttrSDE& sde = dynamic_cast<const AQLMathAttrSDE&>(pPath->getData(sdenames[i], ISNOTNULL).get());
			if (sde.getSDEPathType() == FX)
			{
				pFX = dynamic_cast<AQLMathFXEntity *>(&initialrefs.get(i).get());
			}
