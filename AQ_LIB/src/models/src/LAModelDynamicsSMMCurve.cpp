...
*/
LARatesPathElementSMMCurve::LARatesPathElementSMMCurve(const DoubleArray& tenor, const DoubleArray& delta_tenor, double t) 
...
/*!
	@brief destructor
*/
LARatesPathElementSMMCurve::~LARatesPathElementSMMCurve() 
{
	clear();
}
/*!
...
*/
LARatesPathElementSMMCurve::LARatesPathElementSMMCurve(const LARatesPathElementSMMCurve& v)
...
*/
bool
LARatesPathElementSMMCurve::isTypeOf(pathelement_t id) const
{
	return (id == PE_SMMCURVE ? true : LARatesPathElementCurve::isTypeOf(id));
}
...
*/
pathelement_t
LARatesPathElementSMMCurve::getType() const
{
	return PE_SMMCURVE;
}
...
LARatesPathElementSMMCurve::clone() const
{
    try 
	{
		return new LARatesPathElementSMMCurve(*this);
    }
    catch (bad_alloc & e)
	{
...
LARatesPathElementSMMCurve::operator = (const LARatesPathElementSMMCurve& a)
{
	...
	mpInitialData_L = dynamic_cast<const LARatesPathElementSMMCurve&>(a).mpInitialData_L;
	mpInitialCurve = dynamic_cast<const LARatesPathElementSMMCurve&>(a).mpInitialCurve;
	mpTenor = dynamic_cast<const LARatesPathElementSMMCurve&>(a).mpTenor;
	mpDeltaTenor = dynamic_cast<const LARatesPathElementSMMCurve&>(a).mpDeltaTenor;
	mpRefCount = dynamic_cast<const LARatesPathElementSMMCurve&>(a).mpRefCount;
	mData_L = dynamic_cast<const LARatesPathElementSMMCurve&>(a).mData_L;
...
*/
double
LARatesPathElementSMMCurve::getP (double T) const
{
...
*/
void
LARatesPathElementSMMCurve::set_t (double t)
{
	if (m_t == t) return;
...
*/
void
LARatesPathElementSMMCurve::set_t(void)
{
	if (m_t == 0.0)
	{
...
*/
double
LARatesPathElementSMMCurve::getL (double ts, double te, double delta, const LARatesPathElementCurve& curve) const
{
	return (curve.getP(ts) / curve.getP(te) - 1.0) / delta;
}

double
LARatesPathElementSMMCurve::getS (DoubleArray tenor, DoubleArray deltatenor, const LARatesPathElementCurve& curve) const
{
...
/*
double
LARatesPathElementSMMCurve::getS (double ts, double te, double annuity, const LARatesPathElementCurve& curve) const
{
	return (curve.getP(ts) - curve.getP(te)) / annuity;
}
*/
double
LARatesPathElementSMMCurve::getA (DoubleArray tenor, DoubleArray deltatenor, const LARatesPathElementCurve& curve) const
{
	double ret = 0.0;
...
*/	
//void
//LARatesPathElementSMMCurve::set(const SCALARARRAY& a) 
//{	
//	int i, j;
...
*/	
void
LARatesPathElementSMMCurve::set(const LARatesPathElementBase& a)
{
	if (a.isTypeOf(PE_SMMCURVE))
	{
		*this = dynamic_cast<const LARatesPathElementSMMCurve&>(a);
		return;
	}
...
*/	
void
LARatesPathElementSMMCurve::clear()
{
	if ((*mpRefCount)-- == 1)
	{
		mpInitialData_S->clear();
