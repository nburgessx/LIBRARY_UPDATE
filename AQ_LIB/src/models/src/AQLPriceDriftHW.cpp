...
*/
AQLPriceDriftHW::AQLPriceDriftHW()
: 
...
*/
AQLPriceDriftHW::AQLPriceDriftHW( const AQLString& sdeAttrName)
:
AQLMathDriftFuncBase(),
...
*/
AQLPriceDriftHW::AQLPriceDriftHW(const AQLPriceDriftHW& rhs) 
:
AQLMathDriftFuncBase(rhs),
mSDEAttrName(rhs.mSDEAttrName),
is_cloned(true),
mGL(rhs.mGL),
mpHWtool(rhs.mpHWtool !=0 ? dynamic_cast<AQLMathHWFuncToolForVar*>(rhs.mpHWtool->clone()) : 0),
mpHWtool2(rhs.mpHWtool2 !=0 ? dynamic_cast<AQLMathHWFuncToolForMR*>(rhs.mpHWtool2->clone()) : 0),
mpHWtool3(rhs.mpHWtool3 !=0 ? dynamic_cast<AQLMathHWFuncToolForMRIntegral*>(rhs.mpHWtool3->clone()) : 0),
...
{
    mpInitialCurve = rhs.mpInitialCurve != 0 ? dynamic_cast<const AQLRatesPathElementCurve*>(rhs.mpInitialCurve->clone()) : 0;
}
...
*/
AQLCoreFunctionBase*	
AQLPriceDriftHW::clone() const
{
    try 
	{
		return new AQLPriceDriftHW(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
AQLPriceDriftHW::isTypeOf(function_t id) const
{
	return (id==FN_DRIFTHW ? true : AQLMathDriftFuncBase::isTypeOf(id));
}
...
*/
AQLString
AQLPriceDriftHW::convertToString(void) const
{
	AQLString ret;
	ret += mSDEAttrName;
	ret += ":";
...
*/
void
AQLPriceDriftHW::convertFromString(const AQLString& str)
{
	AQLDataStrings tmp;
...
*/
void
AQLPriceDriftHW::setUp(AQLMathPathEntity& path)
{
	...
	delete mpHWtool;
	mpHWtool =  new AQLMathHWFuncToolForVar(*dynamic_cast<AQLMathHWFuncMR*>(pMR->clone()),
										*dynamic_cast<AQLMathHWFuncSigma*>(pSigma->clone()));
	delete mpHWtool2;
	mpHWtool2 =  new AQLMathHWFuncToolForMR(*dynamic_cast<AQLMathHWFuncMR*>(pMR->clone()),
										*dynamic_cast<AQLMathHWFuncSigma*>(pSigma->clone()));
	delete mpHWtool3;
	mpHWtool3 = new AQLMathHWFuncToolForMRIntegral(*dynamic_cast<AQLMathHWFuncMR*>(pMR->clone()),
										*dynamic_cast<AQLMathHWFuncSigma*>(pSigma->clone()));

	mFuncExpInt_a_inv.SetFunc(*this, &AQLPriceDriftHW::getExpInt_a_inv);
	mFuncExpInt_a_sigma_SQ.SetFunc(*this, &AQLPriceDriftHW::getExpInt_a_sigma_SQ);
	mFuncExpInt_a_sigma_SQ_B.SetFunc(*this, &AQLPriceDriftHW::getExpInt_a_sigma_SQ_B);
	//set Cachesize
	...
	if(pattrsde->getSDE().getTemplate()->isTypeOf(PE_HWCURVETMDPT))
	{
		...
		pcurve->mpHWtoolMR = new AQLMathHWFuncToolForMR(*dynamic_cast<AQLMathHWFuncMR*>(pMR->clone()),
													*dynamic_cast<AQLMathHWFuncSigma*>(pSigma->clone()));

		delete pcurve->mpHWtoolVar; 
		pcurve->mpHWtoolVar = new AQLMathHWFuncToolForVar(*dynamic_cast<AQLMathHWFuncMR*>(pMR->clone()),
													*dynamic_cast<AQLMathHWFuncSigma*>(pSigma->clone()));
	}
	//set InitialCurve into Drift Class
	delete mpInitialCurve;
	mpInitialCurve = dynamic_cast<AQLMathPathYieldCurve*>(pattrsde->getSDE().getInitialValue()->clone());
...
*/
double
AQLPriceDriftHW::operator()(const DoubleArray& x) const
{	
    if ( x.size() != 3 )
	{
...
*/
double
AQLPriceDriftHW::getAlpha(const double t) const
{
	if (mCacheAlpha.end() == mCacheAlpha.find(t))
	{
		double alpha = dynamic_cast<const AQLMathPathYieldCurve*>(mpInitialCurve)->getF(t);
		if (t != 0.0)
		{
...
*/
double
AQLPriceDriftHW::getFwd(const double t) const
{
	return dynamic_cast<const AQLMathPathYieldCurve*>(mpInitialCurve)->getF(t);
}
...
*/
double
AQLPriceDriftHW::getExpInt_a_inv(const double t) const
{
...
*/
double
AQLPriceDriftHW::getExpInt_a_sigma_SQ(const double t) const
{
...
*/
double
AQLPriceDriftHW::getExpInt_a_sigma_SQ_B(const double t) const
{
