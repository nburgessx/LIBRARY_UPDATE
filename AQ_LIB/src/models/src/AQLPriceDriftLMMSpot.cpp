...
////X///////////////////X///////////////////////////////X///////////////////
//  NAME        :       AQLPriceDriftLMMSpot.cpp
//
//  SYNOPSIS    :       AQLPriceDriftLMMSpot
...
//================ AQLPriceDriftLMMSpot ===================================
/*!
	@brief default constructor
	@param[in] Q val for displaced diffusion case
*/
AQLPriceDriftLMMSpot::AQLPriceDriftLMMSpot(double Q)
: AQLPriceDriftLMMBase(Q)
{
...
*/
AQLPriceDriftLMMSpot::AQLPriceDriftLMMSpot(const AQLString& sdeAttrName, unsigned int i, const DoubleArray& tenor, const DoubleArray& delta_tenor, double Q)
: AQLPriceDriftLMMBase(sdeAttrName, i, tenor, delta_tenor, Q)
{
...
*/
AQLPriceDriftLMMSpot::AQLPriceDriftLMMSpot(const AQLPriceDriftLMMSpot& v) 
: AQLPriceDriftLMMBase(v)
{
...
/*!
	@brief destructor
*/
AQLPriceDriftLMMSpot::~AQLPriceDriftLMMSpot() 
{
...
*/
AQLCoreFunctionBase*	
AQLPriceDriftLMMSpot::clone() const	
{
    try 
	{
		return new AQLPriceDriftLMMSpot(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
bool
AQLPriceDriftLMMSpot::isTypeOf(function_t id) const
{
	return (id==FN_DRIFTLMMSPOT ? true : AQLMathDriftFuncBase::isTypeOf(id));
}
...
*/
function_t
AQLPriceDriftLMMSpot::getType() const
{
...
*/
double
AQLPriceDriftLMMSpot::operator()(const DoubleArray& x) const
{
...
*/
double
AQLPriceDriftLMMSpot::integral(const std::vector<std::pair<double,double> >& x) const
{
//	unsigned int pos;
//	AQLAlgorithm::locate<DoubleArray, double>(mTenor, x[0].second, mTenor.size(), pos);
//	if (pos < mTenor.size() && mTenor[pos] == x[0].second) pos++;
	...
	unsigned int pos;
	AQLAlgorithm::locate<DoubleArray, double>(mTenor, x[0].second - INFINITESIMAL, mTenor.size(), pos);
...
*/
void
AQLPriceDriftLMMSpot::setUp(AQLMathPathEntity& path)
{
