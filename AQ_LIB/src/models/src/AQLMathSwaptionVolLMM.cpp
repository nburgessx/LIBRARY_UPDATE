...
/*!
	@brief constructor
*/
AQLMathSwaptionVolLMM::AQLMathSwaptionVolLMM()
:
mCurve0_F(0),
mCurve0_D(0),
mCloned(false)
...
*/
AQLMathSwaptionVolLMM::AQLMathSwaptionVolLMM( AQLRatesPathElementCurve& curve0_F_,
...
mCloned(false)
{
    ...
    if( mItr_OptMat_L < n - 1 ) mItr_OptMat_L = AQLModelUtilities::eq(mT_OptMat, mT_fix_L[mItr_OptMat_L + 1], eps_1W) ? mItr_OptMat_L + 1 : mItr_OptMat_L;
	
    mItr_Expire_L = --upper_bound(mT_pay_L.begin(), mT_pay_L.end(), mT_Expire) - mT_pay_L.begin();
    if( mItr_Expire_L < n - 1 ) mItr_Expire_L = AQLModelUtilities::eq(mT_Expire, mT_pay_L[mItr_Expire_L + 1], eps_1W) ? mItr_Expire_L + 1 : mItr_Expire_L;
    ...
    if( itr_OptMat < m - 1 ) itr_OptMat = AQLModelUtilities::eq(mT_OptMat, mT_pay[itr_OptMat + 1], eps_1W) ? itr_OptMat + 1 : itr_OptMat;

	size_t itr_Expire = --upper_bound(mT_pay.begin(), mT_pay.end(), mT_Expire) - mT_pay.begin();
    if( itr_Expire < m - 1 ) itr_Expire = AQLModelUtilities::eq(mT_Expire, mT_pay[itr_Expire + 1], eps_1W) ? itr_Expire + 1 : itr_Expire;
	...
	if (dynamic_cast<const AQLMathPathYieldCurve&>(*mCurve0_F) == dynamic_cast<const AQLMathPathYieldCurve&>(*mCurve0_D) && mTau_L.size() == mTau.size())
	{
...
*/
AQLMathSwaptionVolLMM::AQLMathSwaptionVolLMM(const AQLMathSwaptionVolLMM& rhs)
:
mCurve0_F( rhs.mCurve0_F != 0 ? dynamic_cast<AQLRatesPathElementCurve*>(rhs.mCurve0_F->clone()) : 0 ),
...
*/
AQLMathSwaptionVolLMM& AQLMathSwaptionVolLMM::operator=(const AQLMathSwaptionVolLMM& rhs)
{
    if( this != &rhs)
    {
        delete mCurve0_F; 
...
/*!
	...
	@param[in] T_pay
*/
double AQLMathSwaptionVolLMM::getSwaptionVol()
{
...
/*!
	...
	@param[in] tau
*/
double AQLMathSwaptionVolLMM::getForwardLIBOR( const AQLRatesPathElementCurve* curve, double T_fix, double T_pay, double tau ) const
{
    if( T_pay < T_fix )
    {
    ...
    if ( AQLModelUtilities::eq(tau, 0.0, eps_1d) && AQLModelUtilities::eq(T_fix, T_pay, eps_1d) )
    {
        AQLString msg("eq(term_rate, 0.0, eps_1d) && eq(T_fix, T_pay, eps_1d)");
	    throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);   
    }
    if (( AQLModelUtilities::eq(tau, 0.0, eps_1d) && !AQLModelUtilities::eq(T_fix, T_pay, eps_1d) ) ||
		   	( !AQLModelUtilities::eq(tau, 0.0, eps_1d) && AQLModelUtilities::eq(T_fix, T_pay, eps_1d) ))
    {
...
/*!
	...
	@param[in] T_pay
*/
double AQLMathSwaptionVolLMM::getForwardRateValue( const AQLRatesPathElementCurve* curve, double T_fix, double T_pay ) const
{
    if ( T_fix < 0.0 )
    {
        AQLString msg("T_fix < 0.0");
...
/*!
    ...
    @param[in] fix_rate
*/
double AQLMathSwaptionVolLMM::getFRA( double T_fix_L, double T_pay_L, double tau_L, double fix_rate ) const
{
...
/*!
	...
	@param[in] tau
*/
double AQLMathSwaptionVolLMM::getAnnuity( const DoubleVector& T_pay, const DoubleVector& tau ) const
{
...
/*!
	...
	@param[in] T_pay
*/
double AQLMathSwaptionVolLMM::getSwapFloat( const DoubleVector& T_fix_L, const DoubleVector& T_pay_L ) const
{
...
/*!
    ...
    @param[in] swap_rate
*/
double AQLMathSwaptionVolLMM::getSwapValue( const DoubleVector& T_fix_L, const DoubleVector& T_pay_L, const DoubleVector& T_pay, const DoubleVector& tau, double swap_rate ) const
{
...
/*!
    ...
    @param[in] tau
*/
double AQLMathSwaptionVolLMM::getSwapRate( const DoubleVector& T_fix_L, const DoubleVector& T_pay_L, const DoubleVector& T_pay, const DoubleVector& tau ) const
{
...
/*!
	...
	@param[in] volatility
*/
double AQLMathSwaptionVolLMM::getSwaptionBlackPrem( double vol, double forwardShift ) const
{
...
/*!
	...
	@param[in] volatility
*/
double AQLMathSwaptionVolLMM::getSwaptionNormalPrem( double vol ) const
{
...
/*!
	@brief constructor
*/
AQLMathSwaptionVolLMMDiscModel::AQLMathSwaptionVolLMMDiscModel()
:
...
/*!
    ...
    @param[in] tau_
*/
AQLMathSwaptionVolLMMDiscModel::AQLMathSwaptionVolLMMDiscModel( AQLRatesPathElementCurve& curve0_F_,
														AQLRatesPathElementCurve& curve0_D_,
...
:
AQLMathSwaptionVolLMM(curve0_F_, curve0_D_, T_fix_L_, T_pay_L_, tau_L_, T_pay_, tau_, T_OptMat_, T_Expire_, volType_),
mCorrLMM(&corr_LMM_),
mCorrCached(false),
mVolLMM(&vol_LMM_),
mVolCached(false),
...
/*!
	@brief copy constructor
*/
AQLMathSwaptionVolLMMDiscModel::AQLMathSwaptionVolLMMDiscModel(const AQLMathSwaptionVolLMMDiscModel& rhs)
:
AQLMathSwaptionVolLMM(rhs),
mItrS(rhs.mItrS),
mItrE(rhs.mItrE),
...
AQLMathSwaptionVolLMMDiscModel& AQLMathSwaptionVolLMMDiscModel::operator=(const AQLMathSwaptionVolLMMDiscModel& rhs)
{
	if( this != &rhs )
	{
        (*this).AQLMathSwaptionVolLMM::operator = (rhs);
...
*/
AQLMathSwaptionVolLMMDiscModel* AQLMathSwaptionVolLMMDiscModel::clone() const
{
    return new AQLMathSwaptionVolLMMDiscModel(*this);
}
...
*/
double AQLMathSwaptionVolLMMDiscModel::getVar()
{
	if( mVolType != Black )
	{
		AQLString msg = "Only black vol is supported! : AQLMathSwaptionVolLMMDiscModel_DD::getVar";
		throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);
	}
...
double AQLMathSwaptionVolLMMDiscModel::getSwaptionPrem()
{
	AQLString msg = "TODO Implementation! : AQLMathSwaptionVolLMMDiscModel::getSwaptionPrem";
	throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);
}
...
/*!
	...
	@param[in] G       
*/
void AQLMathSwaptionVolLMMDiscModel::setParam(const DoubleVector& paramV_,
...
/*!
	...
	@param[in] paramV  
*/
void AQLMathSwaptionVolLMMDiscModel::setParamV(const DoubleVector& paramV_)
{
...
/*!
	...
	@param[in] paramF  
*/
void AQLMathSwaptionVolLMMDiscModel::setParamF(const DoubleVector& paramF_)
{
...
/*!
	...
	@param[in] G  
*/
void AQLMathSwaptionVolLMMDiscModel::setParamG(const DoubleVector& G_)
{
    if( mVolLMM->getG() != G_)
    {
...
*/
double AQLMathSwaptionVolLMMDiscModel::getIntegrateCov( size_t i, size_t  j, double t_s, double t_e)
{
    if( !mCorrCached )
    {
        getCorrCache();
...
*/
void AQLMathSwaptionVolLMMDiscModel::getVolCache()
{
    ...
    if( !mT_VolCached )
    {
        ...
        for( size_t i = mItr_OptMat_L; i < mItr_Expire_L + 1; ++i )
        {
            ...
            if( itr_i != 0 && itr_i < mT_Vol.size() ) itr_i = AQLModelUtilities::eq(mT_fix_L[i], mT_Vol[itr_i], eps_1W) ? itr_i : --itr_i;
            mT_VolItr[i] = itr_i;
        }
        ...
        if( mItrE < mt_Vol.size() - 1 ) mItrE = AQLModelUtilities::eq(mT_OptMat, mt_Vol[mItrE + 1], eps_1W) ? mItrE + 1 : mItrE;
...
*/
void AQLMathSwaptionVolLMMDiscModel::getCorrCache()
{
    ...
    for( size_t i = mItr_OptMat_L; i < mItr_Expire_L + 1; ++i )
    {
        ...
        if( T_corr_i < T_Corr.size() - 1 ) T_corr_i = AQLModelUtilities::eq(mT_fix_L[i], T_Corr[T_corr_i + 1], eps_1W) ? T_corr_i + 1 : T_corr_i;
        ...
        for( size_t j = mItr_OptMat_L; j < mItr_Expire_L + 1; ++j )
        {
            ...
            if( T_corr_j < T_Corr.size() - 1 ) T_corr_j = AQLModelUtilities::eq(mT_fix_L[j], T_Corr[T_corr_j + 1], eps_1W) ? T_corr_j + 1 : T_corr_j;
...
// constructor
AQLMathSwaptionVolLMMDiscModel_DD::AQLMathSwaptionVolLMMDiscModel_DD()
{
}

// constructor
AQLMathSwaptionVolLMMDiscModel_DD::AQLMathSwaptionVolLMMDiscModel_DD(AQLRatesPathElementCurve& curve0_F_,
															AQLRatesPathElementCurve& curve0_D_,
														    AQLMathCorrelationLMM& corr_LMM_,
															AQLMathVolatilityLMMDiscModel& vol_LMM_,
															const DoubleVector& T_fix_L_,
...
:
AQLMathSwaptionVolLMMDiscModel(curve0_F_, curve0_D_, corr_LMM_,vol_LMM_,T_fix_L_,T_pay_L_,tau_L_,T_pay_,tau_,T_OptMat,T_Expire, volType_),
mQ(Q)
{
}

// Get Variance of SR()
double AQLMathSwaptionVolLMMDiscModel_DD::getVar()
{
	if( mVolType != Black )
	{
		AQLString msg = "Only black vol is supported! : AQLMathSwaptionVolLMMDiscModel_DD::getVar";
...
// Get premium of SR(Maturity, Expire)
double AQLMathSwaptionVolLMMDiscModel_DD::getSwaptionPrem()
{
	AQLString msg = "TODO Implementation! : AQLMathSwaptionVolLMMDiscModel_DD::getSwaptionPrem";
	throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);
}
...
// constructor
AQLMathSwaptionPremLMMDiscModel_ShiftedDD::AQLMathSwaptionPremLMMDiscModel_ShiftedDD()
{
}

// constructor
AQLMathSwaptionPremLMMDiscModel_ShiftedDD::AQLMathSwaptionPremLMMDiscModel_ShiftedDD(AQLRatesPathElementCurve& curve0_F_,
																	AQLRatesPathElementCurve& curve0_D_,
																	AQLMathCorrelationLMM& corr_LMM_,
																	AQLMathVolatilityLMMDiscModel& vol_LMM_,
																	const DoubleVector& T_fix_L_,
...
*/
double AQLMathSwaptionPremLMMDiscModel_ShiftedDD::getVar()
{
	...
	{
		AQLString msg = "Only Black and Normal are supported in volType! : AQLMathSwaptionPremLMMDiscModel_ShiftedDD::getVar()";
		throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);
	}
...
/*!
	@brief function to get Variance of Sihfted SR 
*/
double AQLMathSwaptionPremLMMDiscModel_ShiftedDD::getVar_ShiftedDD()
{
	double log2_Q = log(mQ) / log(2.);
	double inv_log2_Q_SQ = (1. - log2_Q) * (1. - log2_Q);
...
// Get premium of SR(Maturity, Expire)
double AQLMathSwaptionPremLMMDiscModel_ShiftedDD::getSwaptionPrem()
{
	double log2_Q = log(mQ) / log(2.);
	...
	{
		AQLString msg = "Shifted swap rate must be positive! : AQLMathSwaptionPremLMMDiscModel_ShiftedDD::getSwaptionPrem";
		throw AQLCoreInvalidData(msg.getCString(),__FILE__,__LINE__);
	}
}
...
*/
void AQLMathSwaptionPremLMMDiscModel_ShiftedDD::getShiftedDDWeight()
{
