...
*/
AQLLinearRatesSwapTradeValueForExo::AQLLinearRatesSwapTradeValueForExo(AQLPriceAccruedInterest* pacc) :
AQLLinearRatesSwapTradeValue(pacc)
{
}
/*!
...
*/
AQLLinearRatesSwapTradeValueForExo::AQLLinearRatesSwapTradeValueForExo(const AQLLinearRatesSwapTradeValueForExo& v)
: AQLLinearRatesSwapTradeValue(v)
{
}
/*!
    @brief destructor
...
*/
AQLLinearRatesSwapTradeValueForExo::~AQLLinearRatesSwapTradeValueForExo()
{
}
/*!
...
*/
bool
AQLLinearRatesSwapTradeValueForExo::isTypeOf(function_t id) const
{
	return (id == FN_IR_PLAINVANILLASWAPTRADEVALUEFOREXO ? true : AQLPriceTradeValue::isTypeOf(id));
}
/*!
...
*/
AQLCoreFunctionBase*
AQLLinearRatesSwapTradeValueForExo::clone() const
{
    try 
	{
  		return new AQLLinearRatesSwapTradeValueForExo(*this);
    }
    catch (bad_alloc & e)
	{
...
*/
function_t			
AQLLinearRatesSwapTradeValueForExo::getType() const
{
...
*/
double
AQLLinearRatesSwapTradeValueForExo::value(const AQLDate& basedate, AQLObject& object,
					const AQLDataValuation& att) const
{
	...
	AQLMathPathEntity &path = dynamic_cast<AQLMathPathEntity &>(refP.get().get());
	...
	AQLMathPlainVanillaEntity* pPath_v = NULL;
	AQLObjectHolder objHolder = objPool.getObject(pathName_v, ENCHKTYPE_NOCHECK);
	if (!objHolder.isDefined())
	{
		pPath_v = new AQLMathPlainVanillaEntity(dataInstance);
		objPool.set(pathName_v, pPath_v);
	
	}
	else
	{
		pPath_v = &dynamic_cast<AQLMathPlainVanillaEntity &>(objPool.getObject(pathName_v).get());
		pPath_v->reset();
	}

	AQLString curveNames = "";
	AQLMathFXEntity *pFX = NULL;
	...
	for (unsigned int i = 0; i < initialRef.getSize(); ++i)
	{
		...
		{
			fxs.push_back(ccy);
			pFX = &dynamic_cast<AQLMathFXEntity &>(initialRef.get(i).get());
	...
	{
		const AQLMathAttrSDE &sde = dynamic_cast<const AQLMathAttrSDE &>(path.getData(sdeNames_simu[i]).get());
		const AQLString &ccy = sde.getCurrency();
		if (ccy.findString('/') >= 0)
		{
			continue;
	...
	if (!ehPah.isDefined())
	{
		throw AQLCoreInvalidData("AQLMathPathEntity reference pool is not set.", __FILE__, __LINE__);
	}
	pPath_v->update();
	...
	if (pFX)
	{
		...
		if (dh.isDefined() && !dh.isNull())
		{
			...
			if (!ehFX.isDefined())
			{
				throw AQLCoreInvalidData("AQLMathFXEntity type USEMODEL reference pool is not set.", __FILE__, __LINE__);
			}
			pFX->update();
	...
	// value
	double dirtyprice1 = AQLLinearRatesSwapTradeValue::value(basedate, object, att);

	// setup object for exo format
	object.remove(PRICING_DATA_PVCURRENCY);
	const AQLLinearRatesSwapTradeValueDataProvider &dataProvider = dynamic_cast<const AQLLinearRatesSwapTradeValueDataProvider&>(att.getDataProvider());
	...
	if (pFX)
	{
		...
		if (dh.isDefined() && !dh.isNull())
		{
			...
			AQLMathFXEntity &fx = dynamic_cast<AQLMathFXEntity &>(refFX.get().get());
			AQLObjectHolder &ehFX = rp.getReference(fxName_orig);
			fx.update();
