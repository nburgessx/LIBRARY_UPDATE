...
/*!
    @brief Constructor
*/
AQLPriceEventExtraCF::AQLPriceEventExtraCF()
: AQLPriceEventBase()
...
*/
void AQLPriceEventExtraCF::destroy()
{
	...
	std::map<AQLDate, std::vector<AQLPriceIndexToolBase*> >::iterator its = mIndexMap.begin();
...
/*! 
	@brief copyIndex
	@param[in] v AQLPriceEventExtraCF object
*/
void AQLPriceEventExtraCF::copyIndex(const AQLPriceEventExtraCF& v)
{
	std::map<AQLDate, std::vector<AQLPriceIndexToolBase*> >::const_iterator it = v.mIndexMap.begin();
	while(it != v.mIndexMap.end())
	{
		std::vector<AQLPriceIndexToolBase*>::const_iterator itv = it->second.begin();
		vector<AQLPriceIndexToolBase*> c_vec;
...
/*!
	...
	@param[in] v AQLPriceEventExtraCF object
*/
AQLPriceEventExtraCF::AQLPriceEventExtraCF(const AQLPriceEventExtraCF& v)
...
*/
AQLCoreFunctionBase*
AQLPriceEventExtraCF::clone() const    
{
    try 
	{
        return new AQLPriceEventExtraCF(*this);
    }
    catch (bad_alloc & e)
	{
        throw AQLCoreSystemError(e.what(), __FILE__, __LINE__);
...
AQLPriceEventExtraCF::operator=(const AQLPriceEventExtraCF &v)
...
*/
bool
AQLPriceEventExtraCF::isTypeOf(function_t id) const
{
	return (id == FN_IR_TRIGGERACTIONEXTRACF ? true : AQLPriceEventBase::isTypeOf(id));
}
...
*/
function_t			
AQLPriceEventExtraCF::getType() const
{
...
*/	
void
AQLPriceEventExtraCF::doAction(const AQLDate& actiondate,
								   double actiontime,
...
*/
void
AQLPriceEventExtraCF::setUp(const AQLDate& basedate,	
			...
			else if (input.findString(INDEX) == 0)
			{
				...
				for (unsigned int j = 0; j < mpActionDates->size() - startpos; j++)
				{
					...
					for (unsigned int k = 0; k < static_cast<unsigned int>(infonum.getSize()); k++)
					{
						...
						if (type == FIXEDRATE)
						{
							mIndex[j][k] = new AQLPriceIndexToolFixed;
						}
						else if (type == CPN)
						{
							mIndex[j][k] = new AQLPriceIndexToolCpn;
						}
						else if (type == CPNCF)
						{
							mIndex[j][k] = new AQLPriceIndexToolCpnCF;
						}
						else
						{
							mIndex[j][k] = new AQLPriceIndexTool;
						}
...
*/	
double
AQLPriceEventExtraCF::calcExtraCF(const AQLDate& actiondate,
								   double actiontime,
								   vector<PayOffToolHolderVector>& payoff,
								   vector<PayOffToolHolderVector>& extrapayoff,
								   vector<AQLPriceEventHolder*>& futureaction,
								   ...
								   AQLPricePayOffToolHolder& ph) const
{
	...
	{		
		...
		for (unsigned int i = 0; i < mInputsInfo.size(); i++)
		{
			...
			{
				map<AQLDate, std::vector<AQLPriceIndexToolBase*> >::const_iterator its = mIndexMap.begin();
				...
				vector<AQLPriceIndexToolBase*>::const_iterator itvec = its->second.begin();
