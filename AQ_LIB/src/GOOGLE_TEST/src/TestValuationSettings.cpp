/*
 * @brief			Test Valuation Settings Class
...
namespace google_test
{
    ...
    {
        // Load Curves
        const std::string curveUSDOIS   = validation_api::tryMeLWOLoad( USDCurveOIS, etrading::JSON );
        const std::string curveUSD3M    = validation_api::tryMeLWOLoad( USDCurve3M, etrading::JSON );
    ...
    TEST_F( TestValuationSettingsUsingObjects, UNIT_CurveHandle_LabelValueBlock )
    {
        ...
        const std::string curveUSDOIS   = validation_api::tryMeLWOLoad( USDCurveOIS, etrading::JSON );
        const std::string curveUSD3M    = validation_api::tryMeLWOLoad( USDCurve3M, etrading::JSON );
		const std::string curveEUROIS   = validation_api::tryMeLWOLoad( EURCurveOIS, etrading::JSON );
        const std::string curveEUR3M    = validation_api::tryMeLWOLoad( EURCurve3M, etrading::JSON );
    ...
    TEST_F( TestValuationSettingsUsingObjects, UNIT_CurveGroup_SingleCell )
    {
		...
		const std::string curveGroupUSD	= validation_api::tryMeCurveGroupCreate( "USDCurves", USDcurves );
    ...
    TEST_F( TestValuationSettingsUsingObjects, UNIT_CurveGroup_LabelValueBlock )
    {
		...
		const std::string curveGroupUSD	= validation_api::tryMeCurveGroupCreate( "USDCurves", USDcurves );
		const std::string curveGroupEUR	= validation_api::tryMeCurveGroupCreate( "EURCurves", EURcurves );
