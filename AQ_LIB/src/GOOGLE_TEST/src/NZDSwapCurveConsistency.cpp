...
namespace google_test
{
    ...
    class NZD_SwapCurve_Consistency : public virtual testing::Test, public virtual google_test::InitializeMLibGoogleTest
    {
    public:

        // Load Curves
    const std::string curveObjectNZDOIS_ = validation_api::tryMeLWOLoad( etrading::getGoogleTestFolder() + fileNameNZDOIS, etrading::JSON );    
	const std::string curveObjectNZD3ML_ = validation_api::tryMeLWOLoad( etrading::getGoogleTestFolder() + fileNameNZD3ML, etrading::JSON );
    };
    ...
    TEST_F( NZD_SwapCurve_Consistency, CONSISTENCY_NZD3ML_Swap_Instrument_Repricing )
    {
        ...
        for ( size_t i = 0; i < swapTerms.size(); ++i )
        {
            ...
            const std::string swapName                  = "NZD3ML_Swap" + MLIB_TO_STRING_FROM_SIZE_T(i);
