...
#define TEST_DIR "ETrading/LWObjects/TestLWOSwapFloatRate/"
...
namespace google_test
{
    ...
    std::shared_ptr<etrading::Swap> getSwapFromCacheWithCurveDataPopulated(const std::string& swapName, etrading::DataProvider& dataProvider)
    {
		...
		validation_api::tryMeLWOSwapPV(swapName, etrading::fromStringToLVB("EURYC"), "", etrading::fromStringToLVB(dataProvider.getValuationSettings().getFixingTableName("", etrading::FLOAT_SCHEDULE_TYPE)));
    ...
    TEST_F( TestLWOSwapFloatRatesFixingsAndStubs, CONSISTENCY_SourceFloatRates_FromFixingTable )
    {
        try
        {
                ...
                std::string fixingTableName          = validation_api::tryMeLWOFixingTableCreate( tableName, currency, curveTenor, fixingDates, fixingValues );
    ...
    TEST_F( TestLWOSwapFloatRatesFixingsAndStubs, CONSISTENCY_SourceFloatRates_FromFirstFixingAndLastFixing_Parameters )
    {
        try
        {
                // Load the Input Files
    ...
    TEST_F( TestLWOSwapFloatRatesFixingsAndStubs, CONSISTENCY_CheckZeroFloatRates_WhenPaymentDateInThePast_NoFixingTable )
    {
        try
        {
               // Load the Input Files
    ...
    TEST_F( TestLWOSwapFloatRatesFixingsAndStubs, CONSISTENCY_SourceFloatRate_FromFixingTable_WhenPaymentDateIsAsOfDate )
    {
        try
        {
                // Load the Input Files
    ...
    TEST_F( TestLWOSwapFloatRatesFixingsAndStubs, CONSISTENCY_CheckFirstFixingParameter_AppliedToCurrentCashflow )
    {
        try
        {
                // Load the Input Files
    ...
    TEST_F( TestLWOSwapFloatRatesFixingsAndStubs, CONSISTENCY_CheckFirstFixingParameterApplied_WhenFirstFixingDateInFuture )
    {
        try
        {
                // Load the Input Files
    ...
    TEST_F( TestLWOSwapFloatRatesFixingsAndStubs, CONSISTENCY_CheckStubPriority_FirstFixingParameter_HasHigherPriorityThan_InternalStubRateMethod )
    {
        try
        {
                // Load the Input Files
    ...
    TEST_F( TestLWOSwapFloatRatesFixingsAndStubs, SNAPSHOT_CheckSwapWithStub_StubRateUsedWhenNoFirstFixing )
    {
        try
        {
                // Load the Input Files
    ...
    TEST_F( TestLWOSwapFloatRatesFixingsAndStubs, CONSISTENCY_CheckSwapWithSingleCashflow_CheckFirstFixingParameter_HasHigherPriorityThan_LastFixingParameter )
    {
        try
        {
                // Load the Input Files
