...
namespace google_test
{
    ...
    //
    ME_BUILD_EURUSD_XCCY_CURVE( TestLWOSwapZeroCouponSwapXccyFixedFloat, USDYC_OIS, USDYC_STD, EURYC_OIS, EURYC_STD, EURYC_3M6M, EURYC_XCCY );
	...
	TEST_F( TestLWOSwapZeroCouponSwapXccyFixedFloat, CONSISTENCY_IsFwdInterCheck )
    {
        try
        {
				...
				EXPECT_THROW(validation_api::tryMeLWOSwapParRate( swapName2, curveCollectionLVB, fixingTableLVB ), LACoreError );  // On Windows LACoreError inherits from std::exception; however on Linux it does not.
        }
        catch( const LACoreError& m )
        {
            std::cout <<  m.getMsg();
