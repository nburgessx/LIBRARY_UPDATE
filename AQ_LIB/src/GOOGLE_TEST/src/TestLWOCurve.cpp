...
#define SK2(NAME,C1_TYPE,C2_TYPE,C1_NAME,C2_NAME) \
	...
	// Declare Test will Set-Up the Test Fixture Class which inherits the MLIBQ tear-down / clean-up class
	DECLARE_TEST_FIXTURE(TestLWOCurve);

    TEST_F( TestLWOCurve, UNIT_LWOCurve )
    {
		// Creating an instance of InitializeMLibGoogleTest loads in the calendars.
		// When this object goes out of scope MLIBQ is shut down gracefully.
        // InitializeMLibGoogleTest instance; // Resolved using the DECLARE_TEST_FIXTURE macro above
