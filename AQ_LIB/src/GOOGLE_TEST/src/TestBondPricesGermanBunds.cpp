    ...
    TEST_F( TestBondPricesGermanBunds, SCENARIO_CheckBondPricesAndYieldsMatchLiveMarketData )
    {
        ...
        try
        {
             for ( i = 0; i < TEST_COUNT; ++i )
             {
                ...
                LAString bondInputFilename      = CreateDataFile::makeFilename( createBondInputFile, i );
                LAString priceInputFilename     = CreateDataFile::makeFilename( priceBondInputFile, i );
                ...
                auto compoundYields = validation::tryMeLWOBondCompoundYields( bondObjectName, settlementDates, actualCleanPrice );
