...
namespace google_test
{
    ...
    class BuildAllEURCurvesAsSTDSwap : public virtual testing::Test, public virtual google_test::InitializeMLibGoogleTest
    {
    public:
	...
	TEST_F( BuildAllEURCurvesAsSTDSwap, SCENARIO_Load_EUR_Curves_x3 )
	{
        ...
        validation_api::tryMeLWOLoad( etrading::getGoogleTestFolder() + fileNameEUROIS, etrading::JSON );
        validation_api::tryMeLWOLoad( etrading::getGoogleTestFolder() + fileNameEUR1ML, etrading::JSON );
		validation_api::tryMeLWOLoad( etrading::getGoogleTestFolder() + fileNameEUR3ML, etrading::JSON );
        validation_api::tryMeLWOLoad( etrading::getGoogleTestFolder() + fileNameEUR6ML, etrading::JSON );
		validation_api::tryMeLWOLoad( etrading::getGoogleTestFolder() + fileNameEUR12ML, etrading::JSON );
    ...
    TEST_F( BuildAllEURCurvesAsSTDSwap, CONSISTENCY_EUR1ML_Swap_Instrument_Repricing )
    {
        ...
        for ( size_t i = 0; i < swapTerms.size(); ++i )
        {
            ...
            const std::string swapName                  = "EUR1ML_Swap" + MLIB_TO_STRING_FROM_SIZE_T(i);
    ...
    TEST_F( BuildAllEURCurvesAsSTDSwap, CONSISTENCY_EUR12ML_Swap_Instrument_Repricing )
    {
        ...
        for ( size_t i = 0; i < swapTerms.size(); ++i )
        {
            ...
            // Use the BuildSwaptradeFromGenerator Helpers to Create the Swap from the Generator
            const std::string swapName                  = "EUR12ML_Swap" + MLIB_TO_STRING_FROM_SIZE_T(i);
