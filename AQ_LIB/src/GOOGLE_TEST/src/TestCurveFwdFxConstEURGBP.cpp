...
#define TEST_DIR "Vanilla/Curves/TestCurveFwdFxConstEURGBP/"
...
namespace
{
    ...
    extern const char EURYC_FWDGBP[]		= TEST_DIR "EURYC_FWDGBP_tryMirSetUpFwdFXConstantCurve_inputs";

    //
    // test call input and reference files
    //
    extern const char OutPutDFReference_32bit[]	= TEST_DIR "EURYC_EURDF_GBPCSA_tryMirOutputCurve_outputs_32bit";
	extern const char OutPutDFReference_64bit[]	= TEST_DIR "EURYC_EURDF_GBPCSA_tryMirOutputCurve_outputs_64bit";
}
...
namespace google_test
{
    ...
    typedef BindFileToClassConstructor<CurveFwdFxConst,	EURYC_FWDGBP>	SourceEURYC_FWDGBP;
    ...
    class TestCurveFwdFxConstEURGBP : public testing::Test, public CurveEURYC_FWDGBP
    {
        void testGraphAtCompileTime()
        {
            // make sure that conversion to test base class is unambiguous
            // static_cast<::google_test::InitializeMLibGoogleTest&>(*this);
    ...
    TEST_F( TestCurveFwdFxConstEURGBP, SNAPSHOT_OutPutDF )
    {
		...
		#if defined(GTEST32)
        const ReadDataFile::Load resultFile( OutPutDFReference_32bit );
		#else
		const ReadDataFile::Load resultFile( OutPutDFReference_64bit );
		#endif
