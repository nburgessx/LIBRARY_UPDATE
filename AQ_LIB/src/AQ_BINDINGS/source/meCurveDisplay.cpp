...
std::vector<std::vector<double> > meCurveDisplay(const std::string& curveCollection, const std::string& curveIndex)
{
    ...
    LADataHolder* dh = &(etrading::InitializeMLibETrading::instance().dataInstance()->getObjectPool().getObject(tmp_curveCollection,ENCHKTYPE_ISDEFINED).getData(IR_CALIBRATION_DATA_DFS2));
