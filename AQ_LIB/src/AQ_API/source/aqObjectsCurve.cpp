...
#include "tryAqObjectsCurveDisplay.h"
...
SWIG_STRINGMATRIX aqObjectsCurveDisplay(const std::string& curveHandle)
{
	...
	etrading::VariantMatrix results = validation::tryAqObjectsCurveDisplay(curveHandle);
...
SWIG_STRINGMATRIX aqObjectsCurveMarketDataDisplay(const std::string& marketDataObjectName, const std::string marketDataKey )
{
	...
	etrading::VariantMatrix results = validation::tryAqObjectsCurveMarketDataDisplay( marketDataObjectName, marketDataKey );
...
SWIG_STRINGMATRIX aqObjectsCurveMarketDataDisplayFromCurve(const std::string& curveObjectName, const std::string marketDataKey )
{
	...
	etrading::VariantMatrix results = validation::tryAqObjectsCurveMarketDataDisplayFromCurve( curveObjectName, marketDataKey );
								 ...
								 const std::string& foreignCurveCollection )
{
	...
	std::string curveIndexAndAliasNames = validation::tryAqObjectsCurveCalibrate( objectName, lwoCurveGeneratorName, lwoCurveMarketDataName, domesticCurveCollection, foreignCurveCollection );
										...
										const std::string& key4, const SWIG_STRINGMATRIX& value4)
{
	...
	std::string result = validation::tryAqObjectsCurveMarketDataCreate( objectName, marketDataKeys, infoBlocks );
														   ...
														   const std::vector<SWIG_STRINGMATRIX>& valueVector )
{
	...
	std::string result = validation::tryAqObjectsCurveMarketDataCreate( objectName, keyVector, infoBlocks );
                                    ...
                                    const std::vector< double >& fixingValues )
{
    ...
    // Call Method
	const std::string result = validation::tryAqObjectsFixingTableCreate( tableName,
                                                                          currency,
                                                                          curveTenor,
...
SWIG_STRINGMATRIX aqObjectsFixingTableDisplay( const std::string& tableName )
{
    ...
    // Call Method
	etrading::VariantMatrix results = validation::tryAqObjectsFixingTableDisplay( tableName );
...
double aqObjectsFixingTableValue( const std::string& tableName, const std::string & fixingDate )
{
    ...
    // Call Method
    const double result = validation::tryAqObjectsFixingTableValue( tableName, gregorianFixingDate );
...
std::vector<double> aqObjectsFixingTableValues( const std::string& tableName, const std::vector< std::string >& fixingDates )
{
    ...
    // Call Method
    const std::vector<double> results = validation::tryAqObjectsFixingTableValues( tableName, gregorianFixingDates );
...
std::vector<double> aqObjectsCurveDiscountFactorsWithSpread( const std::vector<std::string>& paymentDates, const std::string& curveCollection, const std::string& curveIndex, const double& spread, const std::string& fixingTableName )
{
    ...
    // Call Method
    const std::vector<double> discountFactors = validation::tryAqObjectsCurveDiscountFactorsWithSpread( tempPaymentDates, curveCollection, curveIndex, spread, fixingTableName );
