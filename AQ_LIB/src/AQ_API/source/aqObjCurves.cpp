...
#include "tryAqObjCurvesDisplay.h"
...
SWIG_STRINGMATRIX aqObjCurvesDisplay(const std::string& curveHandle)
{
	...
	etrading::VariantMatrix results = validation::tryAqObjCurvesDisplay(curveHandle);
...
*/
SWIG_STRINGMATRIX aqObjCurvesMarketDataDisplay(const std::string& marketDataObjectName, const std::string marketDataKey )
{
	AQ_API_START
    
	etrading::VariantMatrix results = validation::tryAqObjCurvesMarketDataDisplay( marketDataObjectName, marketDataKey );
...
*  @return			A string matrix containing the specified market data block.
*/
SWIG_STRINGMATRIX aqObjCurvesMarketDataDisplayFromCurve(const std::string& curveObjectName, const std::string marketDataKey )
{
	AQ_API_START
    
	etrading::VariantMatrix results = validation::tryAqObjCurvesMarketDataDisplayFromCurve( curveObjectName, marketDataKey );
...
*/
std::string aqObjCurvesMarketDataCreate( const std::string& objectName,
										...
										const std::string& key4, const SWIG_STRINGMATRIX& value4)
{
	...
	std::string result = validation::tryAqObjCurvesMarketDataCreate( objectName, marketDataKeys, infoBlocks );
														   ...
														   const std::vector<SWIG_STRINGMATRIX>& valueVector )
{
	...
	std::string result = validation::tryAqObjCurvesMarketDataCreate( objectName, keyVector, infoBlocks );
...
*/
std::string aqObjRatesFixingTableCreate( const std::string& tableName,
                                    ...
                                    const std::vector< double >& fixingValues )
{
    ...
    // Call Method
	const std::string result = validation::tryAqObjRatesFixingTableCreate( tableName,
                                                                          currency,
                                                                          curveTenor,
...
*/
SWIG_STRINGMATRIX aqObjRatesFixingTableDisplay( const std::string& tableName )
{
    AQ_API_START
    
    // Call Method
	etrading::VariantMatrix results = validation::tryAqObjRatesFixingTableDisplay( tableName );
...
*  @return			swapName
*/
double aqObjRatesFixingTableValue( const std::string& tableName, const std::string & fixingDate )
{
    AQ_API_START
    ...
    // Call Method
    const double result = validation::tryAqObjRatesFixingTableValue( tableName, gregorianFixingDate );
...
*  @return			swapName
*/
std::vector<double> aqObjRatesFixingTableValues( const std::string& tableName, const std::vector< std::string >& fixingDates )
{
    AQ_API_START
    ...
    // Call Method
    const std::vector<double> results = validation::tryAqObjRatesFixingTableValues( tableName, gregorianFixingDates );
...
*/
std::vector<double> aqObjCurvesDiscountFactorsWithSpread( const std::vector<std::string>& paymentDates, const std::string& curveCollection, const std::string& curveIndex, const double& spread, const std::string& fixingTableName )
{
    AQ_API_START
    ...
    // Call Method
    const std::vector<double> discountFactors = validation::tryAqObjCurvesDiscountFactorsWithSpread( tempPaymentDates, curveCollection, curveIndex, spread, fixingTableName );
