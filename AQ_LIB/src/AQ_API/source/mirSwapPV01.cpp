				...
				bool eomRoll)
{
	...
	{
		// marshall all inputs		
		using swig::fromStringToAQLString;
		
		ret = validation::tryMirSwapPV01(etrading::InitializeETrading::instance().dataInstance(),
												notional,
												fromStringToAQLString(payRec),
												fromStringToAQLString(effectDt),
												fromStringToAQLString(maturity),
												fromStringToAQLString(curveId),
												fromStringToAQLString(xFreq),
												fromStringToAQLString(xDayCount),
												fromStringToAQLString(xRollCnv),
												fromStringToAQLString(xCalendar),
												fromStringToAQLString(xFirstStub),
												fromStringToAQLString(xLastStub),
												fromStringToAQLString(xRollDay),
												fromStringToAQLString(xPayLag),
												fromStringToAQLString(xStub),
												tSpread,
												fromStringToAQLString(tFreq),
												fromStringToAQLString(tDayCount),
												fromStringToAQLString(tRollCnv),
												fromStringToAQLString(tCalendar),
												fromStringToAQLString(tFirstStub),
												fromStringToAQLString(tLastStub),
												fromStringToAQLString(tRollDay),
												fromStringToAQLString(tFixLag),
												tFirstFix,
												tLastFix,
												fromStringToAQLString(tPayLag),
												fromStringToAQLString(tStub),
												fromStringToAQLString(interpolation),
												fromStringToAQLString(forecastCurve),
												fromStringToAQLString(discountCurve),
												interpFwds,
												eomRoll);
