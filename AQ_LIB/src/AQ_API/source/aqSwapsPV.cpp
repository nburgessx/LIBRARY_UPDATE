...
/* @brief			swig interface for aqSwapsPV
*  @param [in]		swapLVB			A label value block defining the swap
*  @param [in]		validateKeys	True to validate the all keys provided are valid. Default to True
*  @return			Swap PV
*/
double aqSwapsPV(const std::vector<std::vector <std::string> >& swapLVB, 
						bool validateKeys)
{
    AQ_API_START
	double ret;
	...
	ret = validation::tryAqSwapsPV(lvb, validateKeys);
...
std::vector<std::string> tryAqSwapsPVLVBKeys()
{
    ...
    ret = validation::tryAqSwapsPVLVBKeys();
	return ret;
    AQ_API_END
}
