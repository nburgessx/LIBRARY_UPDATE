#include "TypeUtilities.h"

#include "aqCurveZeroRate.h"
#include "tryAqCurveZeroRate.h"
#include "APISetUp.h"               // AQ_API_START and AQ_API_END Macros


/* @brief			swig interface for aqCurveZeroRatesFromYearFractions
*  @param [in]		yearFractions	Year fraction
*  @param [in]		curveCollection	Curve collection id
*  @param [in]		curveIndex		Curve index name. Default to STD
*  @param [in]		frequency		Frequency. Default to SIMPLE for STD curve, ANNUAL for other curves
*  @param [in]		dayCount		Day count convention. Default to ACT/365
*  @return			Zero rate over a given term from the curve's asof date 
*/
double aqCurveZeroRatesFromYearFractions(double yearFraction, 
						const std::string& curveCollection, 
						const std::string& curveIndex, 
						const std::string& frequency, 
						const std::string& dayCount)
{
    AQ_API_START
	double ret(0.);
	
	// Input marshalling
	DoubleVector yearFractions;
	yearFractions.push_back(yearFraction);

	AQLString tmp_curveCollection(curveCollection.c_str());
	AQLString tmp_curveIndex(curveIndex.c_str());
	AQLString tmp_dayCount(dayCount.c_str());
	AQLString tmp_frequency(frequency.c_str());

	DoubleVector results = validation::tryAqCurveZeroRatesFromYearFractions(yearFractions,
																				tmp_curveCollection, 
																				tmp_curveIndex,
																				tmp_frequency, 
																				tmp_dayCount);
	ret = results[0];
	return ret;
    AQ_API_END
}


/* @brief			swig interface for aqCurveZeroRatesFromYearFractions
*  @param [in]		yearFractions	A list of year fractions
*  @param [in]		curveCollection	Curve collection id
*  @param [in]		curveIndex		Curve index name. Default to STD
*  @param [in]		frequency		Frequency. Default to SIMPLE for STD curve, ANNUAL for other curves
*  @param [in]		dayCount		Day count convention. Default to ACT/365
*  @return			A list of zero rates over a list of given terms from the curve's asof date 
*/
std::vector<double> aqCurveZeroRatesFromYearFractions(const std::vector<double>& yearFractions, 
						const std::string& curveCollection, 
						const std::string& curveIndex, 
						const std::string& frequency, 
						const std::string& dayCount)
{
    AQ_API_START
	std::vector<double> ret;
	
	// Input marshalling
	AQLString tmp_curveCollection(curveCollection.c_str());
	AQLString tmp_curveIndex(curveIndex.c_str());
	AQLString tmp_dayCount(dayCount.c_str());
	AQLString tmp_frequency(frequency.c_str());

	ret = validation::tryAqCurveZeroRatesFromYearFractions(yearFractions,
																tmp_curveCollection, 
																tmp_curveIndex,
																tmp_frequency, 
																tmp_dayCount);

	return ret;
    AQ_API_END
}

/* @brief			swig interface for aqCurveZeroRatesFromTenors
*  @param [in]		tenor			A tenor string
*  @param [in]		curveCollection	Curve collection id
*  @param [in]		curveIndex		Curve index name. Default to STD
*  @param [in]		frequency		Frequency. Default to SIMPLE for STD curve, ANNUAL for other curves
*  @param [in]		dayCount		Day count convention. Default to ACT/365
*  @param [in]		calendar		Calendar 
*  @param [in]		businessDayAdj	Rule that decides if end date should roll forward or backward etc. Default to NO_CHANGE
*  @return			Zero rate over a given term from the curve's asof date 
*/
double aqCurveZeroRatesFromTenors(const std::string& tenor, 
						const std::string& curveCollection, 
						const std::string& curveIndex, 
						const std::string& frequency, 
						const std::string& dayCount, 
						const std::string& calendar, 
						const std::string& businessDayAdj)
{
    AQ_API_START
	double ret(0.0);
	
	AQLStringVector tenors;
	tenors.push_back(tenor.c_str());

	// Input marshalling
	AQLString tmp_curveCollection(curveCollection.c_str());
	AQLString tmp_curveIndex(curveIndex.c_str());
	AQLString tmp_dayCount(dayCount.c_str());
	AQLString tmp_frequency(frequency.c_str());
	AQLString tmp_calendar(calendar.c_str());
	AQLString tmp_businessDayAdj(businessDayAdj.c_str());

	std::vector<double> results = validation::tryAqCurveZeroRatesFromTenors(tenors,
																				tmp_curveCollection, 
																				tmp_curveIndex,
																				tmp_frequency, 
																				tmp_dayCount, 
																				tmp_calendar,
																				tmp_businessDayAdj);
	ret = results[0];

	return ret;
    AQ_API_END
}

/* @brief			swig interface for aqCurveZeroRatesFromTenors
*  @param [in]		tenors			A list of tenors
*  @param [in]		curveCollection	Curve collection id
*  @param [in]		curveIndex		Curve index name. Default to STD
*  @param [in]		frequency		Frequency. Default to SIMPLE for STD curve, ANNUAL for other curves
*  @param [in]		dayCount		Day count convention. Default to ACT/365
*  @param [in]		calendar		Calendar 
*  @param [in]		businessDayAdj	Rule that decides if end date should roll forward or backward etc. Default to NO_CHANGE
*  @return			A list of zero rates over a list of given terms from the curve's asof date 
*/
std::vector<double> aqCurveZeroRatesFromTenors(const std::vector<std::string>& tenors, 
						const std::string& curveCollection, 
						const std::string& curveIndex, 
						const std::string& frequency, 
						const std::string& dayCount, 
						const std::string& calendar, 
						const std::string& businessDayAdj)
{
    AQ_API_START
	std::vector<double> ret;
	
	AQLStringVector tmp_tenors;
	swig::buildStringVector(tmp_tenors, tenors);

	// Input marshalling
	AQLString tmp_curveCollection(curveCollection.c_str());
	AQLString tmp_curveIndex(curveIndex.c_str());
	AQLString tmp_dayCount(dayCount.c_str());
	AQLString tmp_frequency(frequency.c_str());
	AQLString tmp_calendar(calendar.c_str());
	AQLString tmp_businessDayAdj(businessDayAdj.c_str());

	ret = validation::tryAqCurveZeroRatesFromTenors(tmp_tenors,
														tmp_curveCollection, 
														tmp_curveIndex,
														tmp_frequency, 
														tmp_dayCount, 
														tmp_calendar,
														tmp_businessDayAdj);

	return ret;
    AQ_API_END
}

