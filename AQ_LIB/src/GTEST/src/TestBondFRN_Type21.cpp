...
namespace
{
	...
	*/
    extern const std::string frnCreateInputFilePattern  = ".+_tryAqBondObjectCreateFromGenerator_inputs.csv";
    ...
    extern const std::string frnPriceFromYieldInputFileSuffix             = "_tryAqBondObjectFRNPriceFromYield_inputs.csv";
...
namespace google_test
{
    ...
    TEST_F( TestBondFRN_Type21, SNAPSHOT_tryAqBondObjectFRNPriceFromDiscountMargin )
    {
		...
		for (size_t i=0; i< allMatchingFiles.size(); i++ )
		{
			...
			const double calculatedPrice = validation::tryAqBondObjectFRNPriceFromDiscountMargin( bondObjectName, settlementDate, discountMargin, assumedRate, indexToNextCoupon, annualizedNextCouponRate );
	...
	TEST_F( TestBondFRN_Type21, SNAPSHOT_tryAqBondObjectFRNPriceFromYield )
	{
		const double tolerance = 1.0e-8;
		...
		for (size_t i=0; i< allMatchingFiles.size(); i++ )
		{
			...
			// Calculate the bond price
			const double calculatedPrice = validation::tryAqBondObjectFRNPriceFromYield( bondObjectName, settlementDate, yield, assumedRate, indexToNextCoupon, annualizedNextCouponRate );
	...
	TEST_F( TestBondFRN_Type21, CONSISTENCY_tryAqObjBondsFRNYieldRoundTrip )
	{
		...
		for (size_t i=0; i< allMatchingFiles.size(); i++ )
		{
			...
			const double calculatedYield = validation::tryAqBondObjectFRNYieldFromPrice( bondObjectName, settlementDate, calculatedPrice, assumedRate, indexToNextCoupon, annualizedNextCouponRate );
	...
	TEST_F( TestBondFRN_Type21, CONSISTENCY_tryAqObjBondsFRNDiscountMarginRoundTrip )
	{
		...
		for (size_t i=0; i< allMatchingFiles.size(); i++ )
		{
			...
			const double calculatedDiscountMargin = validation::tryAqBondObjectFRNDiscountMarginFromPrice( bondObjectName, settlementDate, calculatedPrice, assumedRate, indexToNextCoupon, annualizedNextCouponRate );
