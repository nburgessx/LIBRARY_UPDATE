...
#define TEST_DIR "ETrading/AQObjects/TestAQObjSwapDeltaWithCurveEngine/"
...
namespace
{
	...
	void buildEngineCurves(const AQLString& ccy, const AQLString& prefix, const std::string& curveCollection, const std::vector<std::string>& curveGeneratorNames)
	{		
		AQLString marketDataFile_OIS = TEST_DIR;
		AQLString marketDataFile_STD = TEST_DIR;
		AQLString marketDataFile_6M  = TEST_DIR;

		marketDataFile_OIS += prefix + AQLString("OIS_MARKETDATA");
		marketDataFile_STD += prefix + AQLString("STD_MARKETDATA");
		marketDataFile_6M  += prefix + AQLString("6M_MARKETDATA");

		std::string marketObj_OIS = google_test::createAQObjMarketDataObjectFromFileName(marketDataFile_OIS);
		std::string marketObj_STD = google_test::createAQObjMarketDataObjectFromFileName(marketDataFile_STD);
		std::string marketObj_6M  = google_test::createAQObjMarketDataObjectFromFileName(marketDataFile_6M);
...
{
	DECLARE_TEST_FIXTURE(TestAQObjSwapDeltaWithCurveEngine);
	
	// This test checks the delta ladder profile of 3 EUR vanilla swaps
	TEST_F(TestAQObjSwapDeltaWithCurveEngine, SNAPSHOT_EUR_SWAP_IR_DELTA)
	{
		// Load all the swaps 
	...
	// TEST_F(TestAQObjSwapDeltaWithCurveEngine, SNAPSHOT_USD_SWAP_IR_DELTA)
	// {
	// 	// Load all the swaps 
