...
#define TEST_DIR "ETrading/AQObjects/TestAQObjSwapZeroCouponSwapFixedFloat/"
...
namespace
{
    ...
    //
    extern const char zeroCouponSwapInputs[]	= TEST_DIR "FIXEDFLOATZEROCOUPONSWAP@1_tryAqSwapObjectCreate_inputs";
    extern const char parRateInputs[]	        = TEST_DIR "FIXEDFLOATZEROCOUPONSWAP@1_tryAqSwapObjectParRate_inputs";
    extern const char parRateOutputs[]	    = TEST_DIR "FIXEDFLOATZEROCOUPONSWAP@1_tryAqSwapObjectParRate_outputs";
    extern const char parRateOutputs64[]	    = TEST_DIR "FIXEDFLOATZEROCOUPONSWAP@1_tryAqSwapObjectParRate_outputs64_";
}
...
namespace google_test
{
    ...
    //
    AQL_BUILD_USD_CURVE( TestAQObjSwapZeroCouponSwapFixedFloat, USDYC_OIS, USDYC_1M, USDYC_STD, USDYC_6M, USDYC_12M);
    
    //
    // Call Test Fixture
    //

    TEST_F( TestAQObjSwapZeroCouponSwapFixedFloat, SNAPSHOT_SwapHardCodedParRateCheck )
    {
        try
        {
                ...
                double actualSwapParRate          = validation::tryAqSwapObjectParRate( swapName, curveCollectionLVB, fixingTableLVB );
    ...
    TEST_F( TestAQObjSwapZeroCouponSwapFixedFloat, SNAPSHOT_SwapPVZeroCheck )
    {
        try
        {
                // 2. Load the Input Files
                ...
                double parRate = validation::tryAqSwapObjectParRate( swapName, curveCollectionLVB, fixingTableLVB );
				...
				validation::tryAqSwapObjectCreateFromLegLVBs(newSwapTradeName, legsLVB[0], legsLVB[1], swapPropertiesLVB, isXccySwap, validateKeys);
                
                double actualPV = validation::tryAqSwapObjectPV(newSwapTradeName, curveCollectionLVB, "", fixingTableLVB);
	  ...
	  TEST_F( TestAQObjSwapZeroCouponSwapFixedFloat, CONSISTENCY_IsFwdInterCheck )
    {
        try
        {
                // 2. Load the Input Files
                ...
                std::string swapName          = validation::tryAqSwapObjectCreate( swapTradeName, swapLVB, swapPropertiesLVB, isXccySwap, validateKeys );
                ...
                validation::tryAqSwapObjectParRate( swapName, curveCollectionLVB, fixingTableLVB );
				...
				std::string swapName2          = validation::tryAqSwapObjectCreateFromLegLVBs( swapTradeName, firstLegLVB, secondLegLVB, swapPropertiesLVB, isXccySwap, validateKeys );
           		auto swap2 = etrading::getSwap(swapName2);

				// Call any pricing function so that the fwdInter flag will be retrieved from a specific curveCollection and staticTable
                validation::tryAqSwapObjectParRate( swapName2, curveCollectionLVB, fixingTableLVB );
	...
	TEST_F( TestAQObjSwapZeroCouponSwapFixedFloat, CONSISTENCY_IsFwdInterCheck_MultiFloatLegs )
    {
        try
        {
                // 2. Load the Input Files
				...
				std::string swapName          = validation::tryAqSwapObjectCreateFromLegLVBs( swapTradeName, firstLegLVB, secondLegLVB, swapPropertiesLVB, isXccySwap, validateKeys );

                AQLStringMatrix curveCollectionLVB = parRateInputFile["curveCollections"];
                AQLStringMatrix fixingTableLVB     = parRateInputFile.getOptional("fixingTableNames", AQLStringMatrix() );
                validation::tryAqSwapObjectParRate( swapName, curveCollectionLVB, fixingTableLVB );

				std::string legName3          = validation::tryAqSwapObjectLegCreate("thirdLeg", thirdLegLVB);
				std::string legName4          = validation::tryAqSwapObjectLegCreate("fourLeg", fourthLegLVB);
				validation::tryAqSwapObjectAddLeg(swapName, legName3);
				validation::tryAqSwapObjectAddLeg(swapName, legName4);
