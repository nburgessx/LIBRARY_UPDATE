...
#define TEST_DIR			  "ETrading/AQObjects/TestAQObjCreditDefaultSwap/"
...
namespace
{
	...
	// API methods
	const char CDS_CALCULATE_PV[]				= TEST_DIR "tryAqCDSObjectPVFromHazardRate_inputs.csv";
	const char CDS_CALCULATE_RISKY_ANNUITY[]	= TEST_DIR "tryAqObjCreditDefaultSwapAnnuityFromHazardRate_inputs.csv";
	const char CDS_CALCULATE_PAR_SPREAD[]		= TEST_DIR "tryAqCDSObjectParSpreadFromHazardRate_inputs.csv";
	const char CDS_CALCULATE_HAZARD_RATE[]		= TEST_DIR "tryAqCDSObjectHazardRateFromParSpread_inputs.csv";

	// Snapshot results
	const char CDS_EXPECTED_PV[]				= TEST_DIR "tryAqCDSObjectPVFromHazardRate_outputs.csv";
	const char CDS_EXPECTED_RISKY_ANNUITY[]		= TEST_DIR "tryAqObjCreditDefaultSwapAnnuityFromHazardRate_outputs.csv";
	const char CDS_EXPECTED_PAR_SPREAD[]		= TEST_DIR "tryAqCDSObjectParSpreadFromHazardRate_outputs.csv";
	...
	*/
	...
	void createAQObjCreditDefaultSwapFromFileName( const AQLString& cdsFileName )
	{
...
namespace google_test
{
	...
	TEST_F( TestCreditDefaultSwap, SNAPSHOT_PV_fromHazardRate )
    {
		...
		createAQObjCreditDefaultSwapFromFileName( GEN_CREDIT_DEFAULT_SWAP );
		...
		const double calculatedPV = validation::tryAqCDSObjectPVFromHazardRate( swapName, curveCollections, hazardRate, recoveryRate, legName.c_str(), includeAccruedInterest );
	...
	TEST_F( TestCreditDefaultSwap, SNAPSHOT_RiskyAnnuity_FromHazardRate )
    {
		...
		const double calculatedAnnuity = validation::tryAqCDSObjectRiskyAnnuityFromHazardRate( swapName, curveCollection, hazardRate, recoveryRate, legName.c_str(), includeAccruedInterest );
	...
	TEST_F( TestCreditDefaultSwap, SNAPSHOT_ParSpread_FromHazardRate )
    {
		...
		const double calculatedParSpread = validation::tryAqCDSObjectParSpreadFromHazardRate( swapName, curveCollections, hazardRate, recoveryRate, premiumLegName.c_str(), protectionLegName.c_str(), includeAccruedInterest );
	...
	TEST_F( TestCreditDefaultSwap, SNAPSHOT_HazardRate_FromParSpread )
    {
		...
		const double calculatedHazardRate = validation::tryAqCDSObjectHazardRateFromParSpread( swapName, curveCollections, parSpread, recoveryRate, premiumLegName.c_str(), protectionLegName.c_str(), includeAccruedInterest );
