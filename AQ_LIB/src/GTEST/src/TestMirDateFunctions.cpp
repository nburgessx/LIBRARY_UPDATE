...
namespace google_test
{
    ...
    TEST( TestMirDateFunctions, UNIT_checkDateFromTerm_ACT_365_ISDA )
    {
		...
		AQLDate dateFromTerm1 = etrading::AQLDateSchedule::getDateFromTerm(baseDate, inputTerm, dayCount);
		...
		double impliedTerm = etrading::AQLDateSchedule::getTerm(baseDate, dateFromTerm1, dayCount);
		double tolerance = 1.0e-9;

		EXPECT_NEAR( inputTerm, impliedTerm, tolerance );

		// Verify that the date from impliedTerm again reproduces the expected date
		AQLDate dateFromTerm2 = etrading::AQLDateSchedule::getDateFromTerm(baseDate, impliedTerm, dayCount);
