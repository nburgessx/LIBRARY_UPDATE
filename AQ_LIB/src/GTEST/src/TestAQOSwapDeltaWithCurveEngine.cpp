...
#define TEST_DIR "ETrading/AQObjects/TestAQOSwapDeltaWithCurveEngine/"
...
namespace
{
	...
	{		
		...
		std::string marketObj_OIS = google_test::createAQOMarketDataObjectFromFileName(marketDataFile_OIS);
		std::string marketObj_STD = google_test::createAQOMarketDataObjectFromFileName(marketDataFile_STD);
		std::string marketObj_6M  = google_test::createAQOMarketDataObjectFromFileName(marketDataFile_6M);
...
{
	DECLARE_TEST_FIXTURE(TestAQOSwapDeltaWithCurveEngine);
	
	// This test checks the delta ladder profile of 3 EUR vanilla swaps
	TEST_F(TestAQOSwapDeltaWithCurveEngine, SNAPSHOT_EUR_SWAP_IR_DELTA)
	{
		// Load all the swaps 
	...
	// TEST_F(TestAQOSwapDeltaWithCurveEngine, SNAPSHOT_USD_SWAP_IR_DELTA)
	// {
	// 	// Load all the swaps 
