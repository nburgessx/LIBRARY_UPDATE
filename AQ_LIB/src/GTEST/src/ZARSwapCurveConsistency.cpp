// ZARSwapCurveConsistency.cpp

/*
 * @brief			ZAR Curve Consistency Tests
...
const std::string TEST_FOLDER = "ETrading/Curves/ZARCurves/";

// Test Files
const std::string fileNameZAR3MJ = TEST_FOLDER + "ZAR_SWAP_3M_CURVE.JSON";


namespace google_test
{
    // Declare Test Fixture Class
    class ZAR_SwapCurve_Consistency : public virtual testing::Test, public virtual google_test::InitializeGoogleTest
    {
    public:

        // Load Curves
        const std::string curveObjectZAR3MJ_ = validation::tryAqObjectsLoad( etrading::getGoogleTestFolder() + fileNameZAR3MJ, etrading::JSON );
    };


    // Call Test Fixture Class
    TEST_F( ZAR_SwapCurve_Consistency, CONSISTENCY_ZAR3MJ_Swap_Instrument_Repricing_Using_Macro )
    {
        ...
        repriceCalibrationInstruments( etrading::CurveMarketDataEnum::SWAP_MARKETDATA,    // Calibration Instrument: STD Interest Rate Swap
                                       { "3Y", "4Y", "5Y", "6Y", "7Y", "8Y", "9Y", "10Y", "12Y", "15Y", "20Y", "25Y", "30Y" }, // Calibration Tenors
                                       curveObjectZAR3MJ_,                               // AQOCurveName
                                       "ZARYC",                                          // CurveCollection
                                       "20200416",                                       // Swap Effective Date
                                       "ZAR_3MJ",                                        // Swap Generator
                                       1.0e-6 );                                         // Test Tolerance
    }
    
    // Call Test Fixture Class
    TEST_F( ZAR_SwapCurve_Consistency, CONSISTENCY_ZAR3MJ_Swap_Instrument_Repricing )
    {
        ...
        for ( size_t i = 0; i < swapTerms.size(); ++i )
        {
            ...
            std::set<std::string> calibrationInstruments = { "3Y", "4Y", "5Y", "6Y", "7Y", "8Y", "9Y", "10Y", "12Y", "15Y", "20Y", "25Y", "30Y" };
