	...
	TEST_F(TestSABRCalibration, SNAPSHOT_USD_ImpliedSwaptionVol)
	{
		...
		for (const auto& swapTenor : TENOR_FOR_TEST)
		{
			for (const auto& expiryTenor : TENOR_FOR_TEST)
			{
				...
				AQLDate expiryDate = etrading::AQLDateSchedule::getDate(asOfDate, AQLString(expiryTenor.c_str()), "", "");
				AQLString expiryDayCount("ACT/365");
				double expiryTerm = etrading::AQLDateSchedule::getTerm(asOfDate, expiryDate, expiryDayCount);
