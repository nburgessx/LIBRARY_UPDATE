...
namespace
{
	extern const char GBP_OIS[] = TEST_DIR "GBPLIVE_OIS_tryAqCurveCalibrateOIS_inputs";
	extern const char GBP_1M[] = "";
	extern const char GBP_3M[] = TEST_DIR "GBPLIVE_STD_tryAqCurveCalibrateSwap_inputs";
...
namespace google_test
{
	...
	TEST_F(TestTradeGBPSwapStubRate, CONSISTENCY_AQObjSwap_subRateFromFixingTable)
	{
		try
		{
			...
			const std::string fixingTableName = validation::tryAqObjectLoad(FixingTableObject, etrading::JSON);
			...
			const std::string swapName = validation::tryAqObjectLoad(SwapObject, etrading::JSON);

			std::vector<std::string> columnList = { "FixingDate", "FloatRate" };

			AnyTypeMatrix fixingDateAndFixingRates = validation::tryAqSwapObjectDisplayCashflows(swapName, etrading::fromStringToLVB("GBPLIVE"), AQLString("leg2:float"), etrading::fromStringToLVB(fixingTableName), false, columnList).front();
