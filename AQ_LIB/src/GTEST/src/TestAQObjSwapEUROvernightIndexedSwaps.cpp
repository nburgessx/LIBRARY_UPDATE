...
namespace
{
    ...
    //
    extern const char oisInputs[]	            = TEST_DIR "EUROIS"; // Test files require a suffix. Format :=  Basename + Index + '_tryAqSwapObjectCreateFromLegLVBs_inputs'

#if defined(GTEST32)
    extern const char oisOutputs[]	    = TEST_DIR "x86_EUROIS"; // Test files require a suffix. Format :=  Basename + Index + '_tryAqSwapObjectCreateFromLegLVBs_outputs'
#else
	extern const char oisOutputs[]	    = TEST_DIR "x64_EUROIS"; // Test files require a suffix. Format :=  Basename + Index + '_tryAqSwapObjectCreateFromLegLVBs_outputs'
#endif
}
...
namespace google_test
{
    ...
    TEST_F( TestAQObjSwapEUROvernightIndexedSwaps, UNIT_CreateOvernightIndexedSwaps )
    {
        ...
        try
        {
            ...
            {
                // Load the input file
                AQLString inputFilename = CreateDataFile::makeFilename( oisInputs, "_tryAqSwapObjectCreateFromLegLVBs_inputs", i ); // Append the Index and Suffix to test file name
                ...
                AQLString outputFilename = CreateDataFile::makeFilename( oisInputs, "_tryAqSwapObjectCreateFromLegLVBs_outputs", i ); // Append the Index and Suffix to test file name
    ...
    TEST_F( TestAQObjSwapEUROvernightIndexedSwaps, SNAPSHOT_CheckOisPV )
    {
        ...
        try
        {
            ...
            {
                // 1. Create the Swap
                AQLString createSwapFilename = CreateDataFile::makeFilename( oisInputs, "_tryAqSwapObjectCreateFromLegLVBs_inputs", i ); // Append the Index and Suffix to test file name
                ...
                // 2. Price the Swap
                 AQLString SwapPVFilename = CreateDataFile::makeFilename( oisInputs, "_tryAqSwapObjectPV_inputs", i ); // Append the Index and Suffix to test file name
                ...
                CheckTestResultsAndRebaseOnRequest( localSwapPV, TEST_DIR, oisOutputs, "_tryAqSwapObjectPV_outputs", tolerance, i ); // Append the Index and Suffix to test file name
            }
        }
        catch( const ReadDataFile::LoadError& )
        {
        ...
        try
        {
            for ( i = 1; ; ++i )
            {
                ...
                // 2. Get the Swap Par Rate
                 AQLString SwapParRateFilename = CreateDataFile::makeFilename( oisInputs, "_tryAqSwapObjectParRate_inputs", i ); // Append the Index and Suffix to test file name
                const ReadDataFile::Load swapParRateFile( SwapParRateFilename );

                    // Read the input file into the tryAqSwapObjectParRate
                    std::string swapObjectName      = swapParRateFile["swapName"];
                    double localSwapParRate         = validation::tryAqSwapObjectParRate( swapObjectName, etrading::fromStringToLVB("EURYC") );
                
                // 3. Load the Tenor Basis Spread Results File and Compare against the Local Spread                   
                const double tolerance = 1e-008;
                CheckTestResultsAndRebaseOnRequest( localSwapParRate, TEST_DIR, oisOutputs, "_tryAqSwapObjectParRate_outputs", tolerance, i ); // Append the Index and Suffix to test file name
            }
        }
        catch( const ReadDataFile::LoadError& )
        {
    ...
    TEST_F( TestAQObjSwapEUROvernightIndexedSwaps, SNAPSHOT_CheckOisPV01 )
    {
        ...
        try
        {
            for ( i = 1; ; ++i )
            {
                ...
                // 2. Price the Swap
                 AQLString SwapPV01Filename = CreateDataFile::makeFilename( oisInputs, "_tryAqSwapObjectPV01_inputs", i ); // Append the Index and Suffix to test file name
                const ReadDataFile::Load swapPV01File( SwapPV01Filename );

                    // Read the input file into the tryAqSwapObjectPV01
                    std::string swapObjectName      = swapPV01File["swapName"];
                    double localSwapPV01            = validation::tryAqSwapObjectPV01( swapObjectName, etrading::fromStringToLVB("EURYC") );
                
                // 3. Load the Tenor Basis Spread Results File and Compare against the Local Spread                   
                const double tolerance = 1e-008;
                CheckTestResultsAndRebaseOnRequest( localSwapPV01, TEST_DIR, oisOutputs, "_tryAqSwapObjectPV01_outputs", tolerance, i ); // Append the Index and Suffix to test file name
            }
        }
        catch( const ReadDataFile::LoadError& )
        {
