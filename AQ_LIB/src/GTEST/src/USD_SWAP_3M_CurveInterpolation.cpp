...
namespace google_test
{
	namespace
	{
		...
		void loadCurves( const std::string interpolation )
		{
			...
			// Load Curves
		    const std::string curveObjectEUROIS_ = validation::tryAqObjectLoad( etrading::getGoogleTestFolder() + fileNameUSDOIS, etrading::JSON );
		    const std::string curveObjectEUR3ML_ = validation::tryAqObjectLoad( etrading::getGoogleTestFolder() + fileNameUSD3ML, etrading::JSON );
		}
	}
