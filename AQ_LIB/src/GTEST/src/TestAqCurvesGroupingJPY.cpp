...
{
	DECLARE_TEST_FIXTURE(TestAqCurvesGroupingJPY);

    //
    // Call Test Fixture
    //

    TEST_F( TestAqCurvesGroupingJPY, SNAPSHOT_CheckMeCurveDisplay )
    {
		// JPYYC OIS, JPYYCLCH_OIS and JPYYCTIBOR_OIS
		AQLString inputDir = JPYYC_OIS;
		setUpAqOISCurve(inputDir);

		inputDir = JPYYCLCH_OIS;
		setUpAqOISCurve(inputDir);

		inputDir = JPYYC_OIS;
		setUpAqOISCurve(JPYYCTIBOR_OIS);

		// JPYYC 6M
		inputDir = JPYYC_6M_STD;
		setUpAqSTDCurve(inputDir);
		...
		setUpAqTenorBasisCurve(inputDir);
