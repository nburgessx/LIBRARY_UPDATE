...
namespace validation
{
	...
	*/
	etrading::NelsonSiegelSvenssonCalibrationResults tryAqBondsCurveNelsonSiegelCalibrate( const DoubleVector& bondMaturities,
	...
	*  @returns		The calibrated parameters, store in a NelsonSiegelSvenssonCalibrationResults structure
	*/
	etrading::NelsonSiegelSvenssonCalibrationResults tryAqBondsCurveSvenssonCalibrate( const DoubleVector& bondMaturities,
																			 const DoubleVector& bondYields,
																			 const etrading::NelsonSiegelSvenssonParameters& initialGuess,
																			 const unsigned int& maxIterations,
																			 const unsigned int& maxStationaryStateIterations,
	 ...
	 */
	etrading::PolynomialCalibrationResults tryAqBondsCurvePolynomialCalibrate( const unsigned int polynomialOrder,
	 ...
	 */
    std::string tryAqBondsCurveCreate( const std::string& objectName,
	...
	*/
	AnyTypeMatrix tryAqBondsCurveDisplay( const std::string& bondCurveName );
	...
	*/
	double tryAqBondsYield(const std::string& bondCurveName, const AQLDate& referenceDate );
	...
	*/
	DoubleVector tryAqBondsCurveNelsonSiegelYield( const double& beta0,
												  const double& beta1,
												  const double& beta2,
												  const double& lambda,
												  const DoubleVector& bondMaturities );
	...
	*  @returns		A vector of interpolated bond yields, one yield per supplied maturity.
	*/
	DoubleVector tryAqBondsCurveSvenssonYield( const double& beta0,
											  const double& beta1,
											  const double& beta2,
											  const double& beta3,
	...
	*/
	DoubleVector tryAqBondsCurvePolynomialYield( const DoubleVector& coefficients, const DoubleVector& bondMaturities );
}