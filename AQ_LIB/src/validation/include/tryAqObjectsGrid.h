...
namespace validation
{
    ...
    /* @brief			validation interface for the tryAqObjectsCreateRange method
    ...
    */
    std::string tryAqObjectsGridCreate(
        const std::string& objectName,
        const TableInfo& tableInfo,
        const bool allowJaggedData = false );

    std::string tryAqObjectsGridSave(
        const std::string& objectName,
    ...
    */
    std::pair<const FlexibleData, std::vector<std::string>>  tryAqObjectsGridDisplay(
                const std::string& objectName );

    std::pair<const bool, std::string> tryAqObjectsGridLoad(
        const std::string& fileName );

    std::vector<std::string> tryAqObjectsGridObjectNames();
    const bool tryAqObjectsGridClearOne( const std::string& objectName );
    const bool tryAqObjectsGridClearAll();
