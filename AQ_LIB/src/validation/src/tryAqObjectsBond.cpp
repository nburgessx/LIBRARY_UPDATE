...
namespace validation
{
	...
	*  @return			Bond Handle
	*/
	std::string tryAqObjectsBondCreate( const std::string& bondObjectName, const LabelValueBlock& bondLVB, const LabelValueBlock& scheduleLVB, bool validateKeys )
    {
	...
	*/
	std::string tryAqObjectsBondCreateFromGenerator( const std::string& bondObjectName, const std::string& bondGeneratorName, const LabelValueBlock& expressionLVB, bool validateKeys )
	{
	    VALID_EXCEPTION_START
    ...
    /* @brief			validation interface for the aqObjectsBondDisplay method
	*  @param [in]		bondObjectName		Bond object name
	...
	*/
	AnyTypeMatrix tryAqObjectsBondDisplaySchedule( const std::string& bondObjectName, const bool& showColumnHeaders, const std::vector<std::string>& columnList )
    {
        VALID_EXCEPTION_START
	...
	*/
	AnyTypeMatrix tryAqObjectsBondDisplayCashflows( const std::string& bondObjectName, const AQLDate& settlementDate, const double& yield, const std::string& yieldCalculationType, const bool& showColumnHeaders, const std::vector<std::string>& columnList )
	{
		VALID_EXCEPTION_START
	...
	*/
    std::vector< double > tryAqObjectsBondYieldOptimized( const etrading::BondPtr bondObject, const std::vector< AQLDate >& settlementDates, const std::vector< double >& prices, const std::string& yieldCalculationType  )
    {
		...
		for (int i=0; i<nSettlementDates; i++)
		{
			...
			yields[i] = tryAqObjectsBondYieldFromObject( myBonds[id], settlementDates[i], prices[i], yieldCalculationType );
		}
    ...
    *  @return			Bond Yield(s)
	*/
	std::vector< double > tryAqObjectsBondYield( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& prices, const std::string& yieldCalculationType , const bool & optimizePerformance )
    {
        VALID_EXCEPTION_START
        ...
        if( calculateUsingThreads && optimizePerformance )
        {
            ...
            // ----------------------------------
            yields = tryAqObjectsBondYieldOptimized( bondObject, settlementDates, prices, yieldCalculationType );
        }
        else
        {
	...
	*/
	double tryAqObjectsBondBPVPerTick( const std::string& bondObjectName, const LabelValueBlock& valuationSettingsLVB, const double price, const std::string& yieldCalculationType, const double tickSize )
	{
		VALID_EXCEPTION_START
	...
	*/
    std::vector< double > tryAqObjectsBondOisSpread( const std::string& bondObjectName, const std::string& marketObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& yields )
	{
		...
		for ( unsigned int i = 0; i < settleDatesSize; ++i )
        {
            const AQLDate settlementDate     = settlementDates[i];
            const double yield              = yields[i];
			double spread                   = tryAqObjectsBondOisSpread( bondObjectName, marketObjectName, settlementDate, yield );
    ...
    /* @brief			Helper method for the tryAqObjectsBondSpreadOIS method. Calculate the O-Spread (BondYield to OIS ParRate spread) using linear interpolation on the OIS ParRate on standard tenors.
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		marketObjectName	Market Data object name
	...
	*/
	 ...
	 */
	const etrading::VariantMatrix tryAqObjectsBondGeneratorDisplay( const std::string& objectName, const std::string& propertyKey )
    {
		VALID_EXCEPTION_START
	...
	*  @return			bondName
	*/
	std::string tryAqObjectsBondCreateFromLVB(const std::string& bondObjectName, const LabelValueBlock& bondLVB, bool validateKeys)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			Helper interface for the aqObjectsBondZSpread method. Calculate the Bond Z Spread.
	*  @param [in]		bondObjectName		Bond object name
	...
	*/
	DoubleVector tryAqObjectsBondZSpreads(const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& bondPrices, const std::vector< std::string >& curveCollections, const std::vector< std::string >& forecastCurves, const bool& continuouslyCompounding)
	{
		...
		for (unsigned int i = 0; i < settleDatesSize; ++i)
		{
			double zSpread = tryAqObjectsBondZSpread(bondObjectName, settlementDates[i], bondPrices[i], curveCollections[i], forecastCurves[i], continuouslyCompounding);
	...
	*/
	...
	/* @brief			Help function for tryAqObjectsBondConversionFactors with multiple settle dates as params. Calculate Bond conversionFactor based on future settle date and future's notional coupon rate.
	*  @param [in]		bondObjectName		Bond object name
	...
	*/
	...
	/* @brief			validation interface for the aqObjectsBondCreateAUDNotionalBond method
