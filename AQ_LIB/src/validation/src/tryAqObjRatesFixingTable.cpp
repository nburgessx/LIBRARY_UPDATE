...
namespace validation
{
	...
	*/
    std::string tryAqObjRatesFixingTableCreate( const std::string& tableName,
                                           ...
                                           const std::vector< double >& fixingValues )
    {
		...
		{
			CreateDataFile file(decorateFilename("tryAqObjRatesFixingTableCreate_inputs", tableName.c_str()));
			file.write("generatorFunction", "tryAqObjRatesFixingTableCreate");
	...
	*/
    ...
    etrading::VariantMatrix tryAqObjRatesFixingTableDisplay( const std::string& tableName )
    {
         VALID_EXCEPTION_START
    ...
    double tryAqObjRatesFixingTableValue( const std::string& tableName, const boost::gregorian::date fixingDate )
    {
        VALID_EXCEPTION_START
	...
	*/
    std::vector< double > tryAqObjRatesFixingTableValues( const std::string& tableName, const std::vector< boost::gregorian::date >& fixingDates )
    {
        VALID_EXCEPTION_START
	...
	*  @return			A vector of fixing values
	*/
    std::vector< double > tryAqObjRatesFixingTableValues( const std::string& tableName, const std::string& currency, const std::string& curveTenor, const std::vector< boost::gregorian::date >& fixingDates )
    {
        VALID_EXCEPTION_START
		...
		if (etrading::CreateDataFile::recordEnabled()) 
		{
			CreateDataFile file(decorateFilename("tryAqObjRatesFixingTableValues_inputs", tableName.c_str() ) );
			file.write("generatorFunction", "tryAqObjRatesFixingTableValues");
			file.write("tableName", tableName);
	...
	*  @return			A vector of fixing values
	*/
	std::vector< double > tryAqObjRatesFixingTableValues(const std::string& tableName, const etrading::LabelValueBlock& parameterLVB, const std::vector< boost::gregorian::date >& fixingDates)
	{
		VALID_EXCEPTION_START;
