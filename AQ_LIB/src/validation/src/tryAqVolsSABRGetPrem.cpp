#include "tryAqVolsSABRGetPrem.h"
...
{
	/* @brief			validation interface for tryAqVolsSABRGetPrem
	...
	*/
	double tryAqVolsSABRGetPrem(const AQLString& expiryPoint,
		...
		const std::string& volType)
	{
		...
		AQ_RECORD_INPUTS(expiryPoint, tenorPoint, strike, sign, forwardID, annuityID, betaID, nuID, rhoID, approxMethod, shift);
		...
		double ret = AQLMathSwaptionVolUtility::getSABRPrem(etrading::InitializeETrading::instance().dataInstance(),
														expiryPoint,
														tenorPoint,
														strike,
														sign,
