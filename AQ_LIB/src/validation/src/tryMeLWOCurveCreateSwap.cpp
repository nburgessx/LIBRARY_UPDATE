...
namespace validation_api
{
    ...
    */
                                            ...
                                            const LAStringMatrix& convexityAdjRates )
    {
		...
		etrading::LAUpdateStaticDataManager::loadStaticDataSwapCurve( etrading::getDataInstance(),
																	  curveCollection,
																	  staticDataTable,
																	  curveConv,
																	  moneyMarketConv,
		...
		// 2. Calibrate Curve
		etrading::LAUpdateStaticDataManager::calibrateSwapCurve( etrading::getDataInstance(), curveCollection, staticDataTable, curveConv );
        ...
        std::vector<LADate> massiveDateVector;
        ...
        etrading::LWOCurve lwoCurve( curveNameForLWOCurve,
        ...
        {
            LAString ret = curveNameForLWOCurve.c_str();
            if ( CreateDataFile::recordEnabled() )
            {
        ...
        {
            MLIB_THROW( ( boost::format( "Unable to create LWOCurve named %s" ) % curveNameForLWOCurve.c_str() ).str().c_str() );
        }
