...
namespace validation
{
    ...
    */
    std::vector<std::string> tryAqSwapPV01LVBKeys()
    {
    ...
    /* @brief			validation interface for the aqSwapPV01 method
    *  @param [in]		swapLVB		A label value block defining the swap
    *  @param [in]		validateKeys	True to validate the all keys provided are valid. Default to True
    *  @return			Swap PV01
    */
    double tryAqSwapPV01( const LabelValueBlock& swapLVB, bool validateKeys )
    {
        VALID_EXCEPTION_START

        etrading::validateKeysForLVB( tryAqSwapPV01LVBKeys(), swapLVB.getKeys(), validateKeys );
        ...
        {
            CreateDataFile file( decorateFilename( "tryAqSwapPV01_inputs", curveCollection ) );
            file.write( "generatorFunction", "tryAqSwapPV01" );
        ...
        if ( CreateDataFile::recordEnabled() )
        {
            CreateDataFile file( decorateFilename( "tryAqSwapPV01_outputs", curveCollection ) );
