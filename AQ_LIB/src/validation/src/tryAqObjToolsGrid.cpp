...
{
    std::string tryAqObjToolsGridCreate(
        ...
        const bool allowJaggedData )
    {
        ...
        if( rangeData.size() <= 0 )
        {
            throw AQLCoreInvalidData( "Empty data table was supplied to tryAqObjToolsGridCreate", __FILE__, __LINE__ );
        }
    ...
    std::pair<const FlexibleData, std::vector<std::string>>  tryAqObjToolsGridDisplay(
                const std::string& objectName )
    {
    ...
    std::pair<const bool, std::string> tryAqObjToolsGridLoad(	const std::string& fileName )
    {
    ...
    std::string tryAqObjToolsGridSave( const std::string& objectName,
    ...
    const bool tryAqObjToolsGridClearOne( const std::string& objectName )
    {
        auto& freeObjectStore = etrading::getObjectStore<etrading::FreeObject>( etrading::Environment::DEFAULT_ENV_NAME );
    ...
    const bool tryAqObjToolsGridClearAll()
    {
        auto& freeObjectStore = etrading::getObjectStore<etrading::FreeObject>( etrading::Environment::DEFAULT_ENV_NAME );
