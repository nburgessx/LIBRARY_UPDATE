...
namespace validation
{
    ...
    */
    std::vector<std::string> tryAqSwapsDV01LVBKeys()
    {
    ...
    /* @brief			validation interface for the aqSwapsDV01 method
    *  @param [in]		swapLVB			A label value block defining the swap
    *  @param [in]		validateKeys	True to validate the all keys provided are valid. Default to TRUE
    *  @return			Swap DV01
    */
    double tryAqSwapsDV01( const LabelValueBlock& swapLVB, bool validateKeys )
    {
        VALID_EXCEPTION_START

        etrading::validateKeysForLVB( tryAqSwapsDV01LVBKeys(), swapLVB.getKeys(), validateKeys );
        ...
        {
            CreateDataFile file( decorateFilename( "tryAqSwapsDV01_inputs", curveCollection ) );
            file.write( "generatorFunction", "tryAqSwapsDV01" );
        ...
        if ( CreateDataFile::recordEnabled() )
        {
            CreateDataFile file( decorateFilename( "tryAqSwapsDV01_outputs", curveCollection ) );
