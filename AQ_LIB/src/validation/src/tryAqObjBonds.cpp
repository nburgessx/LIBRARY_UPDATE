...
namespace validation
{
	...
	*  @return			Bond Handle
	*/
	std::string tryAqObjBondsCreate( const std::string& bondObjectName, const LabelValueBlock& bondLVB, const LabelValueBlock& scheduleLVB, bool validateKeys )
    {
	...
	*/
	std::string tryAqObjBondsCreateFromGenerator( const std::string& bondObjectName, const std::string& bondGeneratorName, const LabelValueBlock& expressionLVB, bool validateKeys )
	{
	    VALID_EXCEPTION_START
    ...
    /* @brief			validation interface for the aqObjBondsDisplay method
	*  @param [in]		bondObjectName		Bond object name
	...
	*/
	AnyTypeMatrix tryAqObjBondsDisplaySchedule( const std::string& bondObjectName, const bool& showColumnHeaders, const std::vector<std::string>& columnList )
    {
        VALID_EXCEPTION_START
	...
	*/
	AnyTypeMatrix tryAqObjBondsDisplayCashflows( const std::string& bondObjectName, const AQLDate& settlementDate, const double& yield, const std::string& yieldCalculationType, const bool& showColumnHeaders, const std::vector<std::string>& columnList )
	{
		VALID_EXCEPTION_START
	...
	*/
    std::vector< double > tryAqObjBondsYieldOptimized( const etrading::BondPtr bondObject, const std::vector< AQLDate >& settlementDates, const std::vector< double >& prices, const std::string& yieldCalculationType  )
    {
		...
		for (int i=0; i<nSettlementDates; i++)
		{
			...
			yields[i] = tryAqObjBondsYieldFromObject( myBonds[id], settlementDates[i], prices[i], yieldCalculationType );
		}
    ...
    *  @return			Bond Yield(s)
	*/
	std::vector< double > tryAqObjBondsYield( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& prices, const std::string& yieldCalculationType , const bool & optimizePerformance )
    {
        VALID_EXCEPTION_START
        ...
        if( calculateUsingThreads && optimizePerformance )
        {
            ...
            // ----------------------------------
            yields = tryAqObjBondsYieldOptimized( bondObject, settlementDates, prices, yieldCalculationType );
        }
        else
        {
	...
	*/
	double tryAqObjBondsBPVPerTick( const std::string& bondObjectName, const LabelValueBlock& valuationSettingsLVB, const double price, const std::string& yieldCalculationType, const double tickSize )
	{
		VALID_EXCEPTION_START
	...
	*/
    std::vector< double > tryAqObjBondsOisSpread( const std::string& bondObjectName, const std::string& marketObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& yields )
	{
		...
		for ( unsigned int i = 0; i < settleDatesSize; ++i )
        {
            const AQLDate settlementDate     = settlementDates[i];
            const double yield              = yields[i];
			double spread                   = tryAqObjBondsOisSpread( bondObjectName, marketObjectName, settlementDate, yield );
	...
	*/
                                             ...
                                             const std::vector<TableInfo>& infoBlocks )
    {
        ...
        {
			throw AQLCoreInvalidData( "#Error: No Property grid data provided to tryAqObjBondsGeneratorCreate", __FILE__, __LINE__ );
        }
		...
		{
			CreateDataFile file( decorateFilename( "tryAqObjBondsGeneratorCreate_inputs", objectName.c_str() ));
			file.write( "generatorFunction", "tryAqObjBondsGeneratorCreate" );
		...
		if ( CreateDataFile::recordEnabled() )
		{
			CreateDataFile file( decorateFilename( "tryAqObjBondsGeneratorCreate_outputs", objectName.c_str() ));
	 ...
	 */
	const etrading::VariantMatrix tryAqObjBondsGeneratorDisplay( const std::string& objectName, const std::string& propertyKey )
    {
		VALID_EXCEPTION_START
	...
	*  @return			bondName
	*/
	std::string tryAqObjBondsCreateFromLVB(const std::string& bondObjectName, const LabelValueBlock& bondLVB, bool validateKeys)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			Helper interface for the aqObjBondsZSpread method. Calculate the Bond Z Spread.
	*  @param [in]		bondObjectName		Bond object name
	...
	*/
	DoubleVector tryAqObjBondsZSpreads(const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& bondPrices, const std::vector< std::string >& curveCollections, const std::vector< std::string >& forecastCurves, const bool& continuouslyCompounding)
	{
		...
		for (unsigned int i = 0; i < settleDatesSize; ++i)
		{
			double zSpread = tryAqObjBondsZSpread(bondObjectName, settlementDates[i], bondPrices[i], curveCollections[i], forecastCurves[i], continuouslyCompounding);
	...
	*/
	...
	/* @brief			validation interface for the aqObjBondsCreateAUDNotionalBond method
