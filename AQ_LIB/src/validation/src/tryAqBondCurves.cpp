...
namespace validation
{
	...
	*/
	etrading::NelsonSiegelSvenssonCalibrationResults tryAqBondCurveNelsonSiegelCalibrate( const DoubleVector& bondMaturities,
	...
	*/
	etrading::NelsonSiegelSvenssonCalibrationResults tryAqBondCurveSvenssonCalibrate( const DoubleVector& bondMaturities,
																			          const DoubleVector& bondYields,
																			          const etrading::NelsonSiegelSvenssonParameters& initialGuess,
																			          const unsigned int& maxIterations,
																			          const unsigned int& maxStationaryStateIterations,
	 ...
	 */
	etrading::PolynomialCalibrationResults tryAqBondCurvePolynomialCalibrate( const unsigned int polynomialOrder,
	 ...
	 */
    std::string tryAqBondCurveCreate( const std::string& objectName,
                                      ...
                                      const etrading::JSONInfoBlockTuples& infoBlocks )
    {
		...
		if (CreateDataFile::recordEnabled()) 
		{
			CreateDataFile file(decorateFilename("tryAqBondCurveCreate_inputs", objectName.c_str()));
			file.write("generatorFunction", "tryAqBondCurveCreate");
		...
		if (CreateDataFile::recordEnabled()) 
		{
			CreateDataFile file(decorateFilename("tryAqBondCurveCreate_outputs", objectName.c_str()));
	...
	*/
	AnyTypeMatrix tryAqBondCurveDisplay( const std::string& bondCurveName )
	{
		VALID_EXCEPTION_START
	...
	*/
	double tryAqBondCurveYield(const std::string& bondCurveName, const AQLDate& referenceDate )
	{
	 VALID_EXCEPTION_START
	...
	*/
	double tryAqBondObjectPriceFromBondCurve( const std::string& bondObjectName, const AQLDate& settlementDate, const std::string & bondCurveName )
	{
		VALID_EXCEPTION_START
	...
	*  @param[in] bondCurveName		Name of the bond curve
	*/
	double tryAqBondObjectYieldFromBondCurve( const std::string& bondObjectName, const AQLDate& settlementDate, const std::string & bondCurveName )
	{
		VALID_EXCEPTION_START
	...
	*/
	DoubleVector tryAqBondCurveNelsonSiegelYield( const double& beta0,
												  const double& beta1,
												  const double& beta2,
												  const double& lambda,
												  const DoubleVector& bondMaturities )
	...
	*  @returns		A vector of interpolated bond yields, one yield per supplied maturity.
	*/
	DoubleVector tryAqBondCurveSvenssonYield( const double& beta0,
											  const double& beta1,
											  const double& beta2,
											  const double& beta3,
	...
	*/
	DoubleVector tryAqBondCurvePolynomialYield( const DoubleVector& coefficients, const DoubleVector& bondMaturities )
	{
		VALID_EXCEPTION_START
