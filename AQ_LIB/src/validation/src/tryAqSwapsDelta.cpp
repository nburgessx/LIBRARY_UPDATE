...
namespace validation
{
    ...
    */
        ...
        bool aggregateRisks )
    {
        ...
        {
            CreateDataFile file( decorateFilename( "tryAqSwapsDelta_inputs", curveCollection ) );
            file.write( "generatorFunction", "tryAqSwapsDelta" );
        ...
        if ( CreateDataFile::recordEnabled() )
        {
            CreateDataFile file( decorateFilename( "tryAqSwapsDelta_outputs", curveCollection ) );
            for ( size_t i = 0; i < pillarNames.size(); ++i )
            {
