...
namespace validation
{
    ...
    /* @brief			validation interface for the aqBondObjectCreate method
	...
	*  @return			Bond Handle
	*/
	std::string tryAqBondObjectCreate( const std::string& bondObjectName, const LabelValueBlock& bondLVB, const LabelValueBlock& scheduleLVB, bool validateKeys )
    {
	...
	*/
	std::string tryAqBondObjectCreateFromGenerator( const std::string& bondObjectName, const std::string& bondGeneratorName, const LabelValueBlock& expressionLVB, bool validateKeys )
	{
	    VALID_EXCEPTION_START
    ...
    /* @brief			validation interface for the aqBondObjectDisplay method
	*  @param [in]		bondObjectName		Bond object name
	*  @return			Bond Input Parameters
	*/
	AnyTypeMatrix tryAqBondObjectDisplay( const std::string& bondObjectName )
	{
		VALID_EXCEPTION_START
    ...
    /* @brief			validation interface for the aqBondObjectDisplaySchedule method
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		showColumnHeaders	Show Schedule Column Headers; Defaults to True
	*  @return			Bond Schedule
	*/
	AnyTypeMatrix tryAqBondObjectDisplaySchedule( const std::string& bondObjectName, const bool& showColumnHeaders, const std::vector<std::string>& columnList )
    {
        VALID_EXCEPTION_START
    ...
    /* @brief			validation interface for the aqBondObjectDisplayCashflows method
	*  @param [in]		bondObjectName		    Bond object name
	...
	*/
	AnyTypeMatrix tryAqBondObjectDisplayCashflows( const std::string& bondObjectName, const AQLDate& settlementDate, const double& yield, const std::string& yieldCalculationType, const bool& showColumnHeaders, const std::vector<std::string>& columnList )
	{
		VALID_EXCEPTION_START
    ...
    /* @brief			validation interface for the aqBondObjectDirtyPrice method
	*  @param [in]		bondObjectName		Bond object name
	...
	*/
	std::vector< double > tryAqBondObjectDirtyPrice( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& yields, const std::string& yieldCalculationType )
	{
		VALID_EXCEPTION_START
    ...
    /* @brief			validation interface for the aqBondObjectAccruedInterestDays method
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		settlementDates		settlement dates
    *  @return			Bond Accrual Day(s)
	*/
	std::vector< int > tryAqBondObjectAccruedInterestDays( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates )
    {
        VALID_EXCEPTION_START
     ...
     /* @brief			validation interface for the aqBondObjectAccruedInterest method
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		settlementDates		settlement dates
    *  @return			Bond Dirty Price(s)
	*/
	std::vector< double > tryAqBondObjectAccruedInterest( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates )
    {
        VALID_EXCEPTION_START
	...
	*/
	double tryAqBondObjectAccruedInterest( const std::string& bondObjectName, const LabelValueBlock& valuationSettingsLVB )
	{
		VALID_EXCEPTION_START
    ...
    /* @brief			validation interface for the aqBondObjectCleanPrice method
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		settlementDates		settlement dates
    *  @param [in]		yields		        yields
    *  @param [in]		yieldCalculationType	yield calculation type	        
    *  @return			Bond Dirty Price(s)
	*/
	std::vector< double > tryAqBondObjectCleanPrice( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& yields, const std::string& yieldCalculationType )
    {
        VALID_EXCEPTION_START
    ...
    /* @brief			validation interface for the aqBondObjectPrice method
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		settlementDates		settlement dates
    *  @param [in]		yields		        yields
    *  @param [in]		yieldCalculationType	yield calculation type	        
    *  @return			Bond Dirty Price(s)
	*/
	std::vector< double > tryAqBondObjectPrice( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& yields, const std::string& yieldCalculationType )
    {
        VALID_EXCEPTION_START
    ...
    /* @brief			validation interface for the aqBondObjectQuote method
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		settlementDates		settlement dates
    *  @param [in]		yields		        yields
    *  @param [in]		yieldCalculationType	yield calculation type	        
    *  @return			Bond Dirty Price(s)
	*/
	std::vector< double > tryAqBondObjectQuote( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& yields, const std::string& yieldCalculationType )
    {
        VALID_EXCEPTION_START
    ...
    *  @return			Bond Yield
	*/
    double tryAqBondObjectYieldFromObject( const etrading::BondPtr bondObject, const AQLDate & settlementDate, const double & price, const std::string& yieldCalculationType )
    {
    ...
    *  @return			Bond Yield(s)
	*/
    std::vector< double > tryAqBondObjectYieldOptimized( const etrading::BondPtr bondObject, const std::vector< AQLDate >& settlementDates, const std::vector< double >& prices, const std::string& yieldCalculationType  )
    {
		...
		for (int i=0; i<nSettlementDates; i++)
		{
			...
			yields[i] = tryAqBondObjectYieldFromObject( myBonds[id], settlementDates[i], prices[i], yieldCalculationType );
		}

        return yields;
    }

    /* @brief			validation interface for the aqBondObjectYield method
	*  @param [in]		bondObjectName		    Bond object name
    *  @param [in]		settlementDates		    settlement dates
    *  @param [in]		prices		            bond prices, internally we check for clean or dirty
    *  @param [in]		yieldCalculationType	yield calculation type	        
    ...
    *  @return			Bond Yield(s)
	*/
	std::vector< double > tryAqBondObjectYield( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& prices, const std::string& yieldCalculationType , const bool & optimizePerformance )
    {
        VALID_EXCEPTION_START
        ...
        if( calculateUsingThreads && optimizePerformance )
        {
            ...
            // ----------------------------------
            yields = tryAqBondObjectYieldOptimized( bondObject, settlementDates, prices, yieldCalculationType );
        }
        else
        {
    ...
    *  @return			Bond Yield
	*/
	double tryAqBondObjectYield( const std::string& bondObjectName, const LabelValueBlock& valuationSettingsLVB, const double price, const std::string& yieldCalculationType )
	{
		VALID_EXCEPTION_START
	...
	/* @brief			validation interface for the tryAqBondObjectBPVPerTick method
	*  @param [in]		bondObjectName		    Bond object name
    *  @param [in]		valuationSettingsLVB	Specifies the settlementDate, plus other parameters required for more complex bonds such as FRNs.
    *  @param [in]		price		            bond price, internally we check for clean or dirty
    *  @param [in]		yieldCalculationType	yield calculation type	        
	...
	*/
	double tryAqBondObjectBPVPerTick( const std::string& bondObjectName, const LabelValueBlock& valuationSettingsLVB, const double price, const std::string& yieldCalculationType, const double tickSize )
	{
		VALID_EXCEPTION_START
	...
	*/
    std::vector< double > tryAqBondObjectCompoundYield( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& prices, const std::string& yieldCalculationType )
    {
      	VALID_EXCEPTION_START
	...
	/* @brief			validation interface for the aqBondObjectDV01Numerical method. Calculate the DV01(s) via numerical bumping.
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		settlementDates		settlement dates
    *  @param [in]		yields		        yields
    ...
    *  @return			Bond DV01(s)
	*/
	std::vector< double > tryAqBondObjectDV01Numerical( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& yields, const double bumpSize, const AQLString& bumpMode, const std::string& yieldCalculationType )
	{
	...
	/* @brief			validation interface for the aqBondObjectDV01 method. Calculate the DV01(s) analytically.
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		settlementDates		settlement dates
    *  @param [in]		yields		        yields
    *  @param [in]		yieldCalculationType	yield calculation type	        
    ...
    *  @return			Bond DV01(s)
	*/
	std::vector< double > tryAqBondObjectDV01( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& yields, const std::string& yieldCalculationType )
	{
	...
	/* @brief			validation interface for the aqBondObjectModifiedDuration method. Calculate the Modified Duration analytically.
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		settlementDates		settlement dates
    *  @param [in]		yields		        yields
    *  @param [in]		yieldCalculationType	yield calculation type	        
	...
	*/
	std::vector< double > tryAqBondObjectModifiedDuration( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& yields, const std::string& yieldCalculationType )
	{
    ...
    /* @brief			validation interface for the aqBondObjectOisSpread method. Calculate the O-Spread (BondYield to OIS ParRate spread) using linear interpolation on the OIS ParRate on standard tenors.
	*  @param [in]		bondObjectName		Bond object name
	...
	*/
    std::vector< double > tryAqBondObjectOisSpread( const std::string& bondObjectName, const std::string& marketObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& yields )
	{
		...
		for ( unsigned int i = 0; i < settleDatesSize; ++i )
        {
            const AQLDate settlementDate     = settlementDates[i];
            const double yield              = yields[i];
			double spread                   = tryAqBondObjectOisSpread( bondObjectName, marketObjectName, settlementDate, yield );
    ...
    /* @brief			Helper method for the tryAqObjBondsSpreadOIS method. Calculate the O-Spread (BondYield to OIS ParRate spread) using linear interpolation on the OIS ParRate on standard tenors.
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		marketObjectName	Market Data object name
	...
	*/
    double tryAqBondObjectOisSpread( const std::string& bondObjectName, const std::string& marketObjectName, const AQLDate& settleDate, const double& bondYield)
    {
	    VALID_EXCEPTION_START
	...
	/* @brief			Validation interface for the aqBondObjectLastCouponDate method. Calculate the bond's last coupon date.
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		settlementDates		Bond's Settlement Dates 
    *  @return			The bond's last coupon date.
	*/
    std::vector< double > tryAqBondObjectLastCouponDate( const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates)
    {
	    VALID_EXCEPTION_START
	...
	/* @brief			validation interface for the aqBondObjectPriceFromDirtyToClean method. 
	*  @param [in]		bondObjectName			Bond object name
    *  @param [in]		valuationSettingsLVB	Specifies the settlementDate, plus other parameters required for more complex bonds such as FRNs.
    *  @param [in]		dirtyPrice				Bond dirtyPrice
    *  @return			Bond Clean Price
	*/
    double tryAqBondObjectPriceFromDirtyToClean( const std::string& bondObjectName, const LabelValueBlock& valuationSettingsLVB, const double dirtyPrice )
	{
	    VALID_EXCEPTION_START
    ...
    /* @brief			validation interface for the aqBondObjectPriceFromCleanToDirty method. 
	*  @param [in]		bondObjectName			Bond object name
    *  @param [in]		valuationSettingsLVB	Specifies the settlementDate, plus other parameters required for more complex bonds such as FRNs.
    *  @param [in]		cleanPrice				Bond cleanPrice
    *  @return			Bond Dirty Price
	*/
    double tryAqBondObjectPriceFromCleanToDirty( const std::string& bondObjectName, const LabelValueBlock& valuationSettingsLVB, const double cleanPrice )
	{
	     VALID_EXCEPTION_START
	...
	/* @brief			validation interface for the aqBondObjectPriceFromCreditModel function. This calculates the price of a defaultable bond.
	*  @param [in]		bondObjectName		Bond object name
    *  @param [in]		settlementDate		Bond's Settlement Date
    *  @param [in]		creditModelName		Credit model name
    *  @return			The calculated Bond price
	*/
	double tryAqBondObjectPriceFromCreditModel( const std::string& bondObjectName, const AQLDate& settlementDate, const std::string& creditModelName )
	{
		VALID_EXCEPTION_START
	 ...
	 */
    std::string tryAqBondGeneratorCreate( const std::string& objectName,
                                             ...
                                             const std::vector<TableInfo>& infoBlocks )
    {
		...
		{
			CreateDataFile file( decorateFilename( "tryAqBondGeneratorCreate_inputs", objectName.c_str() ));
			file.write( "generatorFunction", "tryAqBondGeneratorCreate" );
		...
		if ( CreateDataFile::recordEnabled() )
		{
			CreateDataFile file( decorateFilename( "tryAqBondGeneratorCreate_outputs", objectName.c_str() ));
	 ...
	 */
	const etrading::VariantMatrix tryAqBondGeneratorDisplay( const std::string& objectName, const std::string& propertyKey )
    {
		VALID_EXCEPTION_START
	...
	/* @brief			validation interface for the aqBondObjectCreateFromLVB method
	*  @param [in]		bondName		Bond name
	*  @param [in]		bondLVB			Bond input as single label value block
	*  @param [in]		validateKeys	True to validate the all keys provided are valid. Default to True
	*  @return			bondName
	*/
	std::string tryAqBondObjectCreateFromLVB(const std::string& bondObjectName, const LabelValueBlock& bondLVB, bool validateKeys)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			Helper interface for the aqBondObjectZSpread method. Calculate the Bond Z Spread.
	*  @param [in]		bondObjectName		Bond object name
	...
	*  @return			Z Spread
	*/
	double tryAqBondObjectZSpread(const std::string& bondObjectName, const AQLDate& settlementDate, const double& bondPrice, const std::string& curveCollection, const std::string& forecastCurve, const bool& continuouslyCompounding)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			Validation interface for the aqBondObjectZSpread method. Bond Z Spread.
	...
	*  @return			Z Spreads
	*/
	DoubleVector tryAqBondObjectZSpreads(const std::string& bondObjectName, const std::vector< AQLDate >& settlementDates, const std::vector< double >& bondPrices, const std::vector< std::string >& curveCollections, const std::vector< std::string >& forecastCurves, const bool& continuouslyCompounding)
	{
		...
		for (unsigned int i = 0; i < settleDatesSize; ++i)
		{
			double zSpread = tryAqBondObjectZSpread(bondObjectName, settlementDates[i], bondPrices[i], curveCollections[i], forecastCurves[i], continuouslyCompounding);
	...
	/* @brief			Validation interface for the aqBondObjectZSpreadFromRates method. Bond Z Spread.
	*  @param [in]		bondObjectName		Bond object name
	*  @param [in]		settlementDate		Bond's Settlement Date (Valuation Date)
	*  @param [in]		bondPrice		    Bond price
	...
	*  @return			Z Spread
	*/
	double tryAqBondObjectZSpreadFromRates(const std::string& bondObjectName, const AQLDate& settlementDate, const double& bondPrice, const DoubleVector& zeroRates, const bool& continuouslyCompounding)
	{
		VALID_EXCEPTION_START
	...
	*/
	double tryAqBondObjectForwardPrice(const std::string& bondObjectName, const AQLDate& settleDate, const AQLDate& forwardSettleDate, const double& price, const double& repoRate, const std::string& repoDayCount)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			Validation interface for the aqBondObjectForwardReinvestedCoupon method. Calculate Bond's recieved coupon value at forward settle date, from repo rate.
	*  @param [in]		bondObjectName		Bond object name
	*  @param [in]		settleDate			Bond current settle date
	*  @param [in]		forwardSettleDate   Bond forward settle date
	*  @param [in]		price				Bond price at settle date, can be dirty or clean depends on Bond's isCleanPrice flag
	...
	*/
	double tryAqBondObjectForwardReinvestedCoupon(const std::string& bondObjectName, const AQLDate& settleDate, const AQLDate& forwardSettleDate, const double& price, const double& repoRate, const std::string& repoDayCount)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			Validation interface for the aqBondObjectRepoRate method. Calculate Bond's implied repo rate from bond's forward price at settle date.
	*  @param [in]		bondObjectName		Bond object name
	*  @param [in]		settleDate			Bond current settle date
	*  @param [in]		forwardSettleDate   Bond forward settle date
	*  @param [in]		price				Bond price at settle date, can be dirty or clean depends on Bond's isCleanPrice flag
	...
	*/
	double tryAqBondObjectRepoRate(const std::string& bondObjectName, const AQLDate& settleDate, const AQLDate& forwardSettleDate, const double& price, const double& forwardPrice, const std::string& repoDayCount)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			Validation interface for the aqBondObjectRepoRateFromFuture method. Calculate Bond's implied repo rate from bond's FUTURE price at settle date.
	*  @param [in]		bondObjectName		Bond object name
	*  @param [in]		settleDate			Bond current settle date
	...
	*  @return			Bond implied repo rate
	*/
	double tryAqBondObjectRepoRateFromFuture(const std::string& bondObjectName, const AQLDate& settleDate, const AQLDate& deliveryDate, const double& price, const double& futurePrice, const double& conversionFactor, const std::string& repoDayCount)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			Validation interface for the aqBondObjectFuturePrice method. Calculate Bond's Future price at future settle date, from repo rate.
	*  @param [in]		bondObjectName		Bond object name
	*  @param [in]		settleDate			Bond current settle date
	*  @param [in]		deliveryDate		Bond future settle date
	...
	*/
	double tryAqBondObjectFuturePrice(const std::string& bondObjectName, const AQLDate& settleDate, const AQLDate& deliveryDate, const double& bondPrice, const double& repoRate, const std::string& repoDayCount, const double& conversionFactor)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			Help function for tryAqObjBondsConversionFactors with multiple settle dates as params. Calculate Bond conversionFactor based on future settle date and future's notional coupon rate.
	*  @param [in]		bondObjectName		Bond object name
	...
	*/
	double tryAqBondObjectConversionFactor(const std::string& bondObjectName, const AQLDate& firstFutureSettleDate, const double& notionalBondCouponRate)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			Validation interface for the aqBondObjectGrossBasis method. Calculate the Bond Gross Basis
	*  @param [in]		bondObjectName		Bond object name
	*  @param [in]		settleDate			Bond current settle date
	...
	*/
	double tryAqBondObjectGrossBasis(const std::string& bondObjectName, const AQLDate& settleDate, const double& price, const double& futurePrice, const double& conversionFactor)
	{
	...
	/* @brief			Validation interface for the aqBondObjectNetBasis method. Calculate the Bond Gross Basis
	*  @param [in]		bondObjectName		Bond object name
	*  @param [in]		deliveryDate		Bond future settle date
	...
	*  @return			Bond Gross Basis
	*/
	double tryAqBondObjectNetBasis(const std::string& bondObjectName, const AQLDate& settleDate, const AQLDate& deliveryDate, const double& bondPrice, const double& actualRepoRate, const std::string& repoDayCount, const double& futurePrice, const double& conversionFactor)
	{
	...
	*/
	std::string tryAqBondObjectCheapestToDeliver(const double& futurePrice, const AQLDate& settleDate, const AQLDate& deliveryDate, const std::string& repoDayCount, const std::vector<std::string>& bondObjectNames, const std::vector<double>& bondPrices, const std::vector<double>& conversionFactors)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			Validation interface for the aqBondObjectCheapestToDeliverByNetBasis method. Calculate the Cheapest to deliver (CTD) bond via Net Basis, return the bond name.
	*  @param [in]		futurePrice			Future price
	*  @param [in]		settleDate			Bond's settle date
	*  @param [in]		deliveryDate		Bond's future settle date/delivery date
	*  @param [in]		repoDayCount		Day count for repo year fraction
	...
	*  @return			The Cheapest to deliver (CTD) bond's name
	*/
	std::string tryAqBondObjectCheapestToDeliverByNetBasis(const double& futurePrice, const AQLDate& settleDate, const AQLDate& deliveryDate, const std::string& repoDayCount, const std::vector<std::string>& bondObjectNames, const std::vector<double>& bondPrices, const std::vector<double>& conversionFactors, const std::vector<double>& actualRepoRates)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			validation interface for the aqBondObjectCreateAUDNotionalBond method
	...
	*  @return			bondName
	*/
	std::string tryAqBondObjectCreateAUDNotionalBond(const std::string& bondObjectName, const AQLDate& settleDate, const std::string& maturityDate, const std::string& couponRate, const std::string& payReceive, const std::string& calendar, const std::string& frequency, const std::string& dayCount, const std::string& bondQuoteConv)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			validation interface for the aqToolBondAverageYield method, calculate the yield by averaging the underlying bonds' yields
	*  @param [in]		underlyingBondYields	Bond future's underlying bond yields
	*  @return			Aussie bond future notional bond's yield
	*/
	double tryAqToolBondAverageYield(const std::vector<double>& underlyingBondYields)
	{
		VALID_EXCEPTION_START
	...
	/* @brief			validation interface for the aqToolBondYieldFromFuturePrice method, calculate the yield from futurePrice
	*  @param [in]		futurePrice		Bond future price
	*  @return			Aussie bond future notional bond's yield
	*/
	double tryAqToolBondYieldFromFuturePrice(const double& futurePrice)
	{
		VALID_EXCEPTION_START
	...
	/* @brief	validation interface for the aqBondObjectFRNPriceFromDiscountMargin method
	...
	*/
	double tryAqBondObjectFRNPriceFromDiscountMargin( const std::string& bondObjectName, const AQLDate& settlementDate, const double& discountMargin, const double& assumedRate, const double& indexToNextCoupon, const double& annualizedNextCouponRate )
	{
		VALID_EXCEPTION_START
	...
	/* @brief	validation interface for the aqBondObjectFRNPriceFromYield method
	...
	* @returns	The clean / dirty floating bond price	
	*/
	double tryAqBondObjectFRNPriceFromYield( const std::string& bondObjectName, const AQLDate& settlementDate, const double& yield, const double& assumedRate, const double& indexToNextCoupon, const double& annualizedNextCouponRate )
	{
		VALID_EXCEPTION_START
	...
	/* @brief	validation interface for the aqBondObjectFRNYieldFromPrice method
	...
	*/
	double tryAqBondObjectFRNYieldFromPrice( const std::string& bondObjectName, const AQLDate& settlementDate, const double& price, const double& assumedRate, const double& indexToNextCoupon, const double& annualizedNextCouponRate )
	{
		VALID_EXCEPTION_START
	...
	/* @brief	validation interface for the aqBondObjectFRNDiscountMarginFromPrice method
	...
	*/
	double tryAqBondObjectFRNDiscountMarginFromPrice( const std::string& bondObjectName, const AQLDate& settlementDate, const double& price, const double& assumedRate, const double& indexToNextCoupon, const double& annualizedNextCouponRate )
	{
		VALID_EXCEPTION_START
