...
namespace validation
{
	...
	*/
	std::string tryAqVolatilityObjectCreate(const std::string& objectName, const LabelValueBlock& volLVB, bool validateKeys)
    {
	...
	*/
	std::string tryAqVolatilityObjectSabrMarketDataCreate(const std::string& objectName, const std::vector<std::string>& dataBlockNames, const etrading::JSONInfoBlockTuples& infoBlocks)
	{
		...
		{
			CreateDataFile file(decorateFilename("tryAqVolatilityObjectSabrMarketDataCreate_inputs", objectName.c_str()));
			file.write("generatorFunction", "tryAqVolatilityObjectSabrMarketDataCreate");
	...
	std::string tryAqVolatilityObjectSabrModelCalibrate(const std::string& objectName, const std::vector<std::string>& dataBlockNames, const etrading::JSONInfoBlockTuples& infoBlocks)
	{
		...
		if (CreateDataFile::recordEnabled())
		{
			CreateDataFile file(decorateFilename("tryAqVolatilityObjectSabrModelCalibrate_inputs", objectName.c_str()));
			file.write("generatorFunction", "tryAqVolatilityObjectSabrModelCalibrate");
			file.write("objectName", objectName);
