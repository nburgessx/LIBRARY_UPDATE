...
namespace validation
{
    ...
    bool tryAqObjectExists( const std::string& typeAsString, const std::string& objectName )
    {
    ...
    {
        const std::vector<std::string> listOfPossibleTypes = tryAqObjectType( objectName );
	...
	// Helper Function
    // Function to get the object name from the aqObjectLoad function which returns a tuple
    ...
    std::tuple<std::string, etrading::CachedObjectEnum> tryAqObjectLoadAndReturnTupleResults( const std::string& fileName, const etrading::FileTypeEnum fileType, etrading::Environment& env )
	{
    ...
    std::string tryAqObjectLoad( const std::string& fileName, const etrading::FileTypeEnum fileType, etrading::Environment& env )
	{
        // No thread guard needed here since the underlying function 'tryAqObjectLoadAndReturnTupleResults' owns the thread guard
        VALID_EXCEPTION_START_WITH_NO_THREAD_GUARD
        return getObjectName( tryAqObjectLoadAndReturnTupleResults( fileName, fileType, env ) );
        VALID_EXCEPTION_END
	}
                                                ...
                                                etrading::Environment& env )
    {
        ...
        for ( size_t i = 0; i < objectFilePaths.size(); ++i )
        {
            ...
            AQ_SET_VARIABLE_OR_OVERRIDE_ON_FAILURE( 
                resultsVector[i],
                tryAqObjectLoad( objectFilePaths[i], fileType, env ),
	...
	std::string tryAqObjectSave( const std::string& objectName, const std::string& objectTypeString, const std::string& fileName, const etrading::FileTypeEnum fileType, etrading::Environment& env )
	{
        VALID_EXCEPTION_START

        // Get object type
        std::string objectTypeUsed = objectTypeString;
        std::vector<std::string> objectTypes = tryAqObjectType( objectName );
    ...
    std::vector<std::string> tryAqObjectQuickSave( const std::vector<std::string>& objectNames,
		                                        ...
		                                        etrading::Environment& env )
    {
	    // No thread guard needed here since the underlying function 'tryAqObjectSave' owns the thread guard
        VALID_EXCEPTION_START_WITH_NO_THREAD_GUARD
        ...
        for ( size_t i = 0; i < objectFilePaths.size(); ++i )
        {
            ...
            AQ_SET_VARIABLE_OR_OVERRIDE_ON_FAILURE(
                thisObjectType,
                tryAqObjectTypeAsString( cleansedObjectNames[i] ),
            ...
            AQ_SET_VARIABLE_OR_OVERRIDE_ON_FAILURE( 
                resultsVector[i],
                tryAqObjectSave( cleansedObjectNames[i], thisObjectType, objectFilePaths[i], fileType, env ),
    ...
    std::tuple<std::string, etrading::CachedObjectEnum> tryAqObjectLoadFromString( const std::string& jsonString, etrading::Environment& env )
    {
        VALID_EXCEPTION_START
